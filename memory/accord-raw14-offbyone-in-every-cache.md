...
description: "🛑 INSTRUMENT DEFECT in every route cache _cache_r5e..r73: the row arrays (t, probe) are exactly ONE SAMPLE SHORTER than the raw arrays (raw14_t, raw14_b4). Pairing t with raw14_b4 reads every cave byte ~10 ms early — 28 degrees of phase at 7.79 Hz."
metadata: 
  node_type: memory
  type: reference
...
shorter, forever, in every cache.** Verified `_cache_r5e` · `r61` · `r65` · `r66` · `r66x` · `r67x` ·
...
Audit script: `analysis-2020accord/audit_raw14_offbyone.py` — re-confirms the invariant on every cache
