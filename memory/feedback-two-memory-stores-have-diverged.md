...
3. `accord-4x-lkas-gain-is-the-frozen-variable` — **this one was NOT caught in time.** It is cited as a
   live `[[link]]` in `docs/_v100_arc_map.md` (this session's own arc-map deliverable) and in
   `docs/RESEARCH-2026-08-10-lkas-torque-self-cancellation.md` and two other repo memory files —
...
- **8 were skipped as duplicates** — a same- or similar-named repo file already covers the identical
  ...
  `eps-telem-red-panda-cannot-poll-during-lkas.md` and confirmed the repo version is the corrected
...
- **1 was skipped as a REFUTED claim, deliberately** — `accord-damper-cannot-reach-micro-regime`
  ...
  in repo `memory/accord-base-assist-damper-cannot-reach-the-micro-regime.md`, a differently-named
