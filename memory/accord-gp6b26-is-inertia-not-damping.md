...
description: "gp-0x6c2c is a FIRST DIFFERENCE of the filtered motor rate = ACCELERATION, traced line by line in FUN_00041464. So gp-0x6b26 = -K x accel is an APPARENT-INERTIA term, not a damper. build_v91_tva.py's 'genuinely DISSIPATIVE, it opposes motor rate' is WRONG, and the whole V74/V75/V91/V92 dose direction was aimed at the wrong physics."
metadata:
  type: reference
---
...
core physics claim in `build_v91_tva.py` and therefore the rationale of every build in the family.**
...
- **`build_v91_tva.py`'s "The Y row is NEGATIVE … so the term is genuinely DISSIPATIVE (it opposes
...
`rlog-tools/v93_identify_6b26.py` reconstructed the signed lane from 427 magnitude + the `0x14A` b7
