...
> 4. ⚠ **"V71C's r24 is a cut (0.93x) at rateKey 100" is WRONG** — it is a **1.71x BOOST**. That figure came from `_r58_lib.py` sweeping against `_v70_plain_image.bin` at **mode 10**. The companion **2.59x at rateKey 2000 is correct** (V70's edit did not reach that segment). Corrected model: `analysis-2020accord/_grind2_delivered_lib.py`.
...
the `_r58_lib.py` mode-10/V70-baseline defect and is corrected here; the 2.59× was always right.*
