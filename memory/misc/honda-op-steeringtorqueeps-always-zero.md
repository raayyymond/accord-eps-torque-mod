...
column in `rlog-tools/decode/extract_eps_telemetry.py`) reads **0 for the entire drive** in the Accord rlogs.

**Root cause — an openpilot omission, not a firmware/EPS behaviour:**
- The extractor sources it from `cs.steeringTorqueEps` (`rlog-tools/decode/extract_eps_telemetry.py:86`).
