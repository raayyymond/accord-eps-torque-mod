...
description: Six tooling/hygiene traps found 2026-08-07 — the wrong V76 probe decoder for route 65, a stale V76 plain image that a Glob returns first, builds/v50_v79/build_v75_tva.py's default lever set not producing the flown V75, route 5d's missing raw rlogs, V80's non-discriminating probe, and the broken default python numpy.
metadata:
  type: feedback
---
...
## 1. 🛑 `rlog-tools/probe/decode_v76_probe.py` is the WRONG decoder for route 65
...
(**not** `_scratch/cache/r65/`). Using the wrong decoder silently re-labels the probe bits and will give a
...
## 3. `builds/v50_v79/build_v75_tva.py`'s default lever set does **NOT** produce the flown V75
...
repo**
Only the extracted `_scratch/cache/r5d/*.npz` + `.pkl` survive. **Every downstream V74 conclusion in `STATE.md`
