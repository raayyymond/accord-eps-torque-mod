...
- **Before concluding a measurement cannot be made, count the exposure across the whole corpus.** It is
  one cheap loop over the caches and it takes seconds. `analysis-2020accord/studies/sessions/r47/r47_orchestrator_checks.py
...
- **This is the same failure as V57's probe.** `gp-0x6806`'s polarity sat measured in
  `_scratch/cache/r28`/`_scratch/cache/r29` for a month while every session treated it as an open Ghidra question. The
