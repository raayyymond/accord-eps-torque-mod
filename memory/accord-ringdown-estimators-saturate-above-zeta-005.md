...
description: "BOTH kit ring-down estimators saturate at zeta~0.05 and REVERSE above it - demod reads a zeta=0.20 mode as 0.0084, 24x too lightly damped - and their answer moves 3.5-6.3x with the fit-window length alone. _stock_r97_ringdown.json is UNQUOTABLE and the recorded zeta 0.017-0.036 is an upper bound on Q, not a measurement. Matrix pencil (E3) is the replacement."
metadata:
  type: reference
---
...
`rlog-tools/ringdown_validate.py`. Four estimators, identical synthetic inputs, f_n = 8.16 Hz,
40 reps/cell, SNR 20 dB, truth zeta 0.005 → 0.200 (a 40× span).

| estimator | source | Spearman rho | dynamic range | verdict |
|---|---|---|---|---|
| E1 `hilbert_env` | `stock_r97_ringdown.py` — *"the only estimator that passed"* | +0.829 | **7.6×** | 🛑 does not order |
| E2 `demod` | `r67_ringdown_q2.py` | **+0.371** | **1.7×** | 🛑 does not order |
...
1. 🛑 **`rlog-tools/_stock_r97_ringdown.json` IS UNQUOTABLE.** It runs `fit_s = 2.0` over 3–4 s
