...
## The vibration, re-characterized from fresh V38-behavior data (studies/spectra/analyze_manual_vibration.py + studies/telemetry/manual_speed_split.py)
...
- **GATE 2 (closed-loop stability) — CLOSED** (`analysis-2020accord/studies/models/eps_v50_gate2_lowpass.py`, re-runnable):
...
`analysis-2020accord/builds/v50_v79/build_v50_tva.py` + `studies/caves/v50_cave_asm.py` + `studies/models/eps_v50_gate2_lowpass.py`. V38 + 4 changes,
...
2. **Live RAM watch on gp-0x1500 — a READY read-only probe is BUILT + VERIFIED (2026-07-23), NOT YET
   FLASHED/DRIVEN.** `builds/v50_v79/build_v50probe_tva.py` / `_v50probe_plain_image.bin`: V49P/V31P-CLASS telemetry cave
   ...
   -0x1500,gp,r7 ×2; jarl 0xc4b34,lp). Flash it, drive, decode CAN 330 with `studies/probes/decode_v49p_polarity.py` — if
   ...
   **⚠ The existing manual rlog `aa5b3e0c01` does NOT count as this drive** — it was captured 2026-07-22,
   before this probe existed, and `studies/caves/compare_330_caves.py` already showed it carries no live telemetry cave
