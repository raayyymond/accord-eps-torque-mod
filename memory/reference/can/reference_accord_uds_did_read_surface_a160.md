...
`analysis-2020accord/builds/telemetry/build_v31u_uds_telem_tva.py` = V31 cal + repurpose **idx1 / DID `0x4801`**: handler_ptr
...
byte-diff vs proven V31 = 74 bytes telemetry-only. Superseded builds: `builds/telemetry/build_uds_telem_tva.py` (off-by-one,
cave on 0x4800); `builds/telemetry/build_tier1_telem_tva.py` (0x660 broadcast, gateway-blocked).
...
- **Next step is openpilot-side:** log this DID into rlogs during an LKAS drive (voters only move then) — see
  `docs/handoffs/2026-07/HANDOFF-2026-07-10-v31u-uds-telemetry-working.md` §5.
