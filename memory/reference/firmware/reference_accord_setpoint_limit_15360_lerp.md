...
**The blocker described below is CLEARED.** `builds/v18_v49/build_v38_tva.py` now covers the bank blocks and V38 carries the raise. Historical framing retained because the fails-closed mechanism is worth remembering.

`0xE4180` sits in bootloader CRC block `[0xE4000, 0xE4FFC)` with its trailer at `0xE4FFC` (confirmed by running `lib/verify_bootloader_crc.py` on the stock dump: 49 blocks, 0 mismatches; blk#23 covers curves 0-5, blk#22 the E5 bank, blk#21 the E6 bank).
