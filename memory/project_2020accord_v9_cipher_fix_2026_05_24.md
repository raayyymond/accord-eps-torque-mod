...
- **Pre-flash gate:** decode-as-ECU == `code.bin` AND 49/49 CRC blocks PASS in `verify_bootloader_crc.py`. See [[reference-tva-bootloader-crc-scheme]].
...
- Builder: `analysis-2020accord/old_tools/build_stock_tva_v9.py` (now self-contained — the v3–v8 builders it used to import from were removed). Flasher: `flashing-2020accord/eps-update-tva.py`.
...
- Full recipe writeup (replaces the old per-finding docs): `analysis-2020accord/HOW_TO_BUILD_ACCORD_TVA_RWD.md`. Torque path: `analysis-2020accord/TORQUE_PATH_AND_TABLE.md`.
