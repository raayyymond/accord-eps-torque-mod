...
1. **The comma is a PASSENGER at 21.5 Hz.** Re-derived from route b9 raw CAN 399 (`reanalyze_b9_vibration.py`,
...
2. **The measurement was overstated.** Verified from the b9 PSD: peak 21.48 Hz reproduces, but it is a
   ...
   **Aliasing is UNRESOLVED again** — 21.5 Hz is indistinguishable from an aliased 78.6 Hz (no >100 Hz
   ...
   wrong center.** The `Q_MEAS_4X=13.6` / `|L(21.4)|=0.875` anchors in `eps_loop_gain_model.py` /
   `eps_v48c_gate2_closed_loop.py` are calibrated to the falsified sharp-line reading and need re-fitting.
...
## Why V49 = flip StageC's sign + band-limit it (model: `eps_v49_a382_stagec_flip_model.py`, re-runnable)
...
## V49 build (BUILT + VERIFIED, UNFLASHED — `build_v49_tva.py`)
