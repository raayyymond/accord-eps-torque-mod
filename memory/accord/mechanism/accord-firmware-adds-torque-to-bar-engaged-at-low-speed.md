...
⚠ Corrects `docs/research/FEASIBILITY-SELF-INTERFERENCE-CANCELLATION.md` twice: its `f₀ = 6.8–11.4 Hz` bracket is
...
on 280,598/280,598 frames** ⇒ `rlog-tools/decode/decode_two_angles.py`'s premise is void.
⊕ `tq`'s LSB is **8**, not 1; `ang` repeats on **74%** of frames.

Tools: `rlog-tools/selfint_*.py`, cache `_scratch/cache/selfint/`.
