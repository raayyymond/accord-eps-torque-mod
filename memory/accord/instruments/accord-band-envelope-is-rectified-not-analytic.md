...
description: band_envelope in lib/_r31_common.py and lib/_r2b_common.py returns a RECTIFIED signal not an analytic envelope - every envelope-SHAPE result across ~20 callers is wrong
metadata:
  type: reference
---

🛑🛑 **`band_envelope` IS BROKEN IN TWO SHARED HELPERS** — `analysis-2020accord/lib/_r31_common.py` and
`rlog-tools/lib/_r2b_common.py`. **REPORTED 2026-08-20, NOT FIXED.**
...
- `2 * band_envelope(...)` used as peak-to-peak at
  `studies/gates/analyze_bus_amplitude_vs_detector_T.py:282` is **2× too large**.
...
**~20 callers**: `analyze_r31_*`, `studies/sessions/r35/analyze_r35_v64_arms.py`, `analyze_r37_*`,
`probe/decode_v59_boostindex.py`, `studies/grind/compare_v75_v76_v80_grind.py`, `studies/identification/selfint_transfer.py`,
`studies/gates/analyze_bus_amplitude_vs_detector_T.py`.

✅ **`rlog-tools/lib/r95_lib.py::band_envelope` uses a full complex `ifft` and is CORRECT. Use it.**
