...
`rlog-tools/decode/extract_audio.py` builds third-octave and "wide" band powers with **NFFT 1024 at 16 kHz**
...
- `rlog-tools/decode/extract_audio_ladder.py` — replaces **only** the segment discovery with a glob;
...
- `rlog-tools/decode/extract_audio_env.py` — band-passes the **raw PCM** (4th-order zero-phase Butterworth)
  and takes the **true analytic (Hilbert) envelope**. No FFT bins ⇒ the bin trap cannot apply.
- `rlog-tools/decode/extract_audio_grind.py` — **NFFT 16384 ⇒ 0.9766 Hz bins** for direct sub-100 Hz work,
