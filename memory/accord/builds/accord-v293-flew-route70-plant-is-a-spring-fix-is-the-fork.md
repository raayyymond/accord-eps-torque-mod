...
- **Rev 2 (shipped):** fork `66cf4454a` tables (hold ratio ~0.5/0.97/1.01/1.00 at the band centres after the adversary's low-speed re-bound and the knot-placement fix vs 3.20/1.52/1.54/1.29 for old ×3.3;
...
- **Sample rates (operator asked):** `0x14A`/`0x18F` steering angle/rate are 100 Hz with FRESH samples each frame (parity test), `0xE4` 100 Hz; only the `0x1AB` torque tap is 50 Hz — a 100 Hz fork rate loop has a 100 Hz measurement.
