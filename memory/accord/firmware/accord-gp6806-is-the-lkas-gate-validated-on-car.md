...
direct on-car answer sat in `_scratch/cache/r28` / `_scratch/cache/r29`. **Before opening Ghidra on a signal, check
whether a past probe already flew on it** — `docs/BUILD-LINEAGE.md` lists every probe payload, and
`analysis-2020accord/lib/route_build_registry.py` maps every route to its build.
