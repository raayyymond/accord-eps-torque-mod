...
`build_v69_tva.py` asserts `arm == 512 ⟹ gate byte == 0xc5`.
...
⚠ **`diff_build_vs_stock.py` is SPAN-based and would attribute it without complaint.**
...
`[2304, 2304, 2150, 1946]`. Assuming they matched is an error `verify_v69_image.py` caught on its
first run.

★ **THE GENERAL LESSON, and it is why both files exist:**
**`diff_build_vs_stock.py` attributes by RANGE, so a WRONG VALUE inside an existing `EDITS` span
...
⚠ `verify_v68_image.py` does **not** check `0xC6564` (r26's inert cal base) — `verify_v69_image.py`
