...
**Residual:** the `4096 − 3584 = 512` margin rests on COMP ceiling 2560 (cal `0xC67D8`) + governed clamp 1024
(cal `0xC61B4`); COMP magnitude is uncertain (V30 residual #1) — raise the floor or trace `gp-0x6ac0` to size
exactly if a larger realized COMP is suspected.

## Build

V31 = `build_v31_tva.py` = V30 (GAIN/clamps/ramp + corridor ×4 + float mirror + PN) **+ matched flat boost
...
`../accord-firmware/analysis-2020accord/_v31_plain_image.bin`. **UNFLASHED** (iron rule). Handoff `docs/HANDOFF-2026-06-03-v31.md`.
