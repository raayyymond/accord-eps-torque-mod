...
- **Era 23 — 2026-06-03 (latest at the time) — Accord V29/V30 BUILT (cal-only matched corridor) → V30 FLASHED → drove well but residual soft EME on a hard sustained HANDS-OFF turn → V31 BUILT (boost floor)**:
  ...
  - **Docs:** `docs/handoffs/2026-06/HANDOFF-2026-06-03-v31.md`.
...
- **Era 22 — 2026-06-03 — Accord V27 FLASHED → FAULTED WHEN TURNING → V28 BUILT then ANALYSIS-FALSIFIED → V29 proposed (cal-only matched corridor), UNFLASHED**:
  ...
  - **V28 = `builds/v18_v49/build_v28_tva.py` = V27 + a PROPORTIONAL 2× widen of BOTH corridor consistency monitors** so `2R` fits. 49/49 CRC, cipher round-trips, all readbacks pass, UNFLASHED.
  ...
  - **Doc:** `docs/handoffs/2026-06/HANDOFF-2026-06-03-v28.md` (correction banner at top).
...
- **Era 21 — 2026-06-02 — Accord V26 FLASHED → HARD-FAULTED AT REST → V27 BUILT (first Accord CODE patch; corrected corridor-twin model), UNFLASHED**:
  ...
  - **Doc:** `docs/handoffs/2026-06/HANDOFF-2026-06-02-v27.md`.
...
- **Era 20 — 2026-06-02 (later) — Accord V25 FLASHED → HARD-FAULTED at full lock → V26 corridor-lockstep fix BUILT, UNFLASHED**:
  ...
  - **V26 = `builds/v18_v49/build_v26_tva.py` = V25 + the float-corridor twin ×2** (seven f32 `1.0→2.0`). Restores lockstep exactly. 33 bytes / 19 runs, 49/49 CRC, zero code edits. UNFLASHED (at time of this era — later flashed and hard-faulted at rest, see Era 21).
  ...
  - **Doc:** `docs/handoffs/2026-06/HANDOFF-2026-06-02-v26.md`.
...
- **Era 19 — 2026-06-02 — Accord V24→V25 CLEAN: the corrected "corridor vs envelope" model; the shl/envelope thread was mis-scoped; V25 = GAIN + direction-corridor ×2, UNFLASHED [⚠ V25 later flashed → HARD-FAULTED; see Era 20 — the corridor IS lockstep-monitored]**:
  ...
  - **V25 CLEAN BUILT** (`builds/v18_v49/build_v25_tva.py`) = GAIN + clamps + ramp (V18) + **corridor ×2** + PN. The entire shl / FP-twin-cave / widen / weight-8-exclude cleanup is DROPPED. 19 bytes / 12 runs, 49/49 CRC, zero code edits, UNFLASHED (at time of this era; later flashed and hard-faulted at full lock, see Era 20).
  - **MODE resolved:** A160 command mode `tp+0x74c8`=0 (MODE 0), distinct from variant mode `gp-0x674e`=1.
  - **Doc:** `docs/handoffs/2026-06/HANDOFF-2026-06-02-v25-clean.md`.
...
- **Era 9-and-earlier note:** the Accord platform's activation-path/control-plane verification work (whether LKAS-gated levers can affect manual driving) and the initial V0–V10 build history predate the eras captured in detail here; see `project/project_accord_torque_mod_v0.md` for the full rolling log back to V0.
...
`analysis-2020accord/model/eps_lkas_chain_model.py`; update it continuously and prefer it over stale prose.
...
- `project-v53…` → `reference-accord-fourframe-strb-ssam-defect`: V53 inherits the STRB fix by **byte
  equality with the verified image**, not by re-derivation — `builds/v50_v79/build_v53_tva.py` imports the cave from
  `builds/telemetry/build_vfourframe_tva.py` and asserts a 6-byte diff. ⇒ **new reusable pattern: "existing cave + one cal"
...
Full reconciliation written into both memory files directly (`accord/calibration/accord-c6200-clamps-the-pid-reference.md`
carried it from the start; `accord/mechanism/accord-aggregator-never-rails-loop-is-linear.md` got it added this pass so
...
Every build script since V90 labels `0xC6200` as *"gp-0x6b70's clamp"* (`builds/v80_v107/build_v96_tva.py:701`,
