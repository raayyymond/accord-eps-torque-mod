...
- [★★★★★ Blend slows RISING only; 0xD2000 block is modes 10/11/12 packed, mode-10 edit is isolated; task-5 rate UNRESOLVED](reference_accord_boost_amp_blend_direction_and_d2000_block.md) — TCB class-ptr is identical across all 7 tasks (dead end); on-car measurement recommended for the rate.
...
- [0xC646C 6-reader enum: real feedback path, not the 21Hz driver](reference_accord_c646c_gain_feedback_vs_forward_classification.md) — slow/small-authority; free cal word 0xC6CD0.
...
- [★★★★★★ RTOS SOLVED: 7-task TCB has NO period field; rates come from FUN_00014be4's mod-100 divider /1,/2,/5,/10,/100; TAUJ1I2 is the SOLE tick writer](reference_accord_rtos_task_table_and_rate_scheduler.md) — closes "TCB walker not located"; task 5 = FUN_00022ca0 carries boost+damping (its own rate-class still open — TCB class-ptr is identical across all 7 tasks, a dead end; see session-2 entry). Supersedes "OSTM0 master tick ~1kHz" (OSTM0 is 500Hz, not the control tick).
