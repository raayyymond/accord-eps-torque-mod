...
- [🛑🛑★★★★★ `0xC6200`=8192 clamps **`gp-0x6ad6` INSIDE the PID** @`0x3a7a2` ⇒ `|ref|≥8192` makes P, I **and** D sensitivity to `gp-0x6b70` **EXACTLY ZERO**](reference_accord_c6200_clamps_gp6ad6_inside_the_pid.md) — closes `BUILD-LINEAGE:490`'s 3 unidentified readers; **conditions the 0.2565 authority figure**; term 0 needs only 32 % of its clamp, not 100 %; weights all 1, speed LERP an identity; the rung costs **−8 B**.
- [★★★★★ `iVar6` is REGISTER-ONLY (`FUN_00038148` has exactly 2 stores) — and the fixed 1 kHz call order `0x22416→0x225f6→0x22676→0x22696→0x226a0` makes a cave recomputation ZERO-SKEW between passes](reference_accord_ivar6_register_only_and_the_1khz_call_order.md) — valid hands-ON only (4.9 %/tick vs p50 2,829 vs 188 ct); 5 new encodings; ⊕ `gp-0x6b4a` shadow twin `gp-0x4cd2`.

## 2026-08-13 `0xC63AE` priced end to end — NO-GO at 2048 (`tracer-c63ae`)
- [🛑🛑★★★★★ The "+28 % / +177 ct clears the floor" is a **LEVEL** shift scored against an **AC-calibrated** bracket — in-band is **1.242 lane / 1.021–1.135 delivered**, at/below V85's not-felt 1.088](reference_accord_c63ae_dose_is_a_level_not_an_ac_change.md) — ⊕ AC gain **NON-MONOTONE**: 1280 = **0.902, WORSE than stock**. PIN/WRAP duty **0.0000** at ≤2048; real hazard is a **4.9–9.0× gain EXPANSION** at the `X[8]` corner, margin **1.14× @2048 vs 1.53× @1536**. RULE 7 **PASSED** (non-indexed `tp` scalar, 1 unconditional read). `Y[9]` ≡ the ±clamp cell `0xC6200`. 🛑 Blocker is **SIGN**, not dose.
- [🛑🛑★★★★★ Aggregator junction = **10 `add`, ZERO multiplies** ⇒ unweighted 11-term sum, Path 2 is ONE term ⇒ **φ = Path2/total at one node**; `gp-0x374c` never leaves `FUN_00038148` ⇒ "unaffected by A" CONFIRMED; and **427 used RECTIFIED understates 6-9 Hz by 4.86×** (sign toggles 5.06/s) — live defect in `v98_r81_score.py:541`](reference_accord_aggregator_is_unweighted_and_427_rectification_costs_4.9x.md) — ⊕ φ spec: repoint 427→`gp-0x6b94`, **saturation structurally impossible** (code 800/1023), GATE 1 unchanged, **no null possible**, crossover **R≈387 ct**.
