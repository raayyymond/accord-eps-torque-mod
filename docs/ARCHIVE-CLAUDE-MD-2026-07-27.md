...
> `docs/HANDOFF-2026-07-26-route13-vibration-engagement-dependence.md` first.):**
...
> - **⚠ THE PLANNED RED-PANDA CONFIRMATION MAY NOT DISCRIMINATE** — `docs/RED-PANDA-EPS-SETUP.md` routes
...
> `docs/HANDOFF-2026-07-24-low-speed-steer-lockout.md` first.):**
...
>   suppressed ~10-70× in that band. Scripts: `analysis-2020accord/speed_status_engagement.py` +
>   `speed_efficacy_test.py`.
...
>   no speed term. ⇒ `memory/reference_accord_sub3mph_lkas_openpilot_gate.md` is **FALSIFIED on its
...
> `docs/HANDOFF-2026-07-24-v52c-complete-broad-lowpass.md` first.):**
...
> RWD readback clean. `build_v52c_tva.py` + `v52_cave_asm.py` + `verify_v52c_image.py` +
> `eps_v52c_gate2_broad.py`.
...
>   (`0x2EC66/0x2ECBA`) and 3 dead (`0x2A992`, `0x2D9A2`, `0x2DAE6`). `verify_v52c_image.py` enforces
...
> independent decoders (`decode_v51p_gate1.py` + lead verifier), stock null distinguishable. This is the
...
> UNFLASHED** (`build_v52_tva.py` + `v52_cave_asm.py`; 10 repoints; SHA `bf4bc5b4…`).
...
> - **Path for the cell (still valid):** `build_v51probe_tva.py` = a V51P probe (BUILT/UNFLASHED) reading
...
>   - **`VCANTX-TEST-txgate`** (`build_vcantx_test_tva.py`) — single-frame mechanism test: mailbox 16, ID
>     0x555, fixed payload, gated on `gp-0x1712` bit0. The kit's FIRST active-CAN-TX cave.
>   - **`FOURFRAME`** (`build_vfourframe_tva.py`) — the telemetry: mailboxes 16-19, IDs `0x6a0-0x6a3`, **16
...
> `docs/HANDOFF-2026-07-22-v50-lowpass-ema-cave.md` FIRST, then
> `memory/reference-accord-v50-lowpass-ema-cave.md` +
> `memory/reference-accord-v49-stagec-flip-collocated-damper.md`.):**
...
> polarity-independent. `build_v50_tva.py` + `v50_cave_asm.py` + `eps_v50_gate2_lowpass.py`.
> - **Why a low-pass, not V48B's notch:** fresh V38-behavior data (`analyze_manual_vibration.py` +
>   `manual_speed_split.py`) shows the felt mode is **SPEED-DEPENDENT** — ~21.7 Hz at 3-8 m/s (worst regime,
...
>   (2) the live RAM watch is a **BUILT + verified read-only probe** `build_v50probe_tva.py` (V49P-class:
>   reads `gp-0x1500` into CAN 330 spare bits on the current V38 firmware; the car has no arbitrary-RAM UDS
>   read). Flash the probe, drive, decode CAN 330 with `decode_v49p_polarity.py` — `gp-0x1500` staying 0
...
> - **V49 / V49P (superseded lead, retained):** V49 (`build_v49_tva.py`) = the ratchet + a collocated damper
...
>   stability was never analyzed**. `eps_loop_gain_model.py` Task 4(d)'s "base assist doesn't need 21 Hz /
...
> `docs/VIBRATION-DOSSIER.md` for the vibration diagnosis, `docs/HANDOFF-2026-07-21-v48b-notch-build.md` for
...
>   `analysis-2020accord/eps_loop_gain_model.py`.
...
> - **V48A** (`build_v48a_tva.py`; ratchet + mute type-8 `0xC4120` + `FUN_0003a382` `uVar27`→256,
...
>   `build_v48b_tva.py` + `v48b_cave_asm.py` + `eps_v48b_cave_model.py`. V38 + ratchet + a **138-byte,
...
>   Read `docs/HANDOFF-2026-07-21-v48b-notch-build.md` +
>   `memory/reference-accord-v48b-notch-cave-build.md`.
>
...
> `docs/HANDOFF-2026-07-21-v46-v47-vibration.md` first.** Six-agent audit this session established:
...
>   ±512..1024 (not ±2048). See `memory/reference-accord-damper-two-deadzones-factorC-factorE.md`.
...
>   + plant physics, NOT a firmware gate.** `memory/reference-accord-no-vehicle-speed-input-5mph-is-plant.md`.
...
>   resonance. `memory/reference-accord-dualpinion-arch-one-torsion-sensor.md`.
...
>   `memory/reference-accord-governor-g1-total-command-not-thermal.md`.
...
>   float edit. V47 leaves it stock. `memory/reference-accord-damping-clamp-dtc1d-trap.md`.
...
> candidate.** Read `docs/HANDOFF-2026-07-20-v44-handsoff-damping.md` first.
>
...
>
> `analysis-2020accord/eps_lkas_chain_model.py` is the live golden reference and must be updated
...
project docs and collaterals** — CLAUDE.md current-state, the golden model `eps_lkas_chain_model.py`,
...
- **Latest handoff:** `docs/HANDOFF-2026-07-19-v39-direct-torque-rate-guard.md`; full reading order is in `docs/INDEX.md` (V9→V39).
- `docs/` — technical reference. Key entries: `HONDA-EPS-PID-KNOWLEDGE.md` (canonical PID reference distilled from a 26-day Discord working group — READ before any PID/tuning work), `EPS-FLASH-RUNBOOK.md` + `RED-PANDA-EPS-SETUP.md` (procedure/hardware rig), `GHIDRA-CHECKLIST.md` (human-driven interactive disasm) + `FIRMWARE-DECOMPILE-GUIDE.md` (agent-driven GhidraMCP reference), `GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md` (CAN→motor gating map), `SPEC-uds-can-ram-telemetry-a160.md` (UDS RAM telemetry insertion spec), the `HANDOFF-*.md` chain (**LATEST**: `HANDOFF-2026-07-17-v38.md` + `HANDOFF-2026-07-17-lkas-model-firmware-verification.md`; see `docs/INDEX.md` for the full V9→V38 reading order), `review-safety-redteam.md` (adversarial pre-flash review template).
- `analysis-2020accord/` — the active deep work: `build_vNN_tva.py` per version, Ghidra/decompile notes, gating-map docs, and `old_tools/` (superseded early V10-V17 build scripts + comparison tools).
- `flashing-2020accord/` — `eps-update-tva.py` (the flasher), `encode_eps.py` + `tva_sa_key.py` (container/cipher + SecurityAccess).
...
- **Sport-mode steering tightening is NOT implemented by this EPS** (confident negative, 3 independent grounds). See `memory/reference_accord_assist_curve_family_sport_mode.md`.
- **The LKAS setpoint `±15360` clamp is a DEGENERATE (flat) LERP.** Raising to 16384 = +6.71% top-end at every build tier and is EME-safe. ✅ **The former CRC BUILD BLOCKER is RESOLVED (2026-07-18) and the raise SHIPPED IN V38**: `build_v38_tva.py` now adds `(0xE4000,0xE4FFC)` + `(0xE5000,0xE5FFC)` to `TOUCHED_BLOCKS` — the first build in this kit to touch a bootloader block outside the compact `0xC6xxx` cal block. V38 patches **all 8 selector-reachable records** (sel `{0,1,3,4,6,7,8,9}`, live = sel 1 @`0xE41A8`) so the raise lands regardless of how the variant slot resolves. Chain topology survives because the linked-list page fields at `block_start-8/-6` live in the *preceding* block; the walk still finds 49/49. See `memory/reference_accord_setpoint_limit_15360_lerp.md`.
...
- `V37` (gentle-EME fix — `0xC64B8` DTC-0x49 counter disabled) is FLASHED; on-car it resolved the gentle EME with no dash-lights regression. See `docs/HANDOFF-2026-07-14-v37-dtc0x49-fix.md`.
- The gentle-EME root cause is the `STEER_STATUS`/`gp-0x6807` debounce state machine, live-inlined in `m_steer_torque_arbitration` — see `docs/HANDOFF-2026-07-14-v36-debounce-sm-root-cause.md` and `.claude/agent-memory/firmware-codepath-tracer/reference_accord_fun2a30e_steerstatus_debounce_statemachine.md`.
...
- ⚠ **GOVERNOR BULLET CORRECTION:** the cal→variable chain is now verified. `0xC6202=4762`, the exact A160 Q13 adaptive table, and both `gp-0x4f64` clamp sites are modeled in `eps_lkas_chain_model.py`. The decision not to alter it remains, but because live `z`/budget values are missing and the dynamic protection is intentional — not because the chain is unverified.
...
- `V42` — **BUILT + independently VERIFIED, NOT FLASHED. The current candidate.** V38 + **ONE BYTE**:
  ...
  **35 bytes / 14 runs total** (not 45 — ten r26 bytes were already `0x00`, e.g. 3072 = `0x0C00`).
  ...
  observable symptoms, so a null stays attributable. `docs/HANDOFF-2026-07-20-v42-state4-ratchet.md`.
  ...
  *symmetric*-clamp reading it would NOT have held; that distinction was load-bearing.
  ...
  `0xC63CC` = 0). See `memory/reference-accord-lkas-only-rate-limiter-c6194.md`.
...
`memory/reference-accord-c520c-cap-table-axis-provenance.md`.
...
**Standing operator instruction (2026-07-20): all disassembly and decompilation on this kit goes through GhidraMCP — the `mcp__ghidra__*` tools. Do NOT use radare2, rizin, `r2pipe`, or any other CLI disassembler, and do not call `analysis-2020accord/fw_inventory/decompilation/disasm_v850.py` (a CLI-disassembler wrapper, now retired).** This applies to you and to every subagent you dispatch — **prime each subagent with it explicitly**, because the default instinct is to reach for r2.
...
- 🎭 **RUN AS A PURE ORCHESTRATOR (standing instruction, 2026-07-23).** For substantive sessions the
  ...
  `memory/feedback-orchestrator-mode-delegate-verify-at-end.md`.
...
- 🔁 **DELEGATE ALL RE/DECOMPILATION TO SUBAGENTS BY DEFAULT.** The vast majority of disassembly tracing, instruction decoding, xref walking, and decompilation goes to `firmware-codepath-tracer` / `general-purpose-sonnet`, primed with `gp=0xFEDF8000`, `tp=0xBF000` and the golden model's confirmed findings. The lead steps in **only at the end, to verify**. Do NOT hand-decode opcodes inline because it "looks like just a few instructions." Standing operator instruction, restated 2026-07-19. See `memory/feedback-delegate-firmware-tracing-to-subagents.md`.
- `memory/feedback_rigorous_validation.md` — full byte diff over spot diff; ghidra before declaring victory; don't claim completion prematurely.
- `memory/feedback_operator_lived_experience_overrides_analyst_recs.md` — when the operator reports how the car feels, that overrides abstract dwell-time / theoretical concerns from analysis. Trust the seat-of-pants signal.
