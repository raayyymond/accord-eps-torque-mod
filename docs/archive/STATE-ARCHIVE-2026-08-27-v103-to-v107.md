# STATE ARCHIVE — the V103→V107 superseded blocks, moved out 2026-08-27

Verbatim, nothing edited. These are the dated `STATE.md` blocks for the sessions that ran
V103 through V107, moved out when `STATE.md` reached 160 KB against its ~150 KB working
target (hard cap 256 KB, above which `Read` truncates the tail silently).

🛑 **These are a RECORD, not an instruction.** Each was already marked SUPERSEDED in place.
The live state is `docs/STATE.md`; the per-build ledger is `docs/BUILD-LINEAGE*.md`; the
narrative is the `docs/handoffs/2026-08/` chain. Read this file only when you need the
reasoning behind a specific V103–V107 decision.

⚠ **Their "on the car" and "unflashed" lines are STALE BY CONSTRUCTION.** V108 flew on
2026-08-27; V109 and V111 are built and unflashed.
