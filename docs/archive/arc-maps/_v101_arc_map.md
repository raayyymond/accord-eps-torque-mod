...
script. New reader this pass: `analysis-2020accord/studies/ledger/ledger_v38_to_v100_bytes.py` — a one-build extension of
`studies/ledger/ledger_v38_to_v99_bytes.py` (TARGET V100, PREV_ON_CAR V99, one row added to `BUILDS`). Reproduce:

```
ACCORD_FIRMWARE_ROOT=C:/Users/dudei/Desktop/Projects/accord-firmwares
python analysis-2020accord/studies/ledger/ledger_v38_to_v100_bytes.py frozen      # §1
python analysis-2020accord/studies/ledger/ledger_v38_to_v100_bytes.py delta       # §1c
python analysis-2020accord/studies/ledger/ledger_v38_to_v100_bytes.py diff V99 V100
```
...
- ⚠ **V70 was re-cut under the same build number**, so the second cut silently overwrote the first's plain
  ...
  Open defect: `memory/accord/instruments/accord-recut-overwrites-the-previous-plain-image.md`. Fix recommended, not applied.
...
> *"A landmine catch: `docs/traces/TRACE-2026-08-13-path2-authority.md` §6–7 was still actively recommending
...
> it's heard at."* — `archive/arc-maps/_v100_arc_map.md` §2
...
> **MONOTONE-NON-INCREASING** reshape is defensible."* — `archive/arc-maps/_v99_arc_map.md` D3.C1
...
The structure is real and correctly described (`traces/TRACE-2026-08-13-v100-6ad6-and-ivar6.md:112-128`):
```
...
script. `traces/TRACE-2026-08-10-damping-axis-hunt.md:275` already recorded it: *"`0xC6ABA`/`0xC6ACA` mixer LERP
...
> 512 km/h at 64 ct/km/h."* — `docs/traces/TRACE-2026-08-10-damping-axis-hunt.md:257-260`
...
active"** — corroborated in three further places (`handoffs/2026-07/HANDOFF-2026-07-24-low-speed-steer-lockout.md:249-250`,
`memory/accord/calibration/accord-low-speed-lockout-window-c62ea.md:38`, `memory/accord/firmware/accord-fun45608-authority-slots-not-motoroff.md:45`).
...
`builds/v80_v107/build_v80_tva.py` was in when it wrote *"V80 IS NOT CLEARED TO FLY"* and flew anyway.
...
102, VIRGIN, "pre-gain deadband"**, and which `memory/reference/firmware/reference-accord-pregain-deadband-c61b8.md` files
...
(`memory/accord/builds/accord-v88-flew-grinding-fixed-command-intact.md`):
...
`memory/accord/firmware/accord-r24-r26-two-selectors-one-gate.md` establishes, from the disassembly, that **the single
...
in this file.
🛑 ⚠ **RECORD DEFECT, live:** `accord/firmware/accord-r24-r26-two-selectors-one-gate.md`'s headline (*"there IS a clean
...
| **329.8 ct** | `memory/accord/calibration/accord-damper-is-mode-table-selected.md:54` | **highway peak** |
...
| **X3** | 🛑 **Fix the recall-layer defect** — `memory/accord/builds/accord-cbe74-dose-measured-inert-wrong-mode-record.md`'s **frontmatter asserts a claim its own body refutes**, and `MEMORY.md`'s pointer repeats the stale version. The filename encodes the retracted claim | It is the exact error class `feedback-search-the-kit-before-naming-a-cause` exists to prevent |
