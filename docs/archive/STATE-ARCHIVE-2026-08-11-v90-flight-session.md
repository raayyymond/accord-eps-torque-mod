...
`memory/reference/firmware/reference-accord-gp6bbe-is-rate-derived-not-base-assist.md`) and every statement that
treats `gp-0x6b26` as an inertia term (see `memory/accord/builds/accord-v94-flew-and-the-lane-is-a-damper.md`).

---

## ⊕ SUPERSEDED HEADLINE, 2026-08-11 (V90 flight session) — retained for the reasoning, NOT for its status

Narrative: **`docs/handoffs/2026-08/HANDOFF-2026-08-11-v90-flew-and-the-lever-search-closed.md`.**
Flight scoring: `docs/scoring/SCORING-2026-08-11-v90-flight.md`.

### A. WHAT IS ON THE CAR, AND WHAT HE REPORTS
**V90**, route `00000077--7411859c54`, 21 segments, 1245.3 s, cache `_scratch/cache/r77/`.
...
`0xC520C`). **Then it was measured (`rlog-tools/studies/ratchet/v93_return_to_centre.py`, 4 routes) and it does not
bind:**
...
recommendation. **Score it with `scoring/SCORING-2026-08-11-v90-flight.md` §10.1–10.6 exactly as written.**
...
- 🛑 **`gp-0x6afe` ≡ `gp-0x6b4e` ≡ 0, ALWAYS, ON EVERY BUILD.** `gp-0x3d8c` sums `gp-0x62c8[0..10]`;
  ...
  **there is NO second, independent LKAS injection at the final stage.** **CORRECTS the starred
  `accord/firmware/accord-aggregator-reaches-motor-via-gp6acc-bridge.md` ("CAN/arbitration term") and
...
- 🛑 **`gp-0x6b4a` is a SECOND, direct, unconditional, UNWEIGHTED LKAS-descended term into
  ...
  `model/eps_lkas_chain_model.py:2318-2344` documented only the **sibling** `gp-0x6b4c`. ⊕ **Sign: term 0 is
