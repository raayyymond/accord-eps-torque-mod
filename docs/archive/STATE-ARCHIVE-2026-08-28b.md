# STATE ARCHIVE — sections retired from `docs/STATE.md` on 2026-08-28 (second split)

🛑 **A RECORD, NOT AN INSTRUCTION.** Kept verbatim so no finding or retraction is lost.
Where these disagree with `docs/STATE.md`, **STATE wins**. Split because STATE reached 228 KB,
near the 256 KB cap past which `Read` truncates the tail silently.
