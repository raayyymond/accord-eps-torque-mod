# STATE ARCHIVE — V204→V208 era, split out 2026-08-30

> 🛑 **A RECORD, NOT AN INSTRUCTION.** Split from `docs/STATE.md` when it reached **138.6 KB**,
> against a ~150 KB soft target and a 256 KB hard cap past which a file loads with its tail
> **silently truncated**. Everything here is superseded: the flight candidate is **V222**, the
> ladder is V223–V227, and the notch has been replaced twice since (V217, V222).
> The **standing closures** from this era are summarised back in `STATE.md`; the narrative,
> the numbers behind each one, and the retractions live here.
