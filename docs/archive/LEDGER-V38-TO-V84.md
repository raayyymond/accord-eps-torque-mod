...
> **Method for Part 1** [EVIDENCE]: **`analysis-2020accord/studies/ledger/ledger_v38_to_v84_bytes.py`** (landed in the
...
`docs/handoffs/2026-07/HANDOFF-2026-07-18-v39-opposing-torque-rate-guard.md` — the session immediately after V38 flew. All
30 prior handoffs are silent on all three. `memory/builds/v44-built-handsoff-damping.md` already says it outright:
...
encodings, reproducing `memory/reference/firmware/reference-accord-c646c-shared-gain-not-lkas-only.md` as a 4th independent
...
| **D2** | **`0xC644A` on V43 is 32, not 64**, in `BUILD-LINEAGE.md`, `memory/builds/v43-dirty-derivative-pole-built.md` and the V43 handoff. The image says 32; `builds/v18_v49/build_v43_tva.py:195 POLE_NEW = 32`; V44 reverts `32 → 1024`. **64 is V49's value** | byte read `_v43_plain_image.bin[0xC644A]` |
...
| **D6** | **"The ×1.5 friction was introduced by V73, not V74"** (`…v80…` §7) is misleading in the direction that matters for causation. `builds/v50_v79/build_v73_tva.py:191-193` writes it at **`FRICTION_MODE = 10` only** ⇒ inert; `builds/v50_v79/build_v74_tva.py:77` writes it on the 13 engaged modes ⇒ **V74's copy is the live one** | build scripts + the mode-dereferenced byte read in Part 1.3 |
...
| **D9** | Route `5d`'s raw rlogs are **missing** — every V74 conclusion runs against a cache. Route 68's scoring code and cache live in a session scratchpad, not `rlog-tools/`. `probe/decode_v74_probe.py` / `probe/decode_v75_probe.py` **do not exist** despite being referenced by their own build scripts | filesystem |
