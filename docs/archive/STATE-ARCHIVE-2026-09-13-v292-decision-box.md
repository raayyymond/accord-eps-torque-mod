# ARCHIVE — the superseded `docs/STATE.md` decision box of 2026-09-13 (V292 BUILT and CLEARED)

**A RECORD, NOT AN INSTRUCTION.** Moved out of `docs/STATE.md` at the 2026-09-13 close-out, when V292
FLEW on routes `…0000006d`/`…6e`/`…6f` and came back a **REVERT by its own pre-registration**. Nothing
here was retracted by the move; what is still decision-bearing is folded into the live box in
`docs/STATE.md`. It is kept verbatim because it is the pre-flight statement the flight was scored
against — in particular the predicted ring ×0.55/×0.71, the predicted −14° phase at 10 Hz and the
pre-registered revert signatures, **all of which the wire then falsified or fired.**
