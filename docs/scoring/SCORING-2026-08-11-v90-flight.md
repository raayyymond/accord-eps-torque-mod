...
| artefact | path |
|---|---|
| extractor | `rlog-tools/decode/extract_r77.py` → `analysis-2020accord/_scratch/cache/r77/` |
| scorer | `rlog-tools/probe/decode_v90_probe.py` (`d1` `d3` `d4` `d5` `d6` `d6b`) |
| logs | `rlog-tools/{extract_r77,d1_v90,d3_v90,d4_v90,d5_v90,d6_v90,d6b_v90}.log` |
| JSON | `analysis-2020accord/_scratch/cache/r77/v90_d{1,3,4,5,6,6b}_*.json` |
...
### 4.1a ★ EXTENDED TO 35 Hz, 2026-08-11 — `rlog-tools/studies/impedance/v92_rez_extend.py`, log `_scratch/logs/v92_rez.log`
...
Taps copied from `traces/TRACE-2026-08-10-damping-axis-hunt.md` §1, **not re-derived** (`0xC643C`=37,
...
1. **🛑 `raw14` off-by-one — FIXED PROPERLY, and the obvious fix is WRONG.** `D.extract` appends to
   ...
   **constant lead** (= 1 on r77). `decode/extract_r77.py::_row2raw14` derives it and asserts it elementwise
...
- **(b) LINE — the sharper detector, and the one that matches what V80 actually was.**
  ...
  `rlog-tools/score/v91_prereg_threshold.py`, read-only on the frozen `_scratch/cache/r77`.
...
`rlog-tools/studies/ratchet/v93_return_to_centre.py` (new, read-only) · log `_scratch/logs/v93_rtc.log` · JSON
`_scratch/cache/r77/v93_return_to_centre.json`. Four routes: 77 (V90), 73 (V88), 75/76 (V89).
...
`memory/reference/measurement/reference-accord-rate-scale-4p7121-stands.md`: **`gp-0x6ac0` = |column °/s| × 4.7121**,
...
⊕ **This decodes the prior claim's notation exactly.** `research/FEASIBILITY-8X-LKAS.md` Part 2 says *"V38
...
`research/FEASIBILITY-8X-LKAS.md` contains both of these:
