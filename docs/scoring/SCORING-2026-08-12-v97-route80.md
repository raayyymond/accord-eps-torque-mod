...
**Data:** `75604b0a432fdc89_00000080--6c8b103892--{0,1}--rlog.zst` → `analysis-2020accord/_scratch/cache/r80/`
**Extractor:** `rlog-tools/decode/extract_r80.py` (imports `extract_r7d` → `decode_v84_probe_r6d`; the
instrument is NOT reimplemented — same code that wrote every cache since `_scratch/cache/r6d/`).
**Scorers:** `rlog-tools/studies/v97-r80/v97_r80_census.py`, `studies/v97-r80/v97_r80_vs_v96.py`, `studies/v97-r80/v97_r80_phase.py`,
`studies/v97-r80/v97_r80_matched.py`. Machine-readable: `_scratch/cache/r80/r80_census.json`, `r80_vs_v96.json`,
...
`builds/v80_v107/build_v96_tva.py` made "a first-class reported output" is **0.0000**.
...
⚠ A first pass (`studies/v97-r80/v97_r80_phase.py`) reported coherence 1.000 across the board — an artefact of
`nperseg == len(window)`, i.e. a single Welch segment. Fixed in `studies/v97-r80/v97_r80_matched.py`; the numbers
...
Scorer: `rlog-tools/studies/v97-r80/v97_r80_override_and_1023.py` → `_scratch/cache/r80/r80_override_and_1023.json`.
...
⊕ **I verified the ceiling independently**, from `builds/v80_v107/build_v96_tva.py:128`'s own no-clip proof: the
...
| **The cave's regressor** — the cell the pole directly filters | `M` pinned at 0 on **100 %** of route 80, and `Mlo` duty **exactly 0.0000** (vs 77 / 29 firings on 7e / 7f). Zero residual leverage — the one bit that occasionally fired on V96's routes never fired here. **Known void before the drive:** `builds/v80_v107/build_v97_tva.py:99-100` concedes it in the build script itself. | EVIDENCE |
