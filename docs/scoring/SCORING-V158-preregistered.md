...
Use **`rlog-tools/score/score_band_excess.py`**. 🛑 **NOT `score_v133_creep.py`** — audited 2026-08-29 and its `boot()` is a **WINDOW** bootstrap (`rng.choice(e[:, i], len(e))`), which this kit’s standing rule forbids; measured **2.6x too confident** on synthetic data. The corrected scorer resamples EPISODES. Requirements it already satisfies:
