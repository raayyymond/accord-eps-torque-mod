...
Data: `_scratch/cache/r75/` (16 segments) and `_scratch/cache/r76/` (13 segments), extracted by
`rlog-tools/decode/extract_r75_r76.py`. Comparison arm: `_scratch/cache/r73/` (V88), which already existed and was
**not** re-extracted. Scripts: `analysis-2020accord/studies/sessions/v89/v89_e1_identity.py`, `studies/sessions/v89/v89_e2_h2h3.py`,
`studies/sessions/v89/v89_e3_contrast.py`, `studies/sessions/v89/v89_e4_inertia.py`, `studies/sessions/v89/v89_e5_t2contrast.py`, `studies/sessions/v89/v89_e6_bar_impedance.py`.
Logs and JSON alongside them in `_scratch/cache/r75/`.

---

## 0. Method provenance — the instrument was not reimplemented

`decode/extract_r75_r76.py` adds rows to `decode_v84_probe_r6d.ROUTES` and calls that module's
`extract()` / `split()`, exactly as `decode/extract_r6e.py` / `decode/extract_r6f_r70.py` / `decode/extract_r71.py` /
`decode/extract_r73.py` do. Field names, ZOH/interp convention, IMU axis pick, sentinel definition and
...
`studies/sessions/v89/v89_e1_identity.py` runs four parameter-free statistics with route 73 as the measured control:
...
`studies/sessions/v89/v89_e6_bar_impedance.py`. Channel: `rate_f` = STEER_ANGLE_RATE from **0x18F — the same CAN frame
...
`studies/sessions/v89/v89_e5_t2contrast.py`, 1,229 engaged windows over all three routes. `log e_6-9` and the 32–38 Hz
...
Requested by the orchestrator after the interim. Scripts: `analysis-2020accord/studies/sessions/v89/v89_e7_rung_and_zero.py`,
`studies/sessions/v89/v89_e8_placebo_and_dose.py`, `studies/sessions/v89/v89_e9_placebo_fix.py` → `_scratch/cache/r75/v89_e{7,8,9}_*.json`.
...
```
builds/v80_v107/build_v86_tva.py:406   BIT_SIGN, BIT_NONZERO, BIT_MAG, BIT_GATE, BIT_FINGERPRINT = 0x80, 0x40, 0x20, 0x10, 0x08
builds/v80_v107/build_v86b_tva.py:219  BIT_SIGN, BIT_MAG, BIT_NONZERO = 0x80, 0x40, 0x20   # b6 = MAG, b5 = NONZERO -- SWAPPED vs V86
```
...
turning**. Scripts: `analysis-2020accord/studies/sessions/v89/v89_f1_selfinterference.py`, `studies/sessions/v89/v89_f2_requests.py` →
`_scratch/cache/r75/v89_f1_selfinterference.{log,json}`, `v89_f2_requests.json`.
...
`_scratch/cache/r75/v89_f2_requests.json` so a different screen can be applied without re-deriving it.
