...
four codes allocated and `b3`'s two constant values are already claimed. `builds/v80_v107/build_v103_tva.py`'s own
...
**VERDICT: PASS.** Written to `analysis-2020accord/_scratch/cache/r9e/r9e_identity.json`.
...
`rlog-tools/studies/impedance/rez_control.py` pins the frozen `decode_v90_probe` estimator to **0.00 % on all ten
bands** against `_scratch/logs/v92_rez.log`'s published route-77 table (221/221 windows). **Passed before scoring.**
...
> direction. **This is exactly what `docs/scoring/DRIVE-CARD-V103.md` wrote down before the drive:**
...
`memory/accord/firmware/accord-gp6b4c-is-an-11-slot-assist-sum.md` records V98 (route `0x81`) measuring this at
...
> (`memory/accord/mechanism/accord-the-8x-gain-is-the-carrier.md`). **That trade must not be made without re-scoring
> `f0`.**
>
...
**Cache — `analysis-2020accord/_scratch/cache/r9e/` (20 MB, 107 fields, 64,776 rows @ 101.15 Hz)**
...
Schema is **verbatim** the `extract_r7d` / `_scratch/cache/r6d`-onward schema, plus:
...
| file | what it does |
|---|---|
| `decode/extract_r9e.py` | route registration + extraction + derive + identity + `lane427` (modelled verbatim on `decode/extract_r96_r97.py`) |
| `lib/v103_r9e_lib.py` | shared loader, masks, **episode-block bootstrap**, `f0_of`, band-RMS |
| `studies/v103-r9e/v103_r9e_census.py` | fault / identity / exposure / command census → `_scratch/out/_v103_r9e_census.json` |
| `studies/v103-r9e/v103_r9e_f0.py` | the endpoint, all four arms, covariate, split-half, bands, sign map → `_scratch/out/_v103_r9e_f0.json` |
| `studies/v103-r9e/v103_r9e_symptom.py` | regime band-RMS, top-window localisation, coherence, envelopes → `_scratch/out/_v103_r9e_symptom.json` |
| `studies/v103-r9e/v103_r9e_symptom2.py` | ⚠ **supersedes** parts A/C/D2 of the above (two fixed defects) + speed-matched `f0` |
| `probe/v103_r9e_cave.py` | rung duties, `b7`+427 signed lane, `b3` first pass → `_scratch/out/_v103_r9e_cave.json` |
| `studies/v103-r9e/v103_r9e_b3.py` | `b3` with a proper null, aliasing audit, 427 under-range → `_scratch/out/_v103_r9e_b3.json` |
| `studies/v103-r9e/v103_r9e_final.py` | run-length null, band-share CI, cross-route `b7`, rail duty by regime → `_scratch/out/_v103_r9e_final.json` |
...
1. **Masking on an instantaneous quantity (wheel rate) and then demanding a contiguous window
   ...
   median. `studies/v103-r9e/v103_r9e_symptom.py` parts A(rate)/D/D2 hit this and returned empty tables.
