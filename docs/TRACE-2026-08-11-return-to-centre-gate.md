...
`memory/accord-aggregator-reaches-motor-via-gp6acc-bridge.md`) reduces to `iVar45 = uVar34` — there is
...
return-centre). This corrects both `memory/accord-aggregator-reaches-motor-via-gp6acc-bridge.md`'s
...
`docs/FEASIBILITY-8X-LKAS.md` (2026-08-06), both independently verified and now cross-checked against
...
- `FEASIBILITY-8X-LKAS.md` (item 10, independently derived): **"even at TODAY's 4x, moderately fast
...
- `0xC6CD0` = 3564 (4x LKAS forward-path gain) is CONFIRMED live on the current build (`build_v90_tva.py`
...
- **Per `FEASIBILITY-8X-LKAS.md`'s own prior finding, the aggregator's ±0x2800 total clamp and the
...
(`FEASIBILITY-8X-LKAS.md`). **If this reading holds, the fix is closer to a revert (lowering `0xC6CD0`
...
2. Quantify how much of the operator's felt restriction (if any) would be removed by reverting `0xC6CD0`
   ...
   — DTC-0x1d lockstep and the hard-DTC monitor both sit near this stage per `FEASIBILITY-8X-LKAS.md`
