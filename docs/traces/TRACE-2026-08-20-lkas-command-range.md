...
(`builds/v80_v107/build_v101_tva.py:143`). It reproduces every flown pair exactly:
...
**What the build's assertion actually constrains** (`builds/v80_v107/build_v101_tva.py:418-420`,
...
is the documented limit-cycle coupling in `docs/research/FEASIBILITY-8X-LKAS.md` §2.0/§2.4. **Widening the wire
does not touch it.**
...
policy (`memory/feedback/builds/feedback-no-openpilot-side-modifications.md`).
...
| openpilot side | `CarControllerParams.STEER_MAX` 4096 → 32760 (`interface.py:114-115` `lateralParams.torqueBP/torqueV`) — **barred by `memory/feedback/builds/feedback-no-openpilot-side-modifications.md`** |
...
4. **Is 1 arb count actually below the motor's own resolution floor?** The FOC current loop and its
   ADC/current-sense scaling were not traced (still OPEN from `research/FEASIBILITY-8X-LKAS.md` item 1). If the
