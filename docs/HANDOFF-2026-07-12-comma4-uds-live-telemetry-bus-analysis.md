...
**Builds on** `HANDOFF-2026-07-11-starpilot-eps-telem-rlog-analysis.md` (the fork's in-drive UDS poller) and the
...
the new `--obd` flag) + `comma4_can_inventory.py` + `sniff_can_id.py`. Results (`inventory_scan_20260712.txt`):
...
- `inventory_scan_20260712.txt` (operator-captured) — the on-car sweep + bus inventory this analysis rests on.
