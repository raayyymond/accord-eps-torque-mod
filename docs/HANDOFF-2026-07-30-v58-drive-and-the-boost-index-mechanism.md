...
## 5. 🛑 The trace: `build_v58_tva.py` was wrong, and correcting it produced the mechanism

`build_v58_tva.py` described `gp-0x6b9a` as *"the FIR chain's output, indexing boost's NON-flat table
...
the dumped curves; corrected in `build_v59_tva.py`, the decoder, `STATE.md` and `BUILD-LINEAGE.md`.
...
Decoder: `rlog-tools/decode_v59_boostindex.py`. It **hard-stops** above 1% non-monotonic frames rather
