...
Read alongside the primary deliverable `docs/GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md` and the prior
...
`docs/GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md` — exhaustive enumeration of every point LKAS torque can be
...
- Live output archived: `analysis-2020accord/can-scans/2026-07-07-comma4-live-can-inventory-carrunning.txt`
...
- Once a telemetry frame is flashed, decode via cabana (`--dbc`, live `--zmq <ip>` or a route), plotjuggler, or a
  Python rlog decoder (extend `analysis-2020accord/analyze_telem_0x660.py`). openpilot logs all bus-1 IDs, so a
...
- **Code cave**: `0xC4E00–0xC4FEF` is a ~528-byte `0xFF` run inside CRC block `[0x13000,0xC4FFC)` (CRC @0xC4FFC,
  auto-recomputed by the builder). Guarded 0xFF in `build_v31t_tva.py` (`CAVE_GUARD`). Unreferenced.
...
- Build infra: `build_v31t_tva.py` is the clean base (V31 cals + CRC bookkeeping + guards + readback asserts).
...
| tools | `tools/comma4_can_inventory.py` (read-only inventory), `analysis-2020accord/analyze_telem_0x660.py` (decoder base) |
