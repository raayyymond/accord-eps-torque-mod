...
|---|---|---|
| V31P / V31T / V31U | first telemetry at all — gate-firing flags into `0x14A` spare bits; a `0x660` piggyback; UDS DID `0x4801` | `builds/v18_v49/build_v31p_tva.py`, `builds/v18_v49/build_v31t_tva.py`, `builds/telemetry/build_v31u_uds_telem_tva.py` docstrings |
| V53–V61 | telemetry probes + lane mutes | `builds/v80_v107/build_v96_tva.py` arc summary; `docs/BUILD-LINEAGE.md` |
...
| V93/V94 | the `0xCBE74` CUT — **aborted on-car** | `docs/handoffs/2026-08/HANDOFF-2026-08-12-v94-aborted-and-the-override-regime.md` |
| V96 | **the first build in the arc to telemeter a TRANSFER rather than a SIGNAL** — both ends of one nonlinearity, so the slope between them is the measurement | `builds/v80_v107/build_v96_tva.py` docstring |
| V97 | V96's cave carried byte-for-byte; one calibration byte | `builds/v80_v107/build_v97_tva.py` docstring, lines 99–101 |
...
- **(a) Who wrote `0x14A` byte 7?** **V92 (first ever), V96, V97 — and nobody else.** Every cave from
  V53/V54 through V91 wrote **byte4[7:3] and nothing else** (`builds/v80_v107/build_v92_tva.py`: *"It is the FIRST build
...
- **(b) 427 `MOTOR_TORQUE` repointing, V87 →:** `gp-0x6c18` (stock) → **V87** `gp-0x6b98` (`6894`,
  ...
  against V90 and **wrong against V92** — `builds/v80_v107/build_v96_tva.py` flags this explicitly.
...
|---|---|---|---|
| **V88** | `0x14A` byte4 `b7` = **sign** of `gp-0x6b98`, at 100 Hz, paired with the 427 magnitude | signed `cmd`↔column: `\|tq/cmd\|` **6.24** at 7.79 Hz, phase **−30.9°**, coherence² **0.009 (rectified) → 0.343 (signed)** | **The sign bit turned a yes/no probe into a transfer-function measurement.** It closed the fork: signed ≈ rectified ⇒ *"rectification was NEVER hiding a line; V87's null was CORRECT"* ⇒ **the ratcheting is NOT a tone the EPS commands** — no notch, no phase lever at 7.79 Hz. Cited: `docs/STATE.md` §2 (V88 block); ⚠ **auto-memory root**: `…/memory/accord/builds/accord-v88-flew-grinding-fixed-command-intact.md` |
...
|---|---|---|---|---|
| **V64** | the dwell/ratchet detector's own cells | the cal edits were never in force | **GATE-NEVER-ARMED** — *"the null is on the GATE, not the hypothesis"*; the detector never armed | `memory/accord/builds/accord-v64-null-is-on-the-gate.md`; `docs/handoffs/2026-07/HANDOFF-2026-07-31-v64-the-null-is-on-the-gate.md` |
| **V68** | the detector cell | reads zero | **NO-POSITIVE-CONTROL** — *"the cell has NEVER been non-zero on any build ⇒ its writer and enable gate are OPEN"* | `memory/accord/builds/accord-v68-detector-still-zero-no-positive-control.md` |
| **V69** | three probe rungs | all three failed | **SIZED-AGAINST-THE-WRONG-THING** — rungs sized against a downstream gate's width, not the lane's own reachable output. *"V69 spent all three rungs for nothing."* | `memory/feedback/measurement/feedback-size-probe-rungs-against-lane-reachable-output.md`; `docs/BUILD-LINEAGE.md` GATE 3 |
| **V69 / V70** | the r24 dose ladder | byte-stock | **WRONG-MODE / RULE 7** — the builds wrote mode-10 `gain_B`; the car is TVCA4 (modes 24/26) ⇒ *"the r24 dose ladder NEVER EXISTED"* | `memory/reference/firmware/reference-accord-car-is-tvca4-mode-24-26.md` |
| **V87 (and V90's `b4`)** | `gp-0x6b98` magnitude / `gp-0x6c00<0` | under-ranges to ~1.5 bits / duty **0.000000 railed** | **UNDER-RANGED** and **RAILED-DEAD**. *"a usable SPECTRAL probe, but amplitude claims do NOT travel."* A duty-0 or duty-1 rung carries zero information | `memory/accord/instruments/accord-probe-underranges-to-one-bit-comparator.md`; `docs/specs/SPEC-2026-08-11-telemetry-budget.md` T2 |
| **V91 / V92** | 427 ← `gp-0x6b26` as the dose-in-force channel for the `0xCBE74` ×1.5 dose | engaged stratified ratio **0.99 [0.91, 1.26]** against a pre-registered **1.50** | **INSTRUMENT-CANNOT-MEASURE-ITS-OWN-DOSE** — `gp-0x6b26 = K·α` and α is *what K damps*, so in a stable closed loop the **product is invariant to K**. *"Nobody asked whether the instrument could measure the thing it was pointed at."* | `memory/accord/builds/accord-cbe74-dose-measured-inert-wrong-mode-record.md`; `docs/HANDOFF-2026-08-12-v94...` §1.5 |
| **V92 / V93** | — | V93's flight was wasted because **V93's own instrument could not see V93's edit** | same class, restated as a build rule | `builds/v80_v107/build_v96_tva.py` SIZING section |
...
| **all 427 magnitude probes** | rectified `\|x\|` on `0x1AB` at 50 Hz | — | **ALIASED** — a rectified channel folds on `2f` and the law is `\|2f − 50·round(2f/50)\|`, **not** `f mod 25`. 26/29/31 Hz fold to **2/8/12 Hz** ⇒ a rectified 427 magnitude probe exposes **2–12 Hz, not 19–24**, and cannot separate a genuine 2–12 Hz line from a 26–31 Hz image | `memory/reference/can/reference-accord-427-is-rectified-and-folds-26to31-into-2to12hz.md` |

### 1.4 The channel facts, as they stand (inherited EVIDENCE, re-verified where noted)

| fact | status |
|---|---|
| The CAN TX gateway is a **WHITELIST** — only `0x14A`, `0x18F`, `0x1AB` cross. **A new CAN ID can NEVER reach openpilot.** | `memory/accord/signals/accord-can-tx-gateway-whitelist-and-20-free-bits.md` — inherited, not re-derived here |
| `0x14A` byte4[7:3] = 5 bits @ 100 Hz — in use since ~V53, ~50 flown builds | `docs/specs/SPEC-2026-08-11-telemetry-budget.md` T1 |
| `0x14A` byte7[7:6] = 2 bits @ 100 Hz — **first written by V92**; V96/V97 use both | `docs/BUILD-LINEAGE.md` V92 row; `builds/v80_v107/build_v96_tva.py` |
| `0x1AB`/427 `MOTOR_TORQUE` = 10-bit unsigned magnitude @ **50 Hz** (Nyquist 24.91 Hz), source at `0x55DF2`, scale at `0x55E10` | `builds/v80_v107/build_v96_tva.py`; `docs/HANDOFF-2026-08-12-v94...` §6b |
| `0x18F` has **6 clean spare bits** (byte4[2:0] + byte5[7:6] + byte6[6]) with zero DBC overlap — but **ZERO FLIGHTS**; a second builder hook is a first-flight risk class | `docs/specs/SPEC-2026-08-11-telemetry-budget.md` T1 + ADDENDUM 3 |
| The checksum (`FUN_00057b24`) is the **last** step in every builder ⇒ spare bits are auto-covered | `docs/specs/SPEC-2026-08-11-telemetry-budget.md` T1 |
| CAN-TX base tick **100 Hz** (not 62.5); control task **~1000 Hz** | `memory/accord/signals/accord-can-tx-100hz-base-tick-and-gateway.md`; `memory/misc/control-task-tick-confirmed-1khz.md` |
...
(`FUN_0002214a`) — `builds/v80_v107/build_v96_tva.py` GATE 2 (2): *"the cave hangs off the 100 Hz CAN-TX builder, not
...
in `builds/v80_v107/build_v97_tva.py`'s `FROZEN` table. A measured share tells you whether that is a live field or a
spent one.
...
(`docs/handoffs/2026-08/HANDOFF-2026-08-12-v97-the-loop-pole.md` §8.6). The first half is right; **the conclusion is not.**
...
coh² 0.804) — the regime the operator says is unfixed (`memory/accord/mechanism/accord-rez-antidamping-replicated-three-drives.md`).
...
11-lane table (`docs/specs/SPEC-2026-08-11-telemetry-budget.md` ADDENDUM) ranks r24/r26 and `gp-0x6bbe` as the
...
(`memory/accord/firmware/accord-aggregator-lane-mirrors-6ada-6adc.md`).
...
`memory/accord/mechanism/accord-ratchet-is-a-saturated-resonance.md` (the repo: LKAS-gating at p = 1.09e-08, symmetric
waveform ⇒ not stick-slip, **and the "saturated" half re-framed to BELIEF on 2026-08-04**) and
`~/.claude/projects/…/memory/accord/mechanism/accord-ratchet-is-a-lightly-damped-resonance.md` (auto-memory: the
...
**+7.82° at 7.79 Hz** (`builds/v80_v107/build_v97_tva.py` line 21). ⇒ **Compare `f0` and the ring-down ζ on route `80`
...
Three candidates died and *"the field is empty"* (`docs/handoffs/2026-08/HANDOFF-2026-08-12-v97-the-loop-pole.md` §8.1).
...
1. **🛑 `gp-0x6b70` is NOT always clamped to ±8192.** [EVIDENCE, `disassemble_function(0x38148)`]
   ...
   `builds/v80_v107/build_v96_tva.py`'s no-clip argument (*"the cell is HARD-CLAMPED to ±8192 by `0xC6200`"*) holds only
...
**[EVIDENCE]** Route `0x80`'s 427 channel, read from `_scratch/cache/r80/r80.npz` `ab_mt`, 5,375 frames
...
- **⭐ The better fix — DONE, not deferred.** The Stage-2 LERP is **100 % flash-derived**
  ...
  `analysis-2020accord/sessions/v97/read_ram_lerp_provenance.py` already reads it. See §3.2b below: I ran it and
...
**[EVIDENCE] Step 1 — the cave field on route `0x80`, read from `_scratch/cache/r80/r80.npz`
(`raw14_b4`, `raw14_b7`):**
...
**[EVIDENCE] Step 2 — the LERP's actual knots**, from `sessions/v97/read_ram_lerp_provenance.py` on stock
...
`builds/v80_v107/build_v96_tva.py` rejected a `gp-0x67fa` state-gate rung on **buildability**, arguing it would need to
...
- **V96 and V97 — the builds actually on the car** — hard-wire byte7 `b6` ≡ 1 (`builds/v80_v107/build_v96_tva.py`
...
- Base `0xC4B34`; extent `CAVE_BASE .. CAVE_FREE_END = 0xC4B34 .. 0xC4FF0` = **1212 B free**
  (`builds/v80_v107/build_v96_tva.py`). V96's payload is **112 B / 43 instructions**.
...
operator's own named crux (`docs/handoffs/2026-08/HANDOFF-2026-08-12-v97-the-loop-pole.md` §1). 🛑 **Not a mechanism.**
...
- **BOOTSTRAP OVER EPISODES, never over windows** (`memory/feedback/measurement/feedback-episodes-not-windows.md`).
...
**[EVIDENCE]** Read directly from `analysis-2020accord/_scratch/cache/r80/r80.npz` (10,749 rows, `cc_lat` for
...
| TERTIARY — hands-off returns | **matched engaged/disengaged hands-off returns from similar starting angles**, disengage with the **cancel button** — never by grabbing the wheel or braking | 🛑 **≥ 25 episodes per arm.** 11 vs 7 could not resolve a 2.7× effect (fold-width **3.27×**). Config: `analysis-2020accord/sessions/v97/rtc_measure.json` → `config`; scorer `rlog-tools/studies/ratchet/v97_return_to_centre.py` | ⚠ **Not on a short drive. Do not promise it** |
...
`docs/STATE.md` §A10, and `sessions/v97/read_ram_lerp_provenance.py` already reads it); `0xC63AE` = 1024 so the
...
`+78.0°` on two independent routes** (`builds/v80_v107/build_v97_tva.py` lines 36–43). **Reproducing +78° ± a sensible
...
> (`docs/specs/SPEC-2026-08-11-telemetry-budget.md` T1), and the hook is structurally identical to `0x14A`'s
...
(`docs/specs/SPEC-2026-08-11-telemetry-budget.md` T1). With them, this build could carry, additionally:
...
1. 🛑 **`builds/v80_v107/build_v96_tva.py`'s 427 no-clip argument is incomplete.** *"The cell is HARD-CLAMPED to ±8192 by
...
2. 🛑 **`docs/handoffs/2026-08/HANDOFF-2026-08-12-v97-the-loop-pole.md` §8.6 — `sign(gp-0x6752)` is described as not
...
3. 🛑 **`builds/v80_v107/build_v96_tva.py`'s rejection of the `gp-0x67fa` rung over-states the buildability problem.**
...
6. ⚠ **Citation-root hazard, checked mechanically while writing this spec.** I verified all 18
   ...
   *"four resolve only into the auto-memory directory, which is a separate root"* defect the V94 handoff
   ...
   `memory/accord/mechanism/accord-ratchet-is-a-saturated-resonance.md` and the auto-memory
   `accord/mechanism/accord-ratchet-is-a-lightly-damped-resonance.md`. They are complementary, but the repo copy carries
...
7. ⚠ **Not verified by me, flagged for the record**: `docs/specs/SPEC-2026-08-11-telemetry-budget.md` ADDENDUM
