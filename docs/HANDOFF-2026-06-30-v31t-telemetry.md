...
**Bases:** `gp = 0xFEDF8000`, `tp = 0xBF000`. Builds on `docs/HANDOFF-2026-06-30-sensorA-identity-gate-scale.md`
...
`analyze_telem_0x660.py` → pick `0xC6312` → build V32.
...
## 2. V31T = `build_v31t_tva.py` — the edit
...
## 3. Decoder — `analyze_telem_0x660.py`

`python analyze_telem_0x660.py <route>--rlog.zst [--src N] [--id 0x660]`. Decodes 0x660 (LE words) +
