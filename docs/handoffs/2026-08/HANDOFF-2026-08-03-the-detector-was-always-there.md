...
Read alongside `docs/STATE.md`, `docs/BUILD-LINEAGE.md`, `docs/specs/design/V66-V67-DESIGN.md` and the predecessor
`handoffs/2026-08/HANDOFF-2026-08-02-v67-flew-and-the-highway-grind-is-not-the-rate-lane.md`.
...
**(c) `gp-0x6a5e` is voted VEHICLE SPEED**, relabelled in `model/eps_lkas_chain_model.py` and at
...
**(e) `verify/diff_build_vs_stock.py` was emitting false positives** that made a genuine stray edit
indistinguishable from noise. Fixed, with a `--self-test` that proves it can still fail.

**(f) The retracted "bus = 8 × deg/s" struck from `builds/v50_v79/build_v68_tva.py`.**
...
same events**. Reproduce with `analysis-2020accord/studies/grind2/grind2_trichannel.py`; every number below is
printed by that script and stored in `_scratch/out/_grind2_trichannel.json`.
