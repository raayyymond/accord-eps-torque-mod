...
Read alongside `docs/specs/design/V66-V67-DESIGN.md` (the full design), `docs/STATE.md` (current state) and
...
`analysis-2020accord/studies/models/rate_lane_frequency_response.py`.
...
- ⚠ **`builds/v50_v79/build_v62_tva.py`'s `GAIN_B_LERP_MODE10` tripwire watches only 2 of the 4 records** — blind to an
...
it left a `0x87` decode ambiguity sitting under a build decision. **`analysis-2020accord/lib/route_build_registry.py`**
...
⇒ **V67 does not have to wait for V66's drive.** Reproduce with `analysis-2020accord/verify/validate_gp6806_gate.py`.
🛑 **The lesson is where to look.** This sat in `_scratch/cache/r28`/`_scratch/cache/r29` for a month while every
