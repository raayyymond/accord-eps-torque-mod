...
2. **`memory/accord/firmware/accord-dtc-0x18-hard-eligible-cadence-watchdog.md`** — DTC `0x18` **requires a prior MCU reset**;
...
4. **`reference/firmware/reference_accord_soft_eme_bound_arm_gating.md`** — the COMP ceiling **2560 is at `0xC67DC`**, not
...
`builds/v50_v79/build_v77_tva.py`. Caches: `_scratch/cache/r5e/`, `_scratch/cache/r5e_sym/`. Plots: `analysis-2020accord/figures/aggregator/v78_*.png`.

⚠ **Two traps for the next session.** `probe/decode_v75_probe.py` / `probe/decode_v74_probe.py` **do not exist** despite
...
**`studies/sessions/v74_v75/v75fault_final.py` / `studies/sessions/v74_v75/v75fault_buschange.py` use a strict index split and supersede them.**
