...
`rlog-tools/studies/identification/plant_phase_corner.py`, `studies/identification/plant_Jb_absolute.py`, `studies/identification/plant_falsifiers.py`.
...
`rlog-tools/studies/damping-q/ringdown_validate.py`. f_n = 8.16 Hz, 40 reps/cell, SNR 20 dB, truth ζ 0.005→0.200 (40× span).
...
⇒ 🛑 **`rlog-tools/_scratch/out/_stock_r97_ringdown.json` is UNQUOTABLE.** Route 0x97 has **exactly one qualifying
...
`studies/identification/plant_scale_resolve.py`. `ang == wang == cs_ang` bit-for-bit ⇒ its degree scale is the DBC's. At
...
`memory/accord/signals/accord-427-source-cell-changes-by-build.md`. **Only r71/r73/r75/r76 (V87–V89) carry
...
**Pre-registration written before any drive:** `rlog-tools/score/prereg_maneuver_hs.py`.
...
5. **`_scratch/out/_stock_r97_ringdown.json`** — unquotable (§3.1).
...
9. **`rec2`'s `studies/identification/plant_fit_final.py` `J_w`/`f_n`** — withdrawn by its author (R² 0.01–0.32, negative J²).
...
- **Every decision-bearing claim in this handoff was reproduced by the orchestrator**, not relayed:
  the `carcontroller.py` normalisation, the measured slew, `studies/identification/plant_phase_corner.py`, `studies/identification/plant_Jb_absolute.py`,
  `studies/identification/plant_falsifiers.py`, and the `tan(180−a)` arithmetic.
