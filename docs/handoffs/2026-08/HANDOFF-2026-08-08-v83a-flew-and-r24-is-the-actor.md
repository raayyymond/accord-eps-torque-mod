...
`handoffs/2026-08/HANDOFF-2026-08-08-v81-flew-and-the-aggregator-reaches-the-motor.md`.
...
| decoder | `analysis-2020accord/studies/probes/decode_v84_probe.py` |
...
| 2 | **Route `5d`'s raw rlogs are missing** (V74's clean symptom-measurement flight, 17 segments) while `extract/extract_r5d_cache.py` is its canonical extractor and `BUILD-LINEAGE.md` leans on that cache. **Every V74 conclusion runs against the cache, not the log.** | `analysis-2020accord/rlogs/` |
...
| 5 | **`0xC61B2`/`0xC61B4` are mislabelled as a "pre-gain deadband arm".** They are the **arbitration / LKAS output clamps**, and they have been at **4× Honda (2048) since V38**. Comment-only; changes no byte. | `builds/v80_v107/build_v83a_tva.py:359-360`, `builds/v80_v107/build_v84_tva.py:544-545` |
| 6 | **RULE 9's ~90 s grind-#2 exposure figure is optimistic by ~1.8× and self-inconsistent.** See §7b. | `docs/BUILD-LINEAGE.md` RULE 9 |

⊕ Route 68's scoring code and cache (`_scratch/cache/r68x/`, `score_r68.log`) currently live in the session
...
`analysis-2020accord/model/eps_lkas_chain_model.py`, eight corrections:
...
3. **Promote route 68's scoring code out of the scratchpad** into `rlog-tools/`, alongside a
   `studies/probes/decode_v84_probe.py` run, or route 68's numbers become another cache-only result like route `5d`'s.
