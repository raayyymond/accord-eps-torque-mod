...
405 MB, into `analysis-2020accord/rlogs/`. Extracted with `rlog-tools/decode/extract_r78_r79.py`, which adds
rows to `decode_v84_probe_r6d.ROUTES` and calls **that module's** `extract()`/`split()` — the same
instrument that wrote every cache since `_scratch/cache/r6d/`. **CAN `0x14A` byte 7 is new** (V92 is the first
...
`builds/v80_v107/build_v91_tva.py`'s *"genuinely DISSIPATIVE, it opposes motor rate"* is **wrong**.
...
⊕ A Python mirror of the decompiled arithmetic (`analysis-2020accord/studies/dose/v93_mirror_and_curves.py`) puts
the stock lane's saturation knee at **1.60×** the largest acceleration route 77 produced;
`builds/v80_v107/build_v91_tva.py` derived **1.6014** from measured percentiles by a completely different route.
