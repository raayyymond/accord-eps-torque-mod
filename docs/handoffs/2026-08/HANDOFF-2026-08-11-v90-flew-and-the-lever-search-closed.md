...
`analysis-2020accord/_scratch/cache/r77/`. Scoring: `docs/scoring/SCORING-2026-08-11-v90-flight.md`.
...
the lever** (`rlog-tools/studies/impedance/v92_rez_extend.py`, pre-declared trust gate coh² ≥ 0.10 **and** ≥ 5×
...
🛑 **AND THEN IT WAS MEASURED, AND IT DOES NOT BIND.** `rlog-tools/studies/ratchet/v93_return_to_centre.py`, four
...
⇒ 🛑 **`research/FEASIBILITY-8X-LKAS.md` Part 2 — *"even at TODAY's 4×, moderately fast steering already clips
...
5. 🛑 **`searchsorted` ON `logMonoTime` SILENTLY MISPAIRS ROWS.** `evt.can` can carry **two `0x14A`
   ...
   TIMESTAMP CHECK STILL PASSES.** Only a byte check catches it. `decode/extract_r77.py::_row2raw14` derives
...
- 🛑 **`gp-0x6afe` ≡ `gp-0x6b4e` ≡ 0, ALWAYS, ON EVERY BUILD.** The brief named this cell as "the LKAS
  ...
  (`accord/firmware/accord-aggregator-reaches-motor-via-gp6acc-bridge.md`'s "CAN/arbitration term" label) and
...
- 🛑 **`gp-0x6b4a` is a SECOND, direct, unconditional, unweighted LKAS-descended term into
  ...
  at `model/eps_lkas_chain_model.py:2318-2344` documented only `gp-0x6b4c`, the *sibling*.
...
1. **The golden model's `[VERIFIED]` tag was covering an incomplete picture.**
   `model/eps_lkas_chain_model.py:2318-2344` documents `FUN_00026c80` as `[VERIFIED]` — "~11 LKAS-internal
...
**Read alongside:** `docs/scoring/SCORING-2026-08-11-v90-flight.md` (the flight, and V91's pre-registration) ·
`docs/traces/TRACE-2026-08-10-dampaxis-sizing-and-safety.md` (the `0xCBE74` census, safety and sizing) ·
`docs/review/GATE2-2026-08-11-cbe74-independent.md` (the PID terms and the `Kd` lever) ·
`docs/traces/TRACE-2026-08-10-driver-reference-vs-lkas.md` (`gp-0x6ad6`'s inputs) ·
`docs/review/REDTEAM-2026-08-11-term0-verdict.md` (term 0, and `0xC616C`) ·
`docs/traces/TRACE-2026-08-11-return-to-centre-gate.md` (the return complaint) ·
`docs/specs/SPEC-2026-08-11-telemetry-budget.md` (V92's payload) ·
`docs/archive/arc-maps/_v91_arc_brief.md` (the arc V84→V90 and V90's complete non-stock delta, read from the images).
