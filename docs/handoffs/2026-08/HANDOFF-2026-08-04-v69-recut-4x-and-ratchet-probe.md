...
**Predecessor: `docs/handoffs/2026-08/HANDOFF-2026-08-04-v69-built-speed-shaped-rate-lane.md`** (the ×2 cut, same day).
...
`verify/verify_v69_image.py` all anchors PASS; `verify/diff_build_vs_stock.py v69` **0 unattributed**.
...
**`rlog-tools/probe/decode_v69_ratchet.py`**, linked mechanically: `build_v69_tva.assert_decoder_matches()`
...
4. **Log from before the first engagement**, and decode with `probe/decode_v69_ratchet.py` **before** any
...
the ×4 cut and the ratchet probe (`docs/specs/design/V69-DESIGN.md` §9).
