...
- **The operator's own recorded gloss**, from `studies/sessions/v76/v76_surface.py`: *"FactorC 'FLAT — no taper down, like a
...
🛑 **Named V78, not V77.** `builds/v50_v79/build_v77_tva.py` exists, and the V77/V77B `.rwd`s are already renamed
...
**V78 — BUILT, VERIFIED, UNFLASHED.** Build script `analysis-2020accord/builds/v50_v79/build_v78_tva.py`.
...
**`rlog-tools/probe/decode_v76_probe.py` decodes the SUPERSEDED V76** (the V74-base `GATE-FB-ARM5244` build),
**not the V38-base build that actually flew.** It was caught before use — the census agent wrote a fresh
decoder (`analysis-2020accord/studies/sessions/v76/v76flight_extract.py`) that re-reads the bit weights, thresholds and
`wire_model()` straight out of `builds/v50_v79/build_v76_v38base_tva.py` at import time and cross-checks its own decode
...
does this go?". Find the PWM/timer-compare/FOC duty registers (the kit has `reference/svd_for_ghidra/`), find every
