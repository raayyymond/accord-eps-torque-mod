...
`research/FEASIBILITY-SELF-INTERFERENCE-CANCELLATION.md` §2.1 — *"no compensation, decoupling or cancellation
...
7. 🛑 **THE BOOTLOADER CRC IS NOT UNIFORM 4 KB PAGES — and this was the session's one BRICK-CLASS error,
   ...
   *is* the main block, whose trailer merely **coincides** with `0xC4000 + 0xFFC`. **A coincidence was
   ...
   `lib/verify_bootloader_crc.py`). NEVER from a hard-coded list — including a corrected one.** In this
