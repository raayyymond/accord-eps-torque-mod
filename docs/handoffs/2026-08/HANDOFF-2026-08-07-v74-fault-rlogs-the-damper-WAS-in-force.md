...
4. **`probe/decode_v74_probe.py` still does not exist** despite being referenced by `builds/v50_v79/build_v74_tva.py`.
   Route 61's decode lives in `studies/sessions/v74_v75/v74fault_extract.py`, which `exec`s the builder's own `wire_byte4()`
   and verifies the decode inverts it exhaustively.
5. **`pandaStates` capture is broken** in both `studies/sessions/v74_v75/v75fault_extract.py` and the r5e cache — `ps_t` is
...
`analysis-2020accord/studies/sessions/v74_v75/v74fault_extract.py` (route 61 + IMU, execs the builder's `wire_byte4()`),
`studies/sessions/v74_v75/v74fault_crux.py`, `studies/sessions/v74_v75/v74fault_report.py`, `studies/sessions/v74_v75/v74fault_orchestrator.py` (the orchestrator's own
verification, sentinel-safe), `v74base_{state,damper,manualdamp,imuaxes,bumps,nearmiss,seg1burst}.py`.
Cache: `_scratch/cache/r61/` (`r61.npz`, 71 arrays incl. `state`, `dtc_active`, raw BE angle words, and IMU
under both schemas).
