...
Spec `docs/specs/design/V69-DESIGN.md` · builder `analysis-2020accord/builds/v50_v79/build_v69_tva.py` · verifier
`analysis-2020accord/verify/verify_v69_image.py`. Predecessor:
`handoffs/2026-08/HANDOFF-2026-08-03-v68-flew-the-lane-change-is-28hz.md`.
...
**`verify/diff_build_vs_stock.py` is span-based and would not catch a stray hit.**
...
every gate re-runs on the readback** · `verify/verify_v69_image.py` **all anchors PASS** ·
`verify/diff_build_vs_stock.py v69` **0 unattributed**, V68 unregressed, self-test still fails informatively.

★ **`verify/verify_v69_image.py` caught its own author.** Its first run failed two anchors — I had assumed
...
1. **A value-anchored verifier catches what a span-based differ cannot.** `verify/diff_build_vs_stock.py`
