...
**This corrects a standing kit claim.** `memory/reference/firmware/reference_accord_sub3mph_lkas_openpilot_gate.md` says the
...
path" claim** (`memory/reference/firmware/reference-accord-governor-g1-total-command-not-thermal.md` and
`reference/measurement/reference-accord-no-vehicle-speed-input-5mph-is-plant.md` on that specific point). Note the *direction*:
...
4. **Consider telemetry first.** The kit has a **built, lead-verified, unflashed** new-ID CAN-TX
   capability (`FOURFRAME`, `builds/telemetry/build_vfourframe_tva.py`, mailboxes 16-19, IDs `0x6a0-0x6a3`, 16 signals at
...
5. **On-car validation plan:** flash, then *parked first*, then a low-speed lot. Watch raw CAN 399 for
   ...
   `analysis-2020accord/studies/gates/speed_efficacy_test.py` already computes it.
...
1. **`reference/firmware/reference_accord_sub3mph_lkas_openpilot_gate.md` is WRONG on its central claim.**
...
- `analysis-2020accord/studies/gates/speed_status_engagement.py` — speed × STEER_STATUS × latActive correlation.
- `analysis-2020accord/studies/gates/speed_efficacy_test.py` — the gate-vs-label test (`STEER_CONTROL_ACTIVE`,
