...
**Predecessor:** `handoffs/2026-07/HANDOFF-2026-07-27-fourframe-strb-defect-and-vibration-reframe.md` (same date, earlier
...
Builder: `analysis-2020accord/builds/v50_v79/build_v53_tva.py`.

---

## 2. ★ The build technique worth reusing: import the cave, don't re-type it

`builds/v50_v79/build_v53_tva.py` **imports the 774-byte telemetry cave from `builds/telemetry/build_vfourframe_tva.py`** — the file that
...
Each of these runs inside `builds/v50_v79/build_v53_tva.py` on every build, so it cannot silently rot:
...
Updating `analysis-2020accord/model/eps_lkas_chain_model.py` turned up that its low-speed section had never been
...
The model's own scenario suite still runs clean (`python model/eps_lkas_chain_model.py`, exit 0).
...
- `analysis-2020accord/model/eps_lkas_chain_model.py` — §4 above.
- `analysis-2020accord/builds/v50_v79/build_v53_tva.py` — new.
- `memory/project/project-v53-fourframe2-plus-minsteerspeed0.md` + `memory/MEMORY.md`, and the same fact in the
