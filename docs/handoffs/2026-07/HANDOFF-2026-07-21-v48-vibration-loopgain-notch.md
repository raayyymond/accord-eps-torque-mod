...
unbuilt).** The master reference for the whole issue is now **`docs/research/VIBRATION-DOSSIER.md`** — read it first.
...
**What that means (per the loop-gain model, `analysis-2020accord/studies/models/eps_loop_gain_model.py`):** each single
...
Full detail + citations in `docs/research/VIBRATION-DOSSIER.md`. In brief:
...
`analysis-2020accord/builds/v18_v49/build_v48a_tva.py`. V38 + ratchet (`0x454FE`) + **two carrier mutes:**
...
- **Biquad (validated, `analysis-2020accord/studies/models/eps_v48b_notch_design.py`):** RBJ peaking-dip, f0=21.4 Hz,
...
- **STILL OPEN before V48B can be built:** (1) **RAM: DF-I needs 5 free halfwords; only 3 confirmed
  ...
  the corrected repoints); (4) `builds/v18_v49/build_v48b_tva.py`. **A code cave is the kit's only class that has bricked
...
- `docs/research/VIBRATION-DOSSIER.md` — **master reference for the vibration** (phenomenology, physical model,
  firmware model, full theory ledger + why each lever failed, evidence sources, quantified loop-gain
  table, V48 menu). Read first.
- `analysis-2020accord/studies/models/eps_loop_gain_model.py` — runnable loop-gain/margin model + notch design +
  per-lever prediction.
- `analysis-2020accord/studies/models/eps_v48b_notch_design.py` — runnable biquad design + fixed-point validation.
- `analysis-2020accord/builds/v18_v49/build_v48a_tva.py` — V48A builder (flashed, failed).
