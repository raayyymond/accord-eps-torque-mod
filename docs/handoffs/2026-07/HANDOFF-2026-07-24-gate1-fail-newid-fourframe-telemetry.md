...
- **V50P probe** (`builds/v50_v79/build_v50probe_tva.py`) = V38 + a read-only cave that reads `gp-0x1500` low byte into
...
- **Root cause:** `gp-0x1500` (0xFEDF6B00) = slot 5 of a **40-slot × 8-byte I/O-mailbox array listed at
  ...
  **Static clearance failed on 3 of 8 addresses checked (`gp-0x1500`, `gp-0x14E0`, `gp-0x1700`).** In this
  ...
  `memory/reference/can/reference-accord-b7260-io-mailbox-array.md`.
...
`builds/v50_v79/build_v51probe_tva.py` — reads candidate cells **B = `gp-0x1300`** and **D = `gp-0x1100`** (both outside the
...
See `memory/reference/can/reference-accord-can-tx-architecture-new-id.md`. Single FCN0 (`0xFF480000`); emitter
...
- **`VCANTX-TEST-txgate`** (`builds/telemetry/build_vcantx_test_tva.py`): mailbox 16, ID `0x555`, fixed magic payload, gated
  on `gp-0x1712` bit0. Single-frame mechanism test. Cave 224 B @0xC4B34 on V38.
- **`FOURFRAME`** (`builds/telemetry/build_vfourframe_tva.py`): mailboxes 16-19, IDs `0x6a0-0x6a3`, dual-gated
...
**Levers, categorized** (`memory/reference/builds/reference-accord-vibration-levers-falsified-vs-untested.md`):
...
- **Verify subagent conclusions before acting** paid off twice: the rlog writer finding (re-decoded), and the
  ...
  `memory/feedback/process/feedback-verify-subagent-conclusions.md`.
