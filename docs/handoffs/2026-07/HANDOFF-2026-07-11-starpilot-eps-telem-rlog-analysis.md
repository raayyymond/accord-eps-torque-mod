...
see `docs/handoffs/2026-07/HANDOFF-2026-07-10-v31u-uds-telemetry-working.md`). Openpilot side = the operator's **StarPilot**
fork on a **comma 4** at `/data/openpilot`.

**Builds on** `docs/handoffs/2026-07/HANDOFF-2026-07-10-v31u-uds-telemetry-working.md` §5 ("NEXT SESSION — fork sunnypilot to
...
- **`rlog-tools/decode/extract_eps_telemetry.py`** (NEW) — pulls `epsTelemetry` + steering context into a long-form
  ...
  ```bash
  python rlog-tools/decode/extract_eps_telemetry.py "<route>--*/rlog.zst" -o eme.csv
  python rlog-tools/decode/extract_eps_telemetry.py --eps-only rlog.zst -o eps.csv
  ```
- **Schema mirror** — `rlog-tools/cereal/custom.capnp` + `log.capnp` were updated with the SAME `EpsTelemetry`
  struct + `epsTelemetry @137` (identical `@ids` → identical wire format) so `lib/rlog_parse.py` can decode the
...
- **STATUS: these three files are UNCOMMITTED in this repo** (`rlog-tools/decode/extract_eps_telemetry.py` untracked;
...
LKAS — that's the gentle-EME regime). Confirm first with `python rlog-tools/lib/rlog_parse.py <rlog>` → look for
...
1. Extract with `decode/extract_eps_telemetry.py` → CSV (eps + cs + cc rows, shared `t_ns` timeline).
...
sledgehammers. See `docs/handoffs/2026-07/HANDOFF-2026-07-02-v33.md` and the gentle-EME root-cause handoffs
...
- Analyze STOCK `code.bin` for firmware questions; the RWD/telemetry channel spec is
  `docs/handoffs/2026-07/HANDOFF-2026-07-10-v31u-uds-telemetry-working.md` + `memory/reference/can/reference_accord_uds_did_read_surface_a160.md`.
- **Artifacts this session:** StarPilot fork commits on `raayyymond/StarPilot` (`StarPilot` + `Dom`);
  `rlog-tools/decode/extract_eps_telemetry.py` + schema mirror (uncommitted); this handoff.
