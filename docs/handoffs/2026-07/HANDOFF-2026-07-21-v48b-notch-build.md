...
**UNFLASHED**. Read `docs/research/VIBRATION-DOSSIER.md` first for the *why*; this handoff is the *how* and the
verification record.
...
## 1. Files
- `analysis-2020accord/builds/v18_v49/build_v48b_tva.py` — the builder. Produces the RWD + `_v48b_plain_image.bin`.
- `analysis-2020accord/studies/caves/v48b_cave_asm.py` — the cave assembler (every encoding cross-validated vs ≥2 real
  `code.bin` instructions; the single source of truth for the cave bytes).
- `analysis-2020accord/studies/models/eps_v48b_notch_design.py` — RBJ→biquad design + fixed-point validation (unchanged).
- `analysis-2020accord/studies/models/eps_v48b_cave_model.py` — **NEW** bit-exact model of the cave's integer math
