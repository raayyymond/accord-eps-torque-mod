...
> **CORRECTION 2026-07-18 (V39 session):** the six-lane assist summary below was incomplete. `FUN_0003aa2c` also computes two inline Sensor-B torque-rate lanes (`gp-0x4f62 -> r26/r24`) and adds `FUN_00036682`; the direct `r24` lane can reach +/-8192. The `FUN_000352b4` validity branch was inverted below: normal `gp-0x4f60` inside +/-25600 stores `gp-0x6b86`, not zero. Assist-inclusive `gp-0x6acc` can conservatively reach 7322, not the old 4342/4608 estimate; final `gp-0x6b98` uses the separate `gp-0x6afe+r20` path. The canonical correction is in `model/eps_lkas_chain_model.py` and `handoffs/2026-07/HANDOFF-2026-07-18-v39-opposing-torque-rate-guard.md`.
...
The premise is correct. openpilot's `CAR.HONDA_ACCORD` uses `torqueBP = [[0, 4096]]` (`reference/opendbc/honda/interface.py:114`), so the setpoint reaches `4096 × -4` = **16384** against a **15360** clamp — the top **6.25%** of the command range is clipped.
...
> **SUPERSEDED.** The blocker below is cleared and the raise now ships in V38. See `handoffs/2026-07/HANDOFF-2026-07-17-v38.md` (revised) and `memory/reference/firmware/reference_accord_setpoint_limit_15360_lerp.md`. Historical framing retained — the fails-closed mechanism is worth remembering.

`0xE4180` sits in bootloader CRC block `[0xE4000, 0xE4FFC)` (trailer at `0xE4FFC`), confirmed by running `lib/verify_bootloader_crc.py` on the stock dump (49 blocks, 0 mismatches). Builds through V37 recomputed only `TOUCHED_BLOCKS = [(0xC6000,0xC6FFC), (0x13000,0xC4FFC)]`.
...
**Resolution:** `builds/v18_v49/build_v38_tva.py` adds **both** `(0xE4000,0xE4FFC)` and `(0xE5000,0xE5FFC)`. The 49-block chain survives intact because `walk()` locates each next block via u16 page fields at `block_start-8/-6`, which live in the **preceding** block's CRC range — never inside a patched block. Post-patch walk: 49 blocks, 0 mismatches.
...
`analysis-2020accord/model/eps_lkas_chain_model.py` rewritten (new Section 3B + 6B). Runs clean; both historical regimes still reproduce (V9 soft-cut + gentle EME; V31 fixes soft; V37 fixes gentle); V38 verified at exactly 2× V31 per tick.
...
- `analysis-2020accord/model/eps_lkas_chain_model.py` — Section 3B (assist, explicit), Section 6B (aggregator), corrected `arb_setpoint_limit` block, corrected `gp-0x4f60` identity, `EpsState` assist fields, orchestration rewired.
- **New memories:** `reference/firmware/reference_accord_setpoint_limit_15360_lerp.md`, `reference/firmware/reference_accord_gp4f60_is_sensor_b_column_torque.md`, `reference/firmware/reference_accord_base_assist_lane_architecture.md`, `reference/firmware/reference_accord_assist_curve_family_sport_mode.md`, `reference/tooling/reference_accord_ecu_id_variant_table.md`.
- **Correction banners:** `memory/MEMORY_CONSTELLATION.md` (Era 18), `memory/project/project_accord_torque_mod_v0.md`, `memory/reference/firmware/reference_accord_corridor_lockstep.md` (the CONTESTED identity is now RESOLVED — neither prior candidate was right), `memory/reference/firmware/reference_accord_corridor_vs_envelope.md`, `analysis-2020accord/.claude/agent-memory/firmware-codepath-tracer/reference_accord_segmentD_fun3d04c_full_gate_map.md`.
...
4. **[OPEN]** re-derive the soft-EME bound's IIR arm on the torque (not velocity) interpretation of `gp-0x4f60` — `reference/firmware/reference_accord_corridor_lockstep.md`'s physical reading may shift.
