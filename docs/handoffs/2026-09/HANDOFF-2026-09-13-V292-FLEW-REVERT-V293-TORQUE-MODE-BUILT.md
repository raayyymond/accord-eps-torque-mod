...
**one switch**, `AccordEpsTorqueMode`, instead of four toggles the operator had to keep consistent by hand.
...
| `forkpatch` | the one-switch preset | `AccordEpsTorqueMode` built in the fork, **UNCOMMITTED, OFF by default**: **+306/−4 across 10 files plus a 487-line test file, 25 new tests passing**, suite **306 passed / 303 prior / 3 newly unblocked** — the two Accord rate-plant tests were freed by a **one-line `import math`**. `epsTorqueMode` published in `starpilotLateralState` so the mode is readable from cereal rather than from the params blob. Provisional four-number tune in one place; the checklist in `docs/guides/` encodes the **asymmetric** mismatch hazard as a step order |
...
- **Fork (`../openpilots/raayyymond-StarPilot/StarPilot` @ `Dom`):** the `AccordEpsTorqueMode` preset
  applied, **UNCOMMITTED, OFF by default**; the earlier `ModelCurvatureLead` patch and the operator's own
  SR-map refit untouched. **Not pushed.** ⚠ HEAD had moved past the brief's `305732c85` to `a357cd2b5`;
