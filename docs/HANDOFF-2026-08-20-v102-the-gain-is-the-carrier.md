...
**Chain:** ← `HANDOFF-2026-08-18-v101-eme-audit.md` ← `HANDOFF-2026-08-14-v100-flew-and-six-levers-closed.md`
...
`python build_v102_tva.py` reproduces the flown artifact bit-for-bit. `ACCORD_V102_GAIN=8x`/`4x`
...
1. 🛑 **`gp-0x6b4c` IS NOT THE LKAS COMMAND.** It is `gp-0x3d88 = Σ_{i=0..10} (0xC4118[i]≠0 ?
   ...
   **This explains why its sign agreed with openpilot's command at CHANCE (52.80 % vs 54.36 %) while
   ...
   on the wrong label.
2. 🛑 **V101's GATE 2 premise is MEASURED FALSE.** `build_v101_tva.py`: *"The LKAS command enters the
...
4. 🛑 **`band_envelope` IS BROKEN IN TWO SHARED HELPERS** — `analysis-2020accord/_r31_common.py` and
   `rlog-tools/_r2b_common.py`. Both build `H[band] = 2*X[band]` on a **one-sided** spectrum then call
   ...
   `analyze_bus_amplitude_vs_detector_T.py:282`) is **2× too large**. **Ratios between conditions are
...
11. 🛑 **V102's IDENTITY ASSERTS A *CLEARED* BIT — a first for this kit, and a weakness.**
    ...
    **AND the PASS-3 sign bits (b7, b4) are live and flipping.** `rlog-tools/score_v102.py` implements
...
12. **Cache-layer traps, reported not fixed**: `_cache_r71` lives at the **repo root**, not under
    `analysis-2020accord/` — **two cache roots, and no resolver outside `rlog-tools/v102_xb_lib.py`**.
