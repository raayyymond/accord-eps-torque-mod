...
| file | what |
|---|---|
| `rlog-tools/extract_r82.py` | route-82 cache; adds a `ROUTES` row and calls `extract_r7d.extract_route` — the instrument is NOT reimplemented |
| `rlog-tools/extract_r82_audio.py` | ⭐ NEW — the 16 kHz cabin microphone, never used by this kit before |
| `rlog-tools/v99_r82_score.py` | E1 / E2 / E3 / POS-1..3 / R5b / audible bands / acoustic |
| `rlog-tools/v99_r82_matched.py` | 🛑 the RATE-MATCHED engaged/manual contrast, both routes, + the common-stratification E1 curve |
| `analysis-2020accord/_cache_r82/` | `r82.npz`, `r82s{0,1}.npz`, `r82_audio.npz`, identity/health/census JSON |
| `analysis-2020accord/_v99/v99_r82_score.json` + `.log` | the full machine-readable readout |

Reproduce with:
```
cd rlog-tools
python extract_r82.py            # cache + identity + health + census
python extract_r82_audio.py 82 81
python v99_r82_score.py
python v99_r82_matched.py         # rate-matched arms + common-stratification E1
```
...
Pre-registered (`build_v99_tva.py` ~line 159): *b5 duty ≥ 0.999 over the whole route AND
...
returned NaN phases. Fixed in `v99_r82_score.py:load()`.
...
`v99_r82_matched.py` §D re-derives **both** routes from their own caches on **one** set of edges.
...
🛑 **ALL FOUR BINS MOVED, AND ALL FOUR MOVED DOWN.** `build_v99_tva.py` is unambiguous about what
...
contrast in exactly the direction that manufactures it**. `v99_r82_matched.py` §C therefore matches
...
🛑 `build_v99_tva.py` lists cross-build band ratios as an **UNBUILDABLE ENDPOINT** at this exposure.
...
modulation of a broadband rasp**, so `extract_r82_audio.py` computes the per-band **envelope** on
...
episodes, 12–14 blocks. Script: `rlog-tools/v100_power_gate.py`, readout
`analysis-2020accord/_v99/v100_power_gate.json`.
...
## The defect
`v99_r82_score.py` built its 427 signal **rectified**:
```python
...
from *f* to 2*f*. Inherited from `v98_r81_score.py:541`.

**Fixed** on both lines: `... * np.where(d["sign_6b70"], -1.0, 1.0)`.

⊕ **`extract_r82.py` is CLEAN** — checked. It never builds a signed 427 series; it stores the raw
