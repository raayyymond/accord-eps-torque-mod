...
Reproducer created and on disk: **`analysis-2020accord/observer_leak_model.py`** — runnable and
...
inertia; it does NOT cancel it.** `FEASIBILITY-SELF-INTERFERENCE-CANCELLATION.md` §6.1 / OPEN #3
...
(`build_v81_tva.py:20`, `5203` → `ff01`). Three signed `ld.h` readers, all inside `FUN_00036c12`.
...
2. **`0xCBE74[mode]` is indexed on VOTED DRIVER TORQUE, not speed.** Wrong in
   `FEASIBILITY-SELF-INTERFERENCE-CANCELLATION.md` §6.1 and in the brief that quoted it.
3. **`FEASIBILITY-SELF-INTERFERENCE-CANCELLATION.md` OPEN #3's premise is wrong** — the term's sign
