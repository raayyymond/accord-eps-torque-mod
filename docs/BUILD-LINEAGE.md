...
### V294 — ACCELERATION TRIM on the V293 torque map (two in-place opcode halfwords + four cal cells; the feedforward byte-exact) — 🛑 **THE ENTRY IS IN [`docs/BUILD-LINEAGE-PART6-V291-ONWARD.md`](BUILD-LINEAGE-PART6-V291-ONWARD.md)**

**Stub so `grep V294` lands here.** **BUILT 2026-09-20, NOT FLOWN.** V293's torque map unchanged; `0x28FA4 add→subr` makes r26 the tick difference of the lag-filtered wheel rate (acceleration through the lag pole), `0x29D76 shl 5→2` + Kp 120→960 keep the FF product bit-exact, fb clamp 0→1024 (trim ≤ 25 % of the rail — a backstop; ~1.85 T counts per deg/s of 2 Hz-band wheel rate in practice), pole 923→1011 (2.0 Hz), b 1560→567. Adversarial pass A + B PASS, six non-blocking defects, four fixed. Fork REVERTED (every torque-mode term defaults off, Dom `54ff1ea39`). Image `3143616d…`, rwd `a2b418f0…`. Full entry in PART6.
...
|---|---|---|---|
| `0x2A174` | `ld.hu 0x73ee,tp,r7` (opens the 992/507 output-lag filter) | `jr 0xC4C00` | hook — the one address every route passes every tick; S = r12 (clamped loop output, ±15360) |
| `0xC4C00–0xC4C8B` | 0xFF | 140 B / 52 instr: Q14 TDF-II notch b=[16048,−31842,16048] a=[16384,−31842,15712] + error feedback, clamp to ±cal(0xC61BE), FLAG bits, replicate `ld.hu 0x73ee`, `jr 0x2A178` | notch cave: 20.036 Hz, Q 3.007, −47.6 dB at 20.05, DC exactly 1; state gp-0x6c44/−0x6c40/−0x6c3c, FLAG gp-0x6c3a; scratch r6/r7/r9/r13 |
| `0xC4BD6` / `0xC4BDC–F7` | `jmp [lp]` / 0xFF | `jr 0xC4BDC` / 28 B tail | 0x14A b4.5 = sign(S−y), b4.7 = \|S−y\| ≥ \|y\| from FLAG (mask 0x5F); b4/b6 kept; b0–2 stock |
| `0xC63E8` / `0xC63EA` | 923 / 1560 | 875 / 2301 | fb lag pole 16.53 → 25.03 Hz, DC 30.891 → 30.886 |
...
| region | V282 | V288 r2 | what it is |
|---|---|---|---|
| `0x29D72` | `st.h r16,-0x6a32[gp]` (dead publish of the assist-map output sp) | `jr 0xC4C00` | hook, same length |
| `0xC4C00–0xC4C2F` | 0xFF | 48 B: `ld.w -0x6cf8[gp],r6 ; mov 0x7fffffff,r9 ; cmp r9,r6 ; be st.h` then `ld.h y ; sub ; sar 4 ; +1 fix ; add ; st.h ; ld.h→r16 ; jr 0x29D76` | filter cave: y += (sp−y)>>4, state gp-0x6a32 (zero readers), engage init from Honda's sentinel |
| `0xC4BD6` / `0xC4BDC–FD` | `jmp [lp]` / 0xFF | `jr 0xC4BDC` / 34 B sign rung + relocated epilogue | 0x14A b4.5 = sign(y) (replaces V282's \|r24\| ≥ \|aggregator\|; b4.6 = \|r24\| ≥ \|T\| kept; b4.0–2 STOCK) |
| `0xC4FFC` | 446bb04e | f007c24a | CRC |
Corner 10.3 Hz at 1 kHz, DC gain exactly 1, group delay 15 ms (outer-loop cost ≈ 20° at 3.9 Hz; outer PM never measured), per-tick D kick ÷16 (capped 123-count step: ΔE 3936 → 224, D clamp binds at 640). K is one constant; K=3 = 21 Hz / 7 ms / ÷8. Image `94cabdefd39a103ad10ec34b9c64b6ac94d6552b80bfe7cc192a8977cbbdbd8c`, rwd `43efd0c98446d331a5b529cbaf93f1173e61e9daa132903cf3467078a7207714`, script `builds/v108_plus/build_v288_tva.py`. **Rev 1 (bc8a5b1a…/862cb525…) SUPERSEDED-DO-NOT-FLASH:** FAILED adversary B (state froze across disengagement — the three hook-skipping routes reset every PID cell but not ours; up to 11 ms railed wrong-direction at re-engage after an override) and adversary A (rung wrote 0x14A b4.0, a stock Honda flag). Rev 2: A/B PASS-with-conditions, C/D PASS — `docs/review/ADVERSARIAL-V288-PREREG-2026-09-07.md`. Why this class: the 0xE4 command is NOT a staircase (changes on 92 % of frames) but is slew-capped at 123/frame by openpilot; capped frames carry ~54 % of D-clamp binds; grind #1 is a rung bell (ζ≈0.026) so cutting excitation shrinks it (`rlog-tools/studies/grind/WIRE-0XE4-20HZ-2026-09-07.md`). Page: https://claude.ai/code/artifact/fb9a34f0-9b1c-4850-b673-187c100ce6fd. **On-car: not flown.** Operator on V282 after rejecting V287 ("still felt the grinding").

### V287 rev 2 — V282 + the rate PID's D-TERM CLAMP `0xC61B6` 10240 → 7680 (2026-09-06, NOT FLOWN — a PARTIAL MITIGANT for grind #1, ~5 %, needs ~38 min of ordinary engaged driving to read; rev 1 at 2560 is SUPERSEDED-DO-NOT-FLASH)

**base** V282 · **image** `e75ae7eb…8f15` · **rwd** `71648c0e…52a5` · **376/376** (≈62 load-bearing) · independent rebuild reproduces · **5 bytes vs V282**: `0xC61B7` 28→1e + CRC `0xC6FFC` · adversarial: A PASS · C PASS · D PASS · **B FAIL at 2560 → re-sized to 7680** · **prereg** `rlog-tools/studies/grind/PREREG-V287-LOOP-SHAPE.md` §C5 · design `docs/research/GRIND1-LOOP-SHAPE-V287-2026-09-06.md` App. B/C · reviews `docs/review/ADV-V287-{A,B,C,D}-2026-09-06.md`.

| cell | stock | V282 | V287 r2 | what it is |
|---|---|---|---|---|
| `0xC61B6` (tp+0x71B6) | 10240 | 10240 | **7680** | D = (ΔE·Kd)»3 clamp, symmetric, inclusive, 4 live `ld.hu` readers @0x29EE8/EF2/EF8/F02 (3 more in the unreachable duplicate PID); with Kd 128 rails at \|ΔE\| = 480 (was 640). **Never edited before on any of 287 images.** ⚠ `0xC61BA` = 10240 is the integrator anti-windup — assert on the ADDRESS. |

**WHY.** Grind #1 (18–22 Hz rate-loop crossover resonance) opens on V282 at 3–6× the median demand/rate/angle; the openpilot command is a 100 Hz staircase and 32·Δsp is an impulse train into D — at 10240, 93–100 % of D-binding ticks are setpoint-dominated on a command step. Lowering the clamp limits the kick that rings the mode with the linear loop untouched **where the feedback derivative stays under the clamp**. **Rev 1 (2560)** cut onset envelopes ×0.33–0.79 with steady creep unchanged, but adversary B stratified the mirror over hands-on \|bar\|>700, loaded \|ang\|>60 and fast-wheel >25 deg/s (20–28 % of engaged time): there the feedback derivative binds, effective Kd 95 in the loaded stratum (< the ~118 floor), ring \|L_tot\| 1.038 — re-armed. **7680** is the largest dose with B's admissibility test holding in every stratum (borderline in SUBURBAN at 79.7 % dominance) and ring \|L_tot\| 0.983 = exactly the gate; onset envelope ×0.947 (r39) / ×0.930 (r3c), steady ×1.000; resolvable at 2 SE only with ≥ ~1,150 command-step onsets ≈ 38 min engaged. **Framed as a partial mitigant, not a cure.** What it does NOT change: Kp/Kd/Ki, poles, r24, clamps, cave, tap, DC authority (paired 0–100 ms step torque ×0.99–1.01, n 130–199/route).

**Struck the same day (DO-NOT-FLASH):** output-lag pole `0xC63EC/EE` at ≥ 10 Hz (962/982, 950/1172, 932/1457 → GM 0.99/0.86/0.72×, fires Honda's oscillation detector); the record's shape 4 fb pole (gain-only). **Identified, not built:** a setpoint-interpolation code edit (no cal on the setpoint path has memory — the staircase reaches D unfiltered).
...
### V281 rev 3 — Kp COMPLETELY FLAT at the idx-0 value on the LKAS rate PID  (2026-09-03, **FLOWN 2026-09-03, route `…_00000035`**; rev 1 knot-cap and rev 2 flat-341-from-24 SUPERSEDED)
...
|---|---|---|---|
| 28 map records, every knot | ×2 of the concave stock shape | Y'(X) = round(6·Ytop·X/240): slot 7 = 0,52,86,103,138,275,413,550,688,1032 | the rate reference as a straight line (4.3/idx); ×2.2 at idx 12, ×2.7 at 58, ×3.3 at 96, ×6 at 240 vs stock; top knot = V276's; every other knot UNFLOWN. Damping fraction in V276's ringing frames 0.840 vs rev 3's 0.863 (chain sim; V276 0.576); slope 3.8 (top 912) would hold 0.863 |
...
LINEARISED (V276 ×6 of stock's shape → V278 ×2 of it → V280 a straight line to the same top). New: every knot but the top. **RISK:** the lane pushes WITH a
...
damping 0.30–0.40; 2–4 Hz excess < 1.39 (a 3.9 Hz return → slope 3.8). FAIL sentences in the prereg. Adversarial: `adv280a/b/c`, all clean; docstring
