...
byte4 b7. Engagement from `cc_lat` (`cs_eng` is dead in `_cache_r81`). Pairing asserted
...
enters the record.** Per `TRACE-2026-08-13-v99-arm-levers.md` §1.3, read against the `0x38148`
decompile:
...
`build_v97_tva.py:65-67`, verbatim:
...
and `FUN_00036682`'s hysteresis are not described-function analysed, exactly as `build_v97_tva.py`
itself conceded.
