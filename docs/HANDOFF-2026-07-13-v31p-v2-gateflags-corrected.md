...
**Builds on** `HANDOFF-2026-07-13-v31p-gateflags-330-piggyback.md`. That session BUILT V31P (gate-firing
...
## 4. V31P-V2 build — `analysis-2020accord/build_v31p_v2_tva.py` — BUILT, Ghidra-verified, UNFLASHED
...
- **rlog-tools:** `cereal/custom.capnp` mirror renamed the same; `extract_eps_telemetry.py` — renamed columns,
...
3. **Drive, provoke gentle EMEs**, then `python rlog-tools/extract_eps_telemetry.py <route>--*/rlog.zst -o
...
| build script | `analysis-2020accord/build_v31p_v2_tva.py` |
