...
and again in `build_v86_tva.py:141` where it is called "DEAD".** What is new here is (a) the
...
96** stock + `_v*_plain_image.bin` files. `build_v86_tva.py:141` names it but only to *reject*
...
That premise is false. `build_v86_tva.py:141` reached the same conclusion independently from
