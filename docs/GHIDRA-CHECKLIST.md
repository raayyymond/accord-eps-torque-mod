...
3. **Import the stock binary** (skip if `code.bin` is already imported in the existing project):
   ...
   - ⚠ **Known SLEIGH bug:** the base `V850:LE:32:default` module gets `divq reg2==reg3` semantics wrong (computes quotient where the ISA says remainder) — see `memory/reference_rizin_ghidra_v850_quirks.md`. Don't trust a `divq` result with matching src/dst operands without raw-byte cross-check.
...
Walk the chain the way `docs/GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md`
...
- Confirm the shaper (`FUN_00042af8`) and float consistency-monitor
  ...
  `analysis-2020accord/TORQUE_PATH_AND_TABLE.md` and
  `FUN_00043e44_FLOAT_MONITOR.md`.
...
- If a function body is a run of `setf`/`xori` where you expect `ld.hu`
  ...
  `memory/reference_rizin_ghidra_v850_quirks.md`), not necessarily a
