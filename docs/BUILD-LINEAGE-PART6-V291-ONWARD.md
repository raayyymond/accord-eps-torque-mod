...
**Redo audit (2026-09-23, four fresh Opus agents on disjoint surfaces, FAIL criteria fixed before running; reports + scripts in `studies/v294/redo_2026-09-23/`): ALL FOUR PASS — flash-eligible as built, fork deployable with two conditions.** Bytes/CRC/rebuild: own CRC walker 50/50 on stock and V294, both opcodes hand-decoded from the ISA, own rebuild from V293 reproduces the image hash, own rwd decoder returns it bit-for-bit, FF identity over all 2³² sp, surface at r26 = 0 equals V293 in 154 k chain cases, operand settles to exactly 0 for every x, 28 Kp records by stride; the script's substantive-assertion count is 32 by an independent census (not 66). Decompiler: r26 = s_new − s_old (r26 loads s_old at 0x28F7C, r9 becomes s_new at 0x28FA2; the fb state is gp-0x3d30), polarity cancels (pol·pol), x = 8.00 counts per deg/s from 0x55B48 with the DBC factor −1 and d(angle)/dt ÷ rate = 1.002–1.007 on three routes (the wire study `x_scale_from_v292_wire.py` is circular for the UNIT — it divides by carState rate, which is x/8 by construction — so the scale rests on the bytes and the angle-derivative check), 1874/1874 instruction boundaries identical, no governor/EME/lockstep/DTC reader of any changed or rescaled cell (two derived RAM cells DO change value: gp-0x6a34 = |r26|>>5, gp-0x6cf8 = E_prev; readers dead). Physics (exact 1 kHz discrete + 4th-order continuous, 3,456 plant variants, 1,008 outer-loop variants): sign, 20 Hz ratio (−26.69 dB), stability (|L| 0.023 at the 20.8 Hz crossing, zero unstable loops) and outer-loop benefit (V294 never destabilised a loop stable on V293, stabilised 247) all hold — **but the ζ× table was a QUASI-STATIC estimate: exact poles give ×0.89 / 1.00 / 1.29 / 1.43 / 1.73 at 5 / 8 / 12.5 / 19 / 26 m/s on the map k (×0.92 / 1.05 / 1.38 / 1.79 / 2.25 on the design's levelled k; the orchestrator's own 4th-order roots reproduce these), so below ~8 m/s the wheel mode is slightly LESS damped and slower** (the trim is mostly inertia under its own pole; a 1018 = 0.94 Hz restores ×1.07 at 5 m/s at the cost of outer-loop margin at 26 m/s); the 180° crossing is 19–24 Hz for a 2–3 ms transport delay (not ~26/~40), max |L| 1.1–1.2 (not 1.7), 1.85 is |T/rate| at 2.4 Hz (K_α = 0.21 T per deg/s² below the pole), the trim is one-sided from idx 236 (P clamp), every filter restart is an 80–400 ms braking pulse (peak 73 counts at 50 deg/s, ≤ 615), and "PID" is a misnomer (P-only on 2 Hz-lagged acceleration + FF; Ki/Kd zero by STRUCTURE — I integrates the feedforward, D is a setpoint kick and anti-damps below 3.2 Hz). **The pre-registered instrument FAILED as written on real data** (twenty 20 s hands-off windows of the V293 null routes 70–75 read +0.021, 5–95 % −0.027…+0.087, overlapping a live +0.067; a ±5 % FF-gain error moves it ±0.05) and is **replaced**: byte-exact 1 kHz predictor on the route's own command (fade + 5.05 Hz output lag, tap aligned to the ms), regressor = the output-lag replica of −d/dt LPF_2.03(rate) in deg/s², nuisance FF and dFF/dt; null +0.004 (−0.037…+0.024) vs synthetic-live +0.196 (+0.145…+0.233), 20/20 in each arm: **β > +0.10 = LIVE, |β| < 0.04 = NOT LIVE.** Sign hazard: the v280 cache `rate` column is the RAW 0x18F field = −8 × carState.steeringRateDeg (the firmware writes −x there), so calibrate the regressor's sign against the feedforward on the same route (HP FF torque vs wheel acceleration correlates +0.28…+0.57). Secondary signature: the r1 fork config is route 71's config (Kp 0.9 for 0.85) and on the V293 plant the model reproduces route 71's 2.34 Hz limit cycle above 19 m/s — **if the trim is not live, that limit cycle is the expected symptom at speed.** Fork: the real LatControlTorque at HEAD + r1 is bit-identical to the pre-V293 base and to the generic torque path (two 12,000-frame scenarios; 0 calls of plant FF / hysteresis / rate loop / observer), HEAD + REVERT is bit-identical to rev 6.4; every param default = get_value default = getattr fallback; the codec re-encodes all 22 configs byte-identically; SteerFriction 0.011 / SteerLatAccel 14 cannot be back-filled. **Conditions: import r1 PARKED** (Galaxy permits it onroad; the plant-FF → generic switch steps the law by up to 0.46 torque) and **run Rebuild Params after pulling** (the committed params library knows no Accord keys; the REVERT file would silently lose AccordJerkLpHz 4.0). Defects: five Accord tests broke on 54ff1ea39 (they leaned on the old fallbacks) — fixed by explicit rev-6.4 toggles (`TestLatControl._rev64_accord_toggles`, 25/25 pass) together with a known() guard on LaneChangeTurnGate, left in the operator's working tree (commit/push blocked for the agent); LaneChangeTurnGate stays ON (a lane-change smoothing gate, not a torque-mode term); UseAutoSteerDelay stays true (the operator's own setting); `fork_revert_patch.py` is a partial record (three hand edits not replayed — noted in its header); `toggle-config_V282_rate_servo_REVERT.json` is stale.
