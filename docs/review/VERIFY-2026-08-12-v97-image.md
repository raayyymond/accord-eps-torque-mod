...
`builds/v80_v107/build_v97_tva.py` re-run twice:
...
Independent script (not `builds/v80_v107/build_v97_tva.py`'s internal checks) reads `0xC63AC` as an LE halfword
...
memory (`accord/builds/accord-cbe74-dose-measured-inert-wrong-mode-record.md`) live in the **mode-record mirror
...
`studies/ledger/ledger_v94_cells.py` was **not used** — brief flags it silently ignores `LEDGER_TARGET=V96` in
