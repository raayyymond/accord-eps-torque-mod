...
speed-slope removed, evaluated at a common 40 km/h (`rlog-tools/score_v102_ancova.py`):
...
power (`rlog-tools/v102_estimator_control.py`), a line 0.8 Hz from the edge — V102's exact case —
...
Sliding a 2 Hz window across 16–36 Hz (`rlog-tools/rez_band_tracking.py`), the negative region runs
...
- `rlog-tools/v102_xb_lib.py` `BANDS`: add `"20-28": (20.0, 28.0)`. **Do not remove `"22-26"` or
