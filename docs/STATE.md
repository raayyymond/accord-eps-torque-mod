...
> 🛑 **THE VERDICT (2026-09-20): V294 IS THE CANDIDATE; THE DECISION TO FLASH IS THE OPERATOR'S.** The operator's call: *"the delay kills the idea of a torque mode only control … a PID + feedforward which track acceleration, not rate/velocity … revert all torque mode changes in this next StarPilot update … anticipate grinding and stuttering (relying on feedforward/torque mode as much as possible)."* **What V294 does:** `r26 = clamp(s_new − s_old, ±1024)` — the per-tick change of the lag-filtered wheel rate = the wheel's acceleration through a 2.0 Hz low-pass; `E = 4·sp − r26`; `P = (E·960)>>8 = 15·sp − 3.75·r26`: V293's feedforward EXACTLY (asserted at all 241 demand indices and for every 32-bit sp) minus a trim bounded at 25 % of the rail. Kd 0, D clamp 0, Ki 0, r24 2048, map, tap, cave unchanged. **Why acceleration and why 2 Hz:** with T = −K·H·α and H a lag, every degree of loop lag turns the trim into damping (delayed RATE feedback anti-damps past 90°, V282's 20 Hz crossover resonance); at K_α/J = 1 the wheel mode's ζ goes ×1.02 / 1.16 / 1.49 / 1.82 / 2.05 across 5–26 m/s at a 2 Hz pole (×0.81…1.28 at the stock 16.5 Hz pole — worse at low speed), and the 20 Hz gain sits **−26.7 dB** below the loop V282 ground on. In the 2–3 Hz band the trim is ~97 % damping: **1.85 T counts per deg/s** of wheel rate = a damper comparable to the wheel's own; the 25 % cap binds only above ~230 deg/s. **Image** `3143616d5b79bdb7648d8e4d32e48420c7b481b18325178e89d1853589dbdd85` · **rwd** `a2b418f061160f66…` (exactly one on disk) · script `build_v294_tva.py` (159 assertions, 20/20 mutations caught, zero-edit control reproduces V293) · design `studies/v294/v294_design.py`. **2026-09-21, adversary C (Ghidra):** C1/C2/C4/C5 PASS from the decompiler; its C3 FAIL (x = 1.0–1.7 counts per deg/s) is WITHDRAWN on the wire — three V292 routes with the rate loop live read **x = 7.1–7.8 counts per deg/s** (`studies/v294/x_scale_from_v292_wire.py`), and C itself then retracted from the bytes: the 0x14A rate field is −x>>3 (0x55B48), so **x = 8.00 exactly**; the design's 8 is EVIDENCE twice over and V294 needs no re-cut. **🛑 REDO AUDIT 2026-09-23 (four fresh Opus agents, disjoint surfaces, FAIL criteria written first — `studies/v294/redo_2026-09-23/`): ALL FOUR PASS on flash-eligibility / deployability.** Bytes: own CRC walker, own ISA rebuild reproduces `3143616d…`, own rwd decoder, FF identity over all 2³² sp. Ghidra: r26 = s_new − s_old (state gp-0x3d30), polarity cancels, x = 8.00 from 0x55B48 + DBC factor −1 + d(angle)/dt ÷ rate = 1.002–1.007, 1874/1874 boundaries, no interlock reader. Fork: HEAD + r1 config bit-identical to the pre-V293 base and the generic torque path (real LatControlTorque, 12 k frames); HEAD + REVERT bit-identical to rev 6.4. **Corrections that change what the operator should expect:** (1) the ζ× table was a QUASI-STATIC estimate — exact closed-loop poles (confirmed by the orchestrator's own 4th-order roots) give **×0.89 / 1.00 / 1.29 / 1.43 / 1.73 at 5 / 8 / 12.5 / 19 / 26 m/s** (map k) — **below ~8 m/s the wheel mode is slightly LESS damped and slower** (heavier wheel at car-park speeds; never unstable in 3,456 variants; BELIEF plant; the lever is the pole cell 0xC63E8, a 1018 restores ×1.07 at 5 m/s at a cost of outer-loop margin at 26 m/s); (2) **the pre-registered drive regression FAILED on real V293 null routes** (read +0.021, overlapping a live +0.067; wrong units too) — **replaced** by the byte-exact 1 kHz predictor + output-lag-matched regressor + FF nuisance terms, verified 20/20 on real null windows with a synthetic-live positive control: **β > +0.10 T counts per deg/s² = LIVE, |β| < 0.04 = NOT LIVE**; sign-calibrate the regressor against the feedforward on the same route (the v280 cache `rate` column is the RAW 0x18F field = −8 × carState rate — on it the old rule would call a correct build inverted); (3) every filter restart (engage, bail) is an 80–400 ms braking pulse decaying on the 2 Hz pole (peak 73 counts at 50 deg/s, ≤ 615), not one tick; (4) 180° crossing 19–24 Hz (delay-dependent), max |L| 1.1–1.2, trim one-sided from idx 236; (5) **if the trim is not live, route 71's 2.34 Hz limit cycle is the expected symptom at ≥ 19 m/s** (the r1 config is route 71's config); (6) 'PID' is a misnomer: P-only on 2 Hz-lagged acceleration + FF, Ki/Kd zero by structure. **2026-09-23, operator's questions:** (i) there is NO feedforward block in the firmware — P = Kp·(4·sp − r26) is one P multiply, and the map rides through it only because r26 (lagged acceleration) settles to exactly 0; the only structural acceleration-feedforward path is the D term's setpoint half, zeroed twice; "PID + feedforward" was behaviour, not code (page v4 section 3b, three refuters). (ii) **The r1 fork config is wired correctly but NOT tuned on V294**: Kp 0.9 / Ki 0.3 are the operator's late-V282 toggles (r6f read 0.9000; earlier V282 routes flew 0.6), LAF 14 / friction 0.011 are V293 route-70 measurements; route 71 limit-cycled on these values on V293 and the trim is the BELIEF-plant rescue; margin-safe first-drive alternative SteerKP 0.6. (iii) Operator floated moving the fork's torque-mode machinery (rev 3–6.4: plant FF tables, hold map/level, hysteresis, rate loop, observer, notch, ref filter, dither) to a separate branch — recommended AFTER the first V294 drive so the one-file REVERT path stays available for it; on Dom today all of it is off and dead weight. **Fork deploy conditions:** import the r1 config PARKED (Galaxy allows it onroad; the law steps up to 0.46 torque); run **Rebuild Params** after pulling (the committed params library knows no Accord keys; the REVERT file would silently lose AccordJerkLpHz 4.0). Five Accord tests broke on 54ff1ea39 → fixed (explicit rev-6.4 toggles, 25/25) + LaneChangeTurnGate known() guard — **in the operator's working tree, commit/push was blocked for the agent**. LaneChangeTurnGate stays ON (lane-change gate, not torque mode); UseAutoSteerDelay stays true (operator's own); `toggle-config_V282_rate_servo_REVERT.json` is STALE (its plant FF now runs V293's tables). **Adversarial pass (2026-09-20):** A (arithmetic/sign/scale: four sign legs, the FF identity for every 32-bit sp, int32 4.06×, the −26.7 dB re-derived) and B (build audit/interlocks: independent rebuild reproduces the hash; every changed cell private and in FLASH; the 0x14A cave reads the delivered torque, not the PID cells; no governor/EME reader; V293's D1 survives) both PASS; six non-blocking defects, four fixed in the script, one a record erratum (`gp-0x6a56` has 29 accesses, not 30). **The one new behaviour to know before the drive:** up to 616 counts of lane torque at ZERO command while engaged, opposing wheel acceleration (V293 delivers 0 there; V282 flew 2463). **Next drive** (V294 + the r1 config, `SteerFriction 0.011` checked in initData first): `residual = T_tap − FF_V293(idx)·taper` regressed on −(0x18F rate, 2 Hz low-pass, differenced): slope ≈ +1.85 T counts per deg/s and the FF identity R² ≥ 0.98 = LIVE, RIGHT SIGN; flat = not live; **positive correlation = SIGN INVERTED → stop, revert to V293** (`…_REVERT_to_V293_r64.json` restores the fork). Outcome the operator scores: hard-turn jerk and the 1.6–3 Hz wheel-rate energy, predicted down; any new line in 5–30 Hz is the revert signature. Handoff: `docs/handoffs/2026-09/HANDOFF-2026-09-20-v294-acceleration-trim-built-fork-reverted.md`. **Page:** https://claude.ai/artifact/BcuAb3dgHP84oasqeadVBs.
>
