...
## ✈ THE DECISION, IN ONE PLACE  — updated 2026-09-04 (V283 FLEW and OVERSTEERS; V284 built + attacked but **SHELVED, DO NOT FLASH**; operator's frontier is **V282**; **V285 BUILT = V282 + Kp 0, a BENCH CONFIG, DO NOT FLY**; **V286 SPEC WRITTEN = V282 + Kd 128→160 + a 2-level `\|r24\|` ladder on `0x14A` bits 3/7 — NOW A LEVER, unblocked by `task5rate`'s energy-endpoint resolution, needs the full adversarial pass + empirical threshold placement before any image is written**; understeer moves to the **StarPilot** side)

**ON THE CAR: V283** (= V282 + Ki 50 at `0xC63E6`) — flown 2026-09-03 on routes `…_00000036` (r36, 897.3 s / 633.6 s engaged), `…_00000037` (r37, 484.4 / 260.4), `…_00000038` (r38, 848.3 / 707.5). Tune unmoved: SR 12.50 / LAF 2.110 / kp 0.600. All three tap-attributed to V283 (Ki fitted 51.9 / 52.0 / 52.1 against a 1.2–1.9 control floor; Kp flat 248; V282 bit 6 alive).

**🛑 OPERATOR VERDICT, 2026-09-04 — the integrator is REJECTED, on principle and on the drive:**
> *"This firmware consistently oversteers. I don't like the idea of the integrator anyways, it goes against what openpilot is modelling its output as, a torque. An integrator on steering angle rate is just steering angle, which would NOT be used in a PID loop on angular acceleration (proportional to torque)."*
> *"I don't like the direction this firmware has gone. We should keep Kp fixed, if not 0. … Frontier firmware: V282."*

**WHAT V283 MEASURED (the prereg PASS sentence fired — and it still gets rejected):** stalls 7 → **1** pooled · (b) 87 % · (c) 0.048 · (e) F7 0.00 unchanged. The P-only deadband that V281 rev 3 created was **cured** by Ki 50. But the cure came with the DC face: tight-curve achieved÷asked **0.996 → 1.278**, matched-frame Δ **+0.334 m/s²** on an SR-free instrument (livePose yaw·v, roll removed); inner DC gain **0.36 → 0.76**. **NEW RESIDUAL (verified independently): the EPS integrator does not clear at disengage** — 139–383 counts still delivered 0.5–1.0 s after `STEER_REQUEST` drops; both Ki-0 builds are at zero within 0.5 s. Grinding: V283 holds V281 rev 3's gain (~3× rarer, ~2.2× smaller than V280 rev 2), no r35-class incident, and **Ki does not touch the 20 Hz line** (1.10, p = 0.38).

**🛑 V284 — BUILT, FOUR-WAY ADVERSARIAL PASS, AND *NOT TO BE FLASHED*.** (V282 base + Ki 0 + a shaped Kp table, slot 7 only: X 0,32,36,44,88 · Y 248,248,512,512,248; 12 bytes, 6 runs.) Image `1b46f24f…74ff`, rwd `f42cfc9b…e724`. Page https://claude.ai/code/artifact/e80e5566-f343-4b55-83c8-ae5145772b83. All four attackers returned FLASH-with-caveats; **the build is sound and the LEVER is not.** Two independent grounds, and the operator's own read agrees:
1. **The measured ring loop gain at flat Kp 248 is 0.976 [0.944–0.990]** at the **7.3 Hz strong-turn ring** (per-episode complex-ACF fit, `|ρ(τ)| = exp(−α|τ|)`, `Q = πf₀/α`, 5 episodes / ~8 s; stock-Kp arm 1.027 [1.017–1.044] over 13 episodes / 29.7 s; source `STUTTER-7HZ-V283-r36-r38-2026-09-03.md` §A14.3, per-episode f₀ 6.60–7.72 Hz, median 7.3). Headroom is **2.5 %**. At 7.3 Hz the **P term DOMINATES D** (`|D|/|P| = 0.757`), so V284's raise adds **+13 % of loop gain at idx 68 and +31 % at its idx 36–44 peak** — driving the ring **above unity** (1.106 and 1.277) right across the band, idx 68 included, which is inside the ring's own index range. Model-free dose–response concurs: 0.997 [0.964–1.011]. 🛑 **CORRECTED 2026-09-04 (same day): this number was first written into the record as the ~20.3 Hz GRIND ring. It is not.** The 20 Hz ring has **no usable measured loop gain** — `CREEP-20HZ-LOOP-ID-2026-09-03.md` item 7(a) shows `L_in(line) = −1 by construction` at a spectral line, so any 20 Hz margin figure is tautological. The earlier framing (“headroom 1.0–5.6 %, spends 2.1–5.4 %”) understated the effect ~10× and named the wrong symptom.
2. **The gate that selected the shape was defective** — it scaled the servo arm as if D followed Kp. D does not. True scaling **1.771∠−16.6°**, not 2.065; corrected, M8* reads 1.216 vs flat 341's 1.033, **reversing the ranking**.

⇒ **GAIN IS SPENT AS A LEVER ON THIS LOOP.** No further Kp raise without a new margin measurement.

**🛑 THE NEW PROGRAMME — ZIEGLER–NICHOLS IN THE ACCELERATION FRAME.** The frame mapping is established (`docs/research/PID-FRAME-SIZING-KP-KD-2026-09-04.md`, `studies/pidframe/pid_frame_sizing.py`): relative to ANGLE, our I → proportional, our **P → derivative**, our **D → 2nd derivative**. So in the acceleration frame openpilot actually commands, the loop is a **PI with no derivative term** — accel-frame proportional = our **D** (0.016 s), accel-frame integral = our **P** (0.969), corner **9.64 Hz**, `|D|/|P| = 33.03·sin(πfT)`. Kd = 128 is a lead compensator whose zero cancels the 5.05 Hz output-lag pole; margin is spent by the feedback EMA (`0xC63E8/EA` = 923/1560, pole 16.527 Hz, −50.5° at 20 Hz). **No derivative filter exists anywhere.** The operator's plan: **Kp → 0** (kills accel-frame integral action) then raise **Kd** to find **Ku**.

- **PREDICTED (orchestrator, BELIEF pending `zn285`):** Kp→0 costs **−89.7 %** of loop gain at 1 Hz, **−29.3 %** at 9.64 Hz, only **−9.9 %** at 20 Hz. ⇒ Kp = 0 is nearly a pure **authority cut at lane-keeping frequencies** and barely touches the grind. Expect weak self-steer and a railed openpilot outer loop. **Do not read a quiet 20 Hz line on V285 as a grind result.**
- 🛑 **RESOLVED 2026-09-04, after TWO retractions (mine, then `zn285`'s own).** `Ku ≈ 143–151` (mine) was VOID — it extrapolated a **7.3 Hz** figure at 20 Hz. `Ku = 859` (`zn285` pass 1) was ALSO wrong: that is the Kd at which the **7.3 Hz ring's MAGNITUDE** reaches unity, but a different instability arrives first.
- ⭐ **THE BINDING MODE IS A NYQUIST −180° CROSSING AT 27–32 Hz, REACHED AT `Ku ≈ 227` [217–270], `Tu ≈ 36 ms`.** Anchored on a **MEASURED gain margin** — `CREEP-20HZ-LOOP-ID-2026-09-03.md`'s estimator table, **bar-IV** rows: **`1.75× @ 23.4 Hz`** (Kp 295) and `1.32× @ 22.4 Hz` (Kp 470). `Ku = Kd × GM` = 128×1.75 = **224** vs `zn285`'s independent 217 (3 %). ⚠ **Every OTHER estimator row in that table reads “none” for gain margin — only the bar-IV family finds a crossing at all** (orchestrator's check; carry this caveat).
- 🛑 **CORRECTION 2026-09-04 (late) — THE TWO ROOTS ARE NOT SYMMETRICALLY TRUSTWORTHY, and the lower one UNDERSTATES ITS OWN UNCERTAINTY.**
  - **`Ku ≈ 227` is `s`-INDEPENDENT** [EVIDENCE, verified at source twice]. It derives from a **directly measured plant transfer function** — `CREEP-20HZ-LOOP-ID-2026-09-03.md`'s bar-IV rows, `GM 1.75× @ 23.4 Hz` (Kp 295) and `1.32× @ 22.4 Hz` (Kp 470), `Ku = Kd × GM` — with **no arm split anywhere in the derivation**. No value of the r24 scale `s` can move it. Kd 160 is 1.4× below Ku even at the low end (217).
  - 🛑 **The lower root `118 [102–125]` is the ONLY `s`-sensitive number, and its interval reflects route-to-route PHASE variation ONLY.** The r24 cave bit publishes **sign (phase), not magnitude**, so `s`'s measured 0.30–0.52 amplitude range **has never been propagated through that calculation** — the instrument that would allow it does not exist on the wire. ⇒ **Every statement of the form “today's Kd 128 sits 8 % above the floor” UNDERSTATES its own uncertainty**, including several made earlier in this same document and relayed to the operator. The true lower root may sit closer to 160 than the published interval implies; **nobody has a number for how close.**
  - ⇒ **[SUPERSEDED BELOW, kept for the record] V286 was framed as an INERT TAP, NOT A LEVER** on the grounds that the one input settling Kd-160's safety (`s`) was unmeasured. Not because Kd 160 was shown unsafe — on everything measured it was not — but because the settling input didn't exist on the wire. **This is the *prefer the inert tap to the blind dose* law, applied — see the next bullet for why the Kd raise turned out separately justified anyway.**
  - 🛑 **REVERSED 2026-09-04 (later same day) — `task5rate` UNBLOCKS THE Kd RAISE. V286 = V282 + Kd 128→160 + the ladder, a LEVER again.** `task5rate` resolved the blind-band false-all-clear risk by **ENDPOINT TYPE**: an ENERGY endpoint ("did HF energy grow?") is SAFE on existing 0x18F/427 channels — no anti-alias filter anywhere in the path, so folded power is unattenuated (PSD measured *rising* 1.8–4.8× toward Nyquist on all five routes, where a boxcar sinc rolloff would predict a fall; frame-loss/leakage/fs2-producer/transport artefacts each excluded by its own control). A FREQUENCY endpoint ("PM at 27–32 Hz") is NOT safe — the label could be 68–73 Hz. **⇒ the 1 kHz `dE` cave was never necessary; V286 needed a different endpoint, not a new instrument.** Separately: the measured `|L|=0.976<1` at Kd 128/Kp 248 is `s`-INDEPENDENT (`s` enters the *inferred* root location, not this *measured* ratio) — it proves the operating point sits between the two roots regardless of where `s` places them, and since the loop's minimum sits near Kd≈500, **128→160 moves AWAY from the lower root, toward it**. ⇒ **`s`'s uncertainty never threatened a Kd raise — it threatens a Kd CUT** (e.g. ZN-PI's Kd 122). Independently re-verified this session: `FUN_0003aa2c` (r24's sole producer) has **zero references to the Kd LERP table `0xCB7D4`** — the ladder is Kd-invariant, so the raise and the ladder don't confound.
  - 🛑 **PRE-REGISTER AN ENERGY ENDPOINT, NOT A FREQUENCY ONE — the load-bearing acceptance test.** Score on total engaged HF energy over 0–50 Hz, with the 33–49.9 Hz near-Nyquist shelf as its own explicit sub-band. Do NOT compute or cite a phase margin at 27–32 Hz to justify the 16 % margin spend — `task5rate` states that justification is unsound on this instrumentation. 🛑 **Carry forward, cuts against the build**: engaging LKAS itself raises true >50 Hz content **1.4–3.4×** (loop-driven — a loop-driven resonance at 68–73 Hz is exactly what a raised Kd could create, the specific hazard the energy endpoint exists to catch). `task5rate` **WITHDREW** its earlier ≤22–33 % folded-fraction bound — the folded fraction is currently **unbounded**; do not cite the retracted number in any summary.
  - 🛑 **This build now needs the full four-way adversarial pass before any image is written** — it carries a lever, not just an instrument.
  - 🛑 **RETRACTED 2026-09-04 (same day): the 427-payload placement (4 bits, `gp-0x13CC` b2/5/6 + `gp-0x13CA` b7) is UNUSABLE and must NOT be built.** `0x00057b24` (the Honda checksum) is a **pure function, no side effects** — `FUN_00055d80` (the 427 packer, **50 Hz**) calls it, stores the result, releases. The r24 cave runs at `0x14A`'s rate, **100 Hz — twice per 427 cycle — completely asynchronously**. A cave write landing between one 427 checksum computation and the next **systematically** (not rarely) desyncs the transmitted checksum from the payload, on every threshold crossing — precisely when the instrument is doing its job. Cost: openpilot/panda may drop 427, **losing the `T` tap that rides on the same message**, possibly a DTC. The 4 bits remain genuinely DBC-undefined and firmware-unwritten as a *finding* — they are simply unusable from a cave that runs at a different rate than the packer that checksums them. Full mechanism: `docs/specs/design/V286-R24-LADDER-2026-09-04.md` §5.
  - ⭐ **CURRENT DESIGN: 2-level ladder on `0x14A` bits 3 + 7** (bit 7 = a SACRIFICE of the legacy `sign(gp-0x6b4c)` probe, not a previously-free slot — same class of sacrifice as bit 3). `0x14A`'s checksum is computed **downstream** of the cave's writes, in the **same critical section** the cave already runs under (`0x55C0A jarl 0x1fa42` enter → `0x55C0E` the cave → `0x55C18 jarl 0x57b24` checksum, three instructions later) — structurally identical in kind and timing to the already-flown bits 5/6. No lock needed, ~76 B, fits the 1048-byte free region. Bits 4/5/6 stay untouched.
  - 🛑 **RE-FRAMED ACCEPTANCE CRITERION (`team-lead`, 2026-09-04) — this is NOT about pinning `s` to a point.** The decision is binary: does the ladder bound the Kd-118 floor's **upper bound** below the candidate Kd with margin? Candidates: **Kd 122** (ZN-PI, operator's current preference) sits 3 % above the nominal floor; **Kd 160** (candidate F/ZN-PID) sits 36 % above it. A *narrowing* (three bins + the two existing relative comparators) can answer this even without recovering a point `s` — that reframing is why the build is worth cutting despite not pinning `s`.
  - 🛑 **OPEN PRE-FLASH ITEM — threshold placement, do NOT flash with a guess.** Documented fallback `T1=16, T2=64` raw is a BELIEF placeholder (only the closed-form p50=21/p90=403 as anchors). **Better method, not yet run:** bit 6's measured duty (`\|r24\|≥\|T\|`) on r36/r37/r38, combined with `T`'s own measured distribution (427 tap, already flown/decoded), gives a genuine empirical point on `\|r24\|`'s CDF; bit 5 against the aggregator gives a second. Route-data work, assigned once `task5rate` reports.
  - ⚠ **“DBC-undefined” never means “unwritten”** — in the 427 frame, byte0 bits 3/4 are DBC-undefined **yet firmware-written**. Worth keeping on record even though the 427 placement itself is retracted — it is the general lesson, and it is why the dual writer+DBC standard exists.
- ⭐⭐ **THE STRUCTURAL RESULT — Kd IS BRACKETED FROM BOTH SIDES: `Kd ∈ [118, 227]` at Kp 248.** The two modes move OPPOSITE ways in Kd: the **7.3 Hz ring gets BETTER with more Kd** (lower root **118**; a cut RE-ARMS the cycle) while the **27–32 Hz Nyquist gets WORSE** (Ku 227). **Today's Kd 128 sits near the FLOOR** — 1.08× above the root, 1.77× below Ku. This replaces both “Kd cuts are DO-NOT-FLASH” (still true, now with a reason from ABOVE as well as below) and the withdrawn “F is 5× below Ku”.
- ⭐ **TWO INDEPENDENT METHODS CONVERGE ON Kd ≈ 160.** A Ziegler–Nichols PID of the acceleration frame gives **Kd 162 / Kp 329**; the loop-shape study's **candidate F** gives **Kd 160**, from wholly separate reasoning. **Within 1 %.** F is the best-centred point in the bracket (1.36× above the root, 1.42× below Ku) and spends **16 %** of the blind-band margin (GM 1.77× → 1.48×, i.e. 5.0 → 3.4 dB).
- 🛑 **`zn285`'s pass-1 ZN CONSTANTS ARE RETRACTED AS UNSTABLE** — ZN-PI (Kp 108 / Kd 387) has **GM 0.69×**, ZN-PID (241/515) has **GM 0.51×**. DO NOT BUILD EITHER. **Td has no realisable home** in the firmware (verified from the disassembly: exactly three addends and ONE difference operator, `0x29EE2 sub r27,r8`, single history cell `gp-0x6cf8`) — the reachable form is ZN-**PI**, not PID.
- ⚠ **ZN-PI (revised, Kp 148 / Kd 122) is the ONLY candidate better than today on BOTH gates** (ring 0.936, GM 2.01×) — but Kp 148 cuts DC tracking to ~40 % vs today's 53.5 % (**more understeer**), and its Kd 122 sits only **3 % above the ring root of 118**. Do not fly it without a per-episode arm split first.
- 🛑 **THE BIGGEST OPEN HOLE: the answer bifurcates on the plant's phase above 25 Hz** — a delay model (−3.75°/Hz, = 10.4 ms) gives **Ku 227**; frozen phase gives **Ku 865**. A **3.8× spread**. And **27–32 Hz is ABOVE the 427 tap's 25 Hz Nyquist**; `gp-0x6a56` at 100 Hz reaches it only through the unresolved **80→20 Hz alias** ambiguity. ⇒ **A one-drive Ku estimate on the flying channels is INFEASIBLE for the binding mode.** The instrument that WOULD settle it is an on-ECU **`dE` sign-change rung at the PID's own 1 kHz** (immune to both the Nyquist and the alias; the 24–28 Hz control floor is flat, giving it a null control). Being costed by `telem285`.
- 🛑 **Kp = 0 DELIVERS ZERO STEADY-STATE LANE KEEPING — not weak, ZERO.** The plant is **type 0 in rate** (pinned from four measured (|T|, wheel-rate) pairs held 1–3 s on r34/r35), so with Kp = 0 AND Ki = 0 the controller has `L(0) = 0` **exactly**: steady state ⇒ `dE = 0` ⇒ `D = 0` ⇒ `S = 0`. Orchestrator-verified by integer mirror (0.000 deg/s at every plant gain) as well as by `zn285` (0.0 % of a 25 deg/s request at 4 s; V282 53.5 %, V283 96.6 %). **V285 is a BENCH CONFIG, not a drive.** Every stability metric improves on it (ring 0.861, GM 2.11×, +52° lead at 7.3 Hz) — and it cannot hold a lane.
- **Authority loss from Kp→0, re-derived and confirming the orchestrator's estimate:** −89.7 % @ 1 Hz (exact) · −54.2 % @ 5 Hz · −40.3 % @ 7.3 Hz · −30.3 % @ 9.64 Hz · −20.2 % @ 13.5 Hz · −12.1 % @ 20 Hz · −5.5 % @ 40 Hz. 🛑 And **“removing P removes ~90° of lag” was BACKWARDS** (orchestrator error): **P is phase-FLAT**; removing it ADDS lead — +52° at 7.3 Hz, +25° at 20 Hz.
- **V285 IS BUILT — and it is a BENCH / SYSTEM-IDENTIFICATION CONFIG, NOT A DRIVE CANDIDATE. 🛑 DO NOT FLY.** V282 + Kp slot 7 → 0. Image `7c2cfef7…9605d2`, rwd `83079d43…a652e`, **444/444** (censused 278 substantive / 29 vacuous / 2 tautological / 135 redundant), independent rebuild bit-for-bit, cal-only, **no cave change**. **9 bytes in 6 runs**, not 14 — 248 = `0x00F8`, so only the five LOW bytes change: `0xE5384/86/88/8A/8C` `f8`→`00` + CRC `0xE5FFC` `7ea6f28e`→`9ee32106`. Slot 7 only; X, Kd, Ki, cave, hook and 427 tap all byte-identical.
  - **Safety check it needed, answered NO exhaustively:** all **17** divisions in `FUN_00028ea6` divide by an **X-axis segment width**; **no Y is ever a divisor**, and X is untouched. Y = 0 is degenerate in the safe direction on all three LERP branches.
  - **Cross-family census (after a label error):** MAP (`0xC9A88`, 28×`0x2C`), Kp (`0xCB994`, 28×24) and Kd (`0xCB7D4`, 28×20) **interleave** across `0xE4000`–`0xE82FF`. **84 records, ZERO overlaps**; the edit lands in exactly one — Kp slot 7. Verified twice, independently. **Never assume contiguity in this range.**
  - **Telemetry: no new bytes, deliberately** (`docs/specs/design/V285-TELEMETRY-2026-09-04.md` returned BUILD NOTHING — three candidates redundant with flying channels, the fourth structurally vacuous since `S = D·254/256` leaves ONE live term). Read by `gp-0x6a56` on 0x18F[2:3] (100 Hz), `T` on 427 (50 Hz), r24 bits on 0x14A b4.
  - **DRIVE SPEC if it is ever flown:** the ring is a **damped TRANSIENT after a disturbance, not a standing hum** ⇒ seek bumps, quick corrections, stall releases while LATERALLY engaged; smooth cruising = uninterpretable null. 🛑 **Do NOT read a quiet 20 Hz line as a grind result** (`0xC6446` byte-untouched). The P-only deadband/stutter is expected to **RETURN and worsen** (Ki 50 was its cure).
- 🛑 **RECONCILED 2026-09-04: this bullet described a SUPERSEDED V286 concept (Kd 160 + a 1 kHz `dE` rung). The BUILT SPEC (line 7/§V286 above) is the INERT r24 ladder, no Kd change.** The `dE`-rung thread below is now a **separate, still-open question** — whether a Kd raise could ever be safely instrumented at all — parked pending Task 5's true rate, not a gate on the current V286 spec. Kept for the record, not as an active gate.
  - **The rung's design, and the defect caught in it:** `0x29EE4 mul r7,r8,r0` is **exactly 4 bytes**, sits right after `dE` is formed (`0x29EE2 sub r27,r8`) and right before it is destroyed — a 1-for-1 swap with no length change. `r10` is **provably dead** (`0x29EE8 ld.hu 0x71b6,tp,r10` overwrites it unconditionally). 🛑 **But `jarl …,lp` would CLOBBER A LIVE REGISTER** — the prologue `0x28EA6 prepare {r20…r29, lp}` saves `lp`, yet `0x28EBC ld.hu 0x72ea,tp,lp` then **reuses `lp` as scratch**; independently re-traced and CONFIRMED LIVE end-to-end (`0x29A2C ld.bu -0x6809,gp,lp` → next consumed at `0x2A29A`, a gap spanning `0x29EE4`). **Use `jr` (also 4 B; well inside ±2 MB) — the return site is fixed, so no link register is needed at all.**
  - **Still open, parked, not actively gating anything today:** `gp-0x683c`'s access width (byte-granular region, likely fine) · a physical spare byte for the subroutine (1048 B confirmed at `0xC4BD8`-`0xC4FF0`, ample) · the **1 kHz cycle-budget** measurement, cancelled per operator instruction pending whether the rung is even needed (see the `dE`-vs-`0x18F` reconstruction question below) · **the readout bits are now moot for this design** since the current V286 spec uses `0x14A` bits 3/7 for a different purpose (the r24 ladder), not this rung.
  - 🛑 **THE OPERATOR ASKED WHETHER THE RUNG IS NEEDED AT ALL** — E is reconstructable in formula from wire-adjacent signals (`sp`=`gp-0x6a32`, corrected from an earlier mix-up with the demand index `gp-0x697a`; `fb`=EMA(`gp-0x6a56`,923/1560) two-sample-summed, NOT raw `gp-0x6a56`), but fidelity at 27-32 Hz depends on whether `gp-0x6a56`'s true update rate is bandlimited below the 0x18F stream's 50 Hz Nyquist — i.e. on **Task 5's execution rate**, which THIS SESSION found is unresolved across 3+ prior sessions (a "100 Hz" claim was explicitly RETRACTED earlier because a sibling signal's measured frequency response pointed close to 1 kHz). `OSTM1` (a second hardware timer, never previously checked) was ruled out cleanly as Task 5's driver this session — positive-controlled, zero hits image-wide. **Handed to `task5rate` for the empirical half** (PSD-to-Nyquist folding evidence already found on all five routes); gate (c) stays cancelled until that resolves.
  - 🛑 **`sign(r24)` (0x14A b4.4) is NOT sacrificeable** — it is the direct instrument behind the pump/damp finding that made `0xC6446→512` a DO-NOT-FLASH. A proposal to spend it was withdrawn after the agent found it had conflated `gp-0x6bbe` with `gp-0x6ada`.
- ⭐ **THE PID PUBLISHES ITS OWN INTERNALS EVERY TICK** (live code, below the dead duplicate at `0x2A30E`): `0x2A17C`→`gp-0x6b2e` (output) · `0x2A188`→`gp-0x6b32` (P) · `0x2A18C`→`gp-0x6cf8` (E_prev, `st.w`) · `0x2A19C`→`gp-0x6b36` (D) · `0x2A1A2`→`gp-0x6b34` (sum); `0x29DDA`→`gp-0x697a` (demand idx). ⚠ `D = 16·dE` **only inside ±10240** — on railed frames the sign holds, the magnitude does not, and rails stop being rare in exactly the Kd sweep. ⚠ A halfword scan misses `gp-0x6cf8`: `st.w` encodes disp `-0x6cf8` as `0x9309`, not `0x9308` — **the displacement parity trap, third instance today**.

**THE THREE OPEN SYMPTOMS, in the operator's own words (2026-09-04, against V282 as frontier):** *rare, attenuated grinding* (still barely noticeable, not eliminated) · *stuttering when the wheel is turning* · *understeer on straight roads* (he believes this is a StarPilot-side change).

**STARPILOT SIDE (`docs/research/STARPILOT-SIDE-FIX-AND-FORK-MAINTENANCE-2026-09-04.md`):** the updater does **no remote validation**, so a fork branch works and the Galaxy update button keeps working; a purely local patch is wiped by `reset --hard` + `clean -xdff`, and a local *commit* silently stops updates. The Accord constants block has taken **4** commits since June against the file's 92 → low rebase risk. **`SteerKP` 0.6 → 0.8 is FREE (no fork) and worth ≥ 11–29 % (a lower bound)** — flown alone it is the cheapest open move. No `SteerKI` param exists, so a Ki change needs the fork. 🛑 **HARD CONSTRAINT: a KI fork must fly on a Ki-0 image as a SUBSTITUTE, never on top of V283** — two cascaded integrators is uninterpretable. V282 is Ki-0, so V282 + KI-fork is the legal pairing. **openpilot can never reach 7 Hz or 20 Hz** (3 counts/10 ms slew, 1.2 Hz jerk filter) — the firmware symptoms are not reachable from there.

🛑 **SteerRatio 12.5 is currently an UNDERSTEER bias, not an oversteer brake.** SR enters ONLY `measured_curvature`; lower SR over-reads curvature → under-turns. (Stated backwards once and corrected — do not repeat it.) Do not raise SR while assessing an integrator change; it confounds.

**MEASURED, and it reframes the oversteer:** openpilot is **barely commanding in steady curves** — f +0.800 cancelled by (p −0.300, i −0.392), net +0.108, median signed output **+0.009**; over-delivery moves *inversely* with command. Both free-toggle discriminator drives were therefore withdrawn.

**RETIRED / CORRECTED this session:** the pooled half-power Welch Q estimator is **RETIRED** (it tracks bin width: r38 gave 0.9 / 12.3 / 26.7 across `nperseg`) — replaced by the per-episode complex-ACF fit. **"~1500 counts breaks a stall" is UNSUPPORTED** — reference builds deliver 586–817 counts and still stall 12–19 %; the *direction* survives (V283 took 7 → 0/1/0 stalls at the same tune), only the target number was invented. **r24's magnitude corrected to s = 0.37 [0.24–0.52]** (clean AC-vs-AC cut; 0.43 pooled) — the closed form was ~2.7× too big; **the phase survives** (r24 pumps at 7 Hz, damps at 20 Hz), with a 180° cache sign-convention trap reconciling three independent readings. **The `Re` → bar-amplitude map is UNSUPPORTED** (+4.7 % predicted vs ×0.348 measured) — the deepest open hole in the loop-shape work. **`0xE511C` is not virgin** — V279/V279r1 set it to 0 (confounded, unflown). **V281 rev 2 differs at 10 bytes, not 2** — it lowered the knots (X = 0,24,68,136,208) *and* moved Y; the lineage line "flat 341 from idx 24" is incomplete on both counts.

**Corrections of record, prior session:** the override-taper ARM is a 0xE4 byte-2 field openpilot sends as 0 → the 2240 cliff is never selected; the live post-PID fade is 0xCBBC4 (the ~0.5 mirror slope); 'Lever B unreachable' is true only of c5 images (re-verified on the flown image with a positive control; 14 flagged `gp-0x683c` accesses all decoded to `gp-0x683b` — the parity trap); HondaLateralPidKp/KiScale are inert on the torque path (verified on the car's commit); torqued has never been valid on the modded EPS.
...
| edits | `0xC62E6` 7680→0 (feedback operand forced to 0) · Kd→0 (28 rec) · map Y=2X (28 rec) · Kp flat 256 (28 rec) · packer `0x55DF0`–`0x55E11` = **signed DELIVERED TORQUE**: `(sign(T)<<9) \| (\|T\|>>3)`, T = `gp-0x6b38` |
...
| image | `a165b1a59307ab67867fd5488c287a2271d51e322ff601d527853712ea423485` (rev 2; rev 1 `dca2acbc…` carried the sel/demand/sign(E) tap) |
| rwd | `ea0d7dfdbbc3277141aaff0666aea9f2ebc82cafe8bdd5b64fefd19d3532985b` |
| assertions | **710/710** incl. end-state re-reads from the final image AND the decoded .rwd; rev 1: independent rebuild reproduced, 85/99 mutations caught, the 3 misses closed |
