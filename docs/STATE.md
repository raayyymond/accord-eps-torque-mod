...
## ✈ THE DECISION, IN ONE PLACE  — updated 2026-09-03 late (V281 rev 3 FLEW: 7 Hz cycle gone, deadband arrived; V282 built + passed; V283 = + Ki 50 being built)
...
**ON THE CAR: V281 rev 3 — FLOWN 2026-09-03, route `…_00000035` (r35), tune 12.5 / 2.11 / 0.03 / 0.6.** Operator: oversteering largely gone; prolific understeer; stuttering and rare attenuated grinding still present; a pronounced grind at 23:48:21 seg 16 (being read). MEASURED (`HIGHANGLE-r35-V281R3-…`, `V281R3-READ-r35-…`): the self-sustained 7 Hz cycle is GONE (F7 0.0/100 s vs 6.8; rate power ×0.19; ripple/level 0.18); a damped ring at ~40 % remains with f0 and bar/rate unchanged and the servo share down, r24's not → **r24 is the pump, confirmed on the wire; no further Kp cut**. The predicted COST arrived: seven 1–3 s stalled runs at idx 54–79 (0.62 of V280's torque), dead fraction ×3–8 — the P-only deadband = the remaining 'stutter'. The understeer is MOSTLY SteerRatio 12.5 (the loop settles at ~0.62 of the request; the controller says OVER while the road says UNDER) → SR 16.1 later, after the Ki read (operator: no confounding). **THE 23:48:21 GRIND (`rlog-tools/studies/grind/GRIND-INCIDENT-r35-…`):** a 0.9 s exponential burst of the same 20 Hz mode at a loaded left-turn start (idx 10→137, wheel 37–60 deg/s, hands ON tightening, no rail, chain linear), a 7.5 Hz ripple underneath, killed by the hands; the mirror reproduces it (D 2/3). Route-wide, V281's creep line is 3.5× rarer and 2.5× smaller than V280's at matched idx — Kp flat 248 helped the grind (BELIEF: the closed-loop sensitivity peak shrank). **V282 (built, three-attacker PASS, page https://claude.ai/code/artifact/f471ed6a-b877-4b39-a957-446fd371f9eb) = V281 rev 3 + the inert r24 comparator tap. THE FLIGHT CANDIDATE: V283 = V282 + Ki 50 at 0xC63E6** (the deadband companion; corner 0.25 Hz; < 3° at 7 Hz, ~0 at 20 Hz; reads on the T tap) — being built; prereg `rlog-tools/studies/osc-highangle/PREREG-V283-READ.md`.

**THE TWO DEEP ANALYSES (2026-09-03, `docs/research/7HZ-STRONG-TURN-DEEP-ANALYSIS-…`, `GRINDING-DEEP-ANALYSIS-…`):** the dispute is CLOSED from drives already taken — the cave's sign(gp-0x6ada) bit on 0x14A b4.4 shows r24 PUMPS at 7 Hz (loop share 1.17 vs the servo's 0.81; it sustains the strong-turn ripple) and DAMPS at 20 Hz (~83 % of the aggregator's damping at the grind line). **0xC6446 5244→512 is DO-NOT-FLASH** (fixes 7 Hz, strips the grind damper); Honda's arms sit near the 7 Hz neutral point, V280 flies 2.8× past it. V281 rev 3 predicted to PASS its 7 Hz prereg (return ratio 0.88–0.91) but creates a LOW-DEMAND DEADBAND under road load (idx 26: 555 counts vs a 690-count load, wheel does not move); companion V283 = + Ki 100 at 0xC63E6 (corner 0.5 Hz; ~0 cost at 7–9/20 Hz; no tap needed). Ranked loop shapes for the grind (cal-only, never touched): output-lag pole 5→15 Hz (0xC63EC/EE) + 0xC6446→2048 first; shared cost = unobservable HF gain. **THE ONE NEXT BUILD — BUILT: V282 = V281 rev 3 + 10 bytes (four `ld.h` displacements in the V105 cave + the page CRC): 0x14A b4.6 = |r24| ≥ |T|, b4.5 = |r24| ≥ |aggregator|; keeps the 427 T tap; read-only; image `0ea98d06…`, rwd `61836515…` (`…V282-V281R3BASE-KP.FLAT.Y0-CAVE.R24CMP.BITS5.6…rwd`), firmware repo 7d68974; 357/357; adversarial ×3 in progress. FLY V282 INSTEAD OF V281 rev 3: same Kp edit, plus the instrument; every V281 statistic reads unchanged. Prereg `rlog-tools/studies/grind/PREREG-V282-READ.md`. Prereg: bit-6 duty over engaged hands-off creep ≥ 0.22 → r24 dominant (do not cut it); ≤ 0.10 → the hidden 1024 arm (gp-0x671d) is live.
