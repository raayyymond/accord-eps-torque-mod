...
> 🚩 **FLIGHT ORDER: V221.** = **V217 plus two bytes.** Delta from the CAR (**V122** — corrected 2026-08-29, I had used V108 all session) is **27 payload bytes** — notch 20.50 Hz (grinding) · `0xC63AE` 512 (ratchet) · `0xC6CD0` 6×→8× + clamps (authority) · **`0xC6446` 5244→13107 (Lever B)** · the 427 probe. Drive card: `docs/scoring/DRIVE-CARD-V221.md`. **V217 is the fallback** (`DRIVE-CARD-V217.md`), and its 25-byte delta is otherwise byte-identical. Predecessors V212–V216 remain as arms; V214 is SUPERSEDED. Shelf: `docs/scoring/SHELF.md`. Pre-registered scoring: `docs/scoring/SCORING-V217-preregistered.md` (applies to both).
