...
## ✈ THE DECISION, IN ONE PLACE  — updated 2026-09-04 late (**r39 FLEW: V282 + the variable-SR map + SteerKP 0.8**; the map is CONFIRMED LIVE and STAYS; over-steer is an **EQUILIBRIUM** change the map fully explains; the operator's "grinding" on this drive is **the 7.3 Hz ring, NOT the 20 Hz creep grind**; 🛑 **THE Kd AXIS IS CLOSED** — the measured r24 arm makes Kd worth 1–2 % instead of 26 %, and a raise to 160 sits at `|L| = 0.997`; **NO FIRMWARE CUT THIS SESSION**; the single next move is **`SteerLatAccel` 2.11 → 4.0**, then a **plant-magnitude identification drive**)

**ON THE CAR: V282**, confirmed **on the wire, not from the label** [EVIDENCE]: Ki fitted from the 427 tap = **2.21 [1.28–3.33]** against a Ki-0 control floor of 1.24 (r35) / 1.90 (r34), versus **51.9–52.1** on the three V283 routes; `Kp_hat` 242 at idx 0–40 (matches r35's 242, separates from r34's 285); `0x14A` byte-4 bit 6 duty **0.114** route-wide with 11.7 transitions/s against **exactly 0.0000** on r34/r35 — the r24 comparator cave is live. ⚠ The cave is **byte-identical** V282↔V283 (the only delta is `0xC63E6` 00→0x32 plus its page CRC), so the cave alone can never separate them; the Ki fit is what does.

**THE DRIVE — route r39** (`75604b0a432fdc89_00000039--f56039af87`, 16 seg, 2026-09-04). **952.8 s wall, 869.7 s LATERALLY engaged = 92.5 %**, `latActive` disagreeing with `0xE4 AND 0x18F` on **46 frames (0.05 %)** — essentially no longitudinal-only confound, unusual for this corpus. Engaged |angle| is unusually rich at the top: **63 s above 60°, 43 s above 121°**. 427 saturation **0.000 %** route-wide; outer-loop `saturated` **0.0000** over 810 engaged s. Caches: `_scratch/cache/r39/` (openpilot channels) and `analysis-2020accord/_scratch/cache/r39/r39.npz` (91 fields, CAN + 427).

**THE TUNE AS FLOWN — it differs from r35 by exactly TWO things, both authority INCREASES:** the variable-SR **map** (effective 12.5 → **16.00 near centre**, ×1.26) and **`SteerKP` 0.6 → 0.8**. `SteerLatAccel` 2.11, `SteerFriction` 0.03, `SteerDelay` 0.2, `ForceTorqueController` / `ForceAutoTuneOff` both True — all unchanged. Decoded from the operator's own backup, `analysis-2020accord/reference/toggle-backup_V282_20260904.decoded.json`.

### ⭐ THE VARIABLE-STEER-RATIO MAP IS LIVE, AND IT IS NOT UP FOR REVERSAL
[EVIDENCE, two independent agents, two instruments.] Reconstructing `actualLateralAccel` through the shipped chain and TLS-regressing on the logged channel: `map(|steeringAngleDeg − angleOffsetDeg|)` gives **slope 0.99997, r = 1.000000, residual sd 0.00045 m/s², max residual 0.0199** — two orders better than every alternative (`lpar_sr` 1.211, the old `0.857` scale 1.406, fixed 16.1 0.949). The per-frame solved ratio traces the map **bin for bin**. 🛑 **`liveParameters.steerRatio` is a hard 12.500 with ZERO variance and was NEVER USED** — the Galaxy `SteerRatio` toggle is now structurally inert for the Accord, by design. Anything back-calculated off `lpar_sr` on this route is **off by 28 %**.
**Operator's ruling, 2026-09-04: *"SR is a definite keep for sure."*** It is a correction to a wrong measurement, correct independent of feel. **Do not propose touching it.**

### 🛑 THE OVER-STEER IS AN EQUILIBRIUM/SCALE CHANGE, NOT A TRANSIENT — and the map explains ALL of it
Operator: *"worst-case over-steer on turn at 20+ mph; general over-steer on curves at medium and at high speed; solid, stable lane keep on straights; amazing authority."*
- **Time course, 13 curves ≥ 5 s, SR-free:** `R` = 0.90 → 0.92 → 0.98 → 1.04 → 1.07 → 1.14 → 1.11 → 1.06 → 1.25 → 1.09 at t = 0.25…5.0 s. Starts UNDER, crosses 1.00 at ~0.8 s, stays 5–25 % over for the whole curve. **Quasi-static excess (+0.105) EXCEEDS transient (+0.040).** No peak-and-decay (r34 shows one; r39 does not).
- ⭐ **THE IDENTIFYING SPLIT — the two changes are NOT confounded.** `R_road = R_m × (1/ρ)`; SR enters `latcontrol_torque` in exactly one place so it moves `1/ρ` only, and `SteerKP` can only move `R_m`. Measured: `1/ρ` **0.784 → 0.988** (×1.26, the map's designed 16.00/12.5); `R_m` **1.178 [1.081,1.245] → 1.123 [1.083,1.180]**, CIs overlap, **no resolvable change**. `R_road` 0.923 → **1.116 [1.029,1.205]**, `ΔR = +0.192 [+0.101,+0.281]`. ⇒ **the map explains the whole delta; `SteerKP` 0.8 explains none of it.**
- **Why 20 mph is worst — UNITS, measured.** The lateral-accel excess is flat in speed (+0.08…+0.16 m/s²), but the excess **yaw rate** is **+1.09 deg/s below 7 m/s and +0.72 at 7–11** against **+0.29 at 16–22** — 2.5–3.7× larger, building **1.4–2.2° of heading error in 2 s**. `SteerKP` could not have caused it anyway: the friction term is kp-independent, so 0.6→0.8 is +10 % of P at 9 m/s but +30 % at 25 m/s — *smallest* exactly where the worst case is.
- ⚠ **INSTRUMENT SENSITIVITY, stated:** on the **path** measure (`v·yaw_cal`, no roll term) r39 reads **1.116**; on the **specific-force** measure (`v·yaw_cal − g·sin(roll)`) it reads **0.983**, and `ΔR` there is +0.109 and NOT significant. **Path is the correct instrument** — `calc_curvature`'s own docstring says the curvature *"multiplied by the speed will give the yaw rate"*, its `roll_compensation` returns a **curvature** (not a gravity subtraction), and `yaw_cal` is a gyro with no gravity in it; `torqued` subtracts gravity only because it fits torque→**force**. **`R_m` is 1.123 under BOTH arrays, so the identifying split carries no instrument choice at all.** Q1's verdict is robust on the dwell trajectory and on peak÷settled (r35 1.173 / r39 1.171 — same shape, displaced level); it is **NOT** robust on the quasi-static-vs-transient split, which reverses by 0.011 m/s². **Do not lean on that third discriminator.**
- 🛑 **THE RESIDUAL THE MAP UNMASKED, and no gain knob removes it.** `R_m > 1` in curves on **every** arm (1.112 / 1.178 / 1.123): the loop sits above its own setpoint inside a curve and pays it back on the straight (folded mean error −0.41…−0.49 in curves vs −0.03…−0.08 on straights). ~11 %, present all along, newly visible now the 22 % under-read is gone. Because it is an **equilibrium** and the integrator is unsaturated, **only the ratio or a fork change (ki, or the curve-exit unwind freeze) reaches it.**

### ✈ THE ONE MOVE: `SteerLatAccel` 2.11 → **4.0**, flown ALONE. Keep `SteerKP` 0.8, `SteerFriction` 0.03, keep the map.
Sized from r39's own f/i balance: **steady FF over-commands 1.86×** (net `f+i` = 0.250 against raw `f` = 0.465), and the integrator **opposes** the feedforward in **81.4 %** of engaged frames — it spends **46 % of itself cancelling it** (curve-steady `f/p/i/d` = +0.465 / −0.119 / −0.215 / 0.000). LAF is a **DIVISOR** (`interfaces.py:329`), and `get_friction`'s `±friction·LAF` is then divided by LAF, so **friction's torque contribution is LAF-invariant** ⇒ a clean uniform authority cut on f, p and i. A second estimator (torqued's own window, `car_fit`) puts the endpoint at **9.5** (r34 10.17, r35 10.43); the two sizings disagree **2.4×**, so 4.0 now and 9–10 later — straight to 9.5 is −78 % in one step and not one-drive-interpretable.
⭐ **It treats BOTH symptoms**, because the same over-command is what drives the 7 Hz ring (below). ⚠ It is a **CONDITIONING** fix for the over-steer, not a cure — the discriminator is `R_m`: if it falls toward 1.00 as the integrator's cancellation share drops, the residual was the integrator being busy; if `R_m` does not move, it is structural and needs a fork change.
**Integrator headroom, measured:** `max|i|` **0.909 of a ±2.110 bound (43.1 %)**, p90 0.417, p50 0.212, `saturated` 0.0000 over 810 engaged s.
🛑 **The `SteerLatAccel` ceiling is ALREADY 16.893, not 2.534** — r39's own `initData.gitCommit` is `8a28dcef8` and the device was verified over SSH on that commit (`LAT_ACCEL_FACTOR_MAX_MULT = 10.0`). **Nothing to update.**

### 🛑 SYMPTOM RE-ATTRIBUTION — the "grinding" on r39 is the 7.3 Hz RING, not the 20 Hz creep grind
Both `userBookmark` presses characterised (route t **689.643** and **927.694**; the episode precedes each mark by 6–17 s, as the operator said).
- **Mark 1, episode t 677.5–683.5** — a loaded turn out of creep, angle +19→+141°, v 2.7→5.7, cmd −1969 (idx 122), hands light. **Dominated by 7.68 Hz ×327 (bar 6–10 = 723 raw); the 20 Hz mode is SECONDARY (20.69 Hz ×34, 172 raw).** The 7 Hz envelope **bursts** (+6.81 /s over 0.34 s, peak 874 raw) while the 20 Hz envelope rises at only **+0.14 /s — it rides along, it does not grow.** **NO RAIL anywhere.** Operating point near-identical to the r35 incident (idx 96/147 vs 100/137; rate 36/68 vs 37/63) — **what differs is which mode carries the energy: 20 Hz content 28 % SMALLER than r35's, 7 Hz content 13 % LARGER.**
- **Mark 2, episode t 911–923.5** — a **railed** low-speed manoeuvre (cmd −4096 at idx 240, angle −175→−345→+106°, hands 2756–3091 raw, past the 2240 fade knee) in a segment carrying **1,931 frames of REVERSE gear**, and **lateral disengages 4 s BEFORE the mark**. Not a line at all — a broadband 10–22 Hz hand-back smear. **Treat as a manual/park event, not LKAS.**
- ⭐ **THE STRONGER OUTER LOOP IS DRIVING THE RING ~4×.** At matched demand index, r39's 6–10 Hz bar in the loaded-turn stratum (|ang| ≥ 30, idx ≥ 68) is **p50 178 raw vs r35's 42** (19 vs 12 windows — thin; the DIRECTION is the finding, not the size). ⇒ **the LAF cut reduces the ring's DRIVE independently of any firmware change.**
- **"Various" has a number:** **25 line-present stretches ≥ 1 s** over 193.3 s of engaged v < 6; the two marks are the **2nd and 3rd loudest**, and **there is a LOUDER one he did not mark** (t 883.2, idx 83, 116°, 30.6 deg/s).
- 18–22 Hz band, idx-matched onto r35's own creep idx mass: **r39 33.3 % present / amp 35** vs **r35 13.8 % / 24** vs **r34 57.2 % / 57** — ×2.4 as often as r35 at ×1.46 amplitude, ×0.58 of r34 at ×0.61. ⚠ A second statistic on the same windows (episodes per 100 s: 3.38 / 6.00 / 9.08) runs the **other way** and rests on 2–3 events — too thin to lean on. **Quote the idx-matched window presence.** 🛑 **The reweight removes the EXPOSURE share of the confound, not the BANDWIDTH share** — `SteerKP` 0.8 also raises the outer loop's own bandwidth.

### 🛑🛑 THE Kd AXIS IS CLOSED — the session's main firmware result
The 7.3 Hz ring is a **sum of two arms**: the LKAS **servo** arm (carries Kp and Kd) and the **r24** arm (carries neither — `FUN_0003aa2c` has no Kp/Kd reference). A sustained ring requires `Ls + Lr ≈ +1` in **magnitude AND phase**, so fixing the measured complex sum (`1.0028 ∠ +0.38°`), fixing `Lr`'s phase, and scaling `|Lr|` **FORCES `Ls`** — no free parameter. (Both the naive scaling and the `|Ls|+|Lr|`-preserving renormalisation are wrong; the first predicts `|L_today| = 0.46` against a measured 0.980.)

**The r24 arm is MEASURED at 0.40–0.52 of the modelled magnitude**, by two independent routes that agree: `grind39`'s `s` = **0.41 (creep) / 0.52 (loaded)** — replicating r36–r38's 0.42/0.47 on a different route, different Ki and different tune — and the bit-6 duty ladder **renormalised on r39's own `|T|`** (duty ratio 0.464, magnitude interpolation 0.40; duty is a CDF crossing so it is not linear in magnitude, but the conclusion is identical across the whole band).

| `Lr` scale | forced `Ls` | apart | Kd 128 | **Kd 160** | Kd 192 | Kp 148 |
|---|---|---|---|---|---|---|
| 1.000 (modelled) | 0.550 ∠ +96.0° | 123° | 0.976 | **0.909** | 0.845 | 0.900 |
| 0.500 | 0.548 ∠ +30.4° | 57° | 0.976 | **0.990** | 🛑 1.011 | 0.808 |
| **0.460 (measured)** | 0.575 ∠ +26.4° | 53° | 0.976 | 🛑 **0.997** | 🛑 1.024 | **0.802** |
| 0.400 | 0.620 ∠ +21.1° | 48° | 0.976 | 🛑 **1.006** | 🛑 1.044 | 0.793 |

⭐ **AND THE OPTIMUM COLLAPSES, not just the raise.** Minimising `|L(7.3)|` over the whole usable Kd range: at the **modelled** arm the best Kd is **260, worth 26 %**; at the **measured** arm it is **Kd 40–73, worth 1–2 %**. **Kd is not a weak lever at the real operating point — it is essentially not a lever.** (Kd 40–73 is **NOT** recommended: it would gut the lead that cancels the 5.05 Hz output-lag pole, and 1–2 % sits inside every uncertainty in this framework.)
⭐ **`Kp` is the only cell with real ring authority, and it STRENGTHENS as the arm shrinks** (`|L(7.3)|` at Kp 148 / Kd 128 goes 0.900 → 0.802) because it attacks the arm that then dominates. **But its own cost got worse on r39 at the same time** — see the deadband — so nothing is cut.

**`max|T|` independently disqualifies the Kd raise** on the operator's own actuator criterion: the raise removes the 7.3 Hz peak by **creating a bigger one at 25–28 Hz** — `max|T|` 1.447 (today) → **2.247** (Kd 160) → **3.958** (Kd 192), which is `1/GM` appearing in the closed loop. Kp cuts go the other way (1.181); ZN-PI 148/122 is the flattest candidate at 1.090.

### 🛑 ZIEGLER–NICHOLS IS THE WRONG RECIPE FOR THIS LOOP — structurally, and it points the wrong way
Operator's spec, 2026-09-04: *"Oversteer is probably outerloop responsibility"* and *"Innerloop is merely responsible for accelerating the steering angle as demanded by outerloop."* That makes the inner loop an **ACTUATOR** judged on fidelity, not a regulator judged on disturbance rejection.
1. **ZN designs IN a closed-loop peak of 1.3–1.5** (quarter-amplitude decay ≈ 25 % step overshoot) — exactly what a follower must not have.
2. **ZN's only reachable form here is ZN-PI, a Kd CUT** (`Td` has no cell: three addends, ONE difference operator at `0x29EE2`), while the fidelity optimum wanted Kd RAISED. **It points the wrong way on the one axis that sets peaking.** The one thing it got right — ZN-PID's Kd 162 ≈ the loop-shape study's 160 — is an arithmetic accident. **Keep the number, drop the derivation.**
3. **The no-overshoot family is STRUCTURALLY FORBIDDEN.** Hunting properly at Kp 0 (`Ku = 270`) gives Kd **90** ("some overshoot") and **54** ("no overshoot"); all four low-overshoot forms put `|L(7.3)| ≥ 1.01` at their own Kp. **Because only the servo arm carries Kp/Kd, turning the controller down WITHDRAWS the cancellation that keeps the sum sub-unity.** There is no "turn everything down and it calms" regime on this loop.
4. **Flat is unreachable too:** `max|T| ≤ 1.05` needs `|L_ring| ≤ 0.512`; the best available anywhere in the feasible box is **0.640** (~2.8× peak) at a thin 1.15× margin. The honest question is how much peaking at what margin — never "flat or not".

### ⭐ THE COMMAND BAND IS 0–0.75 Hz — MEASURED, and it retires a whole class of argument
Welch on the engaged LKAS command, 5 routes: **95 % of energy below 0.59–0.74 Hz, 97.5–98.3 % below 1 Hz.** Every dynamic feature of the loop (5.05 Hz lag pole, 7.3 Hz ring, 9.64 Hz corner, 16.53 Hz feedback EMA, ~20 Hz grind) sits **7×–27× ABOVE it**. Consequences: **(a)** "flat across the command band" collapses to **`|T(0)| = 1.0`**, a pure DC statement — today **0.535**, a 1.87× scale error, and a Kp cut to 148 would make it 0.407; **(b)** 🛑 **inner-loop phase lag CANNOT discriminate candidates** — 1.32° (4.9 ms) across every pair, against the operator's own 200 ms `SteerDelay`; the whole Kd 128→160 move is **1.4 ms, 0.7 %** of it. Any "this adds lead and therefore helps" argument is real in direction and worthless in size; **(c)** only **PEAKING** and **LINEARITY** remain as criteria. ⇒ **Nothing the outer loop ASKS for is near where this loop misbehaves — every symptom is the actuator's own dynamics, not tracking error.**

### THE DEADBAND — still there, MASKED not cured, and it is what blocks the Kp lever
`v281r3_read_r35.moving_runs` called directly (it reproduces r35's registered 7 runs / 14.8 s and V283's 1 / 1.4 s exactly): **r39 = 4 stall runs / 6.9 s**, 10.3 per 100 s of hands-light context vs r35's 19.1 — roughly **halved**. 🛑 **But the delivered fraction did NOT move: 0.44 of what the stock Kp LERP would deliver, vs r35's 0.45** (V283 reads 1.02–1.31). What moved is the `rate/ref` distribution the gate cuts (p50 0.65 → 0.80). ⇒ **the 1.70× outer loop is dragging the wheel across the gate with the same weak inner push. A cured deadband would show as a larger DELIVERED fraction, as it does on V283.**
**And a Kp cut makes it worse, ~2× worse than first priced:** on r39's operating point Kp 148 gives **32 stall windows vs 17**, total stalled time ×2.3, and 🛑 **the worst run DEEPENS 1.85 → 2.70 s (+46 %)** — the r35 finding that "the cut adds marginal runs but does not deepen the worst one" **does NOT hold at r39's operating point.** Kp 176 is the shallower point (27 windows).
🛑 **Within Ki = 0 and cal-only, only the MAP reaches stiction.** Measured inside the r35 stall runs: `P` = +5574 counts, `D` at Kd 128 = **+116 (2.1 %)**, `I` at Ki 50 = **+4168…+10240** (railed at anti-windup in 3 of 7). `D` is 29 % of `|P|` in magnitude but **pure AC — DC/AC ratio 0.059**: it is **dither, not breakaway**, so a Kd raise is **~100× short**. The map's benefit would not even register on the metric that currently defines the symptom.

### 🛑 PREREG-V282-READ.md, SCORED AS WRITTEN — no threshold moved
- **FAIL gate PASSES** (creep bit6 0.0906 / bit5 0.0739 over 59.2 s; neither 0 nor 1). Cost-FAIL does not fire (tap decodes on all 47,549 frames, `dtc_active` 0.0000). ⚠ Weaker clearance than a matched-tune drive: the feel changes are the openpilot tune flown on the same drive.
- **(A) = 0.0906 ± 0.019.** ≤ 0.10, so the second sentence fires **literally** — 🛑 **but the prereg's own refutation clause fires WITH it:** the ladder on r39's own bar puts 0.0906 between the **2048 (0.0784)** and **3072 (0.1153)** rungs, **2.3× the 1024 prediction (0.0387)**. **The "1024 arm is live" half is NOT supported by the wire.** Same outcome as r36–r38. The ≥ 0.22 sentence did not fire, so *"`0xC6446` must not be cut"* is **not licensed by this drive** — the standing DO-NOT-FLASH rests on the bit-4 phase, untouched.
- **(B) = 0.0739**, strictly in (0,1) — positive control **PASSES**. **(C) PASSES: +10° at 20.3 Hz** (coh 0.68); at 7 Hz **+144…+175° = PUMP** on all four builds — `accord-r24-pumps-at-7hz-and-damps-at-20hz` **survives its first Ki-0 test**. **(D) = 0.0703, UNINFORMATIVE BY CONSTRUCTION** (bit 6 is 0.0000 in every frame with `|T| ≥ 600`, 0.150 at `|T| < 300`) — same as r36–r38.
- ⚠ **Trap:** bit 6 reads **1.00 while DISENGAGED** (`T = 0`, so `|r24| ≥ 0` trivially). **Gate every duty on lateral engagement.**

### ⭐ THE 7.3 Hz RING IS NOW PINNED — `|L| = 0.980 [0.971, 0.983]`, n = 8
`grind39`'s re-implementation reproduces the **registered** r35–r38 pool to the digit (n 5, 0.976 [0.944, 0.990]) — validating the code path — then r39 alone gives **0.982 [0.977, 0.983]** (n 3, f0 median 7.16 Hz, every α positive, one episode inside bookmark 1). **Pooled: n 8, 0.980 [0.971, 0.983] — interval cut from 0.046 wide to 0.012.** Sensitivity across 4 peak bands × 3 ACF max-lags: **≤ 0.002**. Side discrimination: **11 flat-248 episodes below 1 vs r34's 7 stock-Kp episodes above it.** f0 span 7.02–7.57 Hz, no second mode. 🛑 **Use 0.980, not 0.976, in any future sizing.**

### ✈ NEXT — in order
1. **`SteerLatAccel` 2.11 → 4.0 in Galaxy, alone.** Nothing else changes; firmware stays V282. Score on `R_m` (does it fall toward 1.00?) and on the 6–10 Hz bar in loaded turns (does the ring's drive fall?).
2. ⭐ **THE PLANT-MAGNITUDE IDENTIFICATION DRIVE** — read the 427 `T` tap and the 0x18F rate **simultaneously** under swept/broadband excitation. **Both agents independently ranked this above any further dose.** The plant's **magnitude above ~5 Hz has never been measured** (only its phase at 10/22 Hz and its DC gain), which is why `|T(f)|`, `f_-3dB` and `max|T|` are all model-composed — and this session's arithmetic **inverted twice on that single unmeasured number**. It is a DRIVE design, not a build.
3. Optional control, `grind39`'s suggestion: **one drive of V282 at the OLD tune** (SR 12.5, `SteerKP` 0.6) to make every r35→V282 contrast clean. Costs no firmware change. ⚠ Conflicts with the operator's "SR is a definite keep" — offer it, do not assume it.

### CORRECTIONS MADE THIS SESSION — do not re-derive the old versions
- 🛑 **"The 27–32 Hz crossing rests on a SINGLE estimator family" is RETRACTED.** The estimator table **scans 2–24 Hz only**. Extrapolating each family's own published phase slope: **cmd-IV crosses at 24.7–27.3 Hz (GM 2.2–4.1×) — its "none" is a WINDOW ARTEFACT**; bar-IV reports a number only because it carries the most lag and crosses earliest (22.6–23.8 Hz); the genuine dissenter is **direct**, whose phase *advances* (−145° → −144°) and never approaches −180°. ⇒ **existence supported by 2 of 3 families**, GM spanning ~1.57×–4.11×, whole table BELIEF (coherence 0.3–0.6). **`Ku = 227` is the CONSERVATIVE WORST-FAMILY bound and must never be quoted as MEASURED-grade.**
- 🛑 **"A Kp cut makes the servo arm cancel LESS" is RETRACTED** (self-contradictory: less cancellation would GROW the sum). Corrected, byte-exact: lowering Kp rotates the servo arm toward D's **+88.7°**, moving it **CLOSER** to anti-phase (123° → 137°); the cancelling component **GROWS +3.7 %** while the **QUADRATURE** component **collapses 0.461 → 0.287 (−38 %)** — and quadrature adds in RMS and cannot be cancelled. **The arm becomes a PURER canceller, not a weaker one.** Root-find and corrected mechanism now agree (both roots at `|Ls+Lr| ≈ 1.027`).
- 🛑 **"The lower Kd root is 118" is only true AT Kp 248.** The root is a **function of Kp** and falls as Kp falls: **116.7 @248, 99.4 @200, 92.2 @176, 84.9 @148, 75.0 @100, 63.6 @0.** Any "Kd X sits N % above the root" claim must use **that candidate's own Kp**. ⚠ A **baseline-regime** statement; in the measured (inverted) regime the `|L|`-vs-Kd curve is no longer the same V.
- 🛑 **`0.663` is the ρ-based equilibrium prediction, NOT r35's measured delivery (0.923).** `0.663 × 1.28` multiplied two different objects. Same estimator gives `R_eq` 0.690.
- **`max|i| = 1.225` superseded by r39's measured `0.909`** (43.1 % of bound).

### 🛑 DEFECTS FOUND, REPORTED, NOT YET FIXED
1. **The dongle's route counter RESET.** There are **two** routes numbered `00000031`, `00000035` and `00000037`. The **current-epoch** r35 is `--580292087d` (V281 rev 3); `analysis-2020accord/extract/extract_r35_cache.py` points at a **2026-07-31 V64-era** `--77808fe7ce`. Repo-root `_scratch/cache/{r31,r32,r33,r34,r35,r37,r3a,r3b}` **mixes both epochs**; the current-epoch caches the `osc-highangle` scripts read live in `analysis-2020accord/_scratch/cache/{r34,r35,v280}/`. **Check the epoch of every cache before a cross-route comparison.**
2. **`extract_r35_cache.py`'s `0x14A` byte-4 decode is V64's** (liveness/arm/FSM/override), wrong for V282's r24 comparator map. Carrying it forward would produce five plausible, silently-wrong boolean channels. `extract_r39_cache.py` replaces it with V282's map under names that **fail loudly**.
3. **`backcalc_extract.py` mixes the two `0x0E4` sources** — it takes `m.src >= 128`, catching the stock camera (src 128, `STEER_REQUEST` duty 0.0000) alongside openpilot (src 129, duty 1.0000); they interleave at ~200 Hz so `g["req"]` reads ~0.5 and `g["eng_wire"]` disagrees with `latActive` on **44–48 %** of frames on r34/r35/r39. **Blast radius: ONE consumer** (a printed line at `backcalc_laf_friction.py:320`); `g["cmd"]` is contaminated the same way and is unsafe as "openpilot's command". **No analysis gate uses either.** Fix is `m.src == 129`, which invalidates every existing `*_backcalc.npz`.
4. **`analysis-2020accord/_scratch/cache/r39/r39_1ab.json`'s 427 descriptor contradicts the wire.** It says `gp-0x6B38 sar0`, scale 0.2; but r39's `0x1AB` byte 0 takes only `{128,130}` — structurally identical to r35/r38 — so **bit 9 is the sign and the low 8 bits the magnitude**, and the kit's `sign<<9 | |T|>>3` (×8) decode is the one that matches. A sar-0 decode would cap `|T|` at **207** where the chain and every prior route put it at 400–2500.
5. **A Kp discriminator tried and DISCARDED, recorded so nobody re-derives it:** the naive whole-route correlation of chain `T_sim` under each candidate Kp table against the tap prefers "flat 512" on **r34**, whose Kp is known from the image to be the stock LERP. **A check that condemns a known-truth route is broken.**
