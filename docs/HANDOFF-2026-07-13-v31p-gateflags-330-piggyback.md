...
**Builds on** `HANDOFF-2026-07-12-comma4-uds-live-telemetry-bus-analysis.md`. That session proved live UDS
...
## 2. The firmware edits (build: `analysis-2020accord/build_v31p_tva.py`)
...
- `rlog-tools/extract_eps_telemetry.py` — `epsTelemetry` rows now emit `engage_sm_cut, voter_avg,
...
4. **Analyze:** `python rlog-tools/extract_eps_telemetry.py <route>--*/rlog.zst -o eme.csv`. At each cut
