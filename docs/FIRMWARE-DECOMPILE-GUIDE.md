...
and [`GHIDRA-CHECKLIST.md`](./GHIDRA-CHECKLIST.md). The skill is the
...
Full write-up: `memory/reference_rizin_ghidra_v850_quirks.md`. Summary:
...
`analysis-2020accord/fw_inventory/decompilation/disasm_v850.py` instead
...
```python
import sys
sys.path.insert(0, "analysis-2020accord/fw_inventory/decompilation")
...
`GHIDRA-CHECKLIST.md` (JDK 21 + a Ghidra release zip) if you need local
...
`analysis-2020accord/svd_for_ghidra/UPD70F3508_V850E2Px4.svd` is a
...
Per `memory/feedback_rigorous_validation.md`: "full byte diff > spot diff;
...
- `analysis-2020accord/TORQUE_PATH_AND_TABLE.md` / `TORQUE_PATH_GUIDE.md` — the torque path from CAN input to motor output
- `analysis-2020accord/EME_OVERRIDE_SM_NONVERIFIED.md` — non-verified override state-machine hypotheses (read the verified companions in `.claude/agent-memory/firmware-codepath-tracer/` before trusting these)
- `analysis-2020accord/FUN_00043e44_FLOAT_MONITOR.md` — the float consistency-monitor deep dive
- `analysis-2020accord/SESSION-2026-05-30-EME-RESOLUTION.md` — a resolved EME investigation session
- `analysis-2020accord/HOW_TO_BUILD_ACCORD_TVA_RWD.md` — the build/flash recipe, including the decrypt/cipher/CRC pipeline
- `docs/GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md` — the fullest mapped CAN→motor gating chain
...
`memory/reference_accord_lkas_torque_path.md` for the disasm-verified path.
```
[0x00000000]> /v1 0xE4
```
...
2. Walk forward through the demand aggregator → `m_steer_torque_arbitration`
   ...
   `docs/GENTLE-EME-CAN-TO-MOTOR-GATING-MAP-2026-07-06.md` is the fullest
...
(`analysis-2020accord/build_v38_tva.py` is the latest) for the verification
...
Per `memory/feedback_rigorous_validation.md`: full walk, not spot check.
