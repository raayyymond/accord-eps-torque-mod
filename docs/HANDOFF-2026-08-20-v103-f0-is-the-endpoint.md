...
🛑 **Run `rlog-tools/rez_control.py` before scoring.** It pins the whole estimator to **0.00 % on all ten
bands** against `v92_rez.log`'s published route-77 table, 221/221 windows. **Every Re(Z) number in this
...
`docs/SPEC-2026-08-20-band-definition.md`, which keeps **20–28 Hz as the band-RMS primary** on the
...
**Ring-down** (estimator = `qd_final.py` verbatim; envelope = `scipy.signal.hilbert`, **not** the broken
...
builder analysis-2020accord/build_v103_tva.py
```
...
`eps_loop_gain_model.py`'s own docstring; (2) gain scaling |L| uniformly with fixed phase shape (the
...
🛑 **`docs/GATE2-2026-08-11-cbe74-independent.md` uses a DIFFERENT, self-consistent-but-non-matching sign
...
`rlog-tools/decode_v90_probe.py::_band_transfer` defines the **opposite**, and its docstring says so:
...
| 5 | `score_v102.py`'s `PREREG` key `v101_over_v100 = 5.07` | **Holds an ABSOLUTE, not a ratio.** V101's absolute is 4.2–5.07 depending on window; true V101/V100 is **7.2–8.2×**. **No value is wrong — the key NAME is a misnomer.** |
| 6 | `accord-band-envelope-is-rectified-not-analytic` retraction | **OVER-BROAD.** Measured λ error is only **3.6 %**; envelope **CV** is inflated 22 % (so CV/duty/p50 results ARE corrupted) — **but `qd_final.py`/`qd_lib.envelope_stats`, which produced ζ = 0.017–0.036 / Q = 14–29, call `scipy.signal.hilbert`, NOT `band_envelope`.** ⇒ **the ring-down ζ/Q result was NEVER at risk and should be un-retracted.** |
...
| # | decision |
|---|---|
| **D1** | 🛑 **What to do with V103.** He has said he will decide **after** this close-out. Options as put to him: drive as specced (4 items); trim to item ① only (~2 min ordinary 20–50 mph cruising, hands resting — gets f0 and the probe); or hold pending the allpass pricing. **`docs/DRIVE-CARD-V103.md` is written and HELD, not shipped.** |
...
11. 🛑 **"`0x1AB` MOTOR_TORQUE is identically 0 on stock" — WRONG**, and it would fail a genuinely stock
    route. Full-route: stock nonzero **20.8 %**, 165 distinct. **Corrected in `extract_r97.py`.**
12. ⚠ **A cache-hygiene claim was over-stated three times**: *"no write to `_cache_r97` since the single
...
**Kit:** `analysis-2020accord/build_v103_tva.py` · `docs/SPEC-2026-08-20-band-definition.md` ·
`docs/DRIVE-CARD-V103.md` (**HELD**) · caches `_cache_r96` (owner: `route-v102`), `_cache_r97` (owner:
`route-stock`).

**New/promoted `rlog-tools/`:** `rez_control.py` (🛑 **estimator regression — run before scoring
anything**) · `rez_crossover.py` · `rez_band_tracking.py` · `rez_f0_vs_amplitude.py` ·
`v103_endpoint_power.py` · `burst_rez.py` · `bandref.py` · `splice.py` · `v101_press.py` ·
`bandedge.py` · `ident_r96_r97.py` · `extract_r96_r97.py` · `extract_r97.py` · `score_v102_full.py` ·
`score_v102_matched.py` · `score_v102_peak_cave.py` · `score_v102_ancova.py` ·
`v102_endpoint_reconcile.py` · `v102_leak_audit.py` · `v102_estimator_control.py` ·
`v102_wire_and_427.py` · `v102_lane_phase.py` · `v102_b4_phase_feasibility.py` ·
`v102_b4_phase_ci.py` · `v102_b4_phase_band.py` · `v102_torque_intermittency.py` ·
`stock_r97_baseline.py` · `stock_r97_resonance.py` · `stock_r97_rez_matched.py` ·
`stock_r97_ringdown.py`
