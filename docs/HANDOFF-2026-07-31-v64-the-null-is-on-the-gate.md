...
**Confirmed four independent ways:** raw byte histogram · `rlog-tools/decode_v64_detector.py` (run by the
...
`analysis-2020accord/analyze_bus_amplitude_vs_detector_T.py` has had its docstring corrected in place to
...
**(a) Was the build aimed backwards?** `build_v63_tva.py:63` recorded a live dispute — one trace read
