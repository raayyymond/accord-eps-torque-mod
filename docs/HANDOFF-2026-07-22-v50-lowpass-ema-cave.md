...
frequency/alias/polarity-robust low-pass cave. **Read `memory/reference-accord-v50-lowpass-ema-cave.md` first.**
...
- **Vibration re-characterized (fresh V38-behavior drive aa5b3e0c01, `analyze_manual_vibration.py` +
  `manual_speed_split.py`):** SPEED-DEPENDENT — ~21.7 Hz at 3-8 m/s (worst/most-audible, matches b9),
...
`analysis-2020accord/eps_v50_gate2_lowpass.py` (re-runnable). The EMA is stable under BOTH the pessimistic
...
## 4. The build (`build_v50_tva.py` + `v50_cave_asm.py`, BUILT + VERIFIED, UNFLASHED)
...
pivoted from a raw UDS read to a CAN-330 spare-bit probe (`build_v50probe_tva.py`, V49P/V31P-class,
BUILT+VERIFIED, UNFLASHED) — see `memory/reference-accord-v50-lowpass-ema-cave.md`. **The existing manual
rlog `aa5b3e0c01` does NOT satisfy gate (2)**: it was captured 2026-07-22, a day before this probe existed,
and `compare_330_caves.py` already confirmed it carries no live telemetry cave (CAN 330 pinned to the stock
...
- `analysis-2020accord/decode_v49p_polarity.py`, `compare_330_caves.py` — the V49P-telemetry check (route
  was NOT V49P; 330 spare bits read stock).
- `analysis-2020accord/analyze_manual_vibration.py`, `manual_speed_split.py`, `inspect_op_config.py` — the
  fresh vibration re-characterization + openpilot-config check.
- `analysis-2020accord/eps_v50_gate2_lowpass.py` — GATE 2.
- `analysis-2020accord/v50_cave_asm.py`, `build_v50_tva.py` (+ artifacts) — the candidate.
- `memory/reference-accord-v50-lowpass-ema-cave.md`, `feedback-account-for-prior-iterations-before-new-build.md`,
  `MEMORY.md`.
