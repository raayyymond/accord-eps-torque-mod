...
**extends** `docs/LEDGER-V38-TO-V84.md` (still authoritative for V38→V84 detail) with V85, with a
32-slot mode sweep the old reader could not do, and with the four symptoms the operator named on V85.

> **Reader** [EVIDENCE]: `analysis-2020accord/ledger_v38_to_v85_bytes.py` — a NEW file; the V84 reader is
...
`_cache_r6d/r6d.npz`, V84's probe, fingerprint 100%]**
...
(motor rate). `memory/accord-factord-is-the-angle-error-lever.md` should be corrected: *"the only
...
### E3 — THE FROZEN CELLS (from `build_v85_tva.py`; assert each from the BUILT image)
| cell | must be | why |
|---|---|---|
...
- [ ] `decode_v84_probe.py` pre-registered: *"the predicted step is 0.24 manual → 0.64 engaged (~2.6×).
...
- [ ] **CRC 50/50.** Value-anchored verifier, not span-based (`diff_build_vs_stock.py` is span-based and
