...
`e56fad73`. The two loads are adjacent, exactly as `build_v97_tva.py` describes.
...
V97 carries V96's cave byte-for-byte, which telemeters (`build_v96_tva.py` payload map):
`0x14A` byte4 b7 = `gp-0x6b70 < 0` · **b6 = `(gp-0x374c>>4) < 0`** · b5,b4 = `Mhi` · b3 = mode rung ·
byte7 b7 = `Mlo` · byte7 b6 = identity constant 1.

Decoded from `_cache_r80/r80.npz` (pairing `raw14_t` with `raw14_b4`/`raw14_b7`, per the recorded
...
(`eps_lkas_chain_model.py:118-127`): **PCLK is 40 MHz, not 80** (option-byte Table 6-7 forces
...
**Lead = +7.82°**, 21 Hz throughput ×1.2344 — both reproduce `build_v97_tva.py` exactly. ⚠ The
...
`_cache_r80/r80_vs_v96.json` already reports `80_ENG` with `n = 1` in every band. **A single episode
...
1. 🛑 **Re-scale the cave's regressor rung — now the top item.** `build_v97_tva.py:99-100` concedes it
...
* `C:\Users\dudei\Desktop\Projects\accord-eps-torque-mod\analysis-2020accord\_cache_r80\` (V97 flight)
* `C:\Users\dudei\Desktop\Projects\accord-eps-torque-mod\analysis-2020accord\build_v97_tva.py`
