...
`rlog-tools/extract_ratio_corpus.py` → `analysis-2020accord/_cache_ratio/{route}.npz`
...
**(b) The synthetic-rack control — the decisive one.** `rlog-tools/ratio_null_droop.py` builds
...
| file | what |
|---|---|
| `rlog-tools/extract_ratio_corpus.py` | corpus extractor → `_cache_ratio/*.npz` |
| `rlog-tools/ratio_lib.py` | geometry, masks, block bootstrap, `v_ref` definition |
| `rlog-tools/measure_steering_ratio.py` | nuisance fits, QA, θ₀, curve + first-pass controls → `results.json` |
| `rlog-tools/ratio_controls.py` | synthetic null, secant-from-local, K fit, sensitivity → `controls.json` |
| `rlog-tools/ratio_final2.py` | **the reported numbers** — four estimators, curve, strata → `final2.json` |
| `rlog-tools/ratio_null_droop.py` | **the control that decides the >120° droop** → `null_droop.json` |
| `rlog-tools/ratio_final.py` | superseded first pass (vEgo reference) — kept for the record |
...
**Added 2026-08-13.** `rlog-tools/ratio_two_sided.py` → `_cache_ratio/two_sided.json`.
...
| file | what |
|---|---|
| `rlog-tools/ratio_two_sided.py` | per-side curves, paired bootstrap, θ₀ sweep, injection power control, confounds |
| `analysis-2020accord/_cache_ratio/two_sided.json` | the deliverable — per-side `theta`/`local`/`local_lo`/`local_hi`/`n_bin`, all three sign axes, per-side θ₀, the sweep, every confound |
