...
**Supersedes** `docs/SPEC-uds-can-ram-telemetry-a160.md` (which had the wrong table base + request address —
corrected in that file's banner) and the earlier UDStelem build. Read alongside the corrected memories
`memory/reference_accord_uds_did_read_surface_a160.md` and
...
Builder: `analysis-2020accord/build_v31u_uds_telem_tva.py` (plain image `../accord-firmware/analysis-2020accord/_v31u_plain_image.bin`). Built from
STOCK `code.bin`.
...
- **Artifacts this session** (pushed to `main`): `build_v31u_uds_telem_tva.py`, `../accord-firmware/analysis-2020accord/_v31u_plain_image.bin`, the
