...
**Read after:** `docs/HANDOFF-2026-08-12-v94-aborted-and-the-override-regime.md`.
**Agent outputs:** `analysis-2020accord/_v97/` (7 files, ~190 KB) · figures in
`analysis-2020accord/_plots_r7e_r7f/` (22) and `_v97/plots_rtc/` (20+).
...
39990-TVA,A160-V97-V96BASE-C63AC.102to150-0x13000-0x100000.rwd
  ...
  builder analysis-2020accord/build_v97_tva.py   131/131   BASE = V96
```
...
machine-readable at `analysis-2020accord/_v97/rtc_measure.json` → `config`; the scorer is
`rlog-tools/v97_return_to_centre.py`.
...
3. **`eps_lkas_chain_model.py` is 309 KB** — past the 256 KB `Read` cap, so an agent told to "read the
   golden model" silently gets a truncated tail.
4. **`ledger_v94_cells.py` ignores `LEDGER_TARGET=V96`** in `grid` (silently) and `KeyError`s in `matrix`.
