...
Provenance: recorded in `analysis-2020accord/eps_lkas_chain_model.py` (`vibration_hands_off_analysis`,
...
- **Community scrollback** (`discord-export/`, `docs/HONDA-EPS-PID-KNOWLEDGE.md`): Aragon —
...
Model: `analysis-2020accord/eps_loop_gain_model.py` (runnable). Plant fitted to the measured Q=13.6 and
...
`analysis-2020accord/eps_v48b_notch_design.py`) is the remaining lever, since it attenuates the shared
torsion-bar input ahead of ALL sensor-reading carriers at once. See
`docs/HANDOFF-2026-07-21-v48-vibration-loopgain-notch.md`.

### ✅ V48B (the notch) is now BUILT + GHIDRA-VERIFIED, UNFLASHED (2026-07-21 late)
The notch is no longer just a design — it is a built candidate. `analysis-2020accord/build_v48b_tva.py`
(+ `v48b_cave_asm.py`, `eps_v48b_cave_model.py`): V38 + ratchet + a **138-byte, 41-instruction code cave
...
divergence-trip mechanism exists at any monitor. See `docs/HANDOFF-2026-07-21-v48b-notch-build.md`.
