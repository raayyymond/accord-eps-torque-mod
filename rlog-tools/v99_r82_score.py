#!/usr/bin/env python3
r"""SCORE ROUTE 82 = THE V99 FLIGHT.  Adapted from `v98_r81_score.py`; the statistics are the
pre-registered ones from `build_v99_tva.py` and are NOT re-invented here.
...
Endpoints, verbatim from `build_v99_tva.py`:
...
# V99 byte-4 map, read off build_v99_tva.py's PAYLOAD.  b5 is a HARD-WIRED 1.
M_B7, M_B6, M_B5, M_B4, M_B3 = 0x80, 0x40, 0x20, 0x10, 0x08

ROUTES = {"82": ("_cache_r82", "r82", "V99"),
          "81": ("_cache_r81", "r81", "V98"),
          "80": ("_cache_r80", "r80", "V97"),
...
def main():
    ...
    # 3.933 -> 18.509 on r81 (4.71x).  Regression introduced at V98 (`v98_r81_score.py:541`) and
    ...
    if "81" in D:
        ...
        print("       `build_v99_tva.py` lists cross-build band ratios as an UNBUILDABLE ENDPOINT")
    ...
    # `extract_r82_audio.py` builds the per-band envelope on the SAME 100 Hz grid, so every band
    ...
    if "82" not in audio:
        print("    (no audio cache -- run `python extract_r82_audio.py 82 81` first)")
    else:
        ...
        if "81" in audio:
            ...
            print("         UNBUILDABLE endpoint per build_v99_tva.py.  Mic gain / AGC / HVAC /")
