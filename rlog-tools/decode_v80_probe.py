#!/usr/bin/env python3
"""decode_v80_probe.py -- route 66, the V80 flight: probe decode + the grinding census.
...
WHAT V80 IS  (`analysis-2020accord/build_v80_tva.py`)
-----------------------------------------------------
  ...
  => dose(99 ct of rate) = 412 at EVERY speed = 2.000x V78 = 3.007x V75/V76.
     ...
     ever built. `build_v80_tva.py` states in its own header that GATE 2 is NOT satisfied.
...
Usage:
    python decode_v80_probe.py extract [seg ...]     # -> _cache_r66/r66.npz
    python decode_v80_probe.py surface               # damper trip rates from the built images
    python decode_v80_probe.py report                # the census
    python decode_v80_probe.py all
"""
import json
import os
import re
...
SEGS = list(range(15))
OUT = Path(os.environ.get("R66_CACHE", ROOT / "_cache_r66"))
BASELINE_NPZ = ROOT / "_cache_r65" / "r65.npz"          # route 65 = V76, "flew clean"
...
# ---------------------------------------------------------------------------------------------------
# THE PROBE SPEC -- re-read from build_v78_tva.py (V80 carries that cave verbatim) and cross-checked
# against build_v80_tva.py's own inheritance statement. A drift fails the run loudly.
# ---------------------------------------------------------------------------------------------------
PAYLOAD_SHIFT = 3
...
def _assert_probe_spec():
    ...
    v78 = (ANA / "build_v78_tva.py").read_text(encoding="utf-8")
    v80 = (ANA / "build_v80_tva.py").read_text(encoding="utf-8")
    ...
    for line in ("BIT_DAMP_LO, BIT_DAMP_HI = V78.BIT_DAMP_LO, V78.BIT_DAMP_HI",
        ...
        assert line in v80, f"build_v80_tva.py no longer carries `{line}` -- re-derive the decode"

    m = re.search(r"^def wire_model\(.*?(?=^def _check_wire_model)", v78, re.M | re.S)
    assert m, "wire_model() not found in build_v78_tva.py"
    ...
    exec(compile(m.group(0), "build_v78_tva.py:wire_model", "exec"), ns)
    wire = ns["wire_model"]
    seen = set()
...
# ===================================================================================================
# SPECTRAL MACHINERY -- conventions identical to analyze_r29_grinding.py so numbers compare
# ===================================================================================================
NFFT = 256
HOP = 128
...
def report():
    npz = OUT / "r66.npz"
    if not npz.exists():
        print(f"🛑 {npz} missing -- run `python decode_v80_probe.py extract` first.")
        return 2
    d = load(npz)
