...
coh = json.loads((ROOT / "_cache_r6f/probe_v86_coherence.json").read_text())
cross = json.loads((ROOT / "_cache_r6f/probe_v86_cross_route2.json").read_text())
...
PROV = {
    "tq": "raw CAN 0x18F(399) bytes0:1 x -1 == STEER_TORQUE_SENSOR negated. The 399 packer is "
          ...
          "NOT this one -- see HANDOFF-2026-06-30-sensorA-identity-gate-scale.md.",
...
for tag, f1, f2 in (("6f", "_cache_r6f/probe_v86_physics.json",
                     "_cache_r6f/probe_v86_physics2.json"),
                    ("70", "_cache_r70/probe_v86b_physics.json",
                     "_cache_r70/probe_v86b_physics2.json")):
