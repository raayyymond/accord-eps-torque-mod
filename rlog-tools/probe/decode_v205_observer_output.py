#!/usr/bin/env python3
"""Decode V205's CAN 427 probe: gp-0x6b70, the r24 RATE LANE -- the biggest 8 Hz exciter.
...
terms are live, and V196 halves the one with the SMALLEST clamp:
    ...
    gp-0x6b86  8192   biquad output -- LKAS command, 1-5 Hz
    gp-0x6b4c  8192   11-slot assist sum -- low frequency
    gp-0x6b70   2048   VISCOUS, rate-derived (omega^1)      <-- this probe
...
clamp -- but constants cannot settle that.  This measures gp-0x6b70's actual 8 Hz content.
...
THE ENCODING (V205: sar 3, NOT V194's sar 6)
--------------------------------------------
gp-0x6b70 is SIGNED and the aggregator clamps it to +-8192, so sar 5 fits it with 2x headroom:
...
def main(tag, confirmed=False):
    if not confirmed:
        print('=' * 78)
        print('REFUSING TO DECODE -- this channel is only meaningful on a V205 route.')
        print('')
        ...
        print('    V194        gp-0x6c2c at sar 6   V205        gp-0x6b70 at sar 3  <- this one')
        print('')
        print('  Decoding another build here yields a PLAUSIBLE, SPECIFIC, WRONG number.')
        print(f'  Re-run with --v205 once the route is genuinely a V205 capture:')
    ...
    print(f'  |gp-0x6b70|  p50 {np.percentile(a, 50):7.0f}  p95 {np.percentile(a, 95):7.0f}'
    ...
    if len(eng) > 512:
        ...
        print('  If the observer output dominates, V196 is aimed at a minor exciter and the')
...
if __name__ == '__main__':
    ...
    sys.exit(main(_a[0] if _a else 'r24', any(f in sys.argv for f in ('--v205','--v202','--v203'))))
