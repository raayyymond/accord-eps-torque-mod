#!/usr/bin/env python3
"""Decode V204's CAN 427 probe: gp-0x6b4e, the r24 RATE LANE -- the biggest 8 Hz exciter.
...
terms are live, and V196 halves the one with the SMALLEST clamp:
    ...
    gp-0x6b86  10240   biquad output -- LKAS command, 1-5 Hz
    gp-0x6b4c  10240   11-slot assist sum -- low frequency
    gp-0x6b4e   2048   VISCOUS, rate-derived (omega^1)      <-- this probe
...
clamp -- but constants cannot settle that.  This measures gp-0x6b4e's actual 8 Hz content.
...
THE ENCODING (V204: sar 3, NOT V194's sar 6)
--------------------------------------------
gp-0x6b4e is SIGNED and the aggregator clamps it to +-10240, so sar 5 fits it with 2x headroom:

    positive x (0..10240)    -> raw    0 ..  256
    negative x (-10240..-1)  -> raw  768 .. 1023
...
def main(tag, confirmed=False):
    if not confirmed:
        print('=' * 78)
        print('REFUSING TO DECODE -- this channel is only meaningful on a V204 route.')
        print('')
    ...
    print(f'  |gp-0x6b4e|  p50 {np.percentile(a, 50):7.0f}  p95 {np.percentile(a, 95):7.0f}'
    ...
    if len(eng) > 512:
        ...
        print('  If the observer lane dominates, V196 is aimed at a minor exciter and the')
