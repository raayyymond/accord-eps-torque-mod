#!/usr/bin/env python3
"""Decode V198's CAN 427 probe: gp-0x6ada, the r24 RATE LANE -- the biggest 8 Hz exciter.
...
terms are live, and V196 halves the one with the SMALLEST clamp:
    ...
    gp-0x6ada   2048   VISCOUS, rate-derived (omega^1)      <-- this probe
...
clamp -- but constants cannot settle that.  This measures gp-0x6ada's actual 8 Hz content.
...
THE ENCODING (V198: sar 3, NOT V194's sar 6)
--------------------------------------------
gp-0x6ada is SIGNED and the aggregator clamps it to +-8192, so sar 5 fits it with 2x headroom:
...
def main(tag, confirmed=False):
    if not confirmed:
        print('=' * 78)
        print('REFUSING TO DECODE -- this channel is only meaningful on a V198 route.')
        print('')
        ...
        print('    V194        gp-0x6c2c at sar 6   V198        gp-0x6ada at sar 3  <- this one')
        print('')
        print('  Decoding another build here yields a PLAUSIBLE, SPECIFIC, WRONG number.')
        print(f'  Re-run with --v198 once the route is genuinely a V198 capture:')
        print(f'    python {Path(__file__).name} <route-tag> --v198')
        print('=' * 78)
        return 2
    ...
    print(f'  |gp-0x6ada|  p50 {np.percentile(a, 50):7.0f}  p95 {np.percentile(a, 95):7.0f}'
...
if __name__ == '__main__':
    ...
    sys.exit(main(_a[0] if _a else 'r24', '--v198' in sys.argv))
