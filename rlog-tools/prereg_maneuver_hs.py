...
That sentence is in section 6.  Run: `python prereg_maneuver_hs.py --route <hex>` after extraction.
...
The kit has two mutually inconsistent readings of the same 8 Hz line:
  ...
  (i)  `docs/ANALYSIS-2026-08-20-torsion-bar-and-lane-weight.md` -- "THE TORSION-BAR MODE,
       CONFIRMED": f_n = 8.162 Hz, Q = 10.21, zeta = 0.0490, CI [8.015, 8.187] / [7.49, 13.61].
  (ii) `memory/accord-the-8hz-mode-is-the-loop-not-the-plant.md` (2026-08-21) -- the identified
...
==================================================================================================
  * `ringdown_validate.py`  -- E1 (`stock_r97_ringdown`) and E2 (`r67_ringdown_q2`) SATURATE at
  ...
  * `plant_recon.py` / `plant_zcurve.py` -- `ang` is quantisation-limited in band; `rate_c` is
  ...
  * `plant_phase_corner.py` -- SCALE-FREE: `J_w/b_w` = 21.4-36.6 ms on five routes (STOCK
  ...
  * `plant_fit_final.py` -- the MAGNITUDE fit for J_w, b_w separately is **NOT SUPPORTED**
...
SECOND  `rate_f` -- gives Z = tq/rate_f for the scale-free phase statistic, as a within-drive
                    replication of `plant_phase_corner.py` under a KNOWN excitation.
...
MASKS (fixed now, not after seeing data)
  ...
  M2 hands-off      window p90|tq| < 300 counts (`v97_return_to_centre.py`'s HOLD_OFF).  The suite
...
🛑🛑 THE SENTENCE A NULL WILL LICENSE, WRITTEN BEFORE THE DRIVE
  ...
  >  resonance, and `docs/ANALYSIS-2026-08-20-torsion-bar-and-lane-weight.md` section 2 is
  >  RETRACTED in favour of `memory/accord-the-8hz-mode-is-the-loop-not-the-plant.md`."
...
def analyse(route):
    ...
    if not L.ROUTES[route]["segs"]:
        print("no cache for route 0x%s -- extract it first (see rlog-tools/extract_r9e.py)" % route)
        return
    ...
    if np.isfinite(t0) and len(boot) > 200:
        ...
        print("    prior from existing logs (plant_phase_corner.py): Q = 1.10 - 1.88, five routes")
    else:
