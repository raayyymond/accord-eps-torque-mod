...
(`extract_r7d.extract_route` via `extract_r85.py` / `extract_r95.py`), so field names, ZOH
...
def _cache_dir(route):
    ...
    for the older ones (r71, r75, r76).  `_cache_r71` DOES exist, at the repo root, in the modern
    per-segment ~100 Hz schema; only `_cache_ratio/00000071.npz` (14.6 Hz) is the useless one."""
    for base in (AN, ROOT):
...
def load_seg(route, seg):
    ...
    if "v_rear" not in d:
        # caches before `extract_r85.py` do not carry the rear-wheel speed; fall back to the rear
