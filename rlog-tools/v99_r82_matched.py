...
WHAT IT COMPUTES
  ...
  B  route 81's E1 bins RECOMPUTED from its own cache with THIS script's bin edges -- the V98
     numbers used in `v99_r82_score.py` were quoted from the brief, and a quoted number is not a
...
🛑 `build_v99_tva.py`: *"A change in ALL FOUR bins is an operating-point / route artefact, NOT the
...
ROUTES = {"82": ("_cache_r82", "r82", "V99"), "81": ("_cache_r81", "r81", "V98")}
...
def main():
    ...
    print("    (the V98 numbers used in v99_r82_score.py were QUOTED from the brief; a quoted")
    ...
    if res["D_common"]["all_same_sign"]:
        print("    🛑 ALL FOUR BINS MOVED IN THE SAME DIRECTION.  Per build_v99_tva.py this is the")
