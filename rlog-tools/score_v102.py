#!/usr/bin/env python3
r"""score_v102.py -- THE STANDING POST-FLIGHT SCORER FOR V102 (6x LKAS gain, `0xC6CD0` = 5346).

    python score_v102.py <route>          e.g.  python score_v102.py 96
    python score_v102.py --selftest       runs the whole pipeline on r95 under V101's identity rule,
                                          to prove the machinery works before the drive exists

Everything is PRE-REGISTERED here, before the flight.  Baselines are RECOMPUTED from `_cache_r85`
(V100, 4x) and `_cache_r95` (V101, 8x) at run time by the same estimator, so no stored constant can
...
def main(route, ident_b3=0, ident_code=3, label="V102"):
    ...
    if not cdir.exists() or not L._segs(route):
        print("\n🛑 NO CACHE at %s.  Build it first with the `extract_r95.py` pattern:" % cdir)
