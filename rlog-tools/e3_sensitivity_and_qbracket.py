...
B.  ROUTE 0x97 (STOCK) RING-DOWN, redone with E3 at the criteria `stock_r97_ringdown.py` used --
    FROZEN (4.0 s pre / 4.0 s post) and RELAXED (2.5 / 3.0) -- so the replacement for
    `_stock_r97_ringdown.json` is like-for-like.
    🛑 That file is NOT edited or deleted here.  Reporting the defect, not fixing the record.

C.  THE Q = 10.21 BRACKET.  `ringdown_validate.py` section 5 showed the 2-pole PSD fit is nearly
...
def b_route97():
    hdr("B.  ROUTE 0x97 (STOCK) RING-DOWN, E3, at `stock_r97_ringdown.py`'s own two criteria")
    print("    🛑 `rlog-tools/_stock_r97_ringdown.json` is NOT touched.  This is the replacement")
...
def c_qbracket(routes):
    ...
    print("    `ringdown_validate.py` section 5 had to GUESS (it assumed equal band power).")
