...
def main(tag):
    ...
    if ch is None or len(eps) == 0:
        ...
        if _sg is not None:
            ...
            print()
            print('  !! ALIASING: THIS BAND IS NOT PURELY 30-49 Hz. Every cache runs at')
            print('  fs = 101.01-101.26 Hz, so Nyquist is ~50.5 Hz and anything real in')
            print('  52-71 Hz FOLDS INTO THIS BAND -- a 71 Hz line lands on 30 Hz. The fold')
            print('  source sits entirely above Nyquist, so it can be neither seen nor')
            print('  filtered out after the fact, and no channel escapes it: probe, raw14')
            print('  and raw18 share the 101 Hz frame rate, and the ~50 Hz channels fold')
            print('  30 and 70 Hz onto the SAME ~20 Hz. Read every number here as')
            print('  "30-49 Hz OR its 52-71 Hz alias" -- never as 30-49 Hz alone.')
            print('  Settling it needs a cave zero-crossing COUNTER incremented at the')
            print('  1000 Hz task rate, which is immune to folding by construction:')
            print('  docs/specs/design/PROBE-zero-crossing-rate-counter.md.')
