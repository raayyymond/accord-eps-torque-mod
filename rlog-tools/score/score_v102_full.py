#!/usr/bin/env python3
r"""score/score_v102_full.py -- V102 (route 96, 6x) scored against V101 (r95, 8x), V100 (r85, 4x) and,
for the first time in this kit, **HONDA STOCK (r97, 1x)**.

WHY THIS EXISTS ALONGSIDE `score/score_v102.py`
    `score/score_v102.py` runs the PRE-REGISTERED cross-route matched-cell estimator with a 15 s block
    ...
    V102's 576 s engaged exposure is discarded.  This file adds:
      ...
      1. the LITERAL primary endpoint of `STATE.md` -- the WITHIN-ROUTE shape statistic, the same
         estimator that produced the record's 5.07 / 0.62 (`score/r95_v102_prereg.py`), so no cross-route
...
TRAPS HONOURED
  * `t` is EVENT-DRIVEN.  The within-route statistic (1) reproduces `score/r95_v102_prereg.py` EXACTLY,
...
Usage:  python score/score_v102_full.py
"""
...
ARMS = [("97", "V9b STOCK", "1x", 891),
        ...
        ("96", "V102", "6x", 5346),
        ("95", "V101", "8x", 7128)]
NPZ = {"97": "_scratch/cache/r97/r97.npz", "85": "_scratch/cache/r85/r85.npz",
       "96": "_scratch/cache/r96/r96.npz", "95": "_scratch/cache/r95/r95.npz"}
...
# =====================================================================================================
# 1.  THE WITHIN-ROUTE SHAPE STATISTIC -- estimator VERBATIM from `score/r95_v102_prereg.py`
# =====================================================================================================
...
if __name__ == "__main__":
    ...
    print("       measured by score/score_v102.py on the SAME convention is 1.68x -- USE THAT.)")
    ...
    (ROOT / "analysis-2020accord" / "_scratch/cache/r96" / "score_v102_full.json").write_text(
        json.dumps(out, indent=1, default=float))
    print("\nwrote analysis-2020accord/_scratch/cache/r96/score_v102_full.json")
