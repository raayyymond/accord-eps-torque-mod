...
THE PROBLEM (measured in `plant_zcurve.py` / `plant_impedance.py`)
  ...
  linearly with the choice, which is why `plant_phase_corner.py` deliberately quoted only the
  RATIO `J_w/b_w` (immune to it).  This file removes the ambiguity WITHOUT Ghidra.

THE ANCHOR
  `ang` is openpilot's own decoded `carState.steeringAngleDeg` -- `v97_return_to_centre.py:25`
...
def main():
    ...
    (HERE / "_plant_scale_resolve.json").write_text(json.dumps(OUT, indent=1, default=float))
    print("\nwrote %s" % (HERE / "_plant_scale_resolve.json"))
