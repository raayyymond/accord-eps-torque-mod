...
WHY PHASE AND NOT MAGNITUDE
  `plant_fit_final.py` fitted |Z|^2 = J^2 w^2 + b^2 and got R^2 = 0.01-0.32 with arms dropping out:
  **the magnitude fit is NOT supported by these logs** and its J_w / f_n numbers are withdrawn.
  Two things poison the magnitude and neither touches the phase:
    ...
    * something rolls |Z| off above ~13 Hz (`plant_zcurve.py` §1), un-modelled.
...
def main():
    ...
    (HERE / "_plant_phase_corner.json").write_text(json.dumps(dict(fits=OUT, percurve=percurve),
                                                              indent=1, default=float))
    print("\nwrote %s" % (HERE / "_plant_phase_corner.json"))
