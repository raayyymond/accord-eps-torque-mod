...
TWO INJECTION CONVENTIONS, because they answer different questions:
  prom  : amplitude solved so the recovered PROMINENCE is 70x   (what qd_power.py used)
...
def ringdown_zeta(env, fs, f0, i0, fit_s=2.0, floor_from=2.5):
    """The EXACT decay fit qd_final.py used on the car, isolated so it can be controlled."""
    post = env[i0:]
...
json.dump(OUT, open(ROOT / "_cache_r6f" / "qd_control.json", "w"), indent=1, default=float)
print("\nwrote _cache_r6f/qd_control.json")
