#!/usr/bin/env python3
r"""Extract route `97` -- **THE V9b STOCK FLIGHT** -- into `analysis-2020accord/_cache_r97/`.
...
been scored against has had NO stock arm since the instrument chain was built at `_cache_r6d`.
...
IDENTITY -- FOUR INDEPENDENT LEGS, all measured before this file was written (seg 5):
  ...
  4. `carParams.carFw` EPS version string is **`39990-TVA-A160`** with a HYPHEN.  Every modded
     ...
     (`build_v18_tva.py:66-67`, carried to `build_v102_tva.py`), so a modded ECU reports
...
INSTRUMENT FACTS honoured here (identical to `extract_r85.py` / `extract_r95.py`):
...
Usage:
    python extract_r97.py                 # extract + derive + identity + census
"""
import json
import sys
...
D.ROUTES[ROUTE] = (PREFIX, NSEG_SCAN, "analysis-2020accord/_cache_r97", "r97s", "r97", "V9b-STOCK")
