...
`analysis-2020accord/_cache_r85/`.

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Like `extract_r80.py` / `extract_r81.py` / `extract_r82.py`,
this file only adds a row to `decode_v84_probe_r6d.ROUTES` and calls `extract_r7d`'s
`extract_route()` -- the SAME code that wrote every cache since `_cache_r6d/`.  Field names,
...
Usage:
    python extract_r85.py                 # extract + derive + identity + health + census
    python extract_r85.py extract 85
    python extract_r85.py derive 85
    python extract_r85.py identity 85
    python extract_r85.py health 85
    python extract_r85.py census 85
"""
import json
import sys
...
NEW = {
    ROUTE: (PREFIX, NSEG_SCAN, "analysis-2020accord/_cache_r85", "r85s", "r85", "V100"),
}
for _k, _v in NEW.items():
    D.ROUTES[_k] = _v
D.ROUTES.setdefault("82", ("75604b0a432fdc89_00000082--e30d55731b", 2,
                           "analysis-2020accord/_cache_r82", "r82s", "r82", "V99"))
...
# ---- V100's byte-4 map, read off `build_v100_tva.py`'s PAYLOAD, not inferred.
...
#  "IF IT FAILS, NOTHING IN THE READOUT MAY BE REPORTED."  (build_v100_tva.py)
# ======================================================================================
