...
==================================================================================================
  ...
  denominator  `rate_c`  column rate.  **`plant_scale_resolve.py` settled the scale from the data:**
...
def fit(ws, lo=FIT_LO, hi=FIT_HI, coh_min=COH_MIN):
    ...
    (An earlier version -- `plant_fit_final.py` -- weighted by coherence alone on a quantity
...
def main():
    ...
    (HERE / "_plant_Jb_absolute.json").write_text(json.dumps(dict(fits=OUT, coulomb=coul),
                                                             indent=1, default=float))
    print("\nwrote %s" % (HERE / "_plant_Jb_absolute.json"))
