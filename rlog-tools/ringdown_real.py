...
`ringdown_validate.py`, the only one of four estimators that ordered zeta over 0.005-0.200
(Spearman +1.000, dynamic range 41x) and refused all three nulls (0/40 white noise, 0/40 perfect
step, 0/40 phase-randomised real data).  E1 (`stock_r97_ringdown.py`) and E2 (`r67_ringdown_q2.py`)
...
def main():
    ...
    a = part_a(routes)
    b = part_b()
    (HERE / "_ringdown_real.json").write_text(json.dumps(dict(edges=a, maneuver=b), indent=1,
                                                         default=float))
    print("\nwrote %s" % (HERE / "_ringdown_real.json"))
