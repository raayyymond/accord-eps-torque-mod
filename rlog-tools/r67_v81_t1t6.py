...
`decode_v75_probe.py` and this file calls that module's OWN `identify()` / `report()` rather than
re-deriving the thresholds.  The only difference from running the decoder directly is that the
frames come from `_cache_r67x/` (already extracted) instead of a fresh rlog parse -- the probe byte
is carried through verbatim as `probe`, and the bit decode in `extract_r67_v81.py` imports the same
BIT_* constants.
"""
import json
import sys
