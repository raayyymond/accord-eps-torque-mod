...
Same grid and decode conventions as extract_r2b_cache.py (0x14A src1 arrivals, 0x18F held-last),
...
Usage:  python extract_r2c_cache.py SEG [SEG ...]      # e.g. 0 1 3 4 8 9 10 11 12
"""
import os
import sys
...
RLOGDIR = Path(__file__).resolve().parents[1] / "analysis-2020accord" / "rlogs"
ROUTE = "75604b0a432fdc89_0000002c--eb219f392c"
OUT = Path(os.environ.get("R2C_CACHE", Path(__file__).resolve().parents[1] / "_cache_r2c"))
            ...
            if w == "can":
                for m in evt.can:
                    ...
                    if src == 1 and addr == 0x18F and len(d) >= 5:
                        ...
                        # Same guard as decode_v59_boostindex.py. Do not "fix" this.
                        if last18 is None:
                            continue
