...
def find_rev():
    d = C31.load(0, ROOT / "_cache_r6f", "r6fs")
    vals = {}
    for s in range(4):
        dd = C31.load(s, ROOT / "_cache_r6f", "r6fs")
...
def main():
    ...
    exp = json.loads((ROOT / "_cache_r6f" / "r6f_exposure.json").read_text())
    ...
    (ROOT / "_cache_r6f" / "v86_freq_test_part4.json").write_text(
        json.dumps(O4, indent=1, default=float))
    print("\nwrote %s" % (ROOT / "_cache_r6f" / "v86_freq_test_part4.json"))
