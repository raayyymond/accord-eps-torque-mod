# -*- coding: utf-8 -*-
"""Build the v280-format route cache for the FIRST TWO V289 rev 1 routes (2026-09-09), plus the 0x14A byte-4
probe cache and the userBookmark timestamps, in ONE pass.  Copied VERBATIM from extract_r5e_v280cache.py
(itself verbatim from extract_r3c_v280cache.py) -- SAME field names, SAME grid, SAME decode; only PREFIX,
TAG and NSEG_NOMINAL differ and are now chosen by argv, so the V282/V288 grind census reads these routes
with the same yardstick.

    python rlog-tools/studies/grind/extract_v289_v280cache.py r62_v289   # 75604b0a432fdc89_00000062--1c7daa54e8, 17 segs
    python rlog-tools/studies/grind/extract_v289_v280cache.py r63_v289   # 75604b0a432fdc89_00000063--1d4b188022, 12 segs

*** ROUTE-NUMBER COLLISION: the counter reset; match on the full id, never the counter. ***

*** V289 DECODER NOTE: 0x14A byte 4 bit 5 = sign(S - y) (notched-out component; duty ~0.50 engaged = cave alive);
bit 7 = |S - y| >= |y| (1 while DISENGAGED); bits 4/6 = V282's r24 comparators; bit 3 as V282; bits 0-2 stock. ***
...
ROUTES = {"r62_v289": ("75604b0a432fdc89_00000062--1c7daa54e8", 17),
          "r63_v289": ("75604b0a432fdc89_00000063--1d4b188022", 12)}
TAG = sys.argv[1]
PREFIX, NSEG_NOMINAL = ROUTES[TAG]
