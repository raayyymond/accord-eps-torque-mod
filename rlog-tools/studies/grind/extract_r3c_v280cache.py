# -*- coding: utf-8 -*-
"""Build the v280-format route cache for r3c (V282, SteerLatAccel 3.6) plus the 0x14A byte-4 probe
cache and the userBookmark timestamps, in ONE pass over the 13 segment rlogs.
...
Outputs (analysis-2020accord/_scratch/cache/v280/):
  r3c.npz     -- exactly the schema v280_map_profiles.read_route emits (t18,tq,rate,sca,t14,ang,t1ab,b0,b1,te4,cmd,req,tcs,vego)
  r3c_b4.npz  -- t14b, b4  (V282 cave probe byte on 0x14A byte 4)
  r3c_marks.json -- userBookmark logMonoTimes, segment first/last logMonoTime, route-relative times,
                    AND the present/missing segment split with the wall-clock gaps between segments.

*** ROUTE-NUMBER COLLISION. ***  The dongle's route counter RESET, so `3a` and `3b` each name TWO
different drives on disk.  `3c` currently has only ONE -- `--927965c2b4`, 2026-09-04, V282 -- but
match on the full route id including the hash suffix, never on the `3c` counter alone.

*** ALL 13 SEGMENTS 0..12 ARE PRESENT, so the concatenated time axis has no structural hole. ***
The present/missing bookkeeping is kept anyway so the two new-route extractors read identically.
...
Run: python rlog-tools/studies/grind/extract_r3c_v280cache.py
"""
import glob
import json
import os
...
PREFIX = "75604b0a432fdc89_0000003c--927965c2b4"
TAG = "r3c"
