...
CHANNEL  `tq` (0x18F column torsion bar).  NOT 427: the source cell changes build to build, it is
  ...
  (`_scratch/out/_hf_lf_controls.json`), and the interpolation artefact is 0.8-7.5 % of band amplitude.
...
OUTPUT `rlog-tools/_scratch/out/_hf_lf_discriminator.json`
"""
...
def main():
    ...
    (HERE / "_scratch/out/_hf_lf_discriminator.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
    print("\nwrote", HERE / "_scratch/out/_hf_lf_discriminator.json")
