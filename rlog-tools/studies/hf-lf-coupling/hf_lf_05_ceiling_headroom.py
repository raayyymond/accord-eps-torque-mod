...
WHAT THIS FILE MEASURES -- the two headrooms that decide whether any of it can bite
  ...
  R2  DEMAND headroom.  On routes 85/95 -- and ONLY those two -- CAN 427 carries the real
      aggregator sum `gp-0x6b94` (`analysis-2020accord/verify/check_427_alias.py`; on 96/97/9e the same
...
OUTPUT `rlog-tools/_scratch/out/_hf_lf_ceiling.json`
"""
...
def main():
    ...
    (HERE / "_scratch/out/_hf_lf_ceiling.json").write_text(json.dumps(out, indent=1))
    print("\nwrote", HERE / "_scratch/out/_hf_lf_ceiling.json")
    return 0
