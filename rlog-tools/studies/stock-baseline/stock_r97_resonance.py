#!/usr/bin/env python3
r"""studies/stock-baseline/stock_r97_resonance.py -- TASKS 3/4/5 on the STOCK route 97.
...
🛑 ENVELOPE CORRECTNESS.  `_r31_common.band_envelope` / `_r2b_common.band_envelope` are BROKEN --
   ...
   band-pass -- the same construction `qd_lib.envelope_stats` and `studies/damping-q/qd_final.py` used, which is
...
def r2():
    hdr("R2 -- RING-DOWN zeta / Q ON STOCK.  The record calls this the ONLY damping estimator\n"
        ...
        "     `studies/damping-q/qd_final.py`'s, verbatim: |hilbert| of a 2nd-order band-pass around the line, then\n"
...
if __name__ == "__main__":
    for p in (sys.argv[1:] or list(PARTS)):
        PARTS[p]()
    Path(__file__).with_name("_scratch/out/_stock_r97_resonance.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _scratch/out/_stock_r97_resonance.json")
