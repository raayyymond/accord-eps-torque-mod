#!/usr/bin/env python3
r"""studies/stock-baseline/stock_r97_ringdown.py -- ring-down zeta/Q on STOCK, with the full EDGE ACCOUNTING.
...
`qd_lib.envelope_stats` and `studies/damping-q/qd_final.py` use.  The kit's `_r31_common.band_envelope` /
...
if __name__ == "__main__":
    run(4.0, 4.0, tag="FROZEN (studies/damping-q/qd_final.py's own criterion)")
    run(2.5, 3.0, tag="RELAXED")
    Path(__file__).with_name("_scratch/out/_stock_r97_ringdown.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _scratch/out/_stock_r97_ringdown.json")
