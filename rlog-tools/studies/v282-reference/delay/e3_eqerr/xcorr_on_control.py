...
def orch_xcorr(rate, cmd_on_cs):
    ...
    for a, b in E.runs(mask & (v > 3.0), t_cs, min_s=30.0):
        ...
        cnt += 1
    Ls = sorted(num); vals = np.array([num[L] / cnt for L in Ls]); ib = int(np.argmax(np.abs(vals)))
    return Ls[ib] * 10, float(vals[ib])
...
json.dump(res, open(f"xcorr_on_control_{route}.json", "w"), indent=1)
