"""Positive control across regressor variants x filters, quantised as logged (angle 0.1 deg, rate 1 deg/s)."""
import sys, os, json
import numpy as np
...
cfgs = {f"{var}_lp{lp}": dict(variant=var, band=(None, lp), guard=5, demean_block=True)
        for var in ("rate", "angle", "hyb_rateA", "hyb_accA") for lp in (4.0, 6.0, 8.0)}
out = {}
for D in (0.030, 0.060):
    for J in (8e-5, 3e-4, 1e-3):
        for kfb in (0.0, 1e-3):
            a, r, us = synth.simulate(t_u, u, t_cs, v, D, J, 0.003, 0.02, kfb=kfb, t0=t0, t1=t1)
            m = mask & np.isfinite(a); a = np.nan_to_num(a); r = np.nan_to_num(r)
            key = f"D{D*1e3:.0f}_J{J:g}_kfb{kfb:g}"
...
json.dump(out, open(f"synth_var_{route}.json", "w"), indent=1)
for cn, d in out.items():
    err = np.array([[ds[i] - float(k.split('_')[0][1:]) for i in range(3)] for k, ds in d.items()])
    print(f"{cn:18s} max|err| per bin", np.round(np.max(np.abs(err), 0), 1), " mean err", np.round(err.mean(0), 1))
