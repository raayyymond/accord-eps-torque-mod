#!/usr/bin/env python3
r"""PASSIVE COLUMN IDENTIFICATION, IMPEDANCE FORM -- supersedes `studies/identification/plant_hs.py`'s angle-denominator fit.

WHY THE ANGLE DENOMINATOR FAILED, stated so this file can be audited against it
  `studies/identification/plant_recon.py` Q1 measured the 6-9 Hz band-RMS of `ang` at **0.0155-0.032 deg against a
  0.0071 deg quantiser floor** -- SNR 2.2-4.8, i.e. the angle channel's in-band content is BELOW
  one 0.1 deg LSB.  `studies/identification/plant_hs.py` §1 then showed the consequence directly: the log-log slope of
...
⭐ THE UNIT-FREE PAYOFF (unchanged from `studies/identification/plant_hs.py`)
...
def main():
    ...
    shape_table(routes)
    o = fit_table(routes)
    (HERE / "_scratch/out/_plant_impedance.json").write_text(
        json.dumps(dict(synthetic=syn, fits=o), indent=1, default=float))
    print("\nwrote %s" % (HERE / "_scratch/out/_plant_impedance.json"))
