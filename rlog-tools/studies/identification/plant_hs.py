...
over-determination `memory/accord/mechanism/accord-the-8hz-mode-is-the-loop-not-the-plant.md` asks for in its
...
ESTIMATOR AND ITS CONTROLS
  * H1 = S_{theta,T}/S_{theta,theta} is biased LOW by noise on `theta` (errors-in-variables);
    ...
    BOTH are reported.  `studies/identification/plant_recon.py` measured the angle channel's in-band SNR at 2.2-4.8
    (amplitude) -- at SNR 3 the H1 bias is ~10 % -- so the bracket is not decoration.
  * |H| is DELAY-IMMUNE.  `0x18F` is one frame stale and `studies/identification/plant_recon.py` Q2 measured 20-38 ms of
...
def main():
    ...
    p = HERE / "_scratch/out/_plant_hs.json"
