...
DEFENCE OF THE DEFINITION, and what it deliberately does NOT do
  ...
  * It anchors on ANGLE, not on driver torque or on a `steeringPressed` mask.  The kit's standing
    ...
    already (`memory/reference/measurement/reference-accord-steeringpressed-mask-excludes-the-symptom-regime.md`).
  ...
  * All band-limited quantities are filtered ONCE over the whole route and then sliced.  Filtering
    ...
    in `memory/accord/instruments/accord-ringdown-q-needs-a-step-control.md`.
...
--------------------------------------------------------------------------------------------------
Usage:
    python studies/ratchet/v97_return_to_centre.py                 # routes 7e 7f (V96), writes JSON + PNGs
    python studies/ratchet/v97_return_to_centre.py 80 81           # score a future flight the same way
    python studies/ratchet/v97_return_to_centre.py --no-plots
"""
import json
import sys
...
ROOT = HERE.parent
OUTD = ROOT / "analysis-2020accord" / "sessions/v97"
...
def ringdown(D, ev, half_widths=(1.5, 3.0, 5.0, 8.0), f0=None, tag="engaged"):
    ...
    🛑 `memory/accord/instruments/accord-ringdown-q-needs-a-step-control.md`: two agents independently fitted a
