#!/usr/bin/env python3
r"""studies/impedance/v95_crossbuild_rez_ledger.py -- the cross-build Re(Z) ledger, its detection floor, and the two
...
Usage:  python studies/impedance/v95_crossbuild_rez_ledger.py
"""
...
def leverb():
    ...
    for r, (bd, st) in PREV74.items():
        D = segs(str(Path(__file__).resolve().parents[2].parent / f"_cache_{r}"), r + "s")
        if D is None:
