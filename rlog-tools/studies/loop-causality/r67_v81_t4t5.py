...
Usage:  python studies/loop-causality/r67_v81_t4t5.py [t4|t5|t7|all]
"""
import json
import sys
...
def main():
    ...
    if which in ("all", "t7"):
        t7(W)
    (ROOT / "_scratch/cache/r67x" / "r67_t4t5t7.json").write_text(
        json.dumps(OUT, indent=1, default=lambda o: str(o)))
    print(f"\nwrote {ROOT / '_scratch/cache/r67x' / 'r67_t4t5t7.json'}")
