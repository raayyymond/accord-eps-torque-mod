...
Consolidates qd_score / qd_power / qd_phase / qd_lines into _scratch/cache/r6f/q_damping_score.json.

Usage:  python studies/damping-q/qd_final.py
"""
...
for name in ("q_damping_score", "qd_power", "qd_phase", "qd_lines"):
    p = ROOT / "_scratch/cache/r6f" / f"{name}.json"
    if p.exists():
        OUT[name] = json.load(open(p))
        print(f"  merged {p.name}")
json.dump(OUT, open(ROOT / "_scratch/cache/r6f" / "q_damping_score.json", "w"), indent=1, default=float)
print("\nwrote _scratch/cache/r6f/q_damping_score.json  (consolidated)")
