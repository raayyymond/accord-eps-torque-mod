#!/usr/bin/env python3
"""Follow-up to `studies/misc/relay_fingerprint_r6e.py`: the POSITIVE CONTROLS that make its nulls readable,
the absolute-amplitude check that makes its one POSITIVE result safe, and the micro/macro split.

🛑 WHY THIS FILE EXISTS.  `studies/misc/relay_fingerprint_r6e.py` returned two nulls (no odd-harmonic comb, no
...
Usage:
    python studies/misc/relay_fingerprint_r6e_b.py
"""
...
def main():
    ...
    p3(W)
    p4(W)
    (ROOT / "_scratch/cache/r6e" / "relay_fingerprint_b.json").write_text(json.dumps(OUT, indent=1))
    print(f"\nwrote {ROOT / '_scratch/cache/r6e' / 'relay_fingerprint_b.json'}")
