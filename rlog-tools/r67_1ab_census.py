...
WHY THIS EXISTS
  ...
  stream instead.
  🛑 `_cache_r67x/r67.npz` stores only `raw1ab_b0` (byte 0). The census below needs all three
...
Usage:  python r67_1ab_census.py
"""
import json
import sys
...
def main():
    for tag, (route, segs) in TARGETS.items():
        ...
        OUT[tag] = res
        np.savez_compressed(ROOT / "_cache_r67x" / f"r1ab_{tag.split('/')[0]}.npz",
                            t=A[:, 0], src=A[:, 1], seg=A[:, 2],
                            b0=A[:, 3], b1=A[:, 4], b2=A[:, 5], motor_torque=mt)
    (ROOT / "_cache_r67x" / "r67_1ab_census.json").write_text(json.dumps(OUT, indent=1,
                                                                        default=float))
    print(f"\nwrote {ROOT / '_cache_r67x' / 'r67_1ab_census.json'}")
