#!/usr/bin/env python3
"""Extract routes `6f` and `70` into `_cache_r6f/` and `_cache_r70/`, then ESTABLISH WHICH BUILD FLEW.

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly as `extract_r6e.py` does, this file adds rows to
`decode_v84_probe_r6d.ROUTES` and calls that module's `extract()` / `split()` -- the SAME code that
wrote `_cache_r6d/` (V84) and `_cache_r6e/` (V85).  Field names, ZOH/interp convention, IMU axis pick,
...
the WHOLE route.  The tolerant wrapper from `extract_r6e.py` is carried here verbatim: it yields every
...
Usage:
    python extract_r6f_r70.py                # extract both, then identity + health + exposure
    python extract_r6f_r70.py extract 6f 70
    python extract_r6f_r70.py identity 6f 70
    python extract_r6f_r70.py report 6f 70
"""
import json
import sys
...
import decode_v84_probe_r6d as D  # noqa: E402  -- THE extractor that wrote _cache_r6d/ and _cache_r6e/
...
D.ROUTES["6f"] = ("75604b0a432fdc89_0000006f--80ca318af4", 4, "_cache_r6f", "r6fs", "r6f", "UNVERIFIED")
D.ROUTES["70"] = ("75604b0a432fdc89_00000070--66544f819d", 4, "_cache_r70", "r70s", "r70", "UNVERIFIED")
NEW = ("6f", "70")
...
# =====================================================================================================
# tolerant rlog reader -- carried verbatim from extract_r6e.py, plus a per-segment message count
# =====================================================================================================
...
# =====================================================================================================
# 2.  EXPOSURE + PER-WINDOW SPEED CENSUS -- bins IDENTICAL to score_v85_r6e_bands.py
# =====================================================================================================
...
def _load(route):
    if route in D.ROUTES:
        ...
        cdir, stem = {"6e": ("_cache_r6e", "r6e"), "6d": ("_cache_r6d", "r6d"),
                      "67": ("_cache_r67x", "r67"), "68": ("_cache_r68x", "r68")}[route]
...
def speed_census(route):
    """Per-WINDOW census on the 1.28 s grid -- the form `score_v85_r6e_bands.py` consumes."""
    z = _load(route)
    v = np.abs(z["cs_v"])
    lat = z["cc_lat"] > 0.5
    nw = len(v) // WIN
...
def run_report(routes):
    ...
    (ROOT / "_cache_r6f" / "comparison_routes_census.json").write_text(json.dumps(comp, indent=1))
    print("\n" + "=" * 100 + "\nCOMPARISON ROUTES (same bins) -> _cache_r6f/comparison_routes_census.json")
