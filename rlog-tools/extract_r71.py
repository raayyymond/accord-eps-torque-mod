#!/usr/bin/env python3
r"""Extract route `71` (the V87 drive) into `_cache_r71/`, and tap the FULL 0x1AB payload.

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly as `extract_r6e.py` and `extract_r6f_r70.py` do,
this file adds a row to `decode_v84_probe_r6d.ROUTES` and calls that module's `extract()` / `split()`
-- the SAME code that wrote `_cache_r6d/`.. `_cache_r70/`.  Field names, ZOH/interp convention, IMU
...
DBC layout (Motorola bit numbering), verbatim from `r67_1ab_census.py`:
...
Usage:
    python extract_r71.py                # extract 71, tap 70 + 6f for the cross-build comparison
    python extract_r71.py extract 71
    python extract_r71.py tap 70 6f
"""
import json
import sys
...
D.ROUTES["71"] = ("75604b0a432fdc89_00000071--ac50da2a6a", 4, "_cache_r71", "r71s", "r71",
                  "UNVERIFIED")
...
if __name__ == "__main__":
    ...
    if not args:
        ...
        json.dump({"1ab": rep, "b4": ident},
                  open(ROOT / "_cache_r71" / "r71_identity.json", "w"), indent=1)
    elif args[0] == "extract":
        for r in args[1:]:
            extract_route(r)
    elif args[0] == "tap":
