...
WHAT IS UNDER TEST -- four estimators of (f_n, zeta), all fed EXACTLY the same synthetic inputs
  E1 `hilbert_env`   2nd-order Butterworth band-pass + Hilbert envelope + log-linear fit.
                     This is `rlog-tools/stock_r97_ringdown.py`'s estimator, verbatim in structure.
  E2 `demod`         complex demodulation at f0 + brick-wall LP at B_LP + log-linear fit.
                     This is `rlog-tools/r67_ringdown_q2.py`'s estimator.
  ...
  E4 `twopole_psd`   2-pole (f_n, zeta, gain) least-squares fit to the Welch auto-spectrum.
                     This is the estimator behind `docs/ANALYSIS-2026-08-20-torsion-bar-and-lane-weight.md`
...
Usage:  python ringdown_validate.py            (synthetic only, ~30 s)
        python ringdown_validate.py --real 97  (adds the N3 surrogate built from route 0x97 = STOCK)
"""
...
def stationary_twopole_control():
    ...
    `docs/ANALYSIS-2026-08-20-torsion-bar-and-lane-weight.md` got f_n = 8.162 Hz, Q = 10.21,
...
def main():
    ...
    p = HERE / "_ringdown_validate.json"
