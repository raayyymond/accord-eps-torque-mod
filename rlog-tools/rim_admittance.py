...
CHANNEL IDENTITY, established from the record, not assumed (`_cache_selfint/report.txt`):
...
def main():
    ...
    (ROOT / "_cache_r6f" / "rim_admittance.json").write_text(json.dumps(O, indent=1,
                                                                       default=float))
    print("\nwrote %s" % (ROOT / "_cache_r6f" / "rim_admittance.json"))
