#!/usr/bin/env python3
r"""rez_crossover.py -- THE Re(Z) ZERO-CROSSING FREQUENCY: a better endpoint than a fixed-band sign.

`rez_band_tracking.py` shows the anti-damping is NOT a notch sitting on the mode.  It is a broad
...
`decode_v90_probe`; `rez_control.py` pins it to 0.00 % against `v92_rez.log`.
"""
import json
import sys
...
if __name__ == "__main__":
    main()
    Path(__file__).with_name("_rez_crossover.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _rez_crossover.json")
