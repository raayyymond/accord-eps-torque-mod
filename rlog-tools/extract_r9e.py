#!/usr/bin/env python3
r"""Extract route `9e` (**V103**) into `analysis-2020accord/_cache_r9e/`.

THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly the `extract_r96_r97.py` pattern: add a row to
`decode_v84_probe_r6d.ROUTES`, then call `extract_r7d.extract_route()` -- the SAME code that wrote
every cache since `_cache_r6d/`.  Field names, ZOH/interp convention, sentinel definition, the
...
🛑 IDENTITY.  V103 is the first build since V85 with NO single-frame identity witness: `byte7[7:6]`
   ...
   V102 pins it 0).  `build_v103_tva.py`'s CONCRETE RULE FOR THE SCORER:
...
Usage:
    python extract_r9e.py                 # full pipeline
    python extract_r9e.py extract
    python extract_r9e.py derive
    python extract_r9e.py identity
"""
import json
import sys
...
NEW = {
    "9e": ("75604b0a432fdc89_0000009e--54bb0788af", 11,
           "analysis-2020accord/_cache_r9e", "r9es", "r9e", "V103"),
}
...
def identity(route="9e"):
    """V103 rule -- `build_v103_tva.py`, verbatim: byte7[7:6]==3 (as V101/V102) AND **b3 VARIES**.
