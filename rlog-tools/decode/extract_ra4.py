#!/usr/bin/env python3
r"""Extract route `a4` (**V104**) into `analysis-2020accord/_scratch/cache/ra4/`.

THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly the `decode/extract_r9e.py` pattern: add a row to
...
🛑 **NO `x6b94` AND NO `x6b4c` KEY IS WRITTEN.**  Every previous extractor wrote both regardless of
   ...
   `accord-427-alias-x6b94-is-the-lane-on-three-routes`.  `verify/check_427_alias.py` is extended for
...
Usage:
    python decode/extract_ra4.py                 # full pipeline
    python decode/extract_ra4.py extract
    python decode/extract_ra4.py derive
    python decode/extract_ra4.py identity
    python decode/extract_ra4.py lane427
"""
...
NEW = {
    "a4": ("75604b0a432fdc89_000000a4--bdd0c0aa4e", 16,
           "analysis-2020accord/_scratch/cache/ra4", "ra4s", "ra4", "V104"),
}
