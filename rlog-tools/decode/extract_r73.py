#!/usr/bin/env python3
r"""Extract route `73` (believed the V88 drive) into `_scratch/cache/r73/`, tap the FULL 0x1AB payload,
and run the PRE-REGISTERED build-identity test whose control is already measured on route 71.

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly as `decode/extract_r6e.py`, `decode/extract_r6f_r70.py` and
`decode/extract_r71.py` do, this file adds a row to `decode_v84_probe_r6d.ROUTES` and calls that module's
`extract()` / `split()` -- the SAME code that wrote `_scratch/cache/r6d/` .. `_scratch/cache/r71/`.  Field names,
...
★ THE IDENTITY TEST -- PARAMETER-FREE, AND ITS CONTROL IS A FLOWN ROUTE
    V88 makes two edits that matter here (`builds/v80_v107/build_v88_tva.py`):
...
Usage:
    python decode/extract_r73.py                  # extract 73, then identity + census; control on 71
    python decode/extract_r73.py extract 73
    python decode/extract_r73.py identity 73 71
    python decode/extract_r73.py census 73
"""
...
D.ROUTES["73"] = ("75604b0a432fdc89_00000073--9380c74d52", 11, "_scratch/cache/r73", "r73s", "r73",
                  "UNVERIFIED")
...
# byte-4 alphabets, carried verbatim from `decode/extract_r71.py`
...
if __name__ == "__main__":
    ...
    if not args:
        ...
        json.dump({"b4_alphabet": ident, "b6_vs_427": ver, "census": cen},
                  open(ROOT / "_scratch/cache/r73" / "r73_identity.json", "w"), indent=1, default=float)
    elif args[0] == "extract":
        for r in args[1:]:
            extract_route(r)
    elif args[0] == "tap":
