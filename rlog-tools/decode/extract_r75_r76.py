#!/usr/bin/env python3
r"""Extract routes `75` and `76` (believed the V89 drive) into `_scratch/cache/r75/` and `_scratch/cache/r76/`,
tap the FULL 0x1AB payload, and run the PRE-REGISTERED build-identity test whose control is the
already-flown route 73 (V88).

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly as `decode/extract_r6e.py`, `decode/extract_r6f_r70.py`,
`decode/extract_r71.py` and `decode/extract_r73.py` do, this file adds rows to `decode_v84_probe_r6d.ROUTES` and
calls that module's `extract()` / `split()` -- the SAME code that wrote `_scratch/cache/r6d/` .. `_scratch/cache/r73/`.
...
★ V89's CAVE BIT LAYOUT (`builds/v80_v107/build_v89_tva.py`, structurally V86B's with a new source + `sar 0x6`)
...
Usage:
    python decode/extract_r75_r76.py                    # extract 75 and 76, then identity + census
    python decode/extract_r75_r76.py extract 75 76
    python decode/extract_r75_r76.py identity 75 73
    python decode/extract_r75_r76.py census 75 76
    python decode/extract_r75_r76.py h1 75 76           # H1: the cave rung duties
"""
...
D.ROUTES["75"] = ("75604b0a432fdc89_00000075--9bbcb3f7da", 16, "_scratch/cache/r75", "r75s", "r75",
                  "UNVERIFIED")
D.ROUTES["76"] = ("75604b0a432fdc89_00000076--c81e964e05", 13, "_scratch/cache/r76", "r76s", "r76",
                  "UNVERIFIED")
D.ROUTES.setdefault("73", ("75604b0a432fdc89_00000073--9380c74d52", 11, "_scratch/cache/r73", "r73s", "r73",
                           "V88"))
D.ROUTES.setdefault("71", ("75604b0a432fdc89_00000071--ac50da2a6a", 4, "_scratch/cache/r71", "r71s", "r71",
                           "V87"))

# byte-4 alphabets, carried verbatim from `decode/extract_r73.py`
...
def _read_messages_tapped(path):
    """Tolerant reader (carried verbatim from `decode/extract_r73.py`) + the 0x1AB tap."""
    n = 0
    try:
...
# ======================================================================================
#  EXPOSURE CENSUS -- verbatim from decode/extract_r73.py, plus the operator's RATE regimes
# ======================================================================================
