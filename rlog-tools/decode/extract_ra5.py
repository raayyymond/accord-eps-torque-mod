#!/usr/bin/env python3
r"""Extract route `a5` (**V105 -- the 25.5 Hz NOTCH**) into `analysis-2020accord/_scratch/cache/ra5/`.

THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly the `decode/extract_ra4.py` / `decode/extract_r9e.py` pattern: add a
row to `decode_v84_probe_r6d.ROUTES`, then call `extract_r7d.extract_route()` -- the SAME code that
wrote every cache since `_scratch/cache/r6d/`.

===================================================================================================
ROUTE a5 == V105.  V104 BASE + FOUR BIQUAD FLOATS + TWO CAVE HALFWORDS.  24 B in 8 runs.
===================================================================================================
Byte diff V104 -> V105 (from `builds/v80_v107/build_v105_tva.py`, re-stated here, NOT re-derived):
...
Usage:
    python decode/extract_ra5.py                 # full pipeline
    python decode/extract_ra5.py extract
    python decode/extract_ra5.py derive
    python decode/extract_ra5.py identity
    python decode/extract_ra5.py lane427
"""
...
NEW = {
    "a5": ("75604b0a432fdc89_000000a5--1419044ddf", 12,
           "analysis-2020accord/_scratch/cache/ra5", "ra5s", "ra5", "V105"),
}
...
def identity(route="a5"):
    ...
    ra4_b6 = None
    j4 = ROOT / "analysis-2020accord" / "_scratch/cache/ra4" / "ra4_identity.json"
    if j4.exists():
        try:
