#!/usr/bin/env python3
"""selfint_transfer.py -- measure the operator's self-interference thesis on the car.
...
Usage:  python selfint_transfer.py [--nboot N] [--quick]
Writes: _cache_selfint/selfint.json
"""
import json
import sys
...
def s0_instrument():
    ...
    print("      => there is only ONE angle on this bus.  `decode_two_angles.py`'s topology check\n"
...
MANUAL_EXTRA = {"r5d(V74)": ("_cache_r5d", "r5ds", 70),
                "r54": ("_cache_r54", "r54s", 45),
                "r3b(V65)": ("_cache_r3b", "r3bs", 50),
                "r5a": ("_cache_r5a", "r5as", 40),
                "r47(V67)": ("_cache_r47", "r47s", 95)}
