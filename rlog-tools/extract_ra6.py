#!/usr/bin/env python3
r"""Extract route `a6` (**V106 -- gp-0x6b26 x3.0 ENGAGED**) into `analysis-2020accord/_cache_ra6/`.

THE INSTRUMENT IS NOT REIMPLEMENTED.  Exactly the `extract_ra5.py` pattern: add a row to
...
===================================================================================================
Byte diff V105 -> V106 (from `build_v106_tva.py`, re-stated here, NOT re-derived):
...
There is **no structural witness** distinguishing V106 from V105 in the telemetry.  What IS available:
  ...
  LEG 3  🆕 **`b5` DUTY SHIFT -- THE DOSE PROOF, and the only V106-specific leg.**
         ...
         through the |gp-0x6b26| distribution -- see `ra6_dose.py`.
🛑 **The operator's symptom report is itself a leg**: V106 changed nothing else, and he felt a change.

Usage:
    python extract_ra6.py                 # full pipeline
    python extract_ra6.py extract
    python extract_ra6.py derive
    python extract_ra6.py identity
    python extract_ra6.py lane427
"""
import json
import sys
...
NEW = {
    "a6": ("75604b0a432fdc89_000000a6--78293a259e", 26,
           "analysis-2020accord/_cache_ra6", "ra6s", "ra6", "V106"),
}
