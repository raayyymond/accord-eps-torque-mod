...
🛑 SIGN CONVENTION: `angle(scipy.signal.csd(x, y)) = arg(Y) - arg(X)`.  Positive => y LEADS x.
   Self-checked against a known lag in `v97_r80_vs_v96.py`; re-asserted here.
...
def main():
    ...
    (AN / "_cache_r80" / "r80_phase.json").write_text(json.dumps(out, indent=1, default=float))
    print(f"\nwrote {AN/'_cache_r80'/'r80_phase.json'}")
