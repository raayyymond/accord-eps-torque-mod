...
`analysis-2020accord/_cache_r95/`.
...
wrote every cache since `_cache_r6d/`.  Field names, ZOH/interp convention, sentinel definition and
...
INSTRUMENT FACTS honoured here (identical to `extract_r85.py`):
...
Usage:
    python extract_r95.py                 # extract + derive + identity + lane427 + health + census
    python extract_r95.py extract 95
    python extract_r95.py derive 95
    python extract_r95.py identity 95
"""
import json
import sys
...
NEW = {
    ROUTE: (PREFIX, NSEG_SCAN, "analysis-2020accord/_cache_r95", "r95s", "r95", "V101"),
}
for _k, _v in NEW.items():
    D.ROUTES[_k] = _v
D.ROUTES.setdefault("85", ("75604b0a432fdc89_00000085--cad692c3d3", 21,
                           "analysis-2020accord/_cache_r85", "r85s", "r85", "V100"))
...
# ---- V101's byte-4 map, read off `build_v101_tva.py`'s PAYLOAD, not inferred.
