...
`plant_phase_corner.py` section 3 pre-registered three falsifiers.  Two of them were reported but
...
def main():
    ...
    spread(routes)
    single_bin(routes)
    (HERE / "_plant_falsifiers.json").write_text(json.dumps(OUT, indent=1, default=float))
    print("\nwrote %s" % (HERE / "_plant_falsifiers.json"))
