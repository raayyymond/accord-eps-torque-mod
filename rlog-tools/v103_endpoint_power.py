#!/usr/bin/env python3
r"""v103_endpoint_power.py -- THE V103 ENDPOINT SPEC, with its power calculation MEASURED.
...
🛑 INDEPENDENCE.  The frozen Re(Z) estimator runs NW_Z = 512 with HOP_Z = 256, i.e. **50 % overlap**,
   ...
   `v92_rez.log` and with everything reported this session.

🛑 Estimator is imported read-only from `decode_v90_probe`; `rez_control.py` pins it to 0.00 %
   against `v92_rez.log`'s published route-77 table.  Run that FIRST if anything here looks wrong.
"""
import json
import sys
...
if __name__ == "__main__":
    for k in (sys.argv[1:] or list(PARTS)):
        PARTS[k]()
    Path(__file__).with_name("_v103_endpoint_power.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _v103_endpoint_power.json")
