...
`_cache_r6e/records_v85_score.pkl` -- read-only, never written here.  That guarantees the V67 /
...
Usage:
    python score_v86_r6f_r70.py records    # build V86 + V86B window records (slow)
    python score_v86_r6f_r70.py analyze    # A0-A7  the band scoring
    python score_v86_r6f_r70.py grind2     # B  the 40-49 Hz creep contradiction
    python score_v86_r6f_r70.py imped      # C  V86B's damping claim
    python score_v86_r6f_r70.py matched    # A4b the matched-theta_ddot engaged/manual contrast
    python score_v86_r6f_r70.py all
"""
import json
import pickle
import sys
...
CACHE6F = ROOT / "_cache_r6f"
CACHE70 = ROOT / "_cache_r70"
...
def matched():
    ...
    S.hdr("A4b ENGAGED vs MANUAL AT MATCHED theta_ddot AND MATCHED SPEED, WITHIN ROUTE.\n"
          ...
          "    answer by itself.")
    for lbl, spec in (("V86/r6f", ("_cache_r6f", "r6fs", 4)),
                      ("V86B/r70", ("_cache_r70", "r70s", 4)),
                      ("V85/r6e", ("_cache_r6e", "r6es", 8))):
        SL.ROUTES[lbl] = spec
...
def imped():
    ...
    T23.ROUTES.setdefault("V85/r6e", (ROOT / "_cache_r6e", "r6es", list(range(8)), [7]))
    T23.ROUTES.setdefault("V84/r6d", (ROOT / "_cache_r6d", "r6ds", list(range(12)), [11]))
    T23.ROUTES.setdefault("V83a/r68", (ROOT / "_cache_r68x", "r68xs", list(range(8)), [0, 7]))
