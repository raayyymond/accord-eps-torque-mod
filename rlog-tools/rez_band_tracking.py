#!/usr/bin/env python3
r"""rez_band_tracking.py -- DOES THE ANTI-DAMPING TRACK THE MODE, OR SIT IN A FIXED BAND?
...
Estimator imported read-only from `decode_v90_probe`; `rez_control.py` pins it to 0.00 % against
`v92_rez.log`.  Engaged, hands-off, moving, 29-86 km/h -- the endpoint's own conditioning.
"""
import json
import sys
...
if __name__ == "__main__":
    main()
    Path(__file__).with_name("_rez_band_tracking.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _rez_band_tracking.json")
