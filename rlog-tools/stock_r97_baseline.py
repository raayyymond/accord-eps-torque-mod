#!/usr/bin/env python3
r"""stock_r97_baseline.py -- THE STOCK FLOOR.  Route 97 == V9b STOCK, the kit's first modern
...
`v102_xb_lib` band-RMS, episode bootstrap, matched (speed x wheel-rate) cells.
    ...
    python stock_r97_baseline.py            # everything
    python stock_r97_baseline.py <part>     # census | null | endpoint | spectrum | bands | manual
...
NFFT, HOP = 256, 128                  # 2.56 s Hann, 1.28 s hop -- score_v102.py's own constants
# THE PRE-REGISTERED ENDPOINT (STATE.md), plus the second convention score_v102.py also carries
...
if __name__ == "__main__":
    ...
    for p in want:
        PARTS[p]()
    Path(__file__).with_name("_stock_r97_baseline.json").write_text(
        json.dumps(OUT, indent=1, default=float))
    print("\n  wrote _stock_r97_baseline.json")
