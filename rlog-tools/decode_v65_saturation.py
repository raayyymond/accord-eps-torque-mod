#!/usr/bin/env python3
"""decode_v65_saturation.py -- read V65's 4-level aggregator-saturation ladder out of an rlog.
...
Usage:  python decode_v65_saturation.py RLOG [RLOG ...]
"""
import sys
...
def report(tag, d):
    ...
    # ---- THE VERDICT -------------------------------------------------------------------------------
    sel = ok & eng & creep
    print(f"\n{'-' * 92}\n-- THE VERDICT (pre-committed in build_v65_tva.py, before the drive) --")
    if sel.sum() == 0:
    ...
    if alt_rail >= 2.0 and rail_frac > 0.02:
        ...
        print("      but SEPARATE record -- not a redundancy mirror, see build_v62_tva.py).")
