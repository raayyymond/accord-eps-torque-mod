...
🛑 THE KIT'S OWN RECORD IS SPLIT ON THIS, WHICH IS EXACTLY WHY IT MUST BE MEASURED, NOT CITED:
   * `build_v91_tva.py` calls the term *"genuinely DISSIPATIVE (it opposes motor rate)"* and names the
     whole lever "friction/damping-comp".
   * `memory/accord-gp6c2c-is-motor-rate-derivative.md` calls `gp-0x6c2c` a **motor-rate DERIVATIVE**.
...
Usage:  python v93_identify_6b26.py
"""
import json
import sys
...
def build(shift=0):
    z = np.load(CACHE / "_cache_r78" / "r78.npz", allow_pickle=True)
...
def main():
    ...
    z77 = np.load(CACHE / "_cache_r77" / "r77.npz", allow_pickle=True)
    ...
    (CACHE / "_cache_r78" / "identify_6b26.json").write_text(json.dumps(OUT, indent=1,
                                                                       default=float))
    print("\n  wrote analysis-2020accord/_cache_r78/identify_6b26.json")
