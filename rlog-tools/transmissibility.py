...
🛑 THE POSITIVE CONTROL COMES FIRST.  `imu_ceiling.py` found bar->IMU coherence at the chance
...
def main():
    ...
    (ROOT / "_cache_r6f" / "transmissibility.json").write_text(
        json.dumps(O, indent=1, default=float))
    print("\nwrote %s" % (ROOT / "_cache_r6f" / "transmissibility.json"))
