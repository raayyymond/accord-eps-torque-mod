...
WHAT THE PRECEDING FILES ESTABLISHED (all reproduced by `plant_recon.py` / `plant_impedance.py`)
        ...
        for lat, latl in ((True, "engaged"), (False, "manual")):
            for hold in ("off", "on"):
                ...
                peaks["%s|%s|%s" % (rt, latl, hold)] = dict(f=fpk, pk=float(sm[sub][i]),
                                                            ref=ref, nep=c["nep"])
    (HERE / "_plant_zcurve.json").write_text(json.dumps(dict(peaks=peaks, meta=OUT), indent=1,
                                                        default=float))
    print("\nwrote %s" % (HERE / "_plant_zcurve.json"))
