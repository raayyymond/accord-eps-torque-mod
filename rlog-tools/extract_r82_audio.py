...
TWO INSTRUMENTS ARE BUILT, and the second is the physically right one:
  ...
  (2) ⭐ **THE MODULATION SPECTRUM OF THE AUDIO ENVELOPE.**  A mechanical ratchet at ~8 Hz does not
      ...
      every existing band statistic in `v99_r82_score.py` applies to it unchanged.

🛑 TIME BASE: `t = logMonoTime * 1e-9 - t0_mono`, the SAME convention `extract_r7d.py` uses
   (`extract_r7d.py:145` and `:301`).  Asserted against the cache's own `t` span.
...
Usage:  python extract_r82_audio.py 82 81
...
ROUTES = {
    "82": ("75604b0a432fdc89_00000082--e30d55731b", 2, "_cache_r82", "r82", "V99"),
    "81": ("75604b0a432fdc89_00000081--c7103d2cb4", 3, "_cache_r81", "r81", "V98"),
}
