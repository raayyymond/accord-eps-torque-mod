...
`analysis-2020accord/_cache_r81/`.

🛑 THE INSTRUMENT IS NOT REIMPLEMENTED.  Like `extract_r80.py`, this file only adds a row to
`decode_v84_probe_r6d.ROUTES` and calls `extract_r7d`'s `extract_route()` -- the SAME code that
wrote every cache since `_cache_r6d/`.  Field names, ZOH/interp convention, sentinel definition
...
🛑 **THE ~50-BUILD "byte4[7:3] IS ALWAYS ODD" CONVENTION DOES NOT HOLD HERE.**  `b3` is a
   ...
   Liveness has moved to byte7.  (`build_v98_tva.py`, section VAL, pre-registered.)
...
Usage:
    python extract_r81.py                 # extract + identity + health + census
    python extract_r81.py extract 81
    python extract_r81.py identity 81
"""
import json
import sys
...
NEW = {
    "81": ("75604b0a432fdc89_00000081--c7103d2cb4", 3, "analysis-2020accord/_cache_r81",
           "r81s", "r81", "V98"),
}
...
# ---- V98's byte-4 map, read off `build_v98_tva.py`'s PAYLOAD, not inferred.
