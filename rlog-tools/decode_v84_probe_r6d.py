#!/usr/bin/env python3
"""decode_v84_probe_r6d.py -- route `6d` (and `68`): extract, ESTABLISH THE BUILD, then decode.

This EXTENDS `analysis-2020accord/decode_v84_probe.py` (imported, not copied -- its `classify_log`
is still the refusal gate and its bit map still comes from `build_v84_tva`).  What this file adds:

  1. **an extractor** for routes `6d` and `68`, writing `_cache_r6d/` and `_cache_r68x/` with the
     VERBATIM schema of `extract_r67_v81.py` (which is `compare_v75_v76_v80_grind.extract66`'s), so
...
Usage:
    python decode_v84_probe_r6d.py extract 6d      # -> _cache_r6d/
    python decode_v84_probe_r6d.py extract 68      # -> _cache_r68x/
    python decode_v84_probe_r6d.py identify 6d [68 67]
    python decode_v84_probe_r6d.py rungs 6d
    python decode_v84_probe_r6d.py health 6d
"""
import json
import os
import sys
...
ROUTES = {
    "6d": ("75604b0a432fdc89_0000006d--5d03a5adb4", 12, "_cache_r6d", "r6ds", "r6d", "V84?"),
    "68": ("75604b0a432fdc89_00000068--0b7efae911", 8, "_cache_r68x", "r68xs", "r68", "V83a"),
    "67": ("75604b0a432fdc89_00000067--9b3ebbe218", 14, "_cache_r67x", "r67xs", "r67", "V81"),
}


# ======================================================================================
# 1.  EXTRACTION -- schema-verbatim with extract_r67_v81.py
# ======================================================================================
def extract(route):
