...
ss1  |dtorque| IS A LOWER BOUND -- so how big would the underestimate have to be before V69 was
     ...
     TOWARD the saturation hypothesis, so the refutation in `r50_stiffness.py` ss2 is only as good
...
Writes `_r50_followups.json`.  Usage: python r50_followups.py
"""
import json
import sys
...
print("\n   THE RATCHET AS THE STIFFNESS CANDIDATE -- per-ENGAGED-window rate (from r50_ratchet.py):")
...
(HERE / "_r50_followups.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE / '_r50_followups.json'}")
