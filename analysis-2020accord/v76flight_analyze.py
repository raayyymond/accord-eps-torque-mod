#!/usr/bin/env python3
"""v76flight_analyze.py -- Q1-Q5 for V76's first flight (route 65), off `_cache_r65_records.pkl`.

Loads the cache built by `v76flight_extract.py` and answers, in order:
...
Usage: python v76flight_analyze.py
"""
import pickle
import sys
...
CACHE = HERE / "_cache_r65_records.pkl"
...
say("  can set them (re-derived from `build_v76_v38base_tva.py`'s own `ILLEGAL_MASK`/`wire_model()`,")
...
say("  `decode_v75_probe.py`), so bit4 alone (payload 0x10/0x18/0x90/0x98) is STRUCTURALLY IMPOSSIBLE")
...
say("  `rlog-tools/decode_v76_probe.py`, calls this BIT_UNUSED and asserts it never fires).")
...
say("=" * 100)
say("Checked (mirroring `v75fault_bitmap.py` / `v74fault_extract.py`'s fault fingerprint):")
