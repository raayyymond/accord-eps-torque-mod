...
Every non-probe channel is byte-for-byte `extract_r58_cache.py`'s (and therefore
`extract_r50_cache.py`'s), so `_grind2_lib.wrecs`, `_r31_common.load` and `_r4f_lib.avg_periodogram`
...
re-reads CAVE_HEX out of `rlog-tools/decode_v72_probe.py` at import time and fails this extractor if
...
Usage:  python extract_r59_cache.py            # all 15 segments
        python extract_r59_cache.py 0 1        # chosen segments
"""
import json
import os
import re
...
SEGS = list(range(15))
OUT = Path(os.environ.get("R59_CACHE", ROOT / "_cache_r59"))
PFX = "r59s"
...
def wire_byte4(v69a4, v6bd0, v6ac0, status_bits=0x7):
    ...
    Ported VERBATIM from `rlog-tools/decode_v72_probe.py`. 0xC4B34 movea 0x10,r0,r7 /
...
def _assert_cave_bytes():
    ...
    Re-read `CAVE_HEX` out of `rlog-tools/decode_v72_probe.py` (by REGEX, not by import, so this
    ...
    """
    src = (ROOT / "rlog-tools" / "decode_v72_probe.py").read_text(encoding="utf-8")
    m = re.search(r'^CAVE_HEX\s*=\s*"([0-9a-f]+)"', src, re.M)
    assert m, "CAVE_HEX not found in decode_v72_probe.py -- cannot verify the probe cells"
    ...
    assert mn, "RWD_NAME not found in decode_v72_probe.py"
