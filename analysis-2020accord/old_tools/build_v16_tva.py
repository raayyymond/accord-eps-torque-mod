"""build_v16_tva.py — V16 LKAS 2x + EME (driver-override dropout) FIX for the 2020 Accord (39990-TVA,A160).

LINEAGE: V15B (2x arb gain/clamps + part-number string fix) PLUS the EME root-cause fix.
The EME (whole-power-steering momentary cutout + ~10s ratchet when overriding 2x LKAS on a
sharp low-speed turn, no DTC) was traced this session to the delivered-command slew limiter
being DISABLED (tp+0x71d6 = 0). Whatever momentarily drops the combined motor command
(shaper deadband zero-crossing / governor dip / plausibility re-init), the disabled rate-limiter
turns it into a hard 0 -> hold -> jump-back instead of a soft dip. Re-enabling the slew (to 14,
the value other Honda EPS variants ship) smooths it. 2x magnitude (gain 0xC646C=1782) is untouched.
No fault-detection logic is modified (FUN_00041eec plausibility threshold is OFF-LIMITS).

WHAT V16 EDITS (on top of stock V9 code.bin):
  Calibration block #48 [0xC6000, 0xC6FFC) — halfwords (u16 LE):
    ...
    0xC61D6  SLEW     tp+0x71d6     0   -> 14     (NEW: re-enable delivered-command rate limiter)
    0xC6424  DEADBAND tp+0x7424  29491 -> 20000   (NEW: narrow the low-demand cut)
...
CAL_PATCHES = [
    ...
    (0xC61D6,     0,   14, "SLEW     tp+0x71d6  delivered rate limit   0->14 (re-enable) [EME]"),
    (0xC6424, 29491, 20000,"DEADBAND tp+0x7424  shaper low-demand cut 29491->20000 [EME]"),
]
...
def build(label, code_stock, headers, tag):
    print("=" * 78)
    print(f"{label}: LKAS 2x + EME driver-override fix + PN string   cipher v9b")
...
def main():
    ...
    build("V16", code, headers, tag="LKAS-2x-EMEfix-slew-deadband-ramp-PNfix")
    return 0
