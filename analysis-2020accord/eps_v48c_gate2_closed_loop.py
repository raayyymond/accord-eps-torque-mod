...
# -*- coding: utf-8 -*-
"""
eps_v48c_gate2_closed_loop.py -- GATE 2 (closed-loop stability) for reviving the 21 Hz notch as V48C.
...
IN ISOLATION:
  ...
  * GATE 2 (closed-loop stability) -- THIS FILE. The design only ever inserted the notch's
    single-frequency MAGNITUDE |N(21.4)| into the *LKAS* loop-gain model (eps_loop_gain_model.py),
    ...
    18-26 Hz can matter. eps_loop_gain_model.py Task 4(d) is annotated FALSIFIED for exactly this.
...
-----------------------------------------------------------------------------
It reuses the SAME calibrated model as eps_loop_gain_model.py (the golden loop-gain reference):
...
TOPOLOGY (docs/VIBRATION-DOSSIER.md sec.4, eps_loop_gain_model.py): the 21 Hz feedback closes through
...
Run:  python eps_v48c_gate2_closed_loop.py
"""
...
# ---------------------------------------------------------------------------
# CALIBRATION -- identical to eps_loop_gain_model.py (self-consistent by construction)
# ---------------------------------------------------------------------------
F0_HZ       = 21.4
