...
it is a truncation. The file is kept because `analyze_r31_spectra.py` exists to demonstrate exactly this
failure and needs the artifact to demonstrate it against.
⇒ For frequencies use `analyze_r31_spectra.py` (free 12-30 Hz argmax). For amplitudes use
  `analyze_r31_matched.py` / `analyze_r31_standstill.py`, which are free-band. Both are safe to quote.
...
`analyze_r31_standstill.py` is the arm that gets this right.
...
Usage:  python analyze_r31_grinding.py
"""
import sys
