#!/usr/bin/env python3
"""r47_orchestrator_checks.py -- the orchestrator's own first-hand numbers for the V67 route-47 session.
...
`_grind2_lib.py` rather than depending on it, because its job is to be an INDEPENDENT second method.
...
CREEP_POOLS = {
    ...
    "Kd=gated (V67 r47)":                    ["_cache_r47"],
}
HWY_POOLS = {
    "Kd=1.00 (V58 r2b + V59 r2c)": ["_cache_r2b", "_cache_r2c"],
    "Kd=2.00 (V62 r37 + V65 r3b)": ["_cache_r37", "_cache_r3b"],
    "Kd=2.44 (V67 r47)":           ["_cache_r47"],
}
...
def sec_modeid():
    ...
    for lab, caches, vsel in (
            ("CREEP grind #2 (V65 r3a/r3b, V62 r37)", ["_cache_r3a", "_cache_r3b", "_cache_r37"],
             lambda v: v < 4),
            ("HIGHWAY        (V65 r3b, V67 r47)", ["_cache_r3b", "_cache_r47"], lambda v: v > 20)):
