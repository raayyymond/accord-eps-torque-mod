...
Usage:  python extract/extract_r58_cache.py            # all 16 segments
        python extract/extract_r58_cache.py 0 1        # chosen segments
"""
...
SEGS = list(range(16))
OUT = Path(os.environ.get("R58_CACHE", ROOT / "_scratch/cache/r58"))
PFX = "r58s"
...
def _assert_mirror_byte():
    ...
    way `builds/v50_v79/build_v71c_tva.py` does) and fail if cave+0x1A is not 0x26 = `ld.h -0x6ada[gp],r6`.
    ...
    assert m, "CAVE_HEX_A not found in probe/decode_v71_probe.py -- cannot verify the mirror cell"
