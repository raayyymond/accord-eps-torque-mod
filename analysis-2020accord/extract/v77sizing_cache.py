...
Reads `_scratch/cache/r65/r65.npz` (the route-GLOBAL extract written by `studies/sessions/v77/v77sizing_extract.py`, 63,477
...
Usage:  python extract/v77sizing_cache.py
"""
import json
import sys
...
SRC = ROOT / "_scratch/cache/r65" / "r65.npz"
DST = ROOT / "_scratch/cache/r65"          # same dir as the global extract, per-segment files alongside it
PFX = "r65s"

# Fields `_grind2_lib.wrecs` / `_r31_common.load` / `_r47_lib.augment` read by name, straight
# passthrough (no rename needed -- `studies/sessions/v77/v77sizing_extract.py` already used the kit's field names).
