#!/usr/bin/env python3
"""v70_plots.py -- render the V70 explainer plots to a self-contained HTML page.

Every curve is re-derived from `_v70_plot_data.json`, which `v70_plot_data.py` computes from the
...
doc, so the plots cannot drift from what the build actually contains.
    ...
    python v70_plot_data.py && python v70_plots.py     ->  docs/v70-plots.html
"""
import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
D = json.load(open(HERE / "_v70_plot_data.json"))
OUT = HERE.parent / "docs" / "v70-plots.html"
...
<code>v70_plot_data.py</code> (mirrors <code>FUN_0003aa2c</code> / <code>FUN_0003ad74</code>);
