#!/usr/bin/env python3
"""r4f_ratchet_conditions.py -- WHAT the route-4f ratchet is conditional on.

`r4f_ratchet_inventory.py` establishes the episodes, the frequency, the amplitude and the order
...
def load(s):
    return {k: v for k, v in np.load(ROOT / "_cache_r4f" / f"r4fs{s}.npz").items()}
...
def c4_command_spectrum(rs):
    ...
    print("\n   The kit's PRESENCE threshold is prominence >= 10 (analyze_r37_v62_creep.py).")
