#!/usr/bin/env python3
"""verify_v68_image.py -- INDEPENDENT post-build integrity verifier for V68 (and any V68 derivative).

DESIGN RULE (the V40 process lesson, CLAUDE.md; same rule as verify_v52c_image.py): a verifier and the
builder it checks MUST NOT share an assumption. This file therefore imports NOTHING from
build_v68_tva.py / build_v67_tva.py / build_vfourframe_tva.py. The CRC block walk, the V850E2
...
It answers, mechanically:
  ...
  7. Are the control-path anchors (repoint, arms, sar sites, lockout, private gain) at their
     expected values?
  8. Does rlog-tools/decode_v68_probe.py's CAVE_HEX match the image?

Usage:
    ACCORD_FIRMWARE_ROOT=C:/Users/dudei/Desktop/Projects/accord-firmwares \
        python verify_v68_image.py [--built PATH] [--reference PATH]
...
DECODER = os.path.join(REPO, "rlog-tools", "decode_v68_probe.py")
...
def main():
    ...
    if os.path.exists(DECODER):
        ...
        check(bool(m) and m.group(1) == blk[:used].hex(),
              "rlog-tools/decode_v68_probe.py CAVE_HEX matches the built cave",
