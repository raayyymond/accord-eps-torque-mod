...
no amount of stratification can separate them -- `nearcentre_strict.py` ss3 returns 0 shared cells
...
Usage: python nearcentre_span_mid.py [ep|blk] -> writes _nearcentre_span_mid.json
"""
import json
import sys
...
print("  `nearcentre_strict.py` ss3, where amax x rate had ZERO jointly-occupied cells.\n")
...
(HERE.parent / "_nearcentre_span_mid.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE.parent / '_nearcentre_span_mid.json'}")
