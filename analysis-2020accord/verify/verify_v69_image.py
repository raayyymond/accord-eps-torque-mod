#!/usr/bin/env python3
"""verify/verify_v69_image.py -- EXACT-VALUE anchors for V69, independent of the builder.
...
`builds/v50_v79/build_v69_tva.py` beyond the addresses. Two anchors are here that `verify/verify_v68_image.py` does NOT
carry: **`0xC6564`** (r26's inert cal base) and the **mode-11/12 neighbour records**.

Usage:  python verify/verify_v69_image.py [path-to-_v69_plain_image.bin]
"""
...
def main(path=None):
    ...
    b = p.read_bytes()
    print(f"verify/verify_v69_image.py -- {p}\n  {len(b)} bytes\n")
    ...
    check(bytes(b[0xC6564:0xC6564 + 40]) == bytes(40),
          ...
          "(NOT checked by verify/verify_v68_image.py)")
    ...
    print("   🛑 This checks VALUES. Run verify/diff_build_vs_stock.py for SPAN coverage. Both, always.")
