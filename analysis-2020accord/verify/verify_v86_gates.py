#!/usr/bin/env python3
"""verify/verify_v86_gates.py -- NEGATIVE CONTROLS for `builds/v80_v107/build_v86_tva.py`'s verifier. 🛑 CALIBRATE THE
INSTRUMENT BEFORE USING IT.

★ THE POINT: **a gate that has never fired is not evidence.** The V86 null build passes every assert
in `builds/v80_v107/build_v86_tva.py` -- which proves the asserts RUN, and proves nothing about whether they would
...
🛑 THE HEADLINE CASE IS #4, THE SPAN-VS-VALUE TRAP. `verify/diff_build_vs_stock.py` is SPAN-based: it asks
...
Usage:
    python verify/verify_v86_gates.py
"""
...
@case("🛑 REGRESSION: the count-only residual gate is NOT sufficient, and this proves it",
      ...
      "caught a real defect in the first draft of builds/v80_v107/build_v86_tva.py's verifier")
def _c14b():
    m, _rec = _write_unreachable_m10()
...
def main():
    ...
    print("  ✅ EVERY GATE IN builds/v80_v107/build_v86_tva.py's verifier is DEMONSTRATED to catch its own defect.")
