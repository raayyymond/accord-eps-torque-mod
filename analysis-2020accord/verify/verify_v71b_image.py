#!/usr/bin/env python3
"""verify/verify_v71b_image.py -- EXACT-VALUE anchors for V71B, independent of the builder.

🛑 WHY A SEPARATE VERIFIER. V71A and V71B carry a **BYTE-IDENTICAL 68-byte cave** and differ only in
how r26 is dosed. `verify/diff_build_vs_stock.py` is span-based and cannot tell them apart at all; even
`verify/verify_v71a_image.py` would PASS on several of V71B's bytes. The distinguishing facts are:
...
Usage:  python verify/verify_v71b_image.py [path-to-_v71b_plain_image.bin]
"""
...
def main(path=None):
    ...
    print(f"verify/verify_v71b_image.py -- {p}\n  {len(b)} bytes\n")
