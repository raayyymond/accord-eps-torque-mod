...
(`r5d_duty.py` §1: across 8 routes, duty spans 64x while in-burst amplitude spans 1.24x). The
...
Usage:  python v78_symptom_limitcycle.py   ->  writes _v78_limitcycle.json
"""
import json
import pickle
import sys
...
G.EPKEY = "blk"                 # `r5d_duty.py`'s own default -- the same unit, so it is comparable
...
SRC = ROOT / "_cache_r5d_nearcentre.pkl"
MINE = ROOT / "_cache_r5e_sym_nearcentre.pkl"
...
for key, kl in (("e_18-22", "GRIND #1  18-22 Hz"), ("e_6-9", "MICRO RATCHET  6-9 Hz")):
    ...
    print("  unit, exactly as `r5d_duty.py`. duty = fraction of windows at or above T; in-burst =")
...
with open(ROOT / "_v78_limitcycle.json", "w", encoding="utf-8") as fh:
    json.dump(OUT, fh, indent=1, default=float)
print("\nwrote _v78_limitcycle.json")
