#!/usr/bin/env python3
r"""
V224 -- V222 + THE RATCHET DOSE DEEPENED 512 -> 256   (rung 2, rebased off V222)

WHY THIS BUILD EXISTS
---------------------
V218 was cut off V217. V222 then added TWO things V217 does not have:
...
So flying V218 after V222 would SILENTLY HAND BACK BOTH LEVERS. That is the exact failure
shape that lost V42's ratchet fix at a rebase and that hid the 6-9 Hz damper cut inside every notch
build from V196 to V213. This build is V218's lever, on V222's base.

🛑 STILL RUNG 2, STILL NOT THE PRIMARY. Fly V222 first. This is what to fly if the V222 drive points
at RATCHET rung 2 specifically -- see the decision table on docs/scoring/DRIVE-CARD-V222.md.

EVERYTHING ELSE IS V218, BYTE FOR BYTE.
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V224_WRITE", "").strip().lower()
...
NEW_SCALE = 256                # QUARTER Honda unity 1024 -- rung 2, V217 is rung 1 at 512
...
def build():
    print("=" * 102)
    print("  V218 -- V217 WITH THE RATCHET DOSE DEEPENED 512 -> 256   (rung 2 of the ladder)")
    print("=" * 102)
    ...
    # DOSE LADDER, priced 2026-08-29 from the real curve (assist_map_mirror), input-scaled:
    #   0xC63AE   small-amp loop gain   |gp-0x6b70| cost
    #     1024          1.000x               1.000x        Honda
    #      512          0.500x               0.500x        V217, rung 1
    #      256          0.250x               0.250x        V218, rung 2  <-- this build
    # The trade is EXACTLY 1:1 at small amplitude with NO KNEE, so no dose is analytically
    # preferred -- it is a pure preference between ratchet reduction and assist. Fly V217
    # FIRST; this rung exists so a partial result has an immediate follow-up, and so the two
    # together form a DOSE-RESPONSE, which is this kit's strongest evidence form.
    check(NEW_SCALE * 2 == 512, "V218 is exactly one further halving from V217s 512")
    ...
    FF.assert_x31_checksum(rwd, "V218 output")
    dec = bytearray(base)
    ...
    tag = "V224-V222BASE-C63AE.512.TO.256-RATCHET.RUNG2"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v224_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V224_WRITE=rwd to emit the files")
