#!/usr/bin/env python3
r"""
V205 -- IS THE OBSERVER OUTPUT A RELAY?  Base = V202.  3 bytes, telemetry only.
...
ratchet on exactly that: "Engagement amplifies 6-9 Hz 2.8x via a COMMAND-PROPORTIONAL COULOMB RELAY".
gp-0x6b70 is also the traced route to gp-0x6ad6, the torque-tracking reference.  So this is the
ratchet's named mechanism, sitting on a cell we can read for 3 bytes.

WHY THE CORPUS CANNOT ANSWER IT
-------------------------------
V96-V99 all carried a 427 tap on gp-0x6b70 (V100's changelog repoints it AWAY from gp-0x6b70, which
dates the earlier target unambiguously), and routes r80/r81/r82 are cached.  But the cached `probe`
and `field` columns take only FOUR to SIX distinct values on engaged frames, spaced 64 and 16 apart:

    r80  {15, 79, 143, 207}          r81  {23, 71, 87, 135, 199, 215}

** That is the cave's packed BOOLEAN RUNG byte, not a magnitude. **  A shape statistic computed on it
is meaningless -- and one looked like a finding before the distinct-value count caught it.  There is
no magnitude channel for gp-0x6b70 anywhere in the corpus.  rlog-tools/score/observer_relay_shape.py
now REFUSES a channel with too few levels rather than reporting a number.

WHAT V205 DOES
--------------
Repoints the CAN 427 probe from gp-0x6ac0 onto gp-0x6b70.  Every V202 control cell is carried
unchanged; this adds an instrument only.

    0x55DF2  hw2 of `ld.h disp, gp, r6`   0x9540 (-0x6AC0)  ->  0x9490 (-0x6B70)
    0x55E10  the pack shift               sar 4 (0xA4)      ->  sar 6 (0xA6)

sar 6 because gp-0x6b70 is clamped to +-cal(0xC6200) = 8192: positives raw 0..128, negatives
896..1023, resolution 64, unambiguous to |x| <= 32704.  ** The shift is a property of the SOURCE **
-- three live probes now use three different shifts (5, 5, 6) for exactly that reason.

WHAT IT ANSWERS IN ONE DRIVE
----------------------------
    few distinct levels, mass at the rails   -> the stage IS a relay.  That localises the ratchet's
                                                named mechanism to one LERP, whose knots are then
                                                worth the two hops needed to reach them.
    smooth and spread                        -> the relay hypothesis is wrong at this stage and the
                                                record's "command-proportional Coulomb relay" lives
                                                somewhere else -- which is worth as much.
    saturated at +-8192                      -> the observer is railed and the 41x-corrected 0xC63AA
                                                sensitivity cannot be applied safely at all.
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V205_WRITE", "").strip().lower()
...
NEW_DISP, NEW_SAR = (-0x6B70) & 0xFFFF, 6
...
CARRIED_U16 = {0xC40D2: ("K1 -> Honda (V177)", 102),
               ...
               0x55DF2: ("427 probe source gp-0x6b70, the observer output (V205)", 0x9490)}
CARRIED_B = {0xC40DC: ("accel alpha -> Honda (V179)", 22),
             0x55E10: ("packer sar 6 (V205)", 0xA6)}
PTR_I = 0xCBE74
...
def build():
    ...
    check(0x10000 - d == 0x6B70, f"the probe now reads gp-0x6B70, the observer OUTPUT (hw2 0x{d:04X})")
    ...
    tag = "V205-V202BASE-PROBE-GP6B70"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v205_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V205_WRITE=rwd to emit the files")
