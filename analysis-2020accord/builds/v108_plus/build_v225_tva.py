#!/usr/bin/env python3
r"""
V225 -- V222 + 10x FORWARD GAIN + CLAMPS   (authority rung 2, rebased off V222)

WHY THIS BUILD EXISTS
---------------------
V219 was cut off V217. V222 then added TWO things V217 does not have:
...
So flying V219 after V222 would SILENTLY HAND BACK BOTH LEVERS. That is the exact failure
shape that lost V42's ratchet fix at a rebase and that hid the 6-9 Hz damper cut inside every notch
build from V196 to V213. This build is V219's lever, on V222's base.

🛑 STILL RUNG 2, STILL NOT THE PRIMARY. Fly V222 first. This is what to fly if the V222 drive points
at AUTHORITY rung 2 specifically -- see the decision table on docs/scoring/DRIVE-CARD-V222.md.

EVERYTHING ELSE IS V219, BYTE FOR BYTE.
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V225_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V219 -- V217 WITH AUTHORITY 8x -> 10x   (rung 2 of the authority ladder)")
    print("=" * 102)
    ...
    # hard cap (lane 5011, window [5012, 5119], a 108-count squeeze -- NOT taken).
    #
    # Priced on the record's own exponents, vibration ~ m^1.74 and authority ~ m^0.88, against
    # the notch's measured 3.59x at 22-26 Hz, relative to the CAR at 6x:
    #     8x   authority +28.9 %   vibration 1.650x   NET 0.459x  (quieter)   V217, rung 1
    #    10x   authority +56.0 %   vibration 2.432x   NET 0.677x  (quieter)   V219, rung 2
    # Both stay quieter than the car in the vibration band. ABOVE 29.5 Hz the notch gives
    # nothing back, so 10x raises loop gain there 2.43x vs the car against 8x's 1.65x --
    # a bigger UNOPPOSED raise, which is why rung 1 flies first.
    GAIN_EDITS = ((0xC6CD0, 5346, 8910, "LKAS forward gain 6x -> 10x"),
                  (0xC61B2, 3072, 4608, "forward clamp A, tracking the gain"),
                  (0xC61B4, 3072, 4608, "forward clamp B, tracking the gain"))
    ...
    _m_lane = u16(code, 0xC61B2) - _lane
    _m_wall = _eme - u16(code, 0xC61B2)
    check(_m_lane >= 256 and _m_wall >= 256,
          f"MARGINS: {_m_lane} counts above the lane, {_m_wall} below the EME wall"
          f" -- both >= 256, not a 1-count squeeze")
    ...
    FF.assert_x31_checksum(rwd, "V219 output")
    dec = bytearray(base)
    ...
    tag = "V225-V222BASE-GAIN10X-CLAMPS4608-AUTHORITY.RUNG2"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v225_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V225_WRITE=rwd to emit the files")
