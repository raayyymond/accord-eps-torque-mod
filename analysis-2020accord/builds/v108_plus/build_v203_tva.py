#!/usr/bin/env python3
r"""
V203 -- PROBE THE PATH THE NOTCH CANNOT REACH.  Base = V199.  2 bytes, telemetry only.
...
CORRECTION CARRIED FROM V201'S DOCSTRING
----------------------------------------
V201 quoted the pedestal EMA as passing 64.6 % of a 19.75 Hz input.  That was the value at the
CLAMP CEILING K = 204.  Reading the actual table settles it -- all four knots are K = 20:

    0xC68FE/0900/0902/0904  X = 0, 9830, 26214, 32768      (a FLAT schedule)
    0xC6906/0908/090A/090C  Y = 20, 20, 20, 20

    K = 20  ->  alpha = 0.009766  ->  fc = 1.56 Hz  ->  ** 7.9 % at 19.75 Hz **, not 64.6 %

0xC6382 = 41 is the alternate rate, selected only when the gp-0x6b62 gate is true -- and the record
measures that gate at duty 0.0000 over 75,227 engaged frames, so it does not fire engaged.

** So the bypass is REAL but SMALL: 7.9 %, not 64.6 %. **  The probe is still worth its 3 bytes,
because 7.9 % of a large limiter cut can still dominate what is left after a 25x notch, and nobody
has ever measured whether the friction-hold limiter cuts at all while engaged.  But it is no longer
a threat to the fix, and the earlier framing is retracted.
...
WRITE_MODE = os.environ.get("ACCORD_V203_WRITE", "").strip().lower()
...
CARRIED_U16 = {0xC40D2: ("K1 -> Honda (V177)", 102),
               ...
               0x55DF2: ("427 probe source gp-0x6b7e, the pedestal (V203)", 0x9482)}
CARRIED_B = {0xC40DC: ("accel alpha -> Honda (V179)", 22),
             0x55E10: ("packer sar 5 (V203)", 0xA5)}
PTR_I = 0xCBE74
...
def build():
    ...
    tag = "V203-V202BASE-PROBE-THE-PEDESTAL"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v203_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V203_WRITE=rwd to emit the files")
