#!/usr/bin/env python3
r"""
V138 -- V122 + alpha2 8 -> 2.  ONE cal.  THE BIGGER DOSE OF THE SAME LEVER AS V137.

*** FLY V137 (alpha2 5) FIRST.  This is its follow-on rung, not an alternative. ***

alpha2 is the lever the single-variable on-car ladder identifies (V112 -> V122, alpha2 14 -> 8,
endpoint 4.74 -> 3.38 = 1.35x, against a knee step that bought nothing).  V137 takes the in-family
1.60x step to 5.  THIS build takes 8 -> 2, a 4.00x step, which is LARGER than any alpha2 step ever
flown (biggest: 1.75x).  That is deliberate and it is the reason this is second in the queue.

    |H| over 18-22 Hz:  alpha2 8 -> 5.4903   alpha2 5 -> 4.0982   alpha2 2 -> 1.8490
    => 8 -> 2 is 2.97x less inertia lane;  5 -> 2 is a further 2.22x below V137.

WHY A 4x STEP IS ACCEPTABLE HERE BUT NOT AS THE FIRST BUILD AFTER A REGRESSION
-------------------------------------------------------------------------------
  * the quantization gate passes: the EMA state is 32-BIT and the output is >>9, so at alpha2 = 2
    the truncation deadband is 32 state units = 0.0625 OUTPUT LSB.  SUB-LSB => no stair-stepping.
  * the selectivity gate passes: an EMA has UNITY DC GAIN for any alpha, so on a 100-400 ms DRIVER
    hard reversal the detector loses only 1.03-1.23x while the 18-22 Hz band drops 2.97x.
  * every gp-0x6c2c consumer was in force across V91/V111/V112/V122 at alpha2 22/14/14/8.
  * and it is a REDUCTION of a feedback magnitude, which cannot destabilise a stable loop.
But after V133's regression the kit owes the operator ONE clean in-family step before a 4x one.

BASE = V122, THE LAST FLOWN KNOWN-GOOD BUILD.  Clamp 511, both Lever A arms stock, gain 6x.
"""
...
WRITE_MODE = os.environ.get("ACCORD_V138_WRITE", "").strip().lower()
...
# ---- THE EDIT -------------------------------------------------------------------------------
ALPHA2_CAL, ALPHA2_OLD, ALPHA2_NEW = 0xC40DC, 8, 2
ALPHA2_STOCK = 22
...
def build():
    print("=" * 102)
    print("  V138 -- V138-V122BASE-ALPHA2.2")
    print("=" * 102)
    ...
    print("\n  [4] SIZING -- DELIBERATELY OUT-OF-FAMILY.  V137 (the in-family step) flies FIRST.")
    ...
    print(f"      🛑 THIS STEP IS {ALPHA2_OLD/ALPHA2_NEW:.2f}x -- LARGER than the biggest"
          f" ever flown ({_max_flown:.2f}x).  Deliberate: V137 is the in-family step and must"
          f" fly FIRST.")
    check(ALPHA2_OLD / ALPHA2_NEW > _max_flown,
          f"  this build is DELIBERATELY out-of-family ({ALPHA2_OLD/ALPHA2_NEW:.2f}x) -- were it"
          f" in-family it would BE V137 and this build would not exist")
    _dead_out = (64.0 / ALPHA2_NEW) / 512.0
    check(_dead_out < 1.0,
          f"  🛑 THE QUANTIZATION GATE: truncation deadband {_dead_out:.4f} OUTPUT LSB is"
          f" SUB-LSB on the 32-bit EMA state, so the lane cannot stair-step at alpha2 ="
          f" {ALPHA2_NEW}")
    check(ALPHA2_NEW * 2 <= ALPHA2_OLD,
          "  and it is a REDUCTION of a feedback magnitude, which cannot destabilise a stable"
          " loop whatever its phase -- RAISING is the destabiliser, as V133 demonstrated")
    ...
    FF.assert_x31_checksum(rwd, "V138 output")
    dec = bytearray(base)
    ...
    tag = "V138-V122BASE-ALPHA2.2"
    img_out = plain_image_path(f"_v138_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V138_WRITE=rwd to emit the files")
