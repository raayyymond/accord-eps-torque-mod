#!/usr/bin/env python3
r"""
V180 -- EVERYTHING: all three Honda reverts + the stronger pole.  Base = V179.  3 cells (12 bytes).
...
WRITE_MODE = os.environ.get("ACCORD_V180_WRITE", "").strip().lower()

BASE_NAME = "_v179_V179-V177BASE-ACCEL.EMA.ALPHA.REVERT.HONDA.22_plain_image.bin"
BASE_SHA = "c1e07f2d6e86bc31087ef69a490b968b0fc3626db114812f8e621b700a2d650c"
...
K1_CAL, K1_VAL = 0xC40D2, 102           # V177 revert, asserted CARRIED
ALPHA_CAL, ALPHA_VAL = 0xC40DC, 22      # V179 revert, asserted CARRIED
...
def build():
    print("=" * 102)
    print("  V180 -- EVERYTHING   (base V179: K1 + inertia + accel filter reverts, poles 0.980)")
    print("=" * 102)

    print("\n  [1] BASE")
    base = bytearray(Path(plain_image_path(BASE_NAME)).read_bytes())
    check(hashlib.sha256(bytes(base)).hexdigest() == BASE_SHA,
          f"base image sha256 matches V179 ({BASE_SHA[:16]}...)")
    code = bytearray(base)
    ...
    check(struct.unpack_from("<H", code, K1_CAL)[0] == K1_VAL,
          f"0x{K1_CAL:05X} K1 CARRIED at Honda {K1_VAL} (V177)")
    check(code[ALPHA_CAL] == ALPHA_VAL,
          f"0x{ALPHA_CAL:05X} accel alpha CARRIED at Honda {ALPHA_VAL} (V179)")
    for off, what in ENGAGED_ROWS.items():
    ...
    FF.assert_x31_checksum(rwd, "V180 output")
    dec = bytearray(base)
    ...
    tag = "V180-V179BASE-POLE.0.980-ALL.THREE.HONDA.REVERTS"
    img_out = plain_image_path(f"_v180_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V180_WRITE=rwd to emit the files")
    ...
    print("  ** V177 remains fly-first. V180 is the MAXIMUM build. **")
    print("=" * 102)
