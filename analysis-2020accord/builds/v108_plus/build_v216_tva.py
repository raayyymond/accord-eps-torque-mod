...
WRITE_MODE = os.environ.get("ACCORD_V216_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V216 -- V215 + THE FRICTION LANE BACK TO THE CAR = LIGHTER WHEEL  (base V208)")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V216 output")
    dec = bytearray(base)
    ...
    tag = "V216-V215BASE-FRICTION.LANE.TO.FLOWN.V108"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v216_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V216_WRITE=rwd to emit the files")
