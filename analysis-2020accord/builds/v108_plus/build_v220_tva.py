...
WRITE_MODE = os.environ.get("ACCORD_V220_WRITE", "").strip().lower()
...
def build():
    ...
    tag = "V220-V217BASE-NOTCH.POLES.13.50-GRIND.RUNG2"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v220_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V220_WRITE=rwd to emit the files")
