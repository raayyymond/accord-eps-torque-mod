#!/usr/bin/env python3
r"""
V152 -- HALVE THE OBSERVER CORNER ON BOTH ARMS AT ONCE, PRESERVING HONDA'S EXACT MATCH.
        0xC40D0 : 408 -> 204   AND   0xC63AC : 102 -> 51.   Base = V122.
...
WRITE_MODE = os.environ.get("ACCORD_V152_WRITE", "").strip().lower()
...
POLE_CAL, POLE_OLD, POLE_NEW = 0xC40D0, 408, 204   # MODEL-arm EMA  (alpha = cal/4096)
OBS_CAL,  OBS_OLD,  OBS_NEW  = 0xC63AC, 102,  51   # ACTUAL-arm IIR (alpha = cal/1024)
...
def build():
    ...
    FF.assert_x31_checksum(rwd, "V152 output")
    dec = bytearray(base)
    ...
    tag = "V152-V122BASE-OBSCORNER.HALVED.MATCHED"
    img_out = plain_image_path(f"_v152_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V152_WRITE=rwd to emit the files")
