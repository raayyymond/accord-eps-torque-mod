...
V128 -- RESTORE THE ENGAGED-MODE Y RECORD TO STOCK.  The grinding is OURS.  Base = V127.

THE FINDING
-----------
gp-0x6b26's speed schedule Y is a per-mode LERP record reached through the pointer array at
0xCBE74.  MANUAL_MODES = (24,25), ENGAGED_MODES = (26,27).  Reading the records out of the images:

    mode-26 record (0xD7A54, Y@0xD7A5C)      Y[0]     Y[1]     Y[2]    rails from |c2c| (0/20/90)
...
We tripled the ENGAGED-mode acceleration feedback at V96/V106 and every build since has carried
it.  The rail threshold fell 3.0x / 3.0x / 8.1x.  The detector arms at cal(0xC620A) = 12800, so
on STOCK the term was still LINEAR at the arming threshold at 90 km/h (15974 > 12800); on our
builds it rails from 1963 -- i.e. essentially always.

A railed acceleration term is sign(alpha)*511: a bang-bang Coulomb relay
(accord-v80-damper-relay-and-grind1-inert).  A relay ratchets; it does not damp.

WHY THIS EXPLAINS THE WHOLE RECORD
-----------------------------------
  * The symptom is ENGAGED-ONLY, and this dose is ENGAGED-ONLY (modes 26/27), while the manual
    modes 24/25 are byte-stock on every build.  accord-stock-mode24-equals-mode26-damper-is-ours
    already said "the engaged-only damper is OURS" -- this is the cell.
  * alpha2 works by DE-RAILING.  The lane is
        gp-0x4f50 -> EMA1(alpha0=0xC643C) -> (1 - z^-1) -> EMA2(alpha2=0xC40DC) -> gp-0x6c2c
    so lowering alpha2 shrinks the term's INPUT.  That is why 22 -> 14 -> 8 tracked the
    operator's reports monotonically: each step cut rail duty.
  * V91/V92's x1.5 dose on 0xCBE74 measured INERT (ratio 0.99).  A raise on an already-RAILED
    term is absorbed by the clamp and cannot show up.  This supersedes the "saturated counter
    bypasses the record" explanation offered earlier this session -- that one needs an
    oscillation episode, whereas clamp absorption explains a whole-drive null directly.
  * V107 measured 32.32 % rail duty at 10-25 km/h and noted "the symptom map and the rail-duty
    map are the same map".  This is the cell that set that map.

THE EDIT -- A RESTORATION, NOT AN EXPERIMENT
---------------------------------------------
...
already runs, so they are calibrated-safe by construction.  This is the largest available
de-rail and it is in the direction every piece of evidence points.

IT DOES NOT CONFOUND V127's EDIT
---------------------------------
0xC640A applies ONLY when the reversal counter is SATURATED; the mode record applies ONLY when it
is NOT.  The two branches are MUTUALLY EXCLUSIVE by construction, so this build remains
single-variable *per state* and one drive can still interpret both.  The 427 probe measures
|gp-0x6b26| duty in both states.

RESIDUAL RISK, STATED PLAINLY
------------------------------
[EVIDENCE] the values, the rail arithmetic, the mode assignment and the stock comparison.
[BELIEF]   that restoring improves the symptom.  Below the old rail point this removes 3-8x of
           acceleration feedback, which is real damping in the LINEAR regime.  Stock shipped
           exactly these values, so the risk is bounded to "the car behaves as Honda calibrated
           it", but V96/V106 raised them for a reason and that reason is not re-tested here.
           If grinding WORSENS, the next step is a PARTIAL restore sized to the measured duty,
           not a return to -29490.
...
WRITE_MODE = os.environ.get("ACCORD_V128_WRITE", "").strip().lower()
...
Y_OURS, Y_STOCK = (-29490, -17202, -16000), (-9830, -5734, -1966)   # THE EDIT
...
def build():
    print("=" * 102)
    print("  V128 -- V127 + the ENGAGED-mode Y record (modes 26/27) RESTORED TO STOCK.")
    print("=" * 102)
    ...
    for _m, _a in REC_Y.items():
        ...
        check(tuple(s16(stock, _a + 2 * i) for i in range(3)) == Y_STOCK,
              f"  mode {_m} Y = {list(Y_STOCK)} in STOCK -- this build RESTORES Honda's values")
    ...
    attributed |= {SAR_ADDR}
    for _m, _a in REC_Y.items():                            # THE EDIT -- restore to STOCK
        for _i, _v in enumerate(Y_STOCK):
            struct.pack_into("<h", code, _a + 2 * _i, _v)
            attributed |= {_a + 2 * _i, _a + 2 * _i + 1}
    attributed |= {K1_CAL, K1_CAL + 1}
    ...
    check(all(_r(a) < _r(b2) for a, b2 in zip(Y_OURS, Y_STOCK)),
          "  the restore RAISES every rail threshold -- strictly less relay behaviour")
    check(_r(Y_STOCK[2]) > ARM_VAL,
          f"  at 90 km/h STOCK is LINEAR right up to the arming threshold"
          f" ({_r(Y_STOCK[2])} > {ARM_VAL}); ours rails from {_r(Y_OURS[2])}")
    ...
    for m in ENGAGED_MODES:
        ...
        check(rec_y(code, m) == rec_y(stock, m),
              f"  mode {m} (ENGAGED) now byte-identical to STOCK")
    ...
    check(payload == 12, f"exactly 12 payload bytes ({payload} found)")
    ...
    tag = "V128-V127BASE-MODE26.Y.STOCK"
    img_out = plain_image_path(f"_v128_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V128_WRITE=rwd to emit the files")
