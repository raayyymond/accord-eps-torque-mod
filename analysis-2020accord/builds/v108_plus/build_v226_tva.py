#!/usr/bin/env python3
r"""
V226 -- V222 + NOTCH POLES 13.50   (grind rung 2, rebased off V222)

WHY THIS BUILD EXISTS
---------------------
V220 was cut off V217. V222 then added TWO things V217 does not have:

    0xC6446   5244 -> 13107     Lever B -- the kit's only lever with a measured on-car win
                                (V88: 15-22 Hz 0.549x, 9-12 Hz 0.604x, 0.5-3 Hz NULL), frozen for
                                130+ builds because V160 called 6553 an int16 ceiling. There is no
                                int16 on that path: 0x3AC18 is a 32-bit mul and the only bound is
                                the +-8192 rail, left byte-identical here.
    0xC40BC    600 -> 3000  \  the friction lane's SATURATION. V216 matched the car's unsaturated
    0xC40D2    204 -> 1020  /   slope EXACTLY (0.003984) and left what it saturates at 5x off, so
                                above 250 counts of rate the car's observer residual is annihilated
                                (model_out 7) while V217's stays live (1512) -- 216x. The RATCHET
                                REGIME is 1.0x identical, so no ratchet expectation changes.

So flying V220 after V222 would SILENTLY HAND BACK BOTH LEVERS. That is the exact failure
shape that lost V42's ratchet fix at a rebase and that hid the 6-9 Hz damper cut inside every notch
build from V196 to V213. This build is V220's lever, on V222's base.

🛑 STILL RUNG 2, STILL NOT THE PRIMARY. Fly V222 first. This is what to fly if the V222 drive points
at GRIND rung 2 specifically -- see the decision table on docs/scoring/DRIVE-CARD-V222.md.

EVERYTHING ELSE IS V220, BYTE FOR BYTE.
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V226_WRITE", "").strip().lower()
...
LEVER_B = 0xC6446              # r24 engaged derivative gain, Q10. 1 reader 0x3AC08
...
# 🛑 0xC40D2 WAS LISTED HERE AS 204 AND LABELLED "the FLOWN car". IT IS NOT: the car
# (V122) runs 1020. 204 matches only the car's unsaturated SLOPE, because V216 rescaled it
# alongside the gate; at saturation it leaves 80.1 % of the model standing where the car leaves
# 0.4 %, a 216x gap above 250 counts of rate. That mislabel is exactly why the gap survived
# every check. This build restores BOTH cells, so the assertion moves into gate [3b].
...
def build():
    print("=" * 102)
    print("  V220 -- V217 WITH THE NOTCH WIDENED TO THE 12-DEG BUDGET   (grind rung 2)")
    print("=" * 102)
    ...
    # V220 WIDENS THE NOTCH to the 12-degree phase budget. Same zeros (20.50 Hz -- confirmed
    # optimal under BOTH the median-episode-peak fit and an energy-weighted re-optimisation),
    # poles moved 15.50 -> 13.50, radius unchanged.
    #
    # Ladder, energy-weighted removal over 125 engaged episodes, WITH a passband floor:
    #    budget   design                          dphase@5Hz  passband  removed
    ...
    # Residual grind energy 4.7 % -> 2.8 %, i.e. 1.68x LESS LEFT OVER, for 3.9 deg more phase.
    # The biquad is in the BASE POWER-ASSIST path, so that phase is steering FEEL, not LKAS
    # command tracking -- a different currency from the one that killed V184.
    #
    # !! THE PASSBAND FLOOR IS NOT OPTIONAL. Optimising removal WITHOUT it returns a design
    # with poles at 11.5 Hz and r 0.985 that scores 99.0 % -- by attenuating the ENTIRE
    # passband to 0.62x, i.e. turning the base power assist DOWN 38 %. GATE 2 caps max|H| and
    # does NOT catch that. Close-out [15] now checks the floor.
    #
    # Specified by FORMULA, never by typed decimals -- a 6-dp decimal does not round-trip a
    # float32, and every assertion below is against the ENCODED value read back out.
    import cmath as _cm
    N_FS, N_F0, N_FP, N_R = 1000.0, 20.50, 13.50, 0.9575
    _tz = 2 * math.pi * N_F0 / N_FS
    _tp = 2 * math.pi * N_FP / N_FS
    _b1 = -2.0 * math.cos(_tz)
    _a1 = -2.0 * N_R * math.cos(_tp)
    _a2 = N_R * N_R
    _grid = [0.02 + 0.02 * _k for _k in range(6000)] + [120.0 + 0.5 * _k for _k in range(761)]
    def _mag(aa1, aa2, bb1, gg, f):
        _z = _cm.exp(2j * math.pi * f / N_FS)
        return abs(gg * (_z * _z + bb1 * _z + 1) / (_z * _z + aa1 * _z + aa2))
    _g = 1.0 / max(_mag(_a1, _a2, _b1, 1.0, f) for f in _grid)
    # SOLVE FOR THE ENCODED VALUE, not the design value. float32 rounding of a1/a2/b1/g moves
    # max|H| off the design: the first cut here landed at 1.0000065, i.e. GATE 2 FAILING by
    # rounding alone. Shrink g until the ROUND-TRIPPED coefficients satisfy the gate.
    for _off, _val in ((A8_OFF, _a1), (AC_OFF, _a2), (B0_OFF, _b1)):
        struct.pack_into("<f", code, _off, _val)
        attributed |= set(range(_off, _off + 4))
    _ea = [struct.unpack_from("<f", code, _o)[0] for _o in (A8_OFF, AC_OFF, B0_OFF)]
    for _try in range(200):
        struct.pack_into("<f", code, B4_OFF, _g)
        _ge = struct.unpack_from("<f", code, B4_OFF)[0]
        if max(_mag(_ea[0], _ea[1], _ea[2], _ge, f) for f in _grid) <= 1.0:
            break
        _g *= (1.0 - 1e-7)
    else:
        raise SystemExit("could not seat g under GATE 2 after rounding")
    attributed |= set(range(B4_OFF, B4_OFF + 4))
    print(f"      g seated in {_try} step(s) so the ENCODED coefficients pass GATE 2")
    _e = [struct.unpack_from("<f", code, _o)[0] for _o in (A8_OFF, AC_OFF, B0_OFF, B4_OFF)]
    _mx = max(_mag(_e[0], _e[1], _e[2], _e[3], f) for f in _grid)
    check(_mx <= 1.0, f"GATE 2 on the ENCODED floats: max|H| = {_mx:.7f} <= 1.0")
    _pb = min(_mag(_e[0], _e[1], _e[2], _e[3], f) for f in (0.02, 0.5, 1.0, 2.0, 3.0, 5.0))
    check(_pb >= 0.99, f"PASSBAND FLOOR on the encoded floats: min|H| 0-5 Hz = {_pb:.4f} >= 0.99")
    _d = _mag(_e[0], _e[1], _e[2], _e[3], N_F0)
    check(_d < 0.02, f"notch depth at {N_F0} Hz = {_d:.6f}")
    print(f"      notch widened: poles 15.50 -> {N_FP}, zeros {N_F0} unchanged,"
          f" max|H| {_mx:.6f}, passband {_pb:.4f}")
    ...
    print("\n  [3b] REBASE ONTO V222 -- Lever B and the friction lane's saturation")
    _before = u16(code, LEVER_B)
    check(_before == LEVER_B_OLD, f"0x{LEVER_B:05X} starts at the V217-era value {LEVER_B_OLD}")
    struct.pack_into("<H", code, LEVER_B, LEVER_B_NEW)
    attributed |= {LEVER_B, LEVER_B + 1}
    _onset = 8192 * 1024.0 / LEVER_B_NEW
    print(f"      0x{LEVER_B:05X}  {LEVER_B_OLD} -> {LEVER_B_NEW}   saturation onset {_onset:.0f} counts"
          f"   ({_onset / 146.0:.2f}x route r24's engaged torque-rate p90)")
    check(u16(code, LEVER_B) == LEVER_B_NEW, f"Lever B is now {LEVER_B_NEW}")
    check(_onset > 146.0 and _onset / 146.0 >= 2.0,
          "the onset must stay above r24's engaged p90 with at least 2x margin")
    ...
    for _a, _want in ((FRIC_GATE, 600), (FRIC_K1, 204)):
        check(u16(code, _a) == _want, f"0x{_a:05X} starts at the V217-era value {_want}")
    ...
    print(f"      0x{FRIC_GATE:05X} 600 -> {FRIC_GATE_CAR}, 0x{FRIC_K1:05X} 204 -> {FRIC_K1_CAR}"
          f"   slope {_slope_before:.6f} -> {_slope_after:.6f} (UNCHANGED)")
    check(abs(_slope_after - _slope_before) < 1e-9,
          "the unsaturated slope must be IDENTICAL -- the rebase must not touch the ratchet regime")
    ...
    # V220 DELIBERATELY moves the biquad -- assert it CHANGED, and that only the poles moved.
    check(u32(code, B0_OFF) == u32(base, B0_OFF), "0xC60B0 b1 (the ZEROS) unchanged -- same centre")
    check(u32(code, A8_OFF) != u32(base, A8_OFF), "0xC60A8 a1 moved -- the poles widened")
    ...
    # the widened notch: DERIVE the changed byte count from the encoded floats vs the base,
    # never assume it -- float32 re-encoding changes an unpredictable number of bytes.
    _nb = sum(1 for _o in (A8_OFF, AC_OFF, B0_OFF, B4_OFF) for _k in range(4)
              if code[_o + _k] != base[_o + _k])
    _exp += _nb
    print(f"      biquad re-encode changed {_nb} byte(s) vs the base")
    _exp += sum(1 for _k in range(2)
    ...
    FF.assert_x31_checksum(rwd, "V220 output")
    dec = bytearray(base)
    ...
    tag = "V226-V222BASE-NOTCH.POLES.13.50-GRIND.RUNG2"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v226_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V226_WRITE=rwd to emit the files")
