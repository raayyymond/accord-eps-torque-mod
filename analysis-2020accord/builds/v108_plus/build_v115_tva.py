#!/usr/bin/env python3
r"""
V115 -- V112 (FLOWN, BEST YET) + ALPHA2 14 -> 8.  ONE BYTE ON TOP OF THE BEST BUILD.

WHAT THIS IS
------------
V115 = V112 with ONE byte moved.  0xC40DC (alpha2) 14 -> 8.
The relay knee (0xC40BC = 1800) and K1 (0xC40D2 = 612) are V112's values and are HELD, so this is
single-variable against the build that is ON THE CAR.

WHY THE BASE IS V112 AND NOT V111
---------------------------------
V112 flew as routes 22 and 23, fault-free.  Operator, verbatim: "This is the best firmware ever on
6x torque.  Grind #1 is now rare... the smoothest firmware ever with the least amount of
ratcheting."  And it is not only a comfort win -- it measurably improved COMMAND AUTHORITY, which is
the operator's own standing ask:
...
=> V112 tracks 1.37-1.62x better than V111 at 5-15 deg/s and better in every band.
V114 was built on a V111 base before this was known.  **V115 supersedes V114**; V114's base is
simply the wrong one now.

*** AND THE ARGUMENT THAT WITHDREW V112 IS REFUTED ON-CAR. ***  I predicted V112 would add up to
2.93x anti-damping above 10.6 deg/s and recommended against flying it.  It is the best build yet.
That describing-function argument is not settled physics and is not carried forward.

WHAT REMAINS, AND WHY ALPHA2
----------------------------
The operator's remaining symptom is a FIXED OSCILLATION at the peak of a hard curve.  It is located
exactly -- route 23, segment 7, t = 445.6-448.2 s:

    7.42 Hz
    6-9 Hz torque rms  1630 counts   (corpus max; p50 87, p90 466)
    6-9 Hz RATE   rms 16.86 deg/s    (corpus p99 3.98, p50 0.40 -- 4.2x the p99)

Three hypotheses were tested and killed: the command rail is NOT causal (0.76x [0.22, 1.49]);
driver grip has no reliable effect (0.79x [0.67, 1.01], measured on rate so the split and the
measure use different sensors); duration/angle/speed correlate at r = 0.159 / -0.154 / 0.338.
What gates it is command MAGNITUDE, saturating near 1500 counts.

7.42 Hz sits in the band Re(Z) measures at -43 to -67 across 17 route-arms, while the manual arm is
damped at every band.  Nothing on the command side can reach it: only 6.9% of rate power above
5 Hz is coherent with the command.  => the lever must add DAMPING at 6-9 Hz.

ALPHA2 IS THE ONLY CLEAN ONE, AND IT ROTATES RATHER THAN SCALES
---------------------------------------------------------------
The lane is H(f) = 64 * H_lp * (1 - 1/z) * H_ema, a BANDPASS.  alpha2 sets its upper corner, so
lowering it walks the peak DOWN toward the anti-damped band.  Splitting gp-0x6b26 against the
VELOCITY phasor -- DAMPING ~ |H|*sin(phi), MASS ~ |H|*cos(phi):

    alpha2   peak Hz   6-16Hz DAMPING   6-16Hz MASS   20-30Hz damping   broadband rms
      14      46.5        1.000            1.000           1.000            1.000   (V112)
       8      34.2        1.252            0.796           0.899            0.604   <- V115
       6      29.3        1.318            0.647           0.769            0.463

*** DAMPING UP 25%, APPARENT MASS DOWN 20%. ***  That is the operator's both-at-once directive, and
the mass reduction also serves his authority ask -- less programmed inertia opposing the command.
Dose 8 rather than 6 because 21-27 Hz is where V106's grinding win was measured: -10% give-back
instead of -23%, and grind #1 is still present.

IT CANNOT RAIL (V107's failure mode, inverted)
----------------------------------------------
V107 railed by multiplying the Y ROW -- a magnitude change.  alpha2 does the opposite: peak |H|
9.20 -> 6.15, broadband rms x0.604, and 100 Hz 7.13 -> 4.05.  Every magnitude FALLS, so rail duty
must fall.  The 100 Hz drop also attacks the "higher-pitched, several hundred Hz" grinding directly,
which may help the remaining grind #1 as a side effect.

GATES
-----
GATE 1 is the cleanest in the kit: exactly ONE gp/tp access image-wide
(0x41626  ld.hu 0x50dc,tp,r11  in FUN_00041464), zero writers.
GATE 2: a pole move on an existing first-order EMA -- no new state, no new nonlinearity, every
magnitude falls.  Both lineage conditions on this lever are met: it ships WITH the notch revert
(V108 did it; V112 carries Honda's biquad) and is taken UNCOMPENSATED (Y untouched).

RESIDUAL RISK
-------------
gp-0x6c2c fans out to three consumers; only the gp-0x6b26 damper is verified against a reshaped
signal.  The oscillation detector (FUN_000428d4 vs cal 0xC620A) is the second and fires LESS as
alpha2 falls -- the safe direction.  The third is unenumerated.  V109/V111 already flew this exact
axis (22 -> 14) fault-free.
...
WRITE_MODE = os.environ.get("ACCORD_V115_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V115 -- V112 (FLOWN, BEST YET) + ALPHA2 14 -> 8.  ONE BYTE ON TOP OF THE BEST BUILD.")
    print("=" * 102)
    ...
    check(KNEE_OLD in MEASURED_DUTY,
          f"  the relay ladder below is context only -- V115 does not touch the relay")
    ...
    check(KNEE_NEW == KNEE_OLD,
          f"  the relay knee is HELD at V112's {KNEE_OLD} -- V115 does not touch the relay lane")
    ...
    for a, nm in ((KNEE_CAL, "0xC40BC relay knee HELD at 1800 (V112)"),
    ...
    FF.assert_x31_checksum(rwd, "V115 output")
    dec = bytearray(base)
    ...
    tag = "V115-V112BASE-ALPHA2.14.TO.8"
    img_out = plain_image_path(f"_v115_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V115_WRITE=rwd to emit the files")
