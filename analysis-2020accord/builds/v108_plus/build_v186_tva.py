#!/usr/bin/env python3
r"""
V186 -- MOVE HONDA'S NOTCH ONTO THE RATCHET.  Base = V185.  Four float32 cells.

WHAT IS NEW AS A CLASS
----------------------
Every filter build in the arc (V43 pole, V173/V174/V184 poles) moved the DENOMINATOR -- the poles --
which makes a LOW-PASS.  V186 moves the NUMERATOR.  That has never been done.
...
ratchet attenuation with phase in openpilot's band:
    ...
    V186 (notch on the ratchet)         -6.0 dB        -9.9 deg       0.61   <-- 2.8x better
...
WHERE THE NOTCH GOES -- FITTED TO THE MEASURED LINE, NOT AN ASSUMED BAND
------------------------------------------------------------------------
Pooled 1080 engaged-creep windows across the whole corpus:
    power-weighted centroid 5-12 Hz  8.125 Hz
    peak bin                         8.008 Hz
(f0, pole radius) was then chosen to minimise TOTAL REMAINING 5-12 Hz POWER against that measured
shape -- so the ratchet's real +/-0.71 Hz wander is weighted by how much power actually sits at each
frequency, instead of assuming a flat band.  Result: f0 = 8.30 Hz, pole radius 0.9885,
** 5-12 Hz power falls to 0.2498 of stock = a 4.0x reduction. **
...
WRITE_MODE = os.environ.get("ACCORD_V186_WRITE", "").strip().lower()
...
SEC_FS = 1000.0
F0 = 8.30          # notch centre, Hz -- fitted to 1080 pooled engaged-creep windows
RP = 0.9885        # pole radius     -- fitted with it, subject to the GATE 2 phase budget
...
def build():
    print("=" * 102)
    print("  V186 -- HONDA'S NOTCH MOVED ONTO THE RATCHET   (base V185)")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V186 output")
    dec = bytearray(base)
    ...
    tag = "V186-V185BASE-NOTCH.ON.THE.RATCHET"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v186_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V186_WRITE=rwd to emit the files")

    print("\n" + "=" * 102)
    print(f"  image SHA256 {img_sha}")
    print(f"  .rwd  SHA256 {rwd_sha}")
    print(f"  {_checks[1]}/{_checks[0]} assertions passed")
    print("  ** 5-12 Hz power to 0.2498 of stock = 4.0x, for -9.9 deg in openpilot's band. **")
