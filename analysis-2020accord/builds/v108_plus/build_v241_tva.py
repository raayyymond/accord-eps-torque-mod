# -*- coding: utf-8 -*-
r"""V241 -- THE NOTCH RE-AIMED ON THE INDEPENDENT INSTRUMENT. SIXTEEN BYTES ON V235.

WHY. V235's notch geometry was fitted to a CAN objective -- the EPS's own channels, the same subsystem
the build modifies. The comma IMU is physically independent of the EPS and had no part in choosing it.
Asked the open question (at which frequencies does engagement raise chassis motion above what the road
explains?), speed-matched, each route divided by its OWN road control, it names 22-30 Hz as the largest
engagement-created band in the whole 3-45 Hz range, peaking at 25-26 Hz. V235's BAND is RIGHT.

Its SHAPE is not optimal for that band:

    ON DISK  V235   zero 25.00 Hz  pole 23.50 Hz  r 0.960   cost 0.43254   min|H| 6-15 = 0.9108
    ON DISK  stock  zero 55.23 Hz  pole 42.35 Hz  r 0.797   cost 0.57508   min|H| 6-15 = 0.9344
    V241            zero 29.75 Hz  pole 22.50 Hz  r 0.940   cost 0.31079   min|H| 6-15 >= 0.9344

**V241 removes 28.1 % more of the measured engagement excess than V235** -- and it does so while
cutting LESS of the damping band.

TWO CONSTRAINTS, BOTH THE RECORD'S OWN, NOT MINE.
  * `max|H| <= 1.0000` over 0-50 Hz. V194/V195/V196/V198 were PULLED for max|H| 1.3533-1.7177, and a
    later candidate peaking at 1.0020 was DELETED rather than granted a third exception.
  * `min|H| over 6-15 Hz >= what STOCK achieves (0.9344)`. The lane is measured DAMPING there
    (cos -0.918 / -0.989 / -0.629, 3/3 routes) and the instruction is verbatim: "never notch 6-15 Hz
    on this lane". **V235 sits at 0.9108 -- it cuts about 2.5 % MORE of the damping band than Honda
    does. V241 fixes that.** A first pass used an arbitrary 0.97 floor and rejected STOCK ITSELF; the
    bar is now the car, which is the correction the record already had to make once for a V232
    threshold.

ROBUSTNESS: leave-one-route-out over all 10 routes returns **zero 29.75 / pole 22.50 / r 0.940 on
EVERY fold** -- one distinct winner in ten. The geometry is not fitted to any single route. That is the
same check V235's own geometry passed on the CAN objective.

COEFFICIENTS BY FORMULA, never by decimal -- a 6-dp decimal does not round-trip a float32:

    wz = 2*pi*29.75/1000 ; b1 = -2*cos(wz)
    wp = 2*pi*22.50/1000 ; a1 = -2*0.940*cos(wp) ; a2 = 0.940**2
    c4 = |(1 + a1 + a2) / (1 + b1 + 1)|          # unit DC gain

WHAT IS ASSUMED. The weight is CHASSIS MOTION and the notch filters a TORQUE lane, so this is a
better-founded objective than the one V235 was fitted to -- not the lane's own spectrum. The 28.1 % is
a reduction in the modelled cost against the measured profile, NOT a promise of 28 % less grinding.

BASE: V235. V241 = V235 with four float coefficients rewritten. Everything else byte-identical, so the
pair isolates the notch geometry exactly.
"""
import hashlib
import os
import struct
...
WRITE_MODE = os.environ.get("ACCORD_V241_WRITE", "").strip().lower()
...
TAG = "V241-V235BASE-NOTCH.IMU.29.75-22.50-0.940"
...
def build():
    ...
    FF.assert_x31_checksum(rwd, "V241 output")
    dec = bytearray(base)
    ...
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v241_{TAG}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{TAG}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [9] NOT WRITTEN -- set ACCORD_V241_WRITE=rwd to emit the files")

    print("\n" + "=" * 102)
    print(f"  image SHA256 {img_sha}")
    print(f"  .rwd  SHA256 {rwd_sha}")
    print(f"  {_checks[1]}/{_checks[0]} assertions passed")
    print("  ** V241 RE-AIMS THE NOTCH ON AN INSTRUMENT INDEPENDENT OF THE EPS.                                    **")
    print("  ** V235's geometry was fitted to a CAN objective -- the EPS's own channels, the                       **")
    print("  ** same subsystem the build modifies. The comma IMU had no part in choosing it,                       **")
    print("  ** and names 22-30 Hz as the LARGEST engagement-created motion band (2.481,                           **")
    print("  ** peak 25-26 Hz) in the whole 3-45 Hz range. V235's BAND is right.                                   **")
    print("  ** Its SHAPE is not:                                                                                  **")
    print("  **   V235   zero 25.00  pole 23.50  r 0.960   cost 0.43254  min|H| 6-15 0.9108                        **")
    print("  **   stock  zero 55.23  pole 42.35  r 0.797   cost 0.57508  min|H| 6-15 0.9344                        **")
    print("  **   V241   zero 29.75  pole 22.50  r 0.940   cost 0.31079  min|H| 6-15 >=0.9344                      **")
    print("  ** => V241 removes 28.1 % MORE of the measured excess than V235, while cutting                        **")
    print("  **    LESS of the damping band. V235 sits BELOW Honda's own floor there.                              **")
    print("  ** LEAVE-ONE-ROUTE-OUT: 29.75 / 22.50 / 0.940 wins on ALL TEN folds, one distinct                     **")
    print("  ** winner in ten -- not fitted to any single route.                                                   **")
    print("  ** GATES, both the record's own, recomputed from the WRITTEN bytes:                                   **")
    print("  **   max|H| <= 1.0000  (V194-V198 PULLED at 1.3533-1.7177; a 1.0020 was DELETED)                      **")
    print("  **   min|H| 6-15 Hz >= 0.9344, what STOCK achieves (the lane DAMPS there)                             **")
    print("  ** NOT CLAIMED: that 28.1 % of modelled cost is 28 % less grinding. The weight is                     **")
    print("  ** CHASSIS MOTION and the notch filters a TORQUE lane.                                                **")
    print("=" * 102)
