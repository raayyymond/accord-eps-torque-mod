...
WRITE_MODE = os.environ.get("ACCORD_V145_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V145 -- V144's notch + a BINARY probe on gp-0x6C24, the gate-state mirror.")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V145 output")
    dec = bytearray(base)
    ...
    tag = "V145-V122BASE-NOTCH20HZ-427.GATE.6C24"
    img_out = plain_image_path(f"_v145_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V145_WRITE=rwd to emit the files")
