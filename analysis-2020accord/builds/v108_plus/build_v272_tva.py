...
WRITE_MODE = os.environ.get("ACCORD_V272_WRITE", "").strip().lower()
...
TAG = "V272-V112BASE-LKAS.PID.KI.1"
...
KI_CELL, KI_OLD, KI_NEW = 0xC63E6, 0, 1   # the LKAS-path integral gain
...
def build():
    print("=" * 102)
    print("  V272 -- THE LKAS INTEGRAL AT Ki=1, THE CONSERVATIVE FIRST DOSE.")
    print("=" * 102)
    ...
    check(u16(code, KI_CELL) == KI_NEW,
          f"Ki {KI_OLD} -> {KI_NEW} -- ONE FIFTH of the value driven elsewhere, so time-to-rail is 5x "
          f"longer and 'the loop closed' stays distinguishable from 'it railed' inside one drive")
    ...
    for _p in PUMP_SURF + PUMP_BOOST:
        check(bytes(code[_p:_p + 26]) == bytes(base[_p:_p + 26]),
              f"pump record 0x{_p:06X} UNTOUCHED -- V270 is disjoint from V264/V265/V266/V268")
    ...
    FF.assert_x31_checksum(rwd, "V272 output")
    dec = bytearray(base)
    ...
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v272_{TAG}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{TAG}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [9] NOT WRITTEN -- set ACCORD_V272_WRITE=rwd to emit the files")
