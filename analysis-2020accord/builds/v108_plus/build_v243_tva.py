# -*- coding: utf-8 -*-
r"""V243 -- 10x LKAS GAIN ON THE FINISHED GRINDING WORK. FOUR BYTES ON V242.

WHY THIS BUILD EXISTS. the STRUCTURAL CEILING. Exact tracking would need clamp 5120, which EQUALS the
soft-EME floor 0xC674E and fails the audit, so the clamp is held at 4608 as V219/V225
did. Delivered authority rises 4096 -> 4608; the nominal 10x is NOT fully delivered.

WHAT IT IS. V242 carries every piece of the grinding work -- the IMU-aimed notch, Honda's
...
V222 -- at the car's 6x gain. This raises the gain and its tracking clamps, and changes NOTHING else.
    ...
    0xC6CD0   7128 -> 8910    the forward LKAS gain (8x -> 10x; 0xC646C = 891 is 1x)
    0xC61B2   4096 -> 4608    forward-path clamp A, tracking
    0xC61B4   4096 -> 4608    forward-path clamp B, tracking
...
WRITE_MODE = os.environ.get("ACCORD_V243_WRITE", "").strip().lower()

BASE_NAME = "_v242_V242-V241BASE-GAIN8X.CLAMPS4096_plain_image.bin"
BASE_SHA = "424249b0c7d89fad4e1192fbb89b829fb6295e4260b7959d2a406c95657a5576"
...
G_OLD, G_NEW = 7128, 8910
C_OLD, C_NEW = 4096, 4608
...
TAG = "V243-V242BASE-GAIN10X.CLAMPS4608.CEILING"
...
def build():
    ...
    FF.assert_x31_checksum(rwd, "V243 output")
    dec = bytearray(base)
    ...
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v243_{TAG}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{TAG}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [9] NOT WRITTEN -- set ACCORD_V243_WRITE=rwd to emit the files")

    print("\n" + "=" * 102)
    print(f"  image SHA256 {img_sha}")
    print(f"  .rwd  SHA256 {rwd_sha}")
    print(f"  {_checks[1]}/{_checks[0]} assertions passed")
    print("  ** V243 RAISES THE LKAS GAIN TO 10X ON THE FINISHED GRINDING WORK.                                    **")
    print("  **   0xC6CD0   7128 ->  8910     the forward gain (0xC646C = 891 is 1x)                               **")
    print("  **   0xC61B2   4096 ->  4608     forward clamp A, tracking                                            **")
    print("  **   0xC61B4   4096 ->  4608     forward clamp B, tracking                                            **")
