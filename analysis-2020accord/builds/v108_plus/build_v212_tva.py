...
WRITE_MODE = os.environ.get("ACCORD_V212_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V212 -- NOTCH + SOFT-RELAY DOSE + THE gp-0x6b4e PROBE   (base V208)")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V212 output")
    dec = bytearray(base)
    ...
    tag = "V212-V208BASE-C63AE.512-PROBE.GP6B4E"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v212_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V212_WRITE=rwd to emit the files")

    print("\n" + "=" * 102)
    print(f"  image SHA256 {img_sha}")
    print(f"  .rwd  SHA256 {rwd_sha}")
    print(f"  {_checks[1]}/{_checks[0]} assertions passed")
    print("  ** V208 notch (20.50 Hz) + 0xC63AE 1024->512 + the 427 probe on gp-0x6b4e. **")
    print("  ** ONE build for BOTH named symptoms, instrumented: the notch targets the    **")
    print("  ** 18-22 Hz grinding, the cal targets the ~7.8 Hz ratchet, and the probe     **")
    print("  ** separates them -- so ONE short symptomatic drive is interpretable either  **")
    print("  ** way.  V210 carried both levers but NO instrument.                          **")
