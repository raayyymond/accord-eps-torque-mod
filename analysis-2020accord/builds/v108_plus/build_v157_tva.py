...
That is the failure mode to stay away from, and it bounds the dose:
        creep product = (FactorC_Y0 x FactorE_Y0) >> 10 = (234 x 539) >> 10 = 123   (m27: 122)
        ceiling = 512   =>   this build sits at 24.0% of it   (V156 sits at 6.1%)
BOTH doses are the tables' OWN neighbouring knot values, not inventions:
        FactorC Y[0] -> its own Y[1]  (234 on m26, 233 on m27) => the first segment becomes FLAT
                        from 0 to X[1]=3840 instead of ramping up from zero
        FactorE Y[0] -> its own Y[2]  (539)
=> 4x V156's creep damping, still with a 4.2x margin to the bang-bang ceiling.
=> V80's failure was FactorC FLAT 566 across ALL FOUR knots.  This flattens only the FIRST segment
   and leaves Y[1..3] byte-identical, so the high-speed ramp is untouched.
...
WRITE_MODE = os.environ.get("ACCORD_V157_WRITE", "").strip().lower()
...
FC_NEW_M = {26: 234, 27: 233}                   # FactorC Y[0] -> each mode OWN Y[1]
FE_NEW = 539                                    # FactorE Y[0] -> its own Y[2]
...
def build():
    ...
    for _m in sorted(FC_Y0):
        ...
        print(f"      mode {_m}  0x{FC_Y0[_m]:05X} FactorC Y[0]  0 -> {FC_NEW_M[_m]}")
        print(f"      mode {_m}  0x{FE_Y0[_m]:05X} FactorE Y[0]  0 -> {FE_NEW}")
        check(u16(code, FC_Y0[_m]) == FC_NEW_M[_m] and u16(code, FE_Y0[_m]) == FE_NEW,
              "  both read back")
        check(FC_NEW_M[_m] == u16(base, FC_Y0[_m] + 2),
              f"  \U0001f6d1 mode {_m} FactorC Y[0] = {FC_NEW_M[_m]} is this mode's OWN Y[1]"
              f" -- the first segment becomes FLAT, no invented value")

    print("\n  [4] THE PRODUCT IS NON-ZERO AT CREEP, AND FAR UNDER THE BANG-BANG CEILING")
    prod = (FC_NEW_M[26] * FE_NEW) >> 10
    check(prod > 0,
          f"  \U0001f6d1 THE REACH GATE: creep product = ({FC_NEW_M[26]} x {FE_NEW}) >> 10 = {prod}."
    ...
    check(prod < CEIL // 4,
          f"  \U0001f6d1 THE MARGIN GATE: {prod} leaves a {CEIL/prod:.1f}x margin to the {CEIL}"
          f" ceiling.  V80's failure was FactorC FLAT 566 across ALL FOUR knots; this flattens only"
          f" the FIRST segment and leaves Y[1..3] byte-identical, so the high-speed ramp is"
          f" untouched.")
    ...
    check(payload == 6, f"exactly 6 payload bytes ({payload} found) -- 1 for each FactorC Y[0] (0->234/233, high byte already 0) and 2 for each FactorE Y[0] (0->539), on 2 modes")
    ...
    FF.assert_x31_checksum(rwd, "V157 output")
    dec = bytearray(base)
    ...
    tag = "V157-V122BASE-DAMPER.MICRO.4X"
    img_out = plain_image_path(f"_v157_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V157_WRITE=rwd to emit the files")
