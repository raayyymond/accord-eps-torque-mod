...
WRITE_MODE = os.environ.get("ACCORD_V218_WRITE", "").strip().lower()
...
def build():
    ...
    tag = "V218-V217BASE-C63AE.512.TO.256-RATCHET.RUNG2"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v218_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V218_WRITE=rwd to emit the files")
