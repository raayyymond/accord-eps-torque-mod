...
WRITE_MODE = os.environ.get("ACCORD_V213_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V213 -- NOTCH + SOFT-RELAY DOSE + 8x GAIN + PROBE   ALL THREE SYMPTOMS  (base V208)")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V213 output")
    dec = bytearray(base)
    ...
    tag = "V213-V208BASE-C63AE.512-GAIN8X-PROBE.GP6B4E"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v213_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V213_WRITE=rwd to emit the files")
