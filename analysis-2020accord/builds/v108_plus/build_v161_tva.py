#!/usr/bin/env python3
r"""
V161 -- SINGLE-VARIABLE TWIN OF V160.  Base = V122 (NO damper).  PUSH THE KIT'S ONLY MEASURED "BOTH SYMPTOMS AT ONCE" LEVER TO ITS INT16 CEILING.
...
[NOTE] V161 is V160 WITHOUT the damper.  Both add damping in the creep band by INDEPENDENT mechanisms
...
WRITE_MODE = os.environ.get("ACCORD_V161_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V161 -- V122 + Lever B 5244 -> 6553.  ONE halfword.  The SINGLE-VARIABLE discriminator.")
    print("=" * 102)
    ...
    check(u16(base, LB_CAL) == LB_OLD,
          f"  0x{LB_CAL:05X} Lever B = {LB_OLD} in the V122 base -- the value V88 FLEW")
    ...
    tag = "V161-V122BASE-LEVERB.5244.TO.6553"
    img_out = plain_image_path(f"_v161_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V161_WRITE=rwd to emit the files")
