...
WRITE_MODE = os.environ.get("ACCORD_V147_WRITE", "").strip().lower()
...
SAR_NEW = 0xA1                               # sar 3 -> 1; see the blindness check
...
def build():
    ...
    check(_w(1, SAR_HELD) == _w(0, SAR_HELD),
          f"  THE BLINDNESS CHECK: at sar {SAR_HELD & 0x1F} BOTH gate values map to wire"
          f" {_w(1, SAR_HELD)} -- the probe would have been BLIND, which is why sar moves")
    check(_w(1, SAR_NEW) != _w(0, SAR_NEW),
          f"  SEPARATION: at sar {SAR_NEW & 0x1F} shut -> wire {_w(1, SAR_NEW)}, open -> wire"
          f" {_w(0, SAR_NEW)}; a digital field, so the OPEN DUTY is exactly countable")
    ...
    check(payload == 4, f"exactly 4 payload bytes ({payload} found) -- 1 functional + 3 telemetry")
    ...
    FF.assert_x31_checksum(rwd, "V147 output")
    dec = bytearray(base)
    ...
    tag = "V147-V122BASE-DEADBAND.96-427.GATE.6C24"
    img_out = plain_image_path(f"_v147_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V147_WRITE=rwd to emit the files")
