...
WRITE_MODE = os.environ.get("ACCORD_V217_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V217 -- V216 + THE INERTIA LANE WEIGHT: THE DAMPER FIX, END TO END  (base V208)")
    print("=" * 102)
    ...
    tag = "V217-V216BASE-INERTIA.LANE.WEIGHT.TO.FLOWN.V108"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v217_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V217_WRITE=rwd to emit the files")
