#!/usr/bin/env python3
r"""
V129 -- RESTORE Y[2] TO V106's MEASURED-GOOD VALUE.  -16000 -> -5898.  Base = V127.

WHY V128 WAS WRONG, AND WAS RETRACTED BEFORE FLIGHT
----------------------------------------------------
V128 restored the whole engaged-mode Y row to STOCK on the argument that our x3 raise railed the
term into a Coulomb relay.  The rail arithmetic was right; the CONCLUSION was wrong.

accord-v106-extinguished-the-mode-at-low-speed: V106's x3 raise is the kit's STRONGEST measured
anti-grind result, and the ONLY band-power result ever to clear its own within-drive split-half
null (18-30 a6/V105 = 0.347 vs null [0.482, 1.982]; positive control a6/STOCK = 5.735):
...
A Coulomb relay is a STRONG nonlinear damper, so "railed" and "extinguished the mode" are both
true -- a bang-bang term kills a resonant line while adding broadband roughness.  Restoring to
stock would have undone the best measured fix in this kit's record.  V128's artifacts are DELETED.

WHAT IS ACTUALLY WRONG WITH THE BUILD ON THE CAR
--------------------------------------------------
...
V112 kept V106's Y[1] but inherited V107's Y[2] = -16000, which is 2.7x V106's -5898.  So the
build on the car is NOT the configuration that measured the extinction: it carries V107's
HIGH-SPEED escalation, and V107 is precisely the build whose own session matched its rail-duty
map to the symptom map ("the symptom map and the rail-duty map are the same map").

V106's own residual was a HIGH-SPEED phenomenon -- prominence hwy 40-95 6.5 vs stock 1.3, carried
by the >70 km/h portion, "exactly where Honda's speed taper makes the dose 4.2x weaker".  V107
answered that by raising Y[2] 2.7x.  That is the change this build undoes.
...
4 payload bytes.  The result is EXACTLY V106's Y row -- the configuration that measured the
extinguished mode -- so this is a return to a MEASURED-GOOD state, not a new dose.

WHAT IT DOES AND DOES NOT CHANGE
---------------------------------
  * Rail threshold at 90 km/h: 1963 -> 5325, a 2.7x de-rail where the residual lives.
  * Y[0] and Y[1] are UNTOUCHED, so the low- and mid-speed behaviour that measured the
    extinction is bit-identical to the current build.
  * MANUAL modes 24/25 stay byte-stock, as on every build.
  * It does NOT confound V127's 0xC640A edit: that fires only when the reversal counter is
    SATURATED, this record only when it is NOT.  Mutually exclusive by construction.

EVIDENCE vs BELIEF
------------------
[EVIDENCE] every Y value and rail threshold above, read from the images; V106's flight result and
           its split-half null; V112's inheritance of V107's Y[2].
[BELIEF]   that -5898 is better than -16000 on the current build.  V106 flew with alpha2 = 22 and
           a 6x forward gain; the car now runs alpha2 = 5 and 8x, so |gp-0x6c2c| and the command
           are both different.  This returns the ONE cell that was escalated past the measured
           configuration; it does not re-verify V106's whole result under the new conditions.
"""
...
WRITE_MODE = os.environ.get("ACCORD_V129_WRITE", "").strip().lower()
...
Y_OURS,  Y_V106 = (-29490, -17202, -16000), (-29490, -17202, -5898)   # THE EDIT: Y[2] only
...
def build():
    print("=" * 102)
    print("  V129 -- V127 + engaged-mode Y[2] -16000 -> -5898 (V106's MEASURED-GOOD value).")
    print("=" * 102)
    ...
    attributed |= {SAR_ADDR}
    for _m, _a in REC_Y.items():                            # THE EDIT -- Y[2] only
        struct.pack_into("<h", code, _a + 4, Y_V106[2])
        attributed |= {_a + 4, _a + 5}
    attributed |= {K1_CAL, K1_CAL + 1}
    ...
    check(_r(Y_V106[2]) > _r(Y_OURS[2]),
          f"  Y[2] de-rails {_r(Y_OURS[2])} -> {_r(Y_V106[2])}"
          f" ({_r(Y_V106[2])/_r(Y_OURS[2]):.1f}x) where V106's residual lived")
    check(_r(Y_V106[0]) == _r(Y_OURS[0]) and _r(Y_V106[1]) == _r(Y_OURS[1]),
          "  low/mid-speed rail thresholds UNCHANGED -- the measured extinction is preserved")
    ...
    for _m, _a in REC_Y.items():
        check(tuple(s16(code, _a + 2 * i) for i in range(3)) == Y_V106,
              f"  mode {_m} Y -> {list(Y_V106)} = EXACTLY V106's row, a MEASURED-GOOD state")
        check(s16(code, _a) == s16(base, _a) and s16(code, _a + 2) == s16(base, _a + 2),
              f"  mode {_m} Y[0]/Y[1] UNTOUCHED -- only V107's high-speed escalation is undone")
    ...
    tag = "V129-V127BASE-MODE26.Y2.-5898"
    img_out = plain_image_path(f"_v129_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V129_WRITE=rwd to emit the files")
