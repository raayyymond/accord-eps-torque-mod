#!/usr/bin/env python3
r"""V228 -- V222 WITH THE FORWARD GAIN LEFT AT THE CAR 6x.
ONE difference from V222: 0xC6CD0 stays 5346 and the clamps stay 3072. The 20.50 Hz notch,
Lever B 13107, the friction lane matched to the car, 0xC63AE 512 and the 427 probe are all
byte-identical to V222.  NOT V212, which carries the car Lever B 5244.

V222 -- V221 WITH THE FRICTION LANE FULLY RESTORED TO THE CAR.  0xC40BC 600->3000, 0xC40D2 204->1020.

WHAT V216 GOT HALF-RIGHT
------------------------
V216's stated job was restoring the friction lane to the car. It restored the SLOPE and not the
SATURATION, and nobody noticed because every check that looked at this lane looked at the slope.
...
FUN_0003b8f6 is the PLANT-MODEL OBSERVER. Mirrored from the decompile, address by address:
    ...
    friction  = EMA(|model| * ratio * cal(0xC40D2)/1024 + ratio * cal(0xC4080)/1024)  0x3BB02..
...
The UNSATURATED slope in gp-0x6abc is 12*k1/gate/1024, and V216 matched it exactly:

    car   (V122)        gate 3000  k1 1020   slope 0.003984
    shelf (V216..V221)  gate  600  k1  204   slope 0.003984      <- identical

But what each SATURATES AT is k1/1024 of the whole model, and there V216 is 5x off:
    ...
    |gp-0x6abc|      CAR model_out     SHELF model_out      shelf/car
           ...
           250               7.0              1512.0          216.0x   <- car saturated
           800               7.0              1512.0          216.0x

The car models friction at 1020/1024 = 99.6 % of the whole model once saturated, so ITS OBSERVER
RESIDUAL IS ANNIHILATED above 250 counts of rate -- that lane goes silent on fast steering. The
shelf leaves 80.1 % standing, so the lane stays fully live at every rate.

=> [EVIDENCE] V217..V221 carry an uncharacterised ~216x behavioural difference from the car on FAST
   STEERING, in 4 bytes the drive card described only as "less friction => heavier at high steering
   rate". THE RATCHET REGIME IS UNAFFECTED -- 1.0x at every rate the ratchet occupies -- so this
   changes no ratchet expectation. It is a high-rate claim only.
...
the slope match shows that was the intent; the saturation was simply missed. Restoring BOTH cells
makes the lane byte-identical to what the operator already drives at EVERY rate, and removes the
only uncharacterised delta from the flight candidate. Every deliberate lever is untouched: the
20.50 Hz notch, 0xC63AE at 512, the 8x forward gain, Lever B at 13107 and the 427 probe all carry
over from V221 byte for byte.

WHAT THIS IS NOT
----------------
[NOTE] This is a DE-RISKING build, not a new lever. It should feel like V221 everywhere except fast
steering, where it should feel like the car.
[BELIEF] that the shelf's high-rate behaviour is worse rather than better. It is UNMEASURED in that
regime -- 216x is simply too large to carry unexamined into a drive whose purpose is something else.
If a V221 drive reports nothing odd on quick inputs, this build is unnecessary and V221 stands.
[NOTE] 0xC40D2 is ALSO a lever in its own right, and a clean one: 1 reader (0x3BAFE), 0 writers, and
a REAL structural boundary at k1 = 1024 where the modelled friction equals the whole model, the
residual is identically zero and beyond which the sign INVERTS. That is checked against the golden
model, not asserted from an overflow argument. Sizing it is separate work and is NOT done here.

THE CONFOUND THIS WORK TURNED UP
--------------------------------
The record's headline for this cell -- "de-relaying the Coulomb friction made the ratchet band 2.3x
WORSE", from 0xC40BC 600 vs 6000 across 30 routes -- is NOT IDENTIFIED. Those builds held k1 at 102
while the gate moved 10x, so the SMALL-SIGNAL SLOPE fell 10x at the same time:

    V84/V87/V88  gate  600  k1 102   slope 0.001992
    V85/V86/V86B gate 6000  k1 102   slope 0.000199      <- 10x LESS slope, not just wider

So "de-relaying made it worse" and "10x less modelled friction made it worse" fit that data equally
well, and they are different levers. The second one is separable via 0xC40D2 alone and has never
been tested in isolation.

EVERYTHING ELSE IS V221, BYTE FOR BYTE.
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V228_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V222 -- V221 + THE FRICTION LANE FULLY RESTORED TO THE CAR (216x at high rate)")
    print("=" * 102)
    ...
    # V228 DIFFERS FROM V222 HERE, AND ONLY HERE: the forward gain STAYS at the car 6x.
    GAIN_EDITS = ()
    check(u16(code, 0xC6CD0) == 5346, "0xC6CD0 = 5346 (6x) -- the CAR forward gain, NOT raised")
    check(u16(code, 0xC61B2) == 3072, "0xC61B2 = 3072 -- clamp A left at the car value")
    check(u16(code, 0xC61B4) == 3072, "0xC61B4 = 3072 -- clamp B left at the car value")
    ...
    tag = "V228-V222BASE-GAIN.STAYS.6X.AS.CAR"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v228_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V228_WRITE=rwd to emit the files")
    ...
    print("  ** V208 notch + 0xC63AE 512 + LEVER B 13107 + the 427 probe. NO GAIN STEP.  **")
    print("  ** notch -> 18-22 Hz grinding | Lever B -> 6-9 Hz | 0xC63AE -> ~7.8 Hz      **")
    print("  ** V228 DECLINES the 8x gain that V217-V227 all carry. The notch covers     **")
    print("  ** 22-26 Hz (net 0.463x vs the car) but spans only 15.5-29.8 Hz; at the     **")
    print("  ** RATCHET it does NOTHING (0.997), so 8x would add 1.33-1.65x excitation   **")
    print("  ** at 6-9 Hz against Lever B 2.50x -- same order, no open-loop answer.      **")
    print("  ** The operator flew 8x as V101 and reported vibration at ALL SPEEDS; V102  **")
    print("  ** reverted to 6x at his own choosing. V228 keeps only evidenced levers.    **")
    print("  ** COST, PLAINLY: LKAS authority stays at the car 6x, NOT 8x. For the       **")
    print("  ** authority he must fly V222 and accept the ratchet race.                  **")
    print("  ** V228 is the CONSERVATIVE HALF of V222, not a replacement for it.         **")
    print("  ** NOT V212: that is this base with the CAR Lever B (5244). V228 is the     **")
    print("  ** first build to pair a RAISED Lever B (13107) with 6x.                    **")
