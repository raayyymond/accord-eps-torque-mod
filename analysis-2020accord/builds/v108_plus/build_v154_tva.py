#!/usr/bin/env python3
r"""
V154 -- HALVE THE INERTIA LANE'S WEIGHT IN THE OBSERVER SUM.  0xC63A6 : 1024 -> 512.  Base = V122.
...
WRITE_MODE = os.environ.get("ACCORD_V154_WRITE", "").strip().lower()
...
IN_CAL, IN_OLD, IN_NEW = 0xC63A6, 1024, 512     # w(gp-0x6b26), the INERTIA lane -- THE EDIT
...
def build():
    ...
    check(payload == 1, f"exactly 1 payload byte ({payload} found) -- only the HIGH byte of the lane-weight u16 moves (0x0400 -> 0x0200)")
    ...
    FF.assert_x31_checksum(rwd, "V154 output")
    dec = bytearray(base)
    ...
    tag = "V154-V122BASE-INERTIALANE.1024.TO.512"
    img_out = plain_image_path(f"_v154_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V154_WRITE=rwd to emit the files")
