...
WRITE_MODE = os.environ.get("ACCORD_V270_WRITE", "").strip().lower()
...
TAG = "V270-V112BASE-LKAS.PID.KI.5"
...
def build():
    print("=" * 102)
    print("  V270 -- ENABLE THE LKAS-PATH INTEGRAL TERM.  ONE CAL HALFWORD.")
    print("=" * 102)
    ...
    FF.assert_x31_checksum(rwd, "V270 output")
    dec = bytearray(base)
    ...
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v270_{TAG}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{TAG}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [9] NOT WRITTEN -- set ACCORD_V270_WRITE=rwd to emit the files")
