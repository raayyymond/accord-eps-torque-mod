#!/usr/bin/env python3
r"""
V206 -- HALVE THE SOFT RELAY'S SMALL-SIGNAL GAIN.  Base = V202.  ONE u16 cal.
...
sized rather than guessed.  V206 exists so that if V205 says the range is large, the fix is already
cut.
...
WHAT IS CARRIED
---------------
Everything in V202, bit for bit: the 19.75 Hz notch with poles at 15.25 Hz, the engaged inertia
half-dose, the K1 and accel-alpha reverts, w[3] halved, 0xC407E frozen at 511, the 164-byte cave
byte-identical, and the 427 probe still on gp-0x6ac0.  ** V206 is a lever, not an instrument. **
"""
import hashlib
import math
import os
...
WRITE_MODE = os.environ.get("ACCORD_V206_WRITE", "").strip().lower()
...
def build():
    ...
    check(u16(code, PROBE_HW2) == u16(base, PROBE_HW2)
          ...
          "the 427 probe is UNTOUCHED -- V206 is a lever, not an instrument")
    m194, _ = resp(code, F0)
    ...
    tag = "V206-V202BASE-C63AE.1024.TO.512"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v206_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V206_WRITE=rwd to emit the files")
