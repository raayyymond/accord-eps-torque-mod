#!/usr/bin/env python3
r"""
V187 -- MOVE HONDA'S NOTCH ONTO THE RATCHET.  Base = V185.  Four float32 cells.

WHAT IS NEW AS A CLASS
----------------------
Every filter build in the arc (V43 pole, V173/V174/V184 poles) moved the DENOMINATOR -- the poles --
which makes a LOW-PASS.  V187 moves the NUMERATOR.  That has never been done.
...
ratchet attenuation with phase in openpilot's band:
    ...
    V187 (notch on the ratchet)         -6.0 dB        -9.9 deg       0.61   <-- 2.8x better
...
WHERE THE NOTCH GOES -- MINIMAX OVER ROUTES, NOT THE POOLED AVERAGE
-------------------------------------------------------------------
V186 centred the notch on the pooled corpus (centroid 8.125 Hz, peak 8.008 Hz).  That is the wrong
criterion for a line that MOVES.  Per-route peaks across 67 routes with >=4 engaged-creep windows:

    p10 7.34 Hz   median 7.81 Hz   p90 8.59 Hz

so tuning to the average leaves a shoulder on every route that sits off it -- on r24 (peak 8.59 Hz)
V186's residual peak moved to 9.96 Hz instead of vanishing.  V187 instead chooses (f0, radius) to
minimise the WORST-CASE (p90) remaining 5-12 Hz power across routes: a WIDER, shallower notch that
is robust to where the line actually lands.
...
It wins on the median as well as the p90, so this is not an artifact of the robust criterion.
...
WRITE_MODE = os.environ.get("ACCORD_V187_WRITE", "").strip().lower()
...
SEC_FS = 1000.0
F0 = 8.80          # notch centre, Hz -- MINIMAX over 67 routes, not the pooled average
RP = 0.9795        # pole radius     -- wider than V186 on purpose: the line WANDERS
...
def build():
    print("=" * 102)
    print("  V187 -- NOTCH ON THE RATCHET, FITTED MINIMAX OVER ROUTES   (base V185)")
    print("=" * 102)
    ...
    mags = [(x, resp(code, x)[0]) for x in [7.0 + 0.02 * k for k in range(151)]]
    ...
    print("      response across the ratchet's measured wander:")
    for x in (5.0, 6.0, 7.0, 7.5, 8.0, 8.3, 8.6, 9.0, 10.0, 12.0):
        m, p = resp(code, x)
        mb, _ = resp(base, x)
    ...
    FF.assert_x31_checksum(rwd, "V187 output")
    dec = bytearray(base)
    ...
    tag = "V187-V185BASE-NOTCH.MINIMAX.OVER.ROUTES"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v187_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V187_WRITE=rwd to emit the files")

    print("\n" + "=" * 102)
    print(f"  image SHA256 {img_sha}")
    print(f"  .rwd  SHA256 {rwd_sha}")
    print(f"  {_checks[1]}/{_checks[0]} assertions passed")
    print("  ** 5-12 Hz power: median 0.1661 = 6.0x, p90 0.2584 = 3.9x, for -9.95 deg @3 Hz. **")
