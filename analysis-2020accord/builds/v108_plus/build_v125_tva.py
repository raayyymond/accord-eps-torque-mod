...
WRITE_MODE = os.environ.get("ACCORD_V125_WRITE", "").strip().lower()
...
TAP_DISP_ADDR, TAP_OLD, TAP_NEW = 0x55DF2, (-0x6ABC) & 0xFFFF, (-0x6AF0) & 0xFFFF
SAR_ADDR, SAR_OLD, SAR_NEW = 0x55E10, 0xA3, 0xA4   # sar 3 -> sar 4, sized to +-3072
...
def build():
    print("=" * 102)
    print("  V125 -- V124 + 427 REPOINT to gp-0x6AF0 (reader #3 output), packer sar 3->4.")
    print("=" * 102)
    ...
    check(code[SAR_ADDR] == SAR_NEW and base[SAR_ADDR] == SAR_OLD,
          "  0x55E10 packer sar 3 -> 4 (THIS BUILD)")
    ...
    tag = "V125-V124BASE-427.6AF0.SAR4"
    img_out = plain_image_path(f"_v125_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V125_WRITE=rwd to emit the files")
