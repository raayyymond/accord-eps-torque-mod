#!/usr/bin/env python3
r"""
V200 -- PROBE THE BIGGEST 8 Hz EXCITER.  Base = V196.  3 bytes, telemetry only.
...
=> the rate lanes carry ** 4x the viscous term's clamp and 8x the inertia term's **, and they are the
   ...
   than measuring the second-largest.  ** V200 supersedes V197. **
...
WHAT CHANGED FROM V198
----------------------
V198 was this same probe on a V196 base.  V196 carries a notch whose POLES SIT AT THE ZEROS, which
BUILD-LINEAGE.md names as a trap in its V105 section, and which scores max|H| = 1.7177 over 0-500 Hz
against the lineage bar of stock 1.0000.  It ADDS 72 percent loop gain above 30 Hz.  V199 rebuilt
that notch with the poles BELOW the zeros -- Honda own layout -- landing max|H| = 1.000000, so the
filter can only remove loop gain, never add it.  V200 is that corrected base plus the same probe.
V198 is SUPERSEDED and renamed on disk.
...
WRITE_MODE = os.environ.get("ACCORD_V200_WRITE", "").strip().lower()
...
CARRIED_U16 = {0xC40D2: ("K1 -> Honda (V177)", 102),
               ...
               0x55DF2: ("427 probe source gp-0x6ada, the r24 lane (V200)", 0x9526)}
CARRIED_B = {0xC40DC: ("accel alpha -> Honda (V179)", 22),
             0x55E10: ("packer sar 5 (V200)", 0xA5)}
PTR_I = 0xCBE74
...
def build():
    ...
    tag = "V200-V199BASE-PROBE-THE-R24-RATE-LANE"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v200_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V200_WRITE=rwd to emit the files")
