#!/usr/bin/env python3
r"""
V141 -- V140 (the pump deadband) PLUS a 427 probe on gp-0x6ADA, the lane it acts on.
        ONE functional byte + TWO telemetry bytes.  Base = V122.

*** THIS IS V140 MADE SELF-SCORING.  Prefer it over V140: same fix, and one drive also returns
    the measurement that turns the DOSE from a belief into a number. ***

WHAT V140 DOES, UNCHANGED
---------------------------
cal 0xC61F6, the r24 PUMP-LANE DEADBAND, 3 -> 96.  Honda ships 3 counts = 0.037 % of the +-8192
lane clamp, a quantization floor rather than a functional dead zone, so every micro-oscillation
passes straight into a lane whose polarity (gp-0x6752 = -1) makes it a PUMP.  The deadband
SUBTRACTS rather than clips, so the curve is continuous (0 -> 0 -> 1 -> 2) and there is no
notchiness mechanism.  Full rationale in build_v140_tva.py.

WHAT THIS BUILD ADDS -- AND WHY IT COSTS NOTHING
--------------------------------------------------
    0x55DF2   427 tap displacement   gp-0x6ABC -> gp-0x6ADA      (2 bytes, TELEMETRY ONLY)
The aggregator ENDS with:
    *(short *)(gp - 0x6adc) = (short)iVar21;   // r26 lane mirror
    *(short *)(gp - 0x6ada) = (short)iVar16;   // r24 lane mirror   <- THIS BUILD PROBES THIS
gp-0x6ADA is the r24 lane AFTER the deadband, the polarity and the +-0x2000 clamp, and the kit has
established that NOTHING READS THESE MIRRORS -- they are free, zero-blast-radius telemetry.
The 427 message is TX-only and the kit has established that neither steeringTorqueEps nor raw 427
MOTOR_TORQUE is a delivered-torque or cut anchor, so repointing it cannot affect control.
=> FUNCTIONALLY this build is ONE byte, identical to V140.  The other two bytes only change what a
   CAN message reports.  That distinction is asserted explicitly below.

WHY THIS MEASUREMENT IS THE ONE WORTH TAKING
----------------------------------------------
V140's honest weakness is its DOSE: 96 was centred on an ESTIMATE that the grind is a 1-3 % of
full-scale lane oscillation (50-150 counts).  The lane amplitude during a grind episode has NEVER
been measured by this kit.  Probing gp-0x6ADA discriminates the three outcomes that matter:

    grinding GONE                          -> the deadband worked; the dose was right
    grinding PERSISTS, gp-0x6ADA ~ 0       -> the r24 lane is NOT the grind source.  Stop raising
                                              this deadband; the next lever is elsewhere entirely.
    grinding PERSISTS, gp-0x6ADA LARGE     -> the lane IS active through the deadband; raise to 192

Without the probe, the middle and bottom cases are indistinguishable and the kit would keep
stepping a deadband on a lane that has nothing to do with the symptom -- which is exactly the
failure mode that wasted builds on the base-assist damper family earlier in this session.

THE PROBE SCALING
-----------------
The packer is wire = min((|x| * 5) >> sar, 0x3FF), and V122 already carries sar 3, so this build
does NOT touch 0x55E10.
    lane  96 (the new deadband)  -> wire  60 of 1023    resolution 1.6 lane counts per wire LSB
    lane 192 (the next rung)     -> wire 120
    wire saturates at |lane| >= 1637, i.e. the top 80 % of the +-8192 lane range CLIPS.
That is the correct trade here: the question is about SMALL lane values near the deadband, and a
clipped reading still carries the unambiguous message "large".

BASE = V122, THE LAST FLOWN KNOWN-GOOD BUILD.  alpha2 stays 8, gain stays 6x, b26 clamp stays 511,
...
WRITE_MODE = os.environ.get("ACCORD_V141_WRITE", "").strip().lower()
...
def build():
    print("=" * 102)
    print("  V141 -- V140 (deadband 3 -> 96) + a 427 probe on gp-0x6ADA, the lane it acts on.")
    print("=" * 102)
    ...
    print(f"      0x{TAP_ADDR:05X}  427 tap  gp-0x6ABC -> gp-0x6ADA   (0x{TAP_OLD:04X} ->"
          f" 0x{TAP_NEW:04X})")
    ...
    check(payload == 2, f"exactly 2 payload bytes ({payload} found) -- 1 functional + 1 tap")
    ...
    FF.assert_x31_checksum(rwd, "V141 output")
    dec = bytearray(base)
    ...
    tag = "V141-V122BASE-DEADBAND.96-427.6ADA"
    img_out = plain_image_path(f"_v141_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V141_WRITE=rwd to emit the files")
