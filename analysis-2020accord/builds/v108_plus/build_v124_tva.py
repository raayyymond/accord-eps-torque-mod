...
WRITE_MODE = os.environ.get("ACCORD_V124_WRITE", "").strip().lower()
...
TRIM_CAL, TRIM_OLD, TRIM_NEW = 0xC63D2, 6, 3
...
def build():
    print("=" * 102)
    print("  V124 -- V122 + GAIN 8x + CLAMPS 4096 + ALPHA2 5 + TRIM IIR 0xC63D2 6->3.")
    print("=" * 102)
    ...
    check(u16(base, GAIN_CAL) == GAIN_6X, f"  base 0x{GAIN_CAL:05X} = {GAIN_6X} (6x)")
    ...
    for _c in (CLAMP_A, CLAMP_B):
        ...
        attributed |= {_c, _c + 1}
    struct.pack_into("<H", code, TRIM_CAL, TRIM_NEW)
    attributed |= {TRIM_CAL, TRIM_CAL + 1}
    attributed |= {K1_CAL, K1_CAL + 1}
    ...
    exempt = {GAIN_CAL, GAIN_CAL + 1, ALPHA2_CAL, ALPHA2_CAL + 1,
              CLAMP_A, CLAMP_A + 1, CLAMP_B, CLAMP_B + 1, TRIM_CAL, TRIM_CAL + 1}
    ...
    check(TRIM_NEW < TRIM_OLD,
          "  the trim moves DOWN -- reducing a positive-feedback magnitude is safe by construction")
    ...
    FF.assert_x31_checksum(rwd, "V124 output")
    dec = bytearray(base)
    ...
    tag = "V124-V122BASE-GAIN8X-CLAMPS4096-ALPHA2.5-TRIM.C63D2.3"
    img_out = plain_image_path(f"_v124_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V124_WRITE=rwd to emit the files")
