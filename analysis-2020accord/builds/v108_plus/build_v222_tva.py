...
WRITE_MODE = os.environ.get("ACCORD_V222_WRITE", "").strip().lower()
...
def build():
    ...
    tag = "V222-V221BASE-FRICTION.LANE.SATURATION.TO.CAR"
    if WRITE_MODE == "rwd":
        Path(plain_image_path(f"_v222_{tag}_plain_image.bin")).write_bytes(bytes(code))
        Path(RWD_DIR, f"39990-TVA,A160-{tag}-0x{START:X}-0x{END:X}.rwd").write_bytes(rwd)
        print("\n      WROTE image + rwd")
    else:
        print("\n  [14] NOT WRITTEN -- set ACCORD_V222_WRITE=rwd to emit the files")
