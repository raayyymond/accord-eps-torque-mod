#!/usr/bin/env python3
r"""
V148 -- V147's pump deadband, with the probe REPOINTED at V88's LEVER-B GATE.  Base = V122.
        ONE functional byte + ONE tap byte.

WHY THE PROBE MOVED OFF THE NOTCH GATE
----------------------------------------
V147 probes gp-0x6C24, the notch gate mirror.  That question is now SETTLED without a drive:
  * r21 (V108, sar 5, effectively unclipped) puts >= 99.9 % of engaged frames below |gp-0x6c2c|
    6547 against a detector threshold of 12800;
  * and the notch lane gp-0x6B86 measures 0.06-0.27x the FLAT baseline -- the lowest 18-22 Hz
    content of any lane probed.
=> the notch family is closed on its own evidence, and V147's probe would spend a drive confirming
   something already established.  The tap is worth more elsewhere.

WHAT IT PROBES INSTEAD -- AND WHY IT MATTERS RIGHT NOW
--------------------------------------------------------
The r24 multiplier is selected by a gate read at 0x3AB98:
        ld.bu -0x671d, gp, r6        a437e398
        cmp   r0, r6                 e031        -- tested against ZERO
    gp-0x671d == 0  ->  0xC6440 (2048) or 0xC6446 (5244)   depending on gp-0x683c
    gp-0x671d != 0  ->  0xC6442 (1024)
0xC6446 is V88's "Lever B", set 512 -> 5244 and recorded as the best anti-grind lever in the kit.
IT IS ON THE CAR RIGHT NOW via V122.
=> if gp-0x671d is normally NON-ZERO, the r24 multiplier is a flat 1024 and BOTH 0xC6440 and
   Lever B are INERT -- including on the build the operator is driving.
=> that has NEVER been measured.  It is the same failure mode that made the notch inert, and the
   third lever this session found sitting behind an unmeasured gate.

THE ODD-DISPLACEMENT PROBLEM, AND THE FIX
-------------------------------------------
gp-0x671d is at displacement 0x98E3, which is ODD.  The 427 tap is an `ld.h` and requires an EVEN
displacement, so gp-0x671d cannot be tapped directly.  Tapping the EVEN gp-0x671E instead reads the
halfword spanning both bytes, and on a little-endian target that puts:
        gp-0x671E in bits 7:0        gp-0x671D in bits 15:8
    => tapped value x = (gp-0x671d << 8) | gp-0x671e
    => at sar 3 (V122's existing value, UNCHANGED):  wire = min((|x| * 5) >> 3, 1023)
           gp-0x671d == 0  ->  x in 0..255     ->  wire   0..159
           gp-0x671d == 1  ->  x in 256..511   ->  wire 160..319
       CLEANLY SEPARABLE, with gp-0x671e contributing only the low bits.
The packer sar is NOT touched, so this build is ONE tap byte lighter than V147.

WHAT THE DRIVE THEN ANSWERS
-----------------------------
    wire mostly >= 160  ->  gp-0x671d NON-ZERO  ->  r24 multiplier is a flat 1024
                            => V88's LEVER B IS INERT, and has been on every build since V88.
                               The kit's "best anti-grind lever" would be a mechanism that never ran.
    wire mostly <  160  ->  gp-0x671d ZERO      ->  Lever B (or 0xC6440) IS in force; the mechanism
                               credited to V88 is real.
Either answer is worth a drive, and the deadband rides along as the live lever regardless.

BASE = V122.  alpha2 8, gain 6x, b26 clamp 511, both Lever A arms stock, biquad at Honda's values.
"""
...
WRITE_MODE = os.environ.get("ACCORD_V148_WRITE", "").strip().lower()
...
TAP_NEW = (-0x671E) & 0xFFFF                 # -> gp-0x671E; its HIGH BYTE is gp-0x671D
SAR_NEW = 0xA3                               # sar 3 HELD -- gp-0x671D lands in bits 15:8
...
def build():
    ...
    print(f"      0x{TAP_ADDR:05X}  427 tap  gp-0x6ABC -> gp-0x671E   (0x{TAP_OLD:04X} ->"
          f" 0x{TAP_NEW:04X})")
    ...
    check(base[SAR_ADDR] == SAR_HELD,
          f"  0x{SAR_ADDR:05X} packer sar = 0x{SAR_HELD:02X} in the base")
    ...
    check((SAR_HELD & ~0x1F) == (SAR_NEW & ~0x1F),
          f"  sar {SAR_HELD & 0x1F} -> {SAR_NEW & 0x1F}: OPCODE field untouched, only the"
          f" shift immediate moves")
    check(_w(1, SAR_NEW) == _w(1, SAR_HELD) and SAR_NEW == SAR_HELD,
          f"  the packer sar is HELD at 0x{SAR_HELD:02X} (sar {SAR_HELD & 0x1F}) -- this build is"
          f" ONE tap byte lighter than V147, which had to move it")
    _hi = lambda d671d, d671e: _w((d671d << 8) | d671e, SAR_HELD)
    check(_hi(0, 255) < _hi(1, 0),
          f"  🛑 THE SEPARATION GATE: gp-0x671d == 0 gives wire <= {_hi(0, 255)} (any"
          f" gp-0x671e), and gp-0x671d == 1 gives wire >= {_hi(1, 0)} -- CLEANLY SEPARABLE at"
          f" wire {_hi(1, 0)}, with gp-0x671e contributing only the low bits")
    check(_hi(1, 0) <= 0x3FF and _hi(3, 255) <= 0x3FF,
          f"  and the reading does not clip for gp-0x671d up to 3 (wire {_hi(3, 255)} of 1023)")
    check(TAP_NEW % 2 == 0,
          "  🛑 gp-0x671D is at an ODD displacement (0x98E3) which ld.h CANNOT take;"
          " tapping the EVEN gp-0x671E puts gp-0x671D in the HIGH byte instead")
    ...
    exempt = {DB_CAL, DB_CAL + 1, TAP_ADDR, TAP_ADDR + 1, SAR_ADDR}
    ...
    _functional = sum(hi - lo for lo, hi in runs
                      if not any(lo < t + 4 and t < hi for t in _tr)
                      and not (TAP_ADDR <= lo < TAP_ADDR + 2) and lo != SAR_ADDR)
    check(_functional == 1,
    ...
    FF.assert_x31_checksum(rwd, "V148 output")
    dec = bytearray(base)
    ...
    tag = "V148-V122BASE-DEADBAND.96-427.LEVERB.671E"
    img_out = plain_image_path(f"_v148_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V148_WRITE=rwd to emit the files")
