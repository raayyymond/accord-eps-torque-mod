...
V133 restores V62's Lever A, which MEASURED 42x on this exact symptom.  V135 is a second,
independent follow-up alongside V134; flying either before V133 would confound that test.

WHY THIS RUNG EXISTS AT ALL -- A CLOSURE THAT WAS TOO EARLY
-------------------------------------------------------------
This session recorded "the knee/K1 ladder is EXHAUSTED at V122/V124" because the GAIN-HOLDING
invariant (K1/1024)*(12/knee) = 0.0039844 needs K1 = 1122 at knee 3300, above the 1023 ceiling.
That is true -- and it only closes GAIN-HOLDING steps.  Raising the knee with K1 HELD is a
different move, and the exhausted-ladder note did not cover it.

WHY THE KNEE AND NOT alpha2 -- the reattribution that motivates this build
---------------------------------------------------------------------------
GATE 2 on gp-0x6b26 at the creep band shows the alpha2 ladder is nearly INERT at 20 Hz: across
alpha2 22 -> 5 the lane magnitude falls 7.24 -> 4.10 (1.77x) while its phase rotates 56.3 -> 16.0
deg, leaving the delivered component FLAT (-4.01 vs -3.94).  That is a RATIO between alpha2
settings, so it survives any constant sign/phase offset.
=> V122's reported improvement ("better, rare moments") came from the KNEE/K1, not alpha2.
=> the Coulomb relay is the live creep lever, and this is its last measured rung.

THE EDIT, AND IT IS ON THE MEASURED LADDER
--------------------------------------------
    0xC40BC   3000 -> 3600      K1 (0xC40D2) HELD at 1020

    MEASURED relay saturation duty, engaged HANDS-OFF, 5-10 mph, cmd >= 2048:
        knee  600 -> 0.7439     knee 1800 -> 0.2353     knee 3600 -> 0.0000   <- THIS BUILD
        knee 1200 -> 0.4810     knee 2400 -> 0.0484

3600 is a MEASURED point reading 0.0000, not an interpolation -- and the ladder was measured in
ENGAGED HANDS-OFF CREEP, which is precisely the regime of the operator's remaining symptom
("its just a rare low speed grinding #1").

WHAT IT COSTS, AND WHY THE OPERATOR WANTS THAT COST
-----------------------------------------------------
    small-signal friction slope   0.003984 -> 0.003320  = x0.83   (17 % LESS)
    relay saturation point        53.1 -> 63.7 deg/s
His standing instruction: "Increasing mass and friction should not be our primary approach ...
We want both: low apparent steering mass and friction to LKAS AND no ratcheting."
=> this rung moves BOTH the right way: less friction AND less relay.  It is the only lever in
   this kit's record that does.

K1 IS UNTOUCHED AND STAYS UNDER ITS CEILING
---------------------------------------------
K1 = 1020 with a hard ceiling of 1023: at K1 >= 1024 friction would exceed |model| and the
residual would invert sign.  This build does not move K1, so that bound is not approached.
The builder asserts K1 == 1020 and knee/K1 monotonicity.

EVIDENCE vs BELIEF
------------------
[EVIDENCE] the measured duty ladder including the 3600 = 0.0000 point; the arithmetic of slope
           and saturation; K1 unchanged; and the convention-independent alpha2-inertness ratio
           that motivates targeting the knee rather than alpha2.
[BELIEF]   that lower relay saturation reduces the operator's rare creep grind.  The duty ladder
           is a MECHANISM measurement, not a symptom measurement -- the link from saturation duty
           to what he hears rests on his own dose-response reports across V111/V112/V122, not on
           an instrumented symptom endpoint.
"""
...
WRITE_MODE = os.environ.get("ACCORD_V135_WRITE", "").strip().lower()
...
KNEE_OLD_V135, KNEE_NEW_V135 = 3000, 3600          # THE EDIT -- knee only, K1 HELD
K1_HELD, K1_CEILING = 1020, 1023
...
def build():
    print("=" * 102)
    print("  V135 -- V133 + the last MEASURED rung on the Coulomb relay: knee 3000 -> 3600, K1 held.")
    print("=" * 102)
    ...
    check(u16(base, CEIL_I) == CEIL_I_NEW,
          f"  0x{CEIL_I:05X} = {CEIL_I_NEW} INHERITED from V133")
    check(u16(base, KNEE_CAL) == KNEE_OLD_V135,
          f"  0x{KNEE_CAL:05X} relay knee = {KNEE_OLD_V135} in the base")
    check(u16(base, K1_CAL) == K1_HELD,
          f"  0x{K1_CAL:05X} K1 = {K1_HELD} in the base (ceiling {K1_CEILING})")
    _fb = struct.unpack_from("<f", base, CEIL_F)[0]
    check(abs(_fb * 1024 - (CEIL_I_NEW + 1)) < 1e-6,
          f"  the matched-pair invariant holds on the base ({_fb*1024:.0f} == {CEIL_I_NEW+1})")
    ...
    attributed |= {SAR_ADDR}
    struct.pack_into("<H", code, KNEE_CAL, KNEE_NEW_V135)    # THE EDIT -- knee only
    attributed |= {KNEE_CAL, KNEE_CAL + 1}
    attributed |= {K1_CAL, K1_CAL + 1}
    ...
    lo_k, hi_k = 2400, 3600
    print(f"      knee {KNEE_NEW_V135} IS a measured rung: duty {MEASURED_DUTY[KNEE_NEW_V135]:.4f}")

    check(u16(base, KNEE_CAL) == 3000 and K1_NEW == K1_OLD == 1020 and ALPHA2_NEW == 5,
          "  the base is V133: knee 3000, K1 1020, alpha2 5 -- THIS build moves the KNEE only")
    ...
    check(u16(code, CEIL_I) == u16(base, CEIL_I),
          f"  0x{CEIL_I:05X} clamp INHERITED, unchanged")
    check(u16(code, KNEE_CAL) == KNEE_NEW_V135,
          f"  0x{KNEE_CAL:05X} relay knee {KNEE_OLD_V135} -> {KNEE_NEW_V135}")
    check(u16(code, K1_CAL) == K1_HELD == u16(base, K1_CAL),
          f"  \U0001f6d1 K1 HELD at {K1_HELD} -- NOT the gain-holding ladder, which is exhausted"
          f" (knee 3300 would need K1 1122 > the {K1_CEILING} ceiling)")
    check(K1_HELD < K1_CEILING,
          f"  K1 {K1_HELD} stays under its {K1_CEILING} ceiling, where friction would exceed"
          f" |model| and the residual would invert sign")
    _sl = lambda k: (K1_HELD / 1024.0) * (12.0 / k)
    _RATE = 4.7121
    print(f"      slope {_sl(KNEE_OLD_V135):.6f} -> {_sl(KNEE_NEW_V135):.6f}"
          f"  = x{_sl(KNEE_NEW_V135)/_sl(KNEE_OLD_V135):.2f} friction"
          f"   |   saturation {KNEE_OLD_V135/12/_RATE:.1f} -> {KNEE_NEW_V135/12/_RATE:.1f} deg/s")
    check(_sl(KNEE_NEW_V135) < _sl(KNEE_OLD_V135),
          "  friction DECREASES -- which the operator explicitly asked for")
    check(KNEE_NEW_V135 in MEASURED_DUTY and MEASURED_DUTY[KNEE_NEW_V135] == 0.0,
          f"  \U0001f6d1 THE LADDER GATE: knee {KNEE_NEW_V135} is a MEASURED rung reading"
          f" {MEASURED_DUTY[KNEE_NEW_V135]:.4f} saturation duty -- not an interpolation")
    check(MEASURED_DUTY[2400] > 0.0,
          f"  and the rung below it measures {MEASURED_DUTY[2400]:.4f}, so the ladder is not flat here")
    ...
    # 0xC40BC (the relay knee) is in V106B.FROZEN; THIS build deliberately moves it, so it is
    # exempted BY NAME -- the frozen guard is a check on unintended drift, not a veto on a
    # declared edit.  Everything else in the frozen set must still match the base.
    ...
    tag = "V135-V133BASE-KNEE.3600"
    img_out = plain_image_path(f"_v135_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [9] NOT WRITTEN -- set ACCORD_V135_WRITE=rwd to emit the files")
