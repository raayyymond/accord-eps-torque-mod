#!/usr/bin/env python3
r"""
V164 -- THE LOW-DOSE DAMPER BRANCH.  FactorC Y[0] := each mode's own Y[1].  Base = V158.
        0xD77DA 429 -> 234, 0xD77EE 426 -> 233.  TWO HALFWORDS.

WHEN TO FLY THIS
----------------
Pre-registered branch (docs/scoring/SCORING-V158-preregistered.md): fly this if V158 IMPROVES the
ratchet BUT the wheel feels too heavy at parking speeds.  The damper is a drag term, and the
operator's standing constraint is "low apparent mass and friction to LKAS AND no ratcheting".  The
answer to "better but heavier" is a LOWER DOSE, not abandoning the lever.

THE ARITHMETIC
--------------
        at the measured operating point gp-0x6ac0 = 99, FactorE(99) = 120
        V158   FactorC 429 -> dose 50    viscous 2.733 ct/(deg/s)   total 4.304   x2.74
        V164   FactorC 234 -> dose 27    viscous 1.476 ct/(deg/s)   total 3.047   x1.94
        (baseline: gp-0x6bbe measures 1.571 ct/(deg/s) on-car; stock creep is 0.000)

=> roughly HALF V158's added damping, still nearly DOUBLE what the car has at creep today.

A FREE SIDE EFFECT: THE SHAPE BECOMES MONOTONE
-----------------------------------------------
V158 deliberately accepts a non-monotone FactorC (Y = [429, 234, 429, 908], a dip between 35 and
60 km/h) because FactorC is a SPEED schedule and speed cannot pump a 7.8 Hz mode.  Setting
Y[0] := Y[1] removes that dip as a side effect, giving Y = [234, 234, 429, 908].  So V164 is not
merely lighter, it is strictly better-shaped.

WHAT IS HELD
------------
[EVIDENCE] the FactorE records are BYTE-IDENTICAL to V158 on both engaged modes, so the RATE arm --
which is where rate-proportionality lives, and which GATE 2 actually governs -- is unchanged.  This
build moves ONLY the speed-axis magnitude.
[EVIDENCE] Lever B carried at 5244; the knee, K1, alpha2 and the cave are all V122/V158 values.
=> cal-only, two halfwords, outside the cave/bricking class.
"""
...
WRITE_MODE = os.environ.get("ACCORD_V164_WRITE", "").strip().lower()
...
FC_Y0   = {26: 0xD77DA, 27: 0xD77EE}          # FactorC(L2 speed) Y[0] -- THE EDIT
FC_OLD  = {26: 429, 27: 426}                   # V158 values (each mode own Y[2])
FC_NEW  = {26: 234, 27: 233}                   #   -> each mode own Y[1]: MONOTONE
...
def build():
    print("=" * 102)
    print("  V164 -- LOW-dose damper branch, FactorC Y[0] := Y[1]")
    print("=" * 102)
    ...
    print("\n  [3] THE EDIT -- TWO HALFWORDS, ONE PER ENGAGED MODE")
    for _m in (26, 27):
        check(u16(base, FC_Y0[_m]) == FC_OLD[_m],
              f"  0x{FC_Y0[_m]:05X} mode {_m} FactorC Y[0] = {FC_OLD[_m]} in the V158 base")
        struct.pack_into("<H", code, FC_Y0[_m], FC_NEW[_m])
        attributed |= {FC_Y0[_m], FC_Y0[_m] + 1}
        print(f"      0x{FC_Y0[_m]:05X}  mode {_m} FactorC Y[0]  {FC_OLD[_m]} -> {FC_NEW[_m]}")
        check(u16(code, FC_Y0[_m]) == FC_NEW[_m], f"  reads back {FC_NEW[_m]}")

    print("\n  [4] HALF THE DOSE, HALF THE DRAG -- AND THE SHAPE BECOMES MONOTONE")
    for _m in (26, 27):
        _rC = u32(base, 0xC9E9C + 4*_m)
        _Y = [u16(code, _rC + 0xA + 2*i) for i in range(4)]
        check(_Y == sorted(_Y),
              f"  \U0001f6d1 THE SHAPE GATE: mode {_m} FactorC Y = {_Y} is now MONOTONE"
              f" NON-DECREASING.  V158 deliberately accepted a non-monotone dip here (429 > 234)"
              f" because FactorC is a SPEED schedule; setting Y[0] := Y[1] removes the dip as a"
              f" side effect, so this build is strictly better-shaped as well as lighter.")
    _E99 = 539 * (99 - 12) // (400 - 12)
    _d158 = (FC_OLD[26] * _E99) >> 10
    _d = (FC_NEW[26] * _E99) >> 10
    check(_d < _d158,
          f"  \U0001f6d1 THE DOSE GATE: at the measured operating point gp-0x6ac0 = 99,"
          f" FactorE = {_E99} and dose goes {_d158} -> {_d} = x{_d/_d158:.3f}."
          f"  Viscous damping {2.733*_d158/50:.3f} -> {2.733*_d/50:.3f} ct/(deg/s), so total creep"
          f" viscous {1.571+2.733*_d158/50:.3f} -> {1.571+2.733*_d/50:.3f} against a measured"
          f" gp-0x6bbe baseline of 1.571.")
    check(((FC_NEW[26] * 927) >> 10) <= 512,
          f"  the build-time rule (FactorC x FactorE[3])>>10 = {(FC_NEW[26]*927)>>10} <= 512")
    for _m in (26, 27):
        _rE = u32(base, 0xC9F84 + 4*_m)
        check(rd(code, _rE, 0x12) == rd(base, _rE, 0x12),
              f"  mode {_m} FactorE record byte-identical -- the RATE arm is untouched,"
              f" so rate-proportionality is exactly V158's")
    ...
    tag = "V164-V158BASE-DAMPER.LOWDOSE.FC.Y0.234"
    img_out = plain_image_path(f"_v164_{tag}_plain_image.bin")
    ...
    if WRITE_MODE == "rwd":
        ...
        print("\n  [10] NOT WRITTEN -- set ACCORD_V164_WRITE=rwd to emit the files")
