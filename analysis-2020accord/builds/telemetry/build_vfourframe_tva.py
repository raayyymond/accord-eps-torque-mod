"""
builds/telemetry/build_vfourframe_tva.py -- FOURFRAME = V38 + a FOUR-FRAME passive READ-ONLY CAN telemetry code cave.
EXTENDS the kit's first active-CAN-TX cave (builds/telemetry/build_vcantx_test_tva.py, mailbox 16 -> new ID 0x555, a fixed
...
PURPOSE
    ...
    memory/misc/comma4-eps-uds-poll-comma-vs-redpanda.md). Four dedicated 8-byte frames give full-resolution
    16-bit telemetry on all 16 cells simultaneously, every packer cycle.

★ SIGNAL PROVENANCE -- every gp-offset below is grounded against `model/eps_lkas_chain_model.py` (the kit's live
...
TECHNIQUE (verbatim reuse of builds/telemetry/build_vcantx_test_tva.py's harness + encoders; only the cave BODY is new)
    ...
    builds/v50_v79/build_v51probe_tva.py (op=0x3F, `field=((-disp)&0xFFFE)|1`), rather than deriving a new LD.H form.
...
ENCODING VERIFICATION -- every encoder is EITHER (a) verbatim-reused from builds/telemetry/build_vcantx_test_tva.py or
    builds/v50_v79/build_v51probe_tva.py, already Ghidra/byte-verified there (mov32, movea, movi5, stb, sth, stw, ldw, addi,
...
# -------------------------------------------------------------------------------------------------------
# V850E2 mini-assembler -- encoders verbatim-reused from builds/telemetry/build_vcantx_test_tva.py / builds/v50_v79/build_v51probe_tva.py
...
def mov32(imm32, reg2):
    """MOV imm32,reg2 -- V850E2-native 48-bit form. Verbatim from builds/telemetry/build_vcantx_test_tva.py."""
...
def _gp_field_load(disp_neg):
    """Verbatim from builds/v50_v79/build_v51probe_tva.py: gp-relative disp16 field for the ld.hu W/H-selector form."""
    assert 0 < disp_neg <= 0x8000
    return ((0x10000 - disp_neg) & 0xFFFE) | 1


def ldhu(disp_neg, reg2, reg1=GP):
    """LD.HU -disp_neg[reg1],reg2 (op=0x3F). Verbatim formula from builds/v50_v79/build_v51probe_tva.py, cross-checked
...
# Bcond -- verbatim from builds/telemetry/build_vcantx_test_tva.py, already cross-validated against 54 real code.bin
...
# Gate 1 -- TX-ready gp-0x1712 bit0. LITERAL reuse of builds/telemetry/build_vcantx_test_tva.py's own literal reuse of the
...
def build_cave():
    ...
    for mbx in MAILBOXES:
        ...
        # transmits. 0x80 came from builds/telemetry/build_vcantx_test_tva.py:54 mislabelling bit7 SSOW as "TX direction";
...
def _self_check_encoders():
    ...
    from builds/telemetry/build_vcantx_test_tva.py / builds/v50_v79/build_v51probe_tva.py."""
    ...
    # ld.hu -- verbatim formula from builds/v50_v79/build_v51probe_tva.py, cross-checked there against 4 real instances
    assert ldhu(0x1300, 7).hex() == "e43f01ed", "ldhu formula mismatch vs builds/v50_v79/build_v51probe_tva.py"
