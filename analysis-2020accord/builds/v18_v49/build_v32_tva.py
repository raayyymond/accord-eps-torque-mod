"""builds/v18_v49/build_v32_tva.py - V32 = V31 (UNCHANGED) + raise the GENTLE-EME disengage threshold 320 -> 1280.

WHAT V32 CHANGES (2026-06-30, operator-directed)
=========================================================================================================
The "gentle EME" (LKAS-only torque cut, STEER_STATUS=no_torque_alert_2, no DTC) fires when the sensor-A
column-torque voter output gp-0x6a62 (= 0xFEDF159E) reaches the disengage threshold:

    FUN_00040d58 (engage-SM driver-torque disengage decider), ENGAGED/HOLDING branches:
        if (gp-0x6a62 != 0xffff && gp-0x6a62 < cal[tp+0x7312]=0xC6312)  -> stay engaged
        else                                                            -> DISENGAGE (no debounce)

V32 raises cal 0xC6312 from 320 -> 1280, so a harder column-torque transient is required before LKAS
disengages -> fewer gentle-EME cuts on hard sustained turns / bumps.

SAFETY TRADE (operator's call -- name it plainly)
=========================================================================================================
Raising 320 -> 1280 means the driver must push HARDER on the wheel before sensor-A torque takes authority
from LKAS (driver-override still works -- a real grab vastly exceeds 1280). 1280 is the operator's chosen
value. NOTE (residual): gp-0x6a62 is a SEPARATE sensor from the CAN STEER_TORQUE_SENSOR, with an
un-pinned scale (see docs/handoffs/2026-06/HANDOFF-2026-06-30-sensorA-identity-gate-scale.md), so whether 1280 fully clears
the legitimate hard-turn PEAK of gp-0x6a62 is not provable on paper -- this is an empirical first step. A
live read of gp-0x6a62 (deferred; see V31T telemetry build) would let the value be sized exactly.

LOCKSTEP / SAFETY (cal-only, clean)
=========================================================================================================
0xC6312 (the cal) is read `ld.hu 0x7312[r5=tp]` at EXACTLY 3 sites, all inside FUN_00040d58 (0x40db8/0x40dd0/
0x40df4; the neighboring 0x40dae/0x40dc6/0x40dea are the gp-0x6a62 VALUE reads `ld.hu -0x6a62[r4=gp]`, NOT the
cal -- radare2 v850.gnu-verified 2026-07-02, base-register field decoded from raw bytes). No int/float twin
(the engage SM is integer-only -- NOT the corridor/wall dual-path that hard-faulted V25-V27). The value 320
appears once in the cal block. The runtime value gp-0x6a62 has its own RAM shadow gp-0x4cae (the VALUE's
redundancy, unaffected by changing the THRESHOLD constant). => editing 0xC6312 is a clean cal-only change.
It lives in cal block [0xC6000,0xC6FFC) (CRC @0xC6FFC), already recomputed by the V31 builder. ZERO code edits.
...
OUT_DIR      = RWD_DIR
BIN_OUT      = plain_image_path("_v32_plain_image.bin")
...
# ===================== V32 NEW: gentle-EME disengage threshold =====================
GENTLE_EME_THRESHOLD = [
    (0xC6312, 320, 1280, "GENTLE-EME disengage tp+0x7312  gp-0x6a62>= cut  320->1280 (V32)"),
]
...
# --- NO-CODE-EDIT guard: V32 is cal-only; these stock code sites MUST remain byte-identical ---
NO_CODE_EDIT_SITES = [
    ...
    (0x561C2, b"\x44\x07\xf0\xea", "0x660 packer byte0 zero-store -- stock (V32 is NOT the V31T telemetry build)"),
]
...
def build(label, code_stock, headers, tag):
    print("=" * 78)
    print(f"{label}: V31 (unchanged) + gentle-EME disengage threshold 0xC6312  320 -> 1280")
    ...
    # patches
    patch_cal_u(code, GENTLE_EME_THRESHOLD)    # V32 new
    ...
    assert struct.unpack_from("<H", ecu_plain, 0xC6312 - START)[0] == 1280, "GENTLE-EME threshold lost"
...
def main():
    ...
    print("V32 = V31 (gain/clamps/ramp + corridor x4 + boost floor 4096 + float mirror + PN)")
    print("      + GENTLE-EME disengage threshold 0xC6312  320 -> 1280  (cal-only, NO code edits)\n")
    build("V32", code, headers, tag="LKAS-2x-corridor4x-boostfloor4096-gentleEME-thresh1280-PNfix")
    return 0
