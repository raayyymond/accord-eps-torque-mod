#!/usr/bin/env python3
"""builds/v50_v79/build_v76_v38base_tva.py -- V76 RE-CUT ON A **V38 BASE**. Supersedes V76/V77/V77B.
...
WHY A NEW FILENAME AND NOT `builds/v50_v79/build_v76_tva.py`
---------------------------------------------
`builds/v50_v79/build_v76_tva.py` builds the SUPERSEDED V76 (`…_v76_gate_fb_arm5244_gateprobe_…`), whose .rwd is
...
# ---------------------------------------------------------------------------------------------------
# ✅ TableDesign's VALIDATED arrays, taken from `studies/sessions/v76/v76_cut_spec.py` (which asserts the same V38 sha256
...
# ---------------------------------------------------------------------------------------------------
TABLE_SOURCE = "TableDesign-validated (studies/sessions/v76/v76_cut_spec.py)"
...
def main():
    ...
    print(f"    {len(EXPECTED_WRITES)} halfword writes, {2 * len(EXPECTED_WRITES)} bytes, "
          f"all matching studies/sessions/v76/v76_cut_spec.py and all old-values == STOCK")
