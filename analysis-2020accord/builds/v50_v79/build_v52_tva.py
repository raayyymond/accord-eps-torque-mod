"""
builds/v50_v79/build_v52_tva.py -- V52 = the V50 EMA low-pass, MADE FLASHABLE. "V50 done right."
...
GATES (applied without being asked; a code cave is the kit's only bricking class -- V24/V27/V48B):
    ...
    * GATE 2 (closed-loop stability): the filter is byte-for-byte V50's alpha=74/1024 EMA (see
      studies/models/eps_v50_gate2_lowpass.py: STABLE under pessimistic Q_cl=13.6 and realistic Q_cl~4.8, no resonant
...
CAVE + REPOINTS: studies/caves/v52_cave_asm.py. Trampoline jr at 0x7FEAC displacing cmp r0,r8 + mov r8,r14
...
# probe (builds/v50_v79/build_v51probe_tva.py) reduced the FULL 16-bit cell to a "was any bit ever set" flag, packed it
# into CAN-330 spare bits with a liveness beacon, and drove (rlog 7, 4 segments). Decode (two independent
# decoders, studies/probes/decode_v51p_gate1.py + lead verifier): 0/24000 frames non-zero, beacon 100% live on bus 1,
...
V52_TAG = "LKAS-4x-V38base-ratchet-lowpass-fc12hz-ema-gp1300-rnd-8repoint"
V52_OUT = os.path.join(RWD_DIR, f"39990-TVA,A160-V52-{V52_TAG}-0x{START:X}-0x{END:X}.rwd")
BIN_OUT = str(plain_image_path("_v52_plain_image.bin"))
...
REPOINT_SITES = BASE_REPOINTS + EXTRA_REPOINTS
...
def assert_repoints_sane():
    """Exactly 10 repoint sites (7 base + the 3 FUN_0002eda8 branches), all distinct, in MAIN_BLOCK."""
    assert len(EXTRA_REPOINTS) == 3, (
        "V52 requires the 3 FUN_0002eda8 branch repoints (0x2F318/0x2F330/0x2F33E) -- see the tracer")
    assert len(REPOINT_SITES) == 10, f"expected 10 repoint sites, have {len(REPOINT_SITES)}"
...
def build():
    ...
    assert_clamp_stock(readback, "V52 RWD readback")
...
def main():
    stale = [p for p in glob.glob(os.path.join(RWD_DIR, "39990-TVA,A160-V52-*.rwd"))
