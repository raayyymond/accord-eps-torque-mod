#!/usr/bin/env python3
"""builds/v50_v79/build_v65_tva.py -- V65 = V62, byte-identical in every control path, + a 4-LEVEL SATURATION LADDER.
...
--------------------------------------------------------------------------------------------
    bit6 <-> bit3 ALTERNATING at ~7 Hz  (i.e. ~14 alternations/s)
        => the aggregator is RAIL-TO-RAIL and CLIPPED. The lever is LOOP GAIN UPSTREAM, not damping
           ...
           22's byte-identical but SEPARATE record, not a redundancy mirror -- see builds/v50_v79/build_v62_tva.py).
...
Decoder: rlog-tools/probe/decode_v65_saturation.py
"""
...
def build():
    ...
    if os.path.exists(V59_BIN):
        ...
        print("  V65 vs V59: _v59_plain_image.bin absent -- run builds/v50_v79/build_v59_tva.py for that comparison")
    ...
    print("          in builds/v50_v79/build_v62_tva.py. *** Still CODE in the 1 kHz TX path.")
    ...
    print("     Decode with rlog-tools/probe/decode_v65_saturation.py.")
    return code
