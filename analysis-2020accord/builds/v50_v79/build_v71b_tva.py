#!/usr/bin/env python3
"""builds/v50_v79/build_v71b_tva.py -- V71B = V71A's SIBLING: dose r26 ALONE, SPEED-SHAPED, stock at highway.
...
1. **THE TWO CAVES DIFFER IN EXACTLY ONE BYTE** -- cave+0x1A, `0x26` -> `0x24` -- and **that byte is
   ...
   **.rwd FILENAME is the only pre-drive discriminator**. Their plain images are separate files and
   neither builder will overwrite the other's. `probe/decode_v71_probe.py` now **REFUSES to run without
...
Usage:  python builds/v50_v79/build_v71b_tva.py
"""
...
def build():
    ...
    if assert_decoder_matches(cave_bytes):
        print("    ✅ rlog-tools/probe/decode_v71_probe.py matches this cave and names BOTH siblings")
    ...
    print(f"  EXACT DIFF vs STOCK: {len(d_stock)} bytes -- run `python verify/diff_build_vs_stock.py v71b`")
