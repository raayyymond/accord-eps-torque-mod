#!/usr/bin/env python3
r"""builds/v80_v107/build_v93_tva.py -- V93 = the FIRST build in the kit's history to LOWER the 0xCBE74 term.
...
Five edits, 22 calibration bytes + 12 CRC bytes. NO cave change, NO code change. The V90 instrument
...
🛑 `builds/v80_v107/build_v91_tva.py` says of this same term: *"The Y row is NEGATIVE, so gp-0x6b26 carries the
...
Usage:
    python builds/v80_v107/build_v93_tva.py                    # DRY RUN, writes nothing
    ACCORD_V93_WRITE=bin python builds/v80_v107/build_v93_tva.py
    ACCORD_V93_WRITE=rwd python builds/v80_v107/build_v93_tva.py
"""
...
WRITE_MODE = os.environ.get("ACCORD_V93_WRITE", "").strip().lower()
...
FROZEN = {
    ...
    0xC64FD: (1, 5, "🛑 the FALLBACK-2 gate on gp-0x671a -- V93 does NOT move it"),
...
VARIANT_TOKEN = "V90BASE-CBE74.M24x0.50.M26.M27x0.25-FALLBACKx0.75"
TAG = VARIANT_TOKEN
BIN_OUT = str(plain_image_path(f"_v93_{VARIANT_TOKEN}_plain_image.bin"))
OUT = os.path.join(RWD_DIR, f"39990-TVA,A160-V93-{TAG}-0x{START:X}-0x{END:X}.rwd")
...
def main():
    ...
    check(rd(code, R427_ADDR, 2) == struct.pack("<h", -R427_DISP),
          "CAN 427 still reads gp-0x6b26 (`ld.h -0x6b26[gp],r6`)")
    ...
    check(derived == {0xC6FFC, 0xD6FFC, 0xD7FFC},
          f"DERIVED trailer set {sorted(hex(t) for t in derived)} == "
          f"{{0xc6ffc, 0xd6ffc, 0xd7ffc}} -- derived, then asserted; never hard-coded")
    ...
    check(len(crc_only) == 12, f"exactly 3 CRC trailers = 12 bytes (got {len(crc_only)})")
    ...
    if stock_p.exists():
        ...
        for a, v in sorted(FALLBACKS.items()):
            check(s16(stock, a) == v, f"stock 0x{a:05X} == {v} => V93 is its FIRST movement ever")
    ...
    FF.assert_x31_checksum(rwd, "V93 output")
    back = parse_x31(rwd)
    dec = bytearray(base)
    ...
    if WRITE_MODE in ("", "none"):
        print("  🛑 DRY RUN -- NOTHING WRITTEN. Re-run with ACCORD_V93_WRITE=rwd to cut.")
    else:
        ...
        if WRITE_MODE == "rwd":
            ...
            FF.assert_x31_checksum(shipped, "V93 shipped")
            sd = bytearray(base)
    ...
    print("\n" + "=" * 102)
    print(f"  V93 [{VARIANT_TOKEN}]     {_checks[1]}/{_checks[0]} assertions PASSED")
    print(f"    image SHA256 {img_sha}")
    print(f"    .rwd  SHA256 {rwd_sha}  "
          f"({'WRITTEN' if WRITE_MODE == 'rwd' else 'computed, NOT written'})")
    print("  🛑 22 cal bytes. The FIRST build ever to LOWER this term. Mode 24 x0.50, modes 26/27")
    print("     x0.25, both fallbacks x0.75 -- three factors so the flight names the live branch.")
