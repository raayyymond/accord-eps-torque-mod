#!/usr/bin/env python3
"""builds/v80_v107/build_v85_tva.py -- V85 = V84 + ONE CALIBRATION CELL. The friction RELAY becomes VISCOUS.
...
★ REPRODUCTION: `analysis-2020accord/studies/models/fun3b8f6_friction_relay.py` computes the same table from the
...
Usage:
    python builds/v80_v107/build_v85_tva.py                            # DRY RUN, verifies everything, writes nothing
    ACCORD_V85_WRITE=rwd python builds/v80_v107/build_v85_tva.py       # writes the plain image AND the flashable .rwd
"""
...
# =====================================================================================================
# THE RELAY, AS ARITHMETIC -- reproduced here, and cross-checked against studies/models/fun3b8f6_friction_relay.py
# =====================================================================================================
...
def describing_function_closed(amp_counts, norm):
    """The analytic form -- the SECOND method. `studies/models/fun3b8f6_friction_relay.py` uses exactly this."""
    a = RATE_MUL * amp_counts / float(norm)
    if a <= 1.0:
        return 1.0
...
# 🛑 THE BUILDER->DECODER LINK, MADE MECHANICAL. `studies/probes/decode_v85_probe.py` IMPORTS these names rather than
...
def assert_decoder_module():
    ...
    if not os.path.exists(os.path.join(HERE, "studies/probes/decode_v85_probe.py")):
        print("    ⚠ studies/probes/decode_v85_probe.py not found -- the decoder/image link is NOT verified")
        return False
    import importlib
...
def build():
    ...
    print(f"    ✅ studies/probes/decode_v85_probe.py: "
