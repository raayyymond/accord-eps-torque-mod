#!/usr/bin/env python3
"""build_v84_tva.py -- V84 = V83a + LEVER B RESTORED + THE ENGAGED-ONLY DAMPER DELETED. CAL + 1 BYTE.
...
Usage:
    python build_v84_tva.py                            # DRY RUN, verifies everything, writes nothing
    ACCORD_V84_WRITE=rwd python build_v84_tva.py       # writes the plain image AND the flashable .rwd
"""
import hashlib
import math
import os
...
# 🛑 THE BUILDER->DECODER LINK, MADE MECHANICAL. `decode_v84_probe.py` IMPORTS these names rather
...
def assert_decoder_module():
    ...
    if not os.path.exists(os.path.join(HERE, "decode_v84_probe.py")):
        print("    ⚠ decode_v84_probe.py not found -- the decoder/image link is NOT verified")
        return False
    import importlib
...
def build():
    ...
    print(f"        gp-0x{GATE_DISP:04x} is the LKAS-active flag: agreement with `latActive` "
          f"{gate_val['_cache_r28']['agreement_pct']}% / "
          f"{gate_val['_cache_r29']['agreement_pct']}%")
    print(f"        over {gate_val['_cache_r28']['frames'] + gate_val['_cache_r29']['frames']:,} "
          f"frames at duty {gate_val['_cache_r28']['duty_pct']}% / "
          f"{gate_val['_cache_r29']['duty_pct']}%, "
          f"{gate_val['_cache_r28']['transitions'] + gate_val['_cache_r29']['transitions']} "
          "transitions")
    ...
    print(f"    ✅ decode_v84_probe.py: {'imports THIS build\'s bit map; its self-test PASSES' if decoder_ok else 'NOT FOUND'}")
