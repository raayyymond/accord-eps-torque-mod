...
that produced every burst count already on record (`r58_grind2.py`, `d4_r59_grind2.py`). The tapered
`_grind2_lib.win_env` is NOT substituted: they differ by 1.4-1.9x and cross-comparing them is a
recorded error in this kit.

WHAT IS NEW HERE vs `d4_r59_grind2.py`:
...
Usage:  python grind2_delivered_census.py
"""
import json
import sys
...
ROUTES = [
    ...
    ("V64",  "r35",  ["_cache_r35"]),
    ("V62",  "r37",  ["_cache_r37"]),
    ("V65",  "r3ab", ["_cache_r3a", "_cache_r3b"]),
    ("V67",  "r47",  ["_cache_r47"]),
    ("V68",  "r4e",  ["_cache_v68"]),
    ("V69",  "r4f",  ["_cache_r4f"]),
    ("V70",  "r50",  ["_cache_r50"]),
...
(HERE / "_grind2_delivered_census.json").write_text(json.dumps(OUT, indent=1), encoding="utf-8")
print(f"\nwrote {HERE / '_grind2_delivered_census.json'}")
