...
----------------------------------------------------------------
  ...
  P     = c*G,  A = 1+P,   c = lambda*kappa identified from the 4x/8x gain step (_g2b_kappa.py)
...
Uncertainty: the JOINT episode bootstrap of (G4,G8,Z4,Z8) -> (c, A, G) from _g2b_kappa.py,
...
def ident(lo, hi, nboot=4000, seed=41):
    """Point + episode-bootstrap draws of (c, G4, A4, Z4). Mobius solve, as _g2b_kappa.py."""
    def one(g4, z4, g8, z8):
...
print("[C.2] reproduce _g2b_kappa.py's 6-9 Hz solve: |kG| %.3f (rec 0.630), |A| %.3f (rec 0.440),"
      % (abs(c69 * G69), abs(A69)))
