#!/usr/bin/env python3
"""audit_r47_probe.py -- V67 gate-probe decode + FLIGHT-CLEAN audit for route 47.

Route `75604b0a432fdc89_00000047--3e0b6134c0`, 26 segments (0..25), flown on V67.

Reads the caches written by `extract_r47_cache.py` (`_cache_r47/r47s{N}.npz` +
...
the hold. H counts every 0x18F src-1 frame as it arrived.
    ...
    python analysis-2020accord/audit_r47_probe.py           # A-G, cache only
    python analysis-2020accord/audit_r47_probe.py --raw     # + H (re-parses 26 rlogs, ~3 min)
    python analysis-2020accord/audit_r47_probe.py --raw --refresh   # force the rlog re-parse

H caches its own result to `_cache_r47/r47_raw18f.npz`; `--refresh` rebuilds it.
...
`extract_r47_cache.py` takes the MEDIAN of (clocks.wallTimeNanos - logMonoTime). On s0 the
...
CACHE = Path(os.environ.get("R47_CACHE", ROOT / "_cache_r47"))
...
# The kill band for a gate that switches a control gain (docs/V66-V67-DESIGN.md).
