...
Usage:  python analyze_r47_grind2.py            (all sections)
        python analyze_r47_grind2.py 3 4        (only those sections)
"""
import json
import sys
...
def sec7(store):
    ...
    import glob
    import os
    ok = sorted(glob.glob(str(HERE.parent / "_cache_r47" / "r47s*_imu.npz")))
    if not ok:
        print("  no r47 IMU caches -- run:  python extract_imu_cache.py r47")
        return
    ...
    ctl = paired_offset("V67/r47", "_cache_r47", "r47s",
    ...
    subj = []
    for b, cache, pfx in (("V62/r37", "_cache_r37", "r37s"), ("V65/r3a", "_cache_r3a", "r3as"),
                          ("V65/r3b", "_cache_r3b", "r3bs")):
        want = ckey.get(b, set())
        if not want:
            continue
...
# ================================================================================ §8 ==============
def sec8(store):
    G.hdr("§8  THE ATLAS EPISODES.  Independent episode definitions from `r47_atlas.py`\n"
          "(_cache_r47/r47_maneuvers.json): 21 highway manoeuvres, 21 speed-matched straight-line\n"
