#!/usr/bin/env python3
"""r4a_addendum.py -- the second pass on route `4a`: provenance, golden-model grounding, probe-rung
recommendation, speed sweeps, the corpus-wide LKAS-OFF-at-speed census, and the wheel-order screen.

Companion to `r4a_orchestrator_checks.py` (extraction audit, exposure, grind #1/#2 statistics). This
file exists because three things were asked for AFTER that pass and each of them is a different kind
of question:

  provenance  route -> build -> dose must come from `route_build_registry.py` + docs/BUILD-LINEAGE.md,
  ...
  golden      every channel is tapped somewhere in `eps_lkas_chain_model.py`'s chain, and WHERE it is
...
Usage:  python r4a_addendum.py [section ...]      (default: all)
"""
...
# =================================================================================================
def sec_provenance():
    hdr("A.  ROUTE -> BUILD -> DOSE, from route_build_registry.py and docs/BUILD-LINEAGE.md")
    ...
    print(f"      r47_orchestrator_checks.CREEP_POOLS Kd=1.00 = {['_cache_r2b','_cache_r2c','_cache_r35']}")
...
def sec_golden():
    ...
    print("   Source: analysis-2020accord/eps_lkas_chain_model.py. WHERE a channel is tapped decides")
