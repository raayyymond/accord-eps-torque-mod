#!/usr/bin/env python3
"""v77sizing_extract.py -- extract route 65 (**V76**, driven, operator reports grind#1 + micro-
ratcheting still present at creep) to `_cache_r65/r65.npz`.
...
CLEAR, per `build_v76_v38base_tva.py`'s own exhaustive `_check_wire_model`):
...
🛑 The spec is RE-READ out of `build_v76_v38base_tva.py` at import time -- see `_assert_probe_spec()`,
which execs the builder's OWN `wire_model()` and checks this file's decode inverts it exhaustively.
Mirrors the discipline `v74fault_extract.py` and `v75fault_extract.py` established; if the builder's
...
segment index per sample. (`v77sizing_cache.py` re-splits this into per-segment files with `t`
reset to 0 at each segment, which is the schema `_grind2_lib` / `_r31_common` expect.)

Usage: python v77sizing_extract.py            # all 11 segments -> _cache_r65/r65.npz
"""
import json
import os
import re
...
SEGS = list(range(11))
OUT = Path(os.environ.get("R65_CACHE", ROOT / "_cache_r65"))
