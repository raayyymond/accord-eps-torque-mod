...
🛑 THE CONFLICT.  `ArcAudit` reports the V89 probe "tests |gp-0x6ae2| >= 64, not literal zero".
   ...
   scripts carry the swap explicitly:
       ...
       build_v86_tva.py:406   BIT_SIGN, BIT_NONZERO, BIT_MAG, ... = 0x80, 0x40, 0x20, ...
       build_v86b_tva.py:219  BIT_SIGN, BIT_MAG, BIT_NONZERO      = 0x80, 0x40, 0x20
...
OUTJ = ROOT / "_cache_r75" / "v89_e7_rung_and_zero.json"

ROUTES = {"73": ("_cache_r73", "r73", "V88 (cave on gp-0x6b98)"),
          "75": ("_cache_r75", "r75", "V89 (cave on gp-0x6ae2)"),
          "76": ("_cache_r76", "r76", "V89 (cave on gp-0x6ae2)")}
...
def m1_constants():
    ...
    for f, pat in (("build_v86_tva.py", "BIT_SIGN, BIT_NONZERO, BIT_MAG"),
                   ("build_v86b_tva.py", "BIT_SIGN, BIT_MAG, BIT_NONZERO")):
