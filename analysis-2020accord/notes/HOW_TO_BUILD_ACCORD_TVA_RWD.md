...
`analysis-2020accord/archive/old_tools/build_stock_tva_v9.py` is the builder of record. It:
...
```
python analysis-2020accord/archive/old_tools/build_stock_tva_v9.py
```

Supporting modules (all in `analysis-2020accord/`, do not delete):
- `lib/encode_eps.py` — x31/x5a parse + encode, cipher table builder/inverter (`build_decode_table`, `invert_table`, `encode_x31`, `parse_x31`, `OPS`).
- `lib/verify_bootloader_crc.py` — `walk(img)` replays the ECU's `checkProgrammingDependencies` CRC chain. The pre-flash gate.
- `analysis-2020accord/archive/old_tools/decode_eps.py` — the `.rwd` → `code.bin` decoder + cipher cracker (for inspecting other files).
...
The stock build is the special case where you change nothing. For a real modification (torque table, clamp, gain — see `notes/TORQUE_PATH_AND_TABLE.md` for what lives where):
...
| Builder / validator / flasher | `analysis-2020accord/archive/old_tools/build_stock_tva_v9.py` / `analysis-2020accord/lib/verify_bootloader_crc.py` / `flashing-2020accord/eps-update-tva.py` |
