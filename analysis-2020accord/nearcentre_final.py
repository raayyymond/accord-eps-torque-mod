...
ss1  DOES THE NEAR-CENTRE CONTRAST DECOMPOSE?  `nearcentre_deconfound.py` ss2 showed the angle
...
Usage: python nearcentre_final.py [ep|blk]  -> writes _nearcentre_final.json
"""
import json
import sys
...
print("  `nearcentre_speed_and_band.py` reported); the other rows hold the manoeuvre rate fixed.\n")
...
(HERE.parent / "_nearcentre_final.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE.parent / '_nearcentre_final.json'}")
