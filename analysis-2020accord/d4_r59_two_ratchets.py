...
FOLLOW-UP 1 -- RATCHET-A vs RATCHET-B.
  Ratchet-B (what d4_r59_ratchet.py measured, and what the corpus record describes): ENGAGED,
  ...
  symptom, which `docs/V72-DESIGN.md` s0.1 calls a DIFFERENT symptom): hands-ON, during or after a
  hard turn, transient.
...
Writes `_d4_r59_two_ratchets.json`.
"""
import json
import sys
...
(ROOT / "_d4_r59_two_ratchets.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {ROOT / '_d4_r59_two_ratchets.json'}")
