#!/usr/bin/env python3
r"""v77_dose_math.py -- the damper DOSE at 5 mph, the 150% target, and whether 4 breakpoints reach it.
...
The evaluator mirror itself is `v76_surface.py` (imported, not re-implemented) -- every arithmetic
...
SPEED SCALE -- confirmed, not assumed  [EVIDENCE]
  `eps_lkas_chain_model.py:773`  COUNTS_PER_KMH = 64.0625, from FUN_000522fe (`x*41 >> 6` on a
  0.01 km/h raw value).  Anchor: stock FactorC mode-26 X[0] = 2240 counts, byte-read, and
  2240 / 64.0625 = 34.966 km/h -- the kit's "35.00 km/h" knee.  `v76_surface.py` uses a round 64.0;
  ...
  RATE scale: gp-0x6ac0 = 4.7121 counts per deg/s (`eps_lkas_chain_model.py:1364`).

CEILING -- why 512 and not 512..1024  [EVIDENCE, eps_lkas_chain_model.py:928-936]
...
OUT_JSON = Path(__file__).resolve().parent / "_v77_dose.json"
J = {}
...
def q2(S):
  ...
  SPEED-COUNT CONVERSION, and where it is confirmed:
      ...
      v76_surface.py:95             uses a round 64.0.  0.098% apart; BOTH give 515 counts at 5 mph,
