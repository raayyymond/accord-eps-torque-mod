...
# ------------------------------------------------------------------ the V69 build entry ----------
G.BUILDS["V69/r4f"] = dict(cache=ROOT / "_scratch/cache/r4f", pfx="r4fs", segs=list(range(8)), kd=4.0)
...
# V68 has no `_grind2_lib` cache (its extractor writes `_scratch/cache/v68/4e*.npz` with a different prefix
# convention); it is registered below only if that cache is present.
if (ROOT / "_scratch/cache/v68" / "4es31.npz").exists():
    G.BUILDS["V68/r4e"] = dict(cache=ROOT / "_scratch/cache/v68", pfx="4es", segs=[31, 32, 33, 34], kd=2.0)
...
# (docs/handoffs/2026-08/HANDOFF-2026-08-04-v69-recut-4x-and-ratchet-probe.md ss1).
