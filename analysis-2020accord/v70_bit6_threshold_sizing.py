#!/usr/bin/env python3
"""v70_bit6_threshold_sizing.py -- what `sar` immediate gives bit6 a POSITIVE CONTROL?

V69's bit6 (`gp-0x6ada >= +4096`) read 0.0000% over 47,990 frames of route 4f. The replay in
`r4f_v69_readout.py` showed why: under V69's own 4x surface the route predicted ~1 one-sided hit,
...
METHOD. Same replay as `r4f_v69_readout.py`, which the orchestrator adjudicated as the
...
Usage:  python v70_bit6_threshold_sizing.py
"""
import os
import struct
import sys
...
def main():
    ...
    # the four CONFIRMED ratchet episodes, re-derived exactly as in r4f_v69_readout.py
    ...
    print("\n" + "=" * 102)
    print("3. bit5 / bit4 REACHABILITY AT EACH THRESHOLD  (see v70_rung_reachability.py for the")
