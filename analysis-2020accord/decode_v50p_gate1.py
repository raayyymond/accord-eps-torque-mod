#!/usr/bin/env python3
r"""
decode_v50p_gate1.py -- read V50P (gp-0x1500 -> CAN 330 spare bits) telemetry out of rlog 5,
and compare against a STOCK (non-probe) manual drive to determine whether the probe was
provably live, per the packing in build_v50probe_tva.py:

    CAN 330 (0x14A) byte4 bits 7:3 = gp-0x1500 & 0x1F   (low 5 bits)
    CAN 330 (0x14A) byte7 bits 7:6 = gp-0x1500 & 0x03   (low 2 bits)

Mirrors decode_v49p_polarity.py's parsing/checksum logic but globs TWO drives:
