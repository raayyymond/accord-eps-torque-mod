...
unattenuated -- d4_r59_ratchet.py); **MACRO == a separate, larger, hard-turn-recovery symptom that
V72 fixed.** Nobody has measured MACRO because nobody knew to look for it separately. V42 was
CONFIRMED on-car against a ~10 s hard-turn recovery ratchet, flagged in `docs/V72-DESIGN.md` s0.1 as
...
ROUTES = {
    "V59 r2c":  ("_cache_r2c", "r2cs", [0, 1, 3, 4, 8, 9, 10, 11, 12], [], 1.000),
    "V62 r37":  ("_cache_r37", "r37s", list(range(15)), [], 2.000),
    "V67 r47":  ("_cache_r47", "r47s", list(range(26)), [], 0.250),
    "V69 r4f":  ("_cache_r4f", "r4fs", list(range(8)), [], 1.000),
    "V70 r50":  ("_cache_r50", "r50s", [0, 1, 2], [0], 1.000),
    "V71B r54": ("_cache_r54", "r54s", list(range(21)), [10, 11], 2.000),
    "V71C r58": ("_cache_r58", "r58s", list(range(16)), [12, 13, 14, 15], 1.500),
    "V72 r59":  ("_cache_r59", "r59s", list(range(15)), [12, 13, 14], 0.250),
}
NEW = "V72 r59"
...
(ROOT / "_d4_macro.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {ROOT / '_d4_macro.json'}")
