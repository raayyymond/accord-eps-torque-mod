#!/usr/bin/env python3
r"""build_v86_tva.py -- V86 = ONE CALIBRATION CELL. The command-EMA becomes a FREQUENCY experiment.
...
--------------------------------------------------------------------------------------------------
RECORDED TRAP: `diff_build_vs_stock.py` is **SPAN-based**. It answers "did the right region change?"
...
⊕ **A COUNT WORTH RECORDING, because it differs from the one in circulation.** `_session_v86_arc_map.md`
...
`verify_v86_gates.py` case 14 caught exactly that. `assert_records_vs_base` is the repair: every one of
...
Usage:
    python build_v86_tva.py                        # NULL BUILD: verifies everything, writes nothing
    ACCORD_V86_WRITE=rwd python build_v86_tva.py   # REFUSED until the cell list and token are set
"""
import hashlib
import math
import os
...
def assert_decoder_module():
    """🛑 THE BUILDER->DECODER LINK, MADE MECHANICAL. `decode_v86_probe.py` IMPORTS these names rather
    ...
    if not os.path.exists(os.path.join(HERE, "decode_v86_probe.py")):
        print("    ⚠ decode_v86_probe.py not found -- the decoder/image link is NOT verified")
        return False
    import importlib
...
def assert_anchors(buf, stock, label):
    ...
    🛑 `diff_build_vs_stock.py` is SPAN-based: it passes a build that wrote the right REGION with the
