...
in the same session, both writing `_cache_r4f/r4fs*.npz` with DIFFERENT field sets, and whichever ran
last silently dropped the other's channels. If you need a variant, add a flag, not a file.

Every channel `extract_r5a_cache.py` wrote is written here under the SAME name with the SAME units,
...
2. **BUILD IDENTITY IS THE KNOWN FAILURE MODE ON THIS EXACT FILE PAIR.** Every `V7x` cave writes the
   ...
   legal `gp-0x67FA` states. Run against V73's own flight (route 5a), `decode_v74_probe.py` printed
...
`_assert_cave_bytes()` re-reads `CAVE_HEX` out of `rlog-tools/decode_v74_probe.py` at import time and
...
WHAT THIS EXTRACTOR ADDS OVER `extract_r5a_cache.py`, AND WHY
-------------------------------------------------------------
  ...
  2. ★ **THE IMU, in the SAME PASS**, written to `{tag}_imu.npz` in `extract_imu_cache.py`'s EXACT
     ...
     (`_r47_imu_lib` docstring). `extract_imu_cache.py` has never had routes 50/54/58/59/5a/5d
  ...
  3. **`{tag}_snd.npz` sidecar** in `extract_sound_cache.py`'s schema (adds `spwdb`), so
...
Usage:  python extract_r5d_cache.py              # all 17 segments, then the summary
        python extract_r5d_cache.py 0 1          # chosen segments (no summary)
        python extract_r5d_cache.py --summary    # summary only, from the caches on disk
"""
import json
import os
import re
...
SEGS = list(range(17))
OUT = Path(os.environ.get("R5D_CACHE", ROOT / "_cache_r5d"))
PFX = "r5ds"
SUMMARY = HERE / "_r5d_extract_summary.json"
...
def _assert_cave_bytes():
    ...
    Re-read `CAVE_HEX` out of `rlog-tools/decode_v74_probe.py` (by REGEX, not by import, so this does
    ...
    """
    src = (ROOT / "rlog-tools" / "decode_v74_probe.py").read_text(encoding="utf-8")
    m = re.search(r'^CAVE_HEX\s*=\s*"([0-9a-f]+)"', src, re.M)
    assert m, "CAVE_HEX not found in decode_v74_probe.py -- cannot verify the probe cells"
    ...
    assert mn, "RWD_NAME not found in decode_v74_probe.py"
...
def _write_imu(tag, t0, a_hw, a_mono, a_v, a_st, g_hw, g_mono, g_v, g_st):
    """`{tag}_imu.npz` in `extract_imu_cache.py`'s EXACT schema -- the sensor's OWN lattice.
...
def summarize():
    """Read the caches back off disk and write `_r5d_extract_summary.json`.
