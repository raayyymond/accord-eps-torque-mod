...
WHY THIS EXISTS
---------------
`docs/HANDOFF-2026-08-21-v104-built-c4-boost-and-lever-b.md` sec 4.2 and `docs/STATE.md` both
...
one comment line in eps_lkas_chain_model.py).  This file re-derives them from the ROM mirror so
the V104 dose can be re-priced per speed.

DEFINITIONS -- what `a` is, exactly
-----------------------------------
price_flat_6b86_boost.py:  G(f) = u/T_s   with u = aggregator SUM gp-0x6b94,
...
np.savez(os.path.join(os.path.dirname(os.path.abspath(__file__)), '_a_speed_schedule.npz'),
...
print()
print("  saved _a_speed_schedule.npz")
