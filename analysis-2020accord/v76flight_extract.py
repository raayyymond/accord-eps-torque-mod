#!/usr/bin/env python3
"""v76flight_extract.py -- extract route 65 (V76's first flight) into `_cache_r65_records.pkl`.
...
`build_v76_v38base_tva.py`'s own `wire_model()` / bit-weight constants, NOT hand-copied:
...
🛑 THIS IS NOT THE SAME BUILD AS THE REPO'S `rlog-tools/decode_v76_probe.py`. That file documents
...
(`analysis-2020accord/build_v76_v38base_tva.py`), whose probe bits are completely different
(friction margin / mode index / state==5, not gate/mask/arm). Do not use that decoder on this log.

🛑 THE SPEC IS RE-READ out of `build_v76_v38base_tva.py` at import time, not hand-copied -- see
`_assert_probe_spec()`, which execs the builder's own `wire_model()` and checks that this file's
decode inverts it exhaustively, exactly as `v74fault_extract.py` does for V74.
...
Usage: python v76flight_extract.py            # all 11 segments -> _cache_r65_records.pkl
"""
import json
import os
import pickle
...
SEGS = list(range(11))
OUT_PKL = HERE / "_cache_r65_records.pkl"
...
def _assert_probe_spec():
    ...
    src = (HERE / "build_v76_v38base_tva.py").read_text(encoding="utf-8")

    def hexnum(name):
        m = re.search(rf"^{name}\s*=\s*0x([0-9A-Fa-f]+)", src, re.M)
        assert m, f"{name} not found in build_v76_v38base_tva.py"
    ...
    assert m, "wire_model() not found in build_v76_v38base_tva.py"
    ...
    exec(compile(m.group(0), "build_v76_v38base_tva.py:wire_model", "exec"), ns)
    wire = ns["wire_model"]
