"""build_v31p_tva.py - V31P = V31 (UNCHANGED cals) + gentle-EME GATE-FIRING TELEMETRY
...
OUT_DIR      = RWD_DIR
BIN_OUT      = plain_image_path("_v31p_plain_image.bin")
...
def assemble_cave(base):
    ...
    # ---- decider_stub: r12=refusal code (preserve). bit0 if ==2, bit4 if ==5 ----
    syms["decider"] = here()
    ...
    syms["pack"] = here()
    # byte4[7:3] = (flagbyte & 0x1f) << 3   (r6/r7 scratch, both dead at 0x55c0e)
    emit(H("843f01eb"))                # ld.bu -0x1500[gp],r7
    emit(H("c73e1f00"))                # andi 0x1f,r7,r7
    ...
    # byte7[7:6]: clear then set trump(6)/deliver(7)   (r6/r8 scratch)
    ...
    emit(H("84470398"))                # ld.bu -0x67fe[gp],r8   (TRUMP var)
    emit(H("6242"))                    # cmp 0x2,r8
    emit(H("ba05"))                    # bne +6
    emit(H("c437efea"))                # set1 6,-0x1511[gp]     TRUMP
    emit(H("a447f797"))                # ld.bu -0x6809[gp],r8   (DELIVER var)
    emit(H("6042"))                    # cmp 0x0,r8
    emit(H("b205"))                    # be +6
    emit(H("c43fefea"))                # set1 7,-0x1511[gp]     DELIVER_CUT
...
def main():
    ...
    build("V31P", code, headers, tag="gateflags-330piggyback-caveC4B34")
    return 0
