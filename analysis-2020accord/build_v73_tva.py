#!/usr/bin/env python3
"""build_v73_tva.py -- V73 = V72 CARRIED BYTE-IDENTICALLY, plus three additions and a new probe.
...
---------------------------------------------------
  ...
  ⚠ RISK 4 -- V73 inherits V72's UNGATED rate lane and every one of V72's disclosed risks unchanged.
     Nothing here narrows them; see `build_v72_tva.py`'s docstring.
...
DECODER = os.path.join(HERE, "..", "rlog-tools", "decode_v73_probe.py")
...
def build():
    ...
    if assert_decoder_matches(cave_bytes):
        print("    ✅ rlog-tools/decode_v73_probe.py CAVE_HEX matches the built cave byte-for-byte.")
