#!/usr/bin/env python3
"""v70_plot_data.py -- emit the plotting dataset for "what V70 does", from the image bytes.
...
Emits _v70_plot_data.json next to this file.
"""
import json
...
ROOT = M.ROOT
OUT = Path(__file__).resolve().parent / "_v70_plot_data.json"
...
# ---------------------------------------------------------------------------------------------
# operating points -- the SAME ones build_v70_tva.py prices, so these numbers are comparable to
...
# Kd=2 pool burst RATE per second, per rateKey stratum, from _r4f_rate_axis.json (r4f_rate_axis_
...
_ra = json.load(open(Path(__file__).resolve().parent / "_r4f_rate_axis.json"))
...
# the same point build_v70_tva.py quotes. Pricing it at rateKey 603 instead flatters every build.
