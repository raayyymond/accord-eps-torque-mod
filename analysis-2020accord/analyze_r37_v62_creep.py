...
CONTROLS (all pre-built caches, nothing re-extracted):
    V59 route 2c   _cache_r2c   segs SEGS_2C     -- the reference build the grinding was measured on
    V61 route 31   _cache_r31   segs [0..3]      -- Kd = 0, the signed WORSE result
    V64 route 35   _cache_r35   segs [0,1,2]     -- inert (detector never armed) => a V59 replicate
    V62 route 37   _cache_r37   segs [1..14]     -- 🛑 seg 0 is a stale 07:05 boot, EXCLUDED
...
Usage:  python analyze_r37_v62_creep.py
"""
import json
import sys
...
BUILDS = {
    "V59 r2c": dict(cache=ROOT / "_cache_r2c", pfx="r2cs", segs=SEGS_2C),
    "V61 r31": dict(cache=ROOT / "_cache_r31", pfx="r31s", segs=SEGS_31),
    "V64 r35": dict(cache=ROOT / "_cache_r35", pfx="r35s", segs=SEGS_35),
    "V62 r37": dict(cache=ROOT / "_cache_r37", pfx="r37s", segs=SEGS_37),
}
...
def common_shift():
    ...
    print("   rate_lane_damping_model.py: d(omega_n^2)/omega_n^2 ~ Kd*k*tau/J_c, FREQUENCY-INDEPENDENT.")
