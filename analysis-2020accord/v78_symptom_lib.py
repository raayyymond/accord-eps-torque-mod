...
the thirteenth entry, pointing at `_cache_r5e_sym/` (written by `v78_symptom_cache.py`), so every
statistic `r5d_ratchet.py` / `r5d_falsifiers.py` computed for V74 is computed for V75 by the
IDENTICAL code path.
...
byte-derived in `docs/HANDOFF-2026-08-06-v75-faulted-and-the-gate2-gain.md` §3. It is the
...
PKL = ROOT / "_cache_r5e_sym_records.pkl"


def records(rebuild=False):
    """{build: [window records]} for the 13-build corpus.

    🛑 The twelve prior builds come out of `_cache_r5d_records.pkl` READ-ONLY -- this never writes
...
# The operator's verbal report per build, for the perceptual mapping in `v78_symptom_perception.py`.
