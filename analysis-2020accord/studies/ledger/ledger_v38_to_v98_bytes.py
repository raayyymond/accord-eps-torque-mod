#!/usr/bin/env python3
r"""DELIVERED-BYTES LEDGER, STOCK -> V98.  Reads the PLAIN IMAGES ON DISK, never the build scripts.

Successor to `studies/ledger/ledger_v38_to_v84_bytes.py` (kept, not overwritten) and `studies/ledger/ledger_v94_cells.py`.
Extends the chain through V85..V98 and adds the three empirical products the V84 reader lacked:
a FULL byte diff (not just named sites), FROZEN counts, and VIRGIN verdicts.
...
Usage:
    python studies/ledger/ledger_v38_to_v98_bytes.py            # everything: A + B1 + B2 + B3 + C, writes JSON
    python studies/ledger/ledger_v38_to_v98_bytes.py diff V97 V98
    python studies/ledger/ledger_v38_to_v98_bytes.py matrix | frozen | virgin | delta | mask
"""
import hashlib
import json
import os
...
TARGET = os.environ.get("LEDGER_TARGET", "V98")
PREV_ON_CAR = "V97"          # what was on the car before TARGET
...
def main():
    ...
    print(f"{len(order) - 1} build images on disk (V22..V98); TARGET={TARGET}, "
    ...
    outp = HERE / "v98_vs_stock_delta.json"
