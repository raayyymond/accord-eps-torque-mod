#!/usr/bin/env python3
"""DELIVERED-BYTES LEDGER, V38 -> V84.  Reads the PLAIN IMAGES ON DISK, not the build scripts.
...
# ---- build order, V38 .. V84.  filename stem -> label
BUILDS = [
    ("STOCK", STOCK),
...
def main():
    ...
    outp = Path(os.environ.get("LEDGER_OUT", "ledger_bytes.json"))
    outp.write_text(json.dumps(out, indent=1, default=str))
    print(f"wrote {outp}")

    # ---- human print: scalars, only rows that ever change
    print("\n=== SCALAR SITES (only rows that differ somewhere) ===")
    for addr, w, label, vals in rows:
        uniq = set(vals.values())
        tag = "" if len(uniq) > 1 else "  [CONSTANT]"
    ...
    for fname in ("FactorC", "FactorE", "FactorB", "FactorD", "ceiling", "friction"):
        for m in MODES:
            ...
            if run: segs.append((prev, run))
            if len(segs) == 1:
                print(f"\n{fname} m{m}: [CONSTANT ALL BUILDS] {segs[0][0]}")
            else:
                print(f"\n{fname} m{m}:")
                for v, r in segs:
