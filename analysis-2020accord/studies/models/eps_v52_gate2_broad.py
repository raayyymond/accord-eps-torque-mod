...
# -*- coding: utf-8 -*-
r"""
studies/models/eps_v52_gate2_broad.py -- GATE 2 (closed-loop stability) for the BROAD V52 gp-0x4f60 low-pass
insertion: repointing up to 19 command-path carrier lanes (vs the 7-lane insertion studies/models/eps_v50_gate2_lowpass.py
...
studies/models/eps_v50_gate2_lowpass.py (which itself reused eps_v48c's Nyquist machinery). No new measurements are
introduced; everything here is either that calibration, the V52 build's own coefficients read from
studies/caves/v52_cave_asm.py, or an explicit, labeled model ASSUMPTION.
...
Run:  python studies/models/eps_v52_gate2_broad.py
"""
...
# ---------------------------------------------------------------------------------------------------
# GIVENS reused verbatim from studies/models/eps_v50_gate2_lowpass.py (already-closed 7-lane GATE 2) -- no re-derivation
# ---------------------------------------------------------------------------------------------------
...
# V52's ACTUAL built coefficient (studies/caves/v52_cave_asm.py: ALPHA=74 Q10, sar 10 -- read from the build, not re-fit)
...
# per-lane weighting is unknown (studies/models/eps_loop_gain_model.py Task 5 carries the same "unknown split" caveat
...
# studies/models/eps_v50_gate2_lowpass.py's stability(), generalized to take an arbitrary L(w) so it applies to the
...
def task1():
    ...
    print("The per-lane split is UNKNOWN (same caveat as studies/models/eps_loop_gain_model.py Task 5); the fine f-sweep")
...
def task5():
    ...
    print("matches Python's native >> on ints) -- confirmed against studies/caves/v52_cave_asm.py's sar encoding.\n")
...
def verdict(results):
    ...
    print("  (f=0, already flying at 1.16-4+ dB margin per studies/models/eps_v50_gate2_lowpass.py / this script's f=0 row)")
...
def main():
    ...
    print(f"low-pass insertion. fc(V52 as-built) = {FC_V52:.3f} Hz (alpha=74/1024, from studies/caves/v52_cave_asm.py).")
