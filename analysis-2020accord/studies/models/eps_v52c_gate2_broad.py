...
# -*- coding: utf-8 -*-
r"""
studies/models/eps_v52c_gate2_broad.py -- GATE 2 (closed-loop stability) for V52C, the BROAD gp-0x4f60 low-pass build
that repoints 16 of the 19 command-path carrier lanes to the filtered EMA copy (alpha=74/1024,
fc~=11.9 Hz, round-to-nearest -- frequency response IDENTICAL to V50/V52's alpha, see studies/caves/v52_cave_asm.py),
leaving 3 lanes RAW (builds/v50_v79/build_v52c_tva.py LEAVE_RAW_CARRIERS, verbatim):
...
Q_cl=13.6 anchor, BROAD-SHELF Q_cl~4.8 anchor) validated in studies/models/eps_v50_gate2_lowpass.py (which itself
carries the same Nyquist machinery forward from eps_v48c). No new measurements are introduced.

Distinct from the existing studies/models/eps_v52_gate2_broad.py (written for V52-as-built, 10 repoints, with the 3
...
Run:  python studies/models/eps_v52c_gate2_broad.py
"""
...
A_V52C = 1.0 - 74.0 / 1024.0          # studies/caves/v52_cave_asm.py ALPHA=74/1024 -- IDENTICAL to V50/V52 (round-to-
FC_V52C = -FS * math.log(A_V52C) / (2.0 * math.pi)   # nearest changes quantization only, not frequency response)
A_36682 = 1.0 - 6.0 / 1024.0          # builds/v50_v79/build_v52c_tva.py LEAVE_RAW_CARRIERS: cal 0xC63D2=6/1024
FC_36682 = -FS * math.log(A_36682) / (2.0 * math.pi)
A_3B908 = 1.0 - 3686.0 / 4096.0       # builds/v50_v79/build_v52c_tva.py LEAVE_RAW_CARRIERS: float EMA alpha=3686/4096
...
# ---------------------------------------------------------------------------------------------
N_TOTAL = 19          # total gp-0x4f60 command-path carriers (builds/v50_v79/build_v52c_tva.py REPOINT_SITES == 19)
...
# studies/models/eps_v50_gate2_lowpass.py's loop_bare(), factored so an arbitrary complex blend(w) can be multiplied in.
# ---------------------------------------------------------------------------------------------------
...
# studies/models/eps_v50_gate2_lowpass.py's stability(), generalized to an arbitrary blend(w) so it covers the partial,
