#!/usr/bin/env python3
"""studies/sessions/v74_v75/v74fault_extract.py -- extract route 61 (the V74 drive that HARD-FAULTED) to `_scratch/cache/r61/`.
...
🛑 The spec is RE-READ out of `builds/v50_v79/build_v74_tva.py` at import time, not hand-copied -- see
...
Usage: python studies/sessions/v74_v75/v74fault_extract.py            # all 13 segments -> _scratch/cache/r61/r61.npz
"""
import json
import os
import re
...
SEGS = list(range(13))
OUT = Path(os.environ.get("R61_CACHE", ROOT / "_scratch/cache/r61"))
...
def _assert_probe_spec():
    ...
    src = (HERE / "builds/v50_v79/build_v74_tva.py").read_text(encoding="utf-8")

    def hexnum(name):
        m = re.search(rf"^{name}\s*=\s*0x([0-9A-Fa-f]+)", src, re.M)
        assert m, f"{name} not found in builds/v50_v79/build_v74_tva.py"
    ...
    assert m, "wire_byte4() not found in builds/v50_v79/build_v74_tva.py"
    ns = {"W_DAMP_NZ": W_DAMP_NZ, "STATE_MASK": STATE_MASK, "PAYLOAD_SHIFT": PAYLOAD_SHIFT,
          "PAYLOAD_KEEP_MASK": STATUS_MASK}
    exec(compile(m.group(0), "builds/v50_v79/build_v74_tva.py:wire_byte4", "exec"), ns)
    wire = ns["wire_byte4"]
    for st in range(16):
...
def extract(paths):
    ...
    # `extract/extract_r5d_cache.py` (route 5d = V74 flying CLEAN with this same probe).
    ...
    # time base by the median offset against logMonoTime, exactly as `extract/extract_r5d_cache.py` does.
    ...
    # established mapping is ax=vertical, ay=lateral, az=longitudinal (studies/sessions/r47/analyze_r47_imu.py:1564).
    if len(A):
