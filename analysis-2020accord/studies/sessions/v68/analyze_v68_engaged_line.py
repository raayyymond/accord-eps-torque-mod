...
def main():
    ...
    for route in ("4c", "4e"):
        ...
        for s, d in segs_of(route):
            ...
            nfr += len(d["t"])
            ep = HERE.parent / "_scratch/cache/v68" / f"{route}s{s}_events.json"
            if ep.exists():
    ...
    Path(HERE / "_scratch/out/_v68_engaged_line.json").write_text(json.dumps(res, indent=1, default=str))
    print(f"\nwrote {HERE / '_scratch/out/_v68_engaged_line.json'}")
