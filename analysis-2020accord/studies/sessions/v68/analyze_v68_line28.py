...
PRIOR = {
    "r47/V67": (ROOT / "_scratch/cache/r47", "r47s", list(range(0, 26))),
    "r2b/V58": (ROOT / "_scratch/cache/r2b", "r2bs", list(range(0, 14))),
    "r37/V62": (ROOT / "_scratch/cache/r37", "r37s", list(range(0, 15))),
    "r3b/V65": (ROOT / "_scratch/cache/r3b", "r3bs", list(range(0, 14))),
}
...
def main():
    ...
    for s in [31, 32, 33, 34]:
        p = ROOT / "_scratch/cache/v68" / f"4es{s}_events.json"
        if p.exists():
    ...
    Path(HERE / "_scratch/out/_v68_line28.json").write_text(json.dumps(res, indent=1, default=str))
    print(f"\nwrote {HERE / '_scratch/out/_v68_line28.json'}")
