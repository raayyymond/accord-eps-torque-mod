#!/usr/bin/env python3
"""studies/sessions/v68/v69_surface_math.py -- the mode-10 gain_B surface rebuilt FROM IMAGE BYTES, and every design
...
Authoritative: `builds/v50_v79/build_v69_tva.py` (the SCALE constant), `docs/specs/design/V69-DESIGN.md` §0.

Successor to `studies/sessions/v68/v68_design_math.py`. That file hard-coded the records as literals; this one READS
...
WHAT THIS FILE ADDS OVER studies/sessions/v68/v68_design_math.py
  1. Every record is byte-read through its pointer array (no literals). T1.
  2. 🛑 THE AXIS SCALE IS TREATED AS OPEN. `studies/sessions/v68/v68_design_math.py` prints 4.7121 counts/deg-s as
...
Usage:  python studies/sessions/v68/v69_surface_math.py            (uses $ACCORD_FIRMWARE_ROOT or the default below)
"""
...
CACHE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))), "_scratch/cache/v68")
...
def main():
  ...
  ==> ANY of the designs below must be built as V68 MINUS the gate repoint:
    ...
    print("\n  MEASURED |dtorque| from _scratch/cache/v68, via the firmware's own difference transfer")
