#!/usr/bin/env python3
"""studies/sessions/v68/v68_design_math.py -- the r24 rate-lane gain surface, and what each build actually DELIVERS.
...
Usage:  python studies/sessions/v68/v68_design_math.py
"""
...
def _report():
    ...
    print("\n" + "=" * 100)
    print("6. ROBUSTNESS -- is the highway verdict sensitive to the rate operating point?")
    print("   (a peer flagged the measured highway rate is inflated by the oscillation itself)")
    print(f"   flat first segment = axis [0,400] = [0, {400 / AXIS_PER_DEGS:.1f}] deg/s")
    print(f"   {'assumed deg/s':>14} {'axis':>6} {'stock@110kmh':>13} {'V67 mult':>9} {'DesignA':>8}")
    for d in (0, 5, 20, 35, 60, 85, 100):
        st, da = G(110.0, d), G(110.0, d, DESIGN_A)
        print(f"   {d:14.0f} {d * AXIS_PER_DEGS:6.0f} {st:13d} {V67_ARM / st:9.2f} {da / st:8.2f}")
    print("   => every plausible highway rate lands in the FLAT segment, so the LERP output is")
    print("      2151 and the delivered multiplier is 2.44x REGARDLESS. Deflating the rate")
    print("      estimate changes NO downstream number -- it would only matter above ~85 deg/s,")
    print("      which no highway maneuver on record approaches.")

    print("\n" + "=" * 100)
    print("7. 🛑 WHAT V68 DOES NOT DO, AND THE STRUCTURAL REASON")
    print("  The creep grind #2 shares grind #1's SPEED cells, so no speed schedule separates them.")
    print("  The RATE axis does discriminate (grind #1 ~603, creep grind #2 ~1206 -- both on the")
    print("  [400,1400] rolloff), which is what Design A exploits: 2.00x vs 1.22x at those points.")
    ...
    print("  🛑 A gate cell meaning 'LKAS applying AND low speed' WOULD get both -- but TWO search")
    print("    passes found none, and none exists: this firmware's idiom for speed is 'always LERP,")
    print("    never threshold-and-latch'. Do not repoint the mask arm gp-0x671d to manufacture one:")
    print("    it is a rising-edge counter driving DTC 0x5e, read by 8 functions including 4 reads in")
    print("    the motor-off dispatcher FUN_0003d4a2. Unlike the dead gp-0x683c it is a LIVE fault")
    print("    response, and 0/165k logged frames is what a rare fault path looks like.")
