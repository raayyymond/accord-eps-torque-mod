...
Writes `_scratch/out/_r58_followups.json`.  Usage: python studies/sessions/r58/r58_followups.py
"""
...
ROUTES = {"V71B r54": ("_scratch/cache/r54", "r54s", [s for s in range(21) if s not in (10, 11)]),
          ...
          "V65 r3b": ("_scratch/cache/r3b", "r3bs", list(range(14))),
...
print("   does not move (ratio ~1). Computed from `_scratch/out/_r58_ratchet.json`'s own episode table.\n")
qp = ROOT / "_scratch/out/_r58_ratchet.json"
if qp.exists():
...
rows = [r for r in R47._windows("_scratch/cache/r58", "r58", lambda v: True)
...
(ROOT / "_scratch/out/_r58_followups.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {ROOT / '_scratch/out/_r58_followups.json'}")
