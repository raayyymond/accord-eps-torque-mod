#!/usr/bin/env python3
"""studies/sessions/r47/r47_atlas.py -- the ROUTE ATLAS for V67 route `47` (75604b0a432fdc89_00000047--3e0b6134c0).
...
`_scratch/cache/r47/r47_maneuvers.json` so that every test is cut on the SAME episode bounds.
...
METHOD RULES (each has already retracted a claim in this kit -- see docs/STATE.md METHODOLOGY):
  ...
  WALL CLOCK   🛑 SEGMENT 0's `wall_t0` IS THE STALE PRE-NTP RTC (1751465105 = 2025-07-02, a year
               ...
               the post-sync cluster covers the route. Same trap, same fix as studies/sessions/r37/r37_wallclock.py.
...
`python analysis-2020accord/extract/extract_r47_cache.py 0 1 2 ... 25` if they do not):

    python analysis-2020accord/studies/sessions/r47/r47_atlas.py             # full atlas + write the JSON  (~90 s)
    python analysis-2020accord/studies/sessions/r47/r47_atlas.py --no-write  # print only
    python analysis-2020accord/studies/sessions/r47/r47_atlas.py --schema    # the JSON schema alone
    python analysis-2020accord/studies/sessions/r47/r47_atlas.py --verify    # re-derive every episode from the .npz
                                                        # files through `spans` alone and diff it
                                                        # against the emitted JSON (539 checks)

Set R47_CACHE to point the cache elsewhere; it defaults to <repo>/_scratch/cache/r47.
Output: _scratch/cache/r47/r47_maneuvers.json  (the schema is embedded in the file under "schema").
"""
import json
import sys
import time
...
CACHE = Path(__import__("os").environ.get("R47_CACHE", ROOT / "_scratch/cache/r47"))
PFX = "r47s"
SEGS = list(range(26))
...
SCHEMA = {
    "_about": "Route atlas for V67 route 47 (75604b0a432fdc89_00000047--3e0b6134c0). Written by "
              "analysis-2020accord/studies/sessions/r47/r47_atlas.py. Consumers: cut analysis windows INSIDE these "
    ...
    "_indexing": "i0/i1 index the GLOBAL concatenated route array built by r47_atlas.load_all() "
                 ...
                 "indices into _scratch/cache/r47/r47s<seg>.npz and t0/t1 are that segment's own `t`. An "
