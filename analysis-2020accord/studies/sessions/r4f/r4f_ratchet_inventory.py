#!/usr/bin/env python3
"""studies/sessions/r4f/r4f_ratchet_inventory.py -- the RATCHET on route `4f` (V69), characterised from the analog bus.

WHY THIS EXISTS ALONGSIDE `studies/sessions/r4f/r4f_v69_readout.py`
----------------------------------------------
...
`probe/decode_v69_ratchet.py` defines. On this route that cell is 71.7 s of 481.7 s, and a first pass
...
def load(seg, cache="_scratch/cache/r4f", pfx="r4fs"):
...
def scan(d, fs, seg, tag):
    ...
    v = np.abs(d["cs_v"])
    rpm = None
    p = ROOT / "_scratch/cache/r4f" / f"{pfx_of(tag)}{seg}_rpm.npz"
    if p.exists():
        rpm = np.load(p)["rpm"]
    out = []
...
# ---------------------------------------------------------------- 6. waveform -------------------
def hp(x, fs, fc=4.0):
    """High-pass, NOT band-pass. `studies/sessions/r37/analyze_r37_waveform.py`'s method and its reason: a 6-9 Hz
...
def waveform(eps):
    ...
    print("      Method and thresholds are `studies/sessions/r37/analyze_r37_waveform.py`'s, unchanged.")
...
BUILDS = [
    ...
    ("V61 r31", "_scratch/cache/r31", "r31s", [0, 1, 2, 3]),
    ("V62 r37", "_scratch/cache/r37", "r37s", list(range(0, 15))),
    ("V64 r35", "_scratch/cache/r35", "r35s", [0, 1, 2]),
    ("V65 r3a", "_scratch/cache/r3a", "r3as", None),
    ("V67 r47", "_scratch/cache/r47", "r47s", None),
    ("V68 r4c", "_scratch/cache/v68", "4cs", None),
    ("V69 r4f", "_scratch/cache/r4f", "r4fs", SEGS),
]
