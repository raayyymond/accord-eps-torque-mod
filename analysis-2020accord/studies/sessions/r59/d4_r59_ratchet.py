...
INSTRUMENT: `studies/sessions/r58/r58_ratchet.py` COPIED, not re-derived -- DISJOINT 2.56 s windows (NFFT 256),
...
`studies/sessions/r58/r58_ratchet.py` does not compute (it reports a binary hit rate only). D3-microratchet needs the
factor, not the binary.

Writes `_scratch/out/_d4_r59_ratchet.json`.
"""
...
ROUTES = {
    ...
    "V72 r59":  ("_scratch/cache/r59", "r59s", list(range(15)), [12, 13, 14], True),
}
NEW = "V72 r59"
...
(ROOT / "_scratch/out/_d4_r59_ratchet.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {ROOT / '_scratch/out/_d4_r59_ratchet.json'}")
