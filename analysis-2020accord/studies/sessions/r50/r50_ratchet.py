...
INSTRUMENT is `studies/sessions/r4f/r4f_ratchet_inventory.py`'s, unchanged in every numeric respect, so the counts and
...
Writes `_scratch/out/_r50_ratchet.json`.  Usage: python studies/sessions/r50/r50_ratchet.py
"""
...
ROUTES = {"V70 r50": (ROOT / "_scratch/cache/r50", "r50s", [0, 1, 2]),
          "V69 r4f": (ROOT / "_scratch/cache/r4f", "r4fs", list(range(8))),
          "V62 r37": (ROOT / "_scratch/cache/r37", "r37s", list(range(15)))}
...
(HERE / "_scratch/out/_r50_ratchet.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE / '_scratch/out/_r50_ratchet.json'}")
