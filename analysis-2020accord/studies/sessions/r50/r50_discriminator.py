...
Writes `_scratch/out/_r50_discriminator.json`.  Usage: python studies/sessions/r50/r50_discriminator.py
"""
...
print("\n   (iii) COARSE-MATCHED LADDER (from studies/sessions/r50/r50_grind1_power.py ss2), V70/stock, with its null:")
...
L.hdr("ss4  ★★ BURST #0's MANUAL -> ENGAGED TRANSITION -- the same oscillation either side of the "
      "switch")
d = C.load(1, ROOT / "_scratch/cache/r50", "r50s")
fs = R4F.fs_lattice(d)
t = np.asarray(d["t"], float)
tq = np.asarray(d["tq"], float)
...
(HERE / "_scratch/out/_r50_discriminator.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE / '_scratch/out/_r50_discriminator.json'}")
