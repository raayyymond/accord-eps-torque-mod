#!/usr/bin/env python3
"""v75fault_extract.py -- extract route 5e (the V75 flight that HARD-FAULTED) to `_cache_r5e/`.
...
🛑 The thresholds are read out of `build_v75_tva.py` at import time, not hand-copied -- see
...
Usage: python v75fault_extract.py            # all 7 segments -> _cache_r5e/r5e.npz
"""
import json
import os
import re
...
SEGS = list(range(7))
OUT = Path(os.environ.get("R5E_CACHE", ROOT / "_cache_r5e"))
...
def _assert_probe_spec():
    ...
    src = (HERE / "build_v75_tva.py").read_text(encoding="utf-8")

    def num(name):
        m = re.search(rf"^{name}\s*=\s*([0-9]+)", src, re.M)
        assert m, f"{name} not found in build_v75_tva.py"
