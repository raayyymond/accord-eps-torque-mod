...
BACK, at creep. See docs/HANDOFF-2026-08-04-v69-flew-grind1-back-at-creep.md.

🛑 NAME-COLLISION HISTORY, kept because it silently cost a channel once. Two agents in one session
wrote a route-4f extractor under near-identical names -- `extract_r4f_cache.py` (this file) and
`r4f_extract_cache.py` -- BOTH writing `_cache_r4f/r4fs*.npz`. Whichever ran last won, and their
...
Decode is byte-for-byte `extract_v68_cache.py`'s with ONE substantive change -- the byte4 decoder,
which is BUILD-SPECIFIC and now carries V69's RATCHET probe rather than V68's grind detector. Taken
from `rlog-tools/decode_v69_ratchet.py` (which `build_v69_tva.assert_decoder_matches()` links
...
Usage:  python extract_r4f_cache.py            # all 8 segments
        python extract_r4f_cache.py 0 1 2      # chosen segments
"""
import json
import os
import sys
...
SEGS = list(range(0, 8))
OUT = Path(os.environ.get("R4F_CACHE", ROOT / "_cache_r4f"))
PFX = "r4fs"
