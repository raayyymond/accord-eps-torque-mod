#!/usr/bin/env python3
"""build_v81_tva.py -- V81 = THE FLOWN V75, with Honda's friction clamp restored. CAL-ONLY.
...
and V75 carried it. Both variants remain implemented and byte-attributed; A is what shipped:
  ...
  THE OPERATOR'S FOUR REASONS, recorded here because they are the build's justification:
   1. A MEASURED ZERO BACKS IT. V76 flew route 65 with stock friction AND 0xC407E = 511. Its cave
      ...
      🛑 Cite `V76-V38BASE-...-probe-6b26-63fd` (extracted by `v76flight_extract.py` ->
      `_cache_r65_records.pkl`), NOT `rlog-tools/decode_v76_probe.py`, which documents the
...
Usage:
    python build_v81_tva.py                          # DRY RUN, variant A, writes nothing
    ACCORD_V81_WRITE=rwd python build_v81_tva.py     # ✅ reproduces THE FLASHABLE ARTEFACT
    ACCORD_V81_FRICTION=V75 python build_v81_tva.py  # DRY RUN of variant B. 🛑 never with WRITE.
"""
import hashlib
import os
import struct
...
# =====================================================================================================
# 🛑 `diff_build_vs_stock.py` is SPAN-based and has missed things. This is the strongest statement
