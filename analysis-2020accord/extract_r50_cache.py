...
Decode is byte-for-byte `extract_r4f_cache.py`'s with ONE substantive change -- the byte4 decoder,
which is BUILD-SPECIFIC and now carries V70's SIGN probe rather than V69's ratchet rungs. Taken
from `rlog-tools/decode_v70_probe.py`, whose CAVE_HEX `build_v70_tva.assert_decoder_matches()` links
...
Usage:  python extract_r50_cache.py            # all 3 segments
        python extract_r50_cache.py 0 1        # chosen segments
"""
import json
import os
import sys
...
SEGS = [0, 1, 2]
OUT = Path(os.environ.get("R50_CACHE", ROOT / "_cache_r50"))
PFX = "r50s"
