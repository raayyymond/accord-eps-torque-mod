"""build_v17_tva.py — V17 LKAS 2x + EME (driver-override dropout) FIX for the 2020 Accord (39990-TVA,A160).

LINEAGE: V15B (2x arb gain/clamps + part-number string fix) PLUS the EME deadband fix.
Following deep disassembly analysis (User A constraint analysis), the V16 'slew' fix (0xC61D6 0->14)
was rejected. Slew=0 does not mean a passthrough limiter; it means the step size is zero, freezing
a torque-shaping signal. Changing it to 14 activates a dormant signal upstream of the final clamp,
which is dangerous and unproven.

Instead, V17 isolates the verified safe 'deadband' narrowing (0xC6424: 29491->20000). The deadband
safely trims the drop-out window during EME driver interventions without turning on dormant signals.
Ramp steps and slew are preserved at their stock values.

WHAT V17 EDITS (on top of stock V9 code.bin):
  Calibration block #48 [0xC6000, 0xC6FFC) — halfwords (u16 LE):
    0xC646C  GAIN     tp+0x746c   891   -> 1336   (x1.5, backed off from x2 to limit EME snap severity)
    0xC61B4  CLAMP    tp+0x71b4   512   -> 1024   (x2, from V14/V15)
    0xC61B2  CLAMP    tp+0x71b2   512   -> 1024   (x2, from V14/V15)
    0xC6424  DEADBAND tp+0x7424  29491 -> 20000   (INERT without slew, acting only as structural tracker)
  Calibration block #48 — single byte (u8):
    0xC64DE  RAMPSTEP tp+0x74de   0x11  -> 0x1B   (NEW: 17->27, faster re-engage ramp)
...
CAL_PATCHES = [
    (0xC646C,   891, 1336, "GAIN     tp+0x746c  arb Q15 output gain  891->1336 (x1.5)"),
    ...
    (0xC6424, 29491, 20000,"DEADBAND tp+0x7424  shaper low-demand cut 29491->20000 (INERT)"),
]
...
def build(label, code_stock, headers, tag):
    print("=" * 78)
    print(f"{label}: LKAS 1.5x + PN string   cipher v9b")
...
def main():
    ...
    print(f"baseline = V9 stock; edits = 4 cal halfwords + 2 PN string bytes\n")
    build("V17", code, headers, tag="LKAS-1.5x-deadband-PNfix")
    return 0
