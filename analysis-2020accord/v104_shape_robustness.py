...
🛑 AND ONE RECORD CHECK: price_flat_6b86_boost.py's 204,000-corner sweep uses
...
print("=" * 112)
src = open(os.path.join(HERE, 'price_flat_6b86_boost.py'), encoding='utf-8').read()
m = re.search(r"A_GRID\s*=\s*np\.round\(np\.arange\(([^)]*)\)", src)
print("  price_flat_6b86_boost.py:  A_GRID = np.arange(%s)" % m.group(1))
...
print("  ⊕ v104_reprice_k185.py, run on the measured a_filt bootstrap, already reported")
...
bc = np.array([one(rng.integers(0, len(G4s), len(G4s)), rng.integers(0, len(G8s), len(G8s)))
               for _ in range(4000)])
bA = 1 + bc * G0
AF = np.load(os.path.join(HERE, '_v103_natexp.npz'))['a69'].real
