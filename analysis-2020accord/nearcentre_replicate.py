...
`nearcentre_final.py` ss1/ss2 found the operator's angle conditional does not survive, and that the
...
Usage: python nearcentre_replicate.py [ep|blk] -> writes _nearcentre_replicate.json
"""
import json
import sys
...
(HERE.parent / "_nearcentre_replicate.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"\nwrote {HERE.parent / '_nearcentre_replicate.json'}")
