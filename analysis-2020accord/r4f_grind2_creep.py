...
THIS SCRIPT COVERS THE CREEP ARM ONLY. The highway arm is `r4f_highway_bands.py`; the lane-change
transient is `r4f_lanechange.py`.
...
against 7.62 expected, P(0) = 0.0005 (`_r4a_results.json`). So a zero-burst creep result on `4f`
...
in `r4f_highway_bands.py` uses the lattice rate instead.

Usage:  python r4f_grind2_creep.py [--json OUT]
"""
...
POOLS = {
    ...
    "Kd=4x<50 (V69 r4f)  *** THIS ROUTE ***": ["_cache_r4f"],
}
...
def sec_identity():
    ...
    print("     `decode_v69_ratchet.py` warns that V66/V67 also emit bit3 = 0 and that their")
