...
`ledger_v94_cells.py` labels them only by index ("lane weight [0]"…"[5]"). The mapping is load-bearing
...
## 1. VIRGINITY CENSUS (EVIDENCE — `_v97/ledger_v97_virginity.py`, 89 images)
...
- `build_v18_tva.py` already wrote "(byte)" in its comment — the halfword framing entered later, via the
...
`memory/eps-deliver-cut-gp6809-broken.md` establishes `gp-0x6809` has **zero writers — dead code** — and
...
`_v97/ledger_v97_poles.py`, mirroring the integer arithmetic.
...
**On the "must-not-move list" — and why that is weaker than it sounds.** `build_v83a_tva.py:159` says
...
**VIRGIN across all 89 images.** ⚠ It is **not a scalar** — `build_v43_tva.py:240` documents it as a
