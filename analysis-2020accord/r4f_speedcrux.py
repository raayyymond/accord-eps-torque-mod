#!/usr/bin/env python3
"""DELIVERABLE 3 (the crux) done properly, plus the two controls `r4f_crossbuild.py` could not run.

THREE FIXES OVER `r4f_crossbuild.py`, each of which changed a number there:
  ...
  2. ★ A WITHIN-WINDOW NORMALISED METRIC. In `r4f_crossbuild.py` ss4 the creep contrast moved
...
Writes `_r4f_speedcrux.json`.  Usage: python r4f_speedcrux.py [ep|blk]
"""
import json
import sys
...
print("  same call returned 0 cells in r4f_crossbuild.py ss5.\n")
veto = {}
...
(HERE / "_r4f_speedcrux.json").write_text(json.dumps(OUT, indent=1, default=float))
print(f"wrote {HERE / '_r4f_speedcrux.json'}")
