#!/usr/bin/env python3
"""build_v67_tva.py -- V67 = V66 PLUS the grind #1 fix, GATED ON LKAS.
...
`docs/V66-V67-DESIGN.md`; the executable arithmetic is `analysis-2020accord/v66_v67_explained.py`.
...
⚠ The mirror in `v66_v67_explained.py` is therefore off by one at the boundary. It is NOT corrected
...
Decoder: rlog-tools/decode_v67_gate.py
"""
import hashlib
import itertools
import json
...
DECODER = os.path.join(os.path.dirname(HERE), "rlog-tools", "decode_v67_gate.py")
...
def _self_check_wire():
    ...
    # ⚠ CORRECTION. `docs/V66-V67-DESIGN.md` and v66_v67_explained say "|dtorque| >= 1599". That is
...
def assert_decoder_matches(cave_bytes, label="V67"):
    ...
    V62 vs V64), so the link is made MECHANICAL: decode_v67_gate.py carries the built cave's hex and
...
def build():
    ...
    print(f"\n  decoder link: rlog-tools/decode_v67_gate.py CAVE_HEX "
    ...
    print("     Decode with rlog-tools/decode_v67_gate.py.")
    return code
