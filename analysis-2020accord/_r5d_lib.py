...
else. Cache written by `extract_r5d_cache.py`; exposure census in `_r5d_extract_summary.json`.
...
PKL = ROOT / "_cache_r5d_records.pkl"
...
G.BUILDS["V74/r5d"] = dict(cache=ROOT / "_cache_r5d", pfx="r5ds", segs=list(range(17)), kd=2.44)
...
# `_r5d_extract_summary.json` reports every segment separately.
...
RAIL_CHANNELS = {
    ...
    "e4tq": ("CAN 0x0E4 bytes 0:1, `can` src 129 (the panda's TX echo) -- OPENPILOT'S STEER_TORQUE "
             ...
             "`0.03*STEER_MAX` term at eps_lkas_chain_model.py:2238). ZERO frames exceed it; the "
...
def load_imu(seg):
    ...
    """
    return dict(np.load(ROOT / "_cache_r5d" / f"r5ds{seg}_imu.npz"))


def load_snd(seg):
    return dict(np.load(ROOT / "_cache_r5d" / f"r5ds{seg}_snd.npz"))
