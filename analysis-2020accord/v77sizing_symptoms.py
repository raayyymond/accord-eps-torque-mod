...
T4 (grind#2 vs its design prediction), T5 (dose-response, extending `v78_symptom_dose.py`'s ladder).
...
come out of `v78_symptom_lib.records()` UNCHANGED (read-only, exactly as `v78_symptom_dose.py`
consumes them); V76/r65 comes out of `v77sizing_lib.records()`, this session's own extract.

Usage:  python v77sizing_symptom_dose.py   ->  writes _v77sizing_symptoms.json
"""
import json
import sys
...
with open(ROOT / "_v77sizing_symptoms.json", "w", encoding="utf-8") as fh:
    json.dump(OUT, fh, indent=1, default=float)
print("wrote _v77sizing_symptoms.json")
