#!/usr/bin/env python3
"""r4a_orchestrator_checks.py -- extraction audit + exposure + grind #1/#2 statistics for route `4a`.
...
Usage:  python r4a_orchestrator_checks.py [section ...]      (default: all)
...
OUTJSON = HERE / "_r4a_results.json"
...
def sec_exposure():
    ...
    print(f"   {'TOTAL':>14s} {hist[:, 0].sum():11.1f} {hist[:, 1].sum():10.1f} "
          f"{hist.sum():9.1f}")
    print(f"\n   creep = 0.3 < v < 4.0 m/s -- the SAME definition r47_orchestrator_checks.py uses "
    ...
    c47 = Counter()
    for s in range(26):
        p = ROOT / "_cache_r47" / f"r47s{s}.npz"
        if not p.exists():
            continue
        d = load(s, ROOT / "_cache_r47", "r47s")
        fs = fs_of(d)
        dt = float(d["t"][-1] - d["t"][0]) / (len(d["t"]) - 1)
        eff = np.abs(sustained(np.asarray(d["tq"], float), fs))
...
def _pools(vsel):
    ...
    P["Kd=gated (V67 r4a)"] = ["_cache_r4a"]
    P["Kd=gated (V67 r47+r4a)"] = ["_cache_r47", "_cache_r4a"]
...
def _records():
    ...
    if _RECS is not None:
        return _RECS
    with open(ROOT / "_cache_r47" / "r47_grind1_records.pkl", "rb") as fh:
        st = pickle.load(fh)
...
def sec_grind2():
    ...
    CP = {"Kd=2.00 (V62 r37 + V65 r3a/r3b)": ["_cache_r37", "_cache_r3a", "_cache_r3b"],
          "Kd=gated (V67 r47)": ["_cache_r47"], "Kd=gated (V67 r4a)": ["_cache_r4a"],
          "Kd=gated (V67 r47+r4a)": ["_cache_r47", "_cache_r4a"]}
