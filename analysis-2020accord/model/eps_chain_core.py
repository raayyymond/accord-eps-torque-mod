"""model/eps_chain_core.py -- SECTIONS 0-1 of the golden model: calibration constants, the signal/
state containers, and the shared integer helpers. Split out of `model/eps_lkas_chain_model.py` on
...
class EpsState:
    ...
    #   the PRE-RETRACTION sign, and the 3 call sites in model/eps_chain_lanes.py inherit it.
    #   ⚠ It is NOT fixed here because _self_check()'s expected values were themselves computed at
    #   +1 (e.g. `_inline_torque_rate_b(st) == 1533` @model/eps_chain_delivery.py) and flipping the
