...
🛑🛑 KNOWN MODELLING GAPS AND LABEL CORRECTIONS — added 2026-08-21, V104 close-out.
    ...
    2. THE ASSIST MAP'S AXES ARE THE OPPOSITE OF THE OBVIOUS READING. `0xC7B40` is a pointer array
       ...
       studies/models/assist_map_mirror.py`. Runtime slope `a` = 0.069 pooled engaged, SPEED-SCHEDULED
       0.123 (parking) -> 0.046 (120 km/h). The budget's 0.098/0.117 is high by 1.4x/1.7x.

    3. LABEL CORRECTIONS in `model/eps_chain_control.py` -- the code is right, the comments are not:
...
🛑 THIS FILE IS NOW A THIN FACADE. The model itself lives in four modules beside it.
    ...
    WHERE THE CODE ACTUALLY IS — grep/read these, not this file:
      model/eps_chain_core.py       SECTIONS 0-1  calibration constants, Calibration/CanSteeringControl/
                                            SensorInputs/EpsState, the integer helpers. No intra-kit
                                            imports — the bottom of the graph.
      model/eps_chain_lanes.py      SECTIONS 2-3  CAN intake + LKAS setpoint, driver torque sensor + voter,
                                            base driver assist, boost index, the assist-shaping
                                            rate lanes (r24/r26).
      model/eps_chain_control.py    SECTIONS 4-6  engage decider, steer torque arbitration, limit/pack ->
                                            distribute -> mixer -> gate, the demand aggregator, the
                                            motor-rate governor, and the analysis functions.
      model/eps_chain_delivery.py   SECTIONS 7-10 soft-EME windup shaper, hard-DTC lockstep monitor,
...
-------------------------------------------------------------------------------------------------------
  ...
  891 for the feedback readers only. See memory/reference/firmware/reference-accord-c646c-shared-gain-not-lkas-only.md.
...
-------------------------------------------------------------------------------------------------------
  ...
  🛑 V52C ("halved the mode") did NOT halve anything: -6.1 dB IS 0.496x IS the filter's own transfer
     ...
     no V52C rlog exists. See memory/accord/instruments/accord-a-caveat-can-mutate-into-a-result.md.
...
-------------------------------------------------------------------------------------------------------
  BASE TICK    : 🛑 CORRECTED 2026-07-31 -- this line used to read "OSTM0 timer, compare 79999 ->
                 ...
                 See the TASK RATE entry below and memory/accord/firmware/accord-task5-is-100hz-damper-cannot-damp-21hz.md.
  ...
  TASK RATE    : ✅ RESOLVED 2026-07-31. The dispatcher is FUN_00014be4, a mod-100 rate divider on the
                 ...
                 See memory/accord/firmware/accord-task5-is-100hz-damper-cannot-damp-21hz.md.
                 ...
                 (1) FUN_0003b66a holds a REAL BAND-PASS at 8.13 Hz -- backward difference x 17.453293
                     ...
                     See memory/accord/calibration/accord-c63b8-8hz-bandpass-is-a-rectified-boost-index.md.
                 (2) FUN_00043e44 is a FLOAT TWIN of the shaper and it BLOCKS filter insertion. It
                     ...
                     See memory/accord/calibration/accord-shaper-float-twin-blocks-filter-insertion.md.
