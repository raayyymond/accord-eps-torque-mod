...
This is the ONE place a V71B reader goes wrong, and `rlog-tools/decode_v71_probe.py`'s prose has a
...
disagrees with that prose and wins:
  * `build_v71b_tva.py:194`  `MIRROR = A.R26_MIRROR_DISP`  (0x6ADC, `st.h r26,-0x6adc[gp]` @0x3AD4E)
  * `build_v71b_tva.py:531`  asserts `code[CAVE_BASE + 0x1A] == 0x24` (V71A's is 0x26)
...
Usage:  python extract_r54_cache.py            # all 21 segments
        python extract_r54_cache.py 0 1        # chosen segments
"""
import json
import os
import re
...
SEGS = list(range(21))
OUT = Path(os.environ.get("R54_CACHE", ROOT / "_cache_r54"))
PFX = "r54s"
...
def _assert_mirror_byte():
    ...
    way `build_v71b_tva.py` does) and fail if cave+0x1A is not 0x24 = `ld.h -0x6adc[gp],r6`.
    ...
    assert m, "CAVE_HEX_B not found in decode_v71_probe.py -- cannot verify the mirror cell"
